...
export class BraceMatcher {
  ...
  private static getMatchingCloseBrace(
    currentToken: PositionedToken,
    position: number,
    result: TextSpan[]
  ) {
    if (currentToken.start() === position) {
      var closingBraceKind =
        BraceMatcher.getMatchingCloseBraceTokenKind(currentToken);
        ...
        for (var i = 0, n = parentElement.childCount(); i < n; i++) {
          ...
          if (element !== null && element.fullWidth() > 0) {
            if (element.kind() === closingBraceKind) {
              ...
              var range2 = new TextSpan(
                currentPosition + element.leadingTriviaWidth(),
                element.width()
              );
  ...
  private static getMatchingOpenBrace(
    currentToken: PositionedToken,
    position: number,
    result: TextSpan[]
  ) {
    ...
    if (currentToken !== null && currentToken.start() === position - 1) {
      var openBraceKind =
        BraceMatcher.getMatchingOpenBraceTokenKind(currentToken);
      if (openBraceKind !== null) {
        var parentElement = currentToken.parentElement();
        var currentPosition =
          currentToken.parent().fullStart() + parentElement.fullWidth();
        for (var i = parentElement.childCount() - 1; i >= 0; i--) {
          var element = parentElement.childAt(i);
          if (element !== null && element.fullWidth() > 0) {
            if (element.kind() === openBraceKind) {
              var range1 = new TextSpan(
                position - 1,
                currentToken.token().width()
              );
              var range2 = new TextSpan(
                currentPosition -
                  element.trailingTriviaWidth() -
                  element.width(),
                element.width()
              );
              result.push(range1, range2);
  ...
  private static getMatchingCloseBraceTokenKind(
    ...
    switch (element.kind()) {
      ...
      case SyntaxKind.LessThanToken:
        return SyntaxUtilities.isAngleBracket(positionedElement)
  ...
  private static getMatchingOpenBraceTokenKind(
    positionedElement: PositionedElement
  ): SyntaxKind {
    var element = positionedElement !== null && positionedElement.element();
    switch (element.kind()) {
