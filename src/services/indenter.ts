...
import { EditorOptions } from './languageService';
...
export class Indenter {
  public static getIndentation(
    ...
    if (
      currentNode.kind() === SyntaxKind.StringLiteral ||
      currentNode.kind() === SyntaxKind.RegularExpressionLiteral
    ) {
      return indentation;
    }
    ...
    while (parent !== null) {
      ...
      if (parent.fullStart() !== currentNode.fullStart()) {
        if (Indenter.isInContainerNode(parent.element(), currentElement)) {
          ...
          var listIndentation = Indenter.getCustomListIndentation(
            parent.element(),
            currentElement
          );
  ...
  private static belongsToBracket(
  ...
  private static isInContainerNode(
  ...
  private static getCustomListIndentation(
    list: ISyntaxElement,
    element: ISyntaxElement
  ): number {
    switch (list.kind()) {
      ...
      case SyntaxKind.ArgumentList:
        ...
        // MD: Renamed from arguments to argumentListArgs to avoid conflict with arguments keyword
        var argumentListArgs = argumentList.args;
        if (
          argumentListArgs !== null &&
          argumentList.closeParenToken === element
        ) {
          return Indenter.getListItemIndentation(
            argumentListArgs,
            argumentListArgs.childCount() - 1
          );
        }
        break;
      ...
      case SyntaxKind.TypeArgumentList:
        ...
        if (
          typeArguments !== null &&
          typeArgumentList.greaterThanToken === element
        ) {
          return Indenter.getListItemIndentation(
            typeArguments,
            typeArguments.childCount() - 1
          );
        }
        break;
      ...
      case SyntaxKind.TypeParameterList:
        ...
        if (
          typeParameters !== null &&
          typeParameterList.greaterThanToken === element
        ) {
          return Indenter.getListItemIndentation(
            typeParameters,
            typeParameters.childCount() - 1
          );
        }
        break;
    }
  ...
  private static getListItemIndentation(
    list: ISyntaxElement,
    elementIndex: number
  ): number {
    for (var i = elementIndex; i > 0; i--) {
      ...
      if (
        (child !== null && child.leadingTrivia().hasNewLine()) ||
