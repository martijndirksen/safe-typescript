...
import { ISimpleText } from '../../compiler/text/text';
import { TextSpan } from '../../compiler/text/textSpan';
import { ITextSnapshotLine, TextSnapshotLine } from './textSnapshotLine';
...
export class TextSnapshot implements ITextSnapshot {
  ...
  private getLineFromLineNumberWorker(lineNumber: number): ITextSnapshotLine {
    ...
    if (lineMapIndex < 0 || lineMapIndex >= lineMap.length)
      throw new Error(
        getDiagnosticMessage(DiagnosticCode.Invalid_line_number_0, [
          lineMapIndex.toString(),
        ])
      );
    ...
    if (lineMapIndex == lineMap.length) {
      ...
      endIncludingLineBreak =
        lineMapIndex >= lineMap.length - 1
          ? this.snapshot.length()
          : lineMap[lineMapIndex + 1];
