  ...
  export class MultipleTokenIndenter extends IndentationTrackingWalker {
    ...
    public indentToken(
      ...
      var tokenSpan = new TextSpan(
        this.position() + token.leadingTriviaWidth(),
        token.width()
      );
      ...
      var commentIndentationString = Indentation.indentationString(
        commentIndentationAmount,
        this.options
      );

      // Record any needed indentation edits
      this.recordIndentationEditsForToken(
        token,
        indentationString,
        commentIndentationString
      );
    }
    ...
    private recordIndentationEditsForToken(
      token: ISyntaxToken,
      indentationString: string,
      commentIndentationString: string
    ) {
      ...
      if (triviaList) {
        for (
          var i = 0, length = triviaList.count();
          i < length;
          i++, position += trivia.fullWidth()
        ) {
          var trivia = triviaList.syntaxTriviaAt(i);
          // Skip this trivia if it is not in the span
          if (
            !this.textSpan().containsTextSpan(
              new TextSpan(position, trivia.fullWidth())
            )
          ) {
            continue;
          }
          ...
          switch (trivia.kind()) {
            case SyntaxKind.MultiLineCommentTrivia:
              ...
              this.recordIndentationEditsForMultiLineComment(
                ...
                commentIndentationString,
                leadingWhiteSpace,
                !indentNextTokenOrTrivia /* already indented first line */
              );
            ...
            case SyntaxKind.WhitespaceTrivia:
              ...
              var nextTrivia =
                length > i + 1 && triviaList.syntaxTriviaAt(i + 1);
              var whiteSpaceIndentationString =
                nextTrivia && nextTrivia.isComment()
                  ? commentIndentationString
                  : indentationString;
              if (indentNextTokenOrTrivia) {
                if (!(nextTrivia && nextTrivia.isNewLine())) {
                  this.recordIndentationEditsForWhitespace(
                    trivia,
                    position,
                    whiteSpaceIndentationString
                  );
                }
                indentNextTokenOrTrivia = false;
              }
    ...
    private recordIndentationEditsForSingleLineOrSkippedText(
    ...
    private recordIndentationEditsForWhitespace(
      trivia: ISyntaxTrivia,
      fullStart: number,
      indentationString: string
    ): void {
    ...
    private recordIndentationEditsForMultiLineComment(
      trivia: ISyntaxTrivia,
      fullStart: number,
      indentationString: string,
      leadingWhiteSpace: string,
      firstLineAlreadyIndented: boolean
    ): void {
      ...
      var segments =
        Syntax.splitMultiLineCommentTriviaIntoMultipleLines(trivia);
      ...
      var whiteSpaceColumnsInFirstSegment =
      ...
      for (var i = startIndex; i < segments.length; i++) {
        ...
        this.recordIndentationEditsForSegment(
          segment,
          position,
          indentationColumns,
          whiteSpaceColumnsInFirstSegment
        );
        position += segment.length;
      }
    }

    private recordIndentationEditsForSegment(
      segment: string,
      fullStart: number,
      ...
      var deltaFromFirstSegment =
        leadingWhiteSpaceColumns - whiteSpaceColumnsInFirstSegment;
