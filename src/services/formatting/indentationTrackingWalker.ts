...
module TypeScript.Services.Formatting {
  export class IndentationTrackingWalker extends SyntaxWalker {
    ...
    constructor(
      ...
      public options: IFormattingOptions
    ) {
      super();
    ...
    public visitToken(token: ISyntaxToken): void {
      ...
      if (tokenSpan.intersectsWithTextSpan(this._textSpan)) {
        ...
        this._lastTriviaWasNewLine =
          ...
          trivia.syntaxTriviaAt(trivia.count() - 1).kind() ==
    ...
    public visitNode(node: SyntaxNode): void {
      ...
      if (nodeSpan.intersectsWithTextSpan(this._textSpan)) {
        ...
        this._parent = this._indentationNodeContextPool.getNode(
          ...
          indentation.indentationAmountDelta
        );
    ...
    private getTokenIndentationAmount(token: ISyntaxToken): number {
      ...
      if (
        this._parent.node().firstToken() === token ||
        ...
        (token.kind() === SyntaxKind.WhileKeyword &&
          this._parent.node().kind() == SyntaxKind.DoStatement)
      ) {
    ...
    private getNodeIndentation(
      node: SyntaxNode,
      newLineInsertedByFormatting?: boolean
    ): { indentationAmount: number; indentationAmountDelta: number } {
      ...
      var parentIndentationAmountDelta =
        this._parent.childIndentationAmountDelta();
      ...
      switch (node.kind()) {
        default:
          ...
          indentationAmount =
            parentIndentationAmount + parentIndentationAmountDelta;
      ...
      if (parent) {
        if (
          !newLineInsertedByFormatting /*This could be false or undefined here*/
        ) {
          var parentStartLine = this._snapshot.getLineNumberFromPosition(
            this._parent.start()
          );
          var currentNodeStartLine = this._snapshot.getLineNumberFromPosition(
            this._position + node.leadingTriviaWidth()
          );
          if (
            parentStartLine === currentNodeStartLine ||
            newLineInsertedByFormatting ===
              false /*meaning a new line was removed and we are force recomputing*/
          ) {
            indentationAmount = parentIndentationAmount;
            indentationAmountDelta = Math.min(
    ...
    private forceRecomputeIndentationOfParent(
      tokenStart: number,
      newLineAdded: boolean /*as opposed to removed*/
    ): void {
      var parent = this._parent;
      if (parent.fullStart() === tokenStart) {
        // Temporarily pop the parent before recomputing
        this._parent = parent.parent();
        var indentation = this.getNodeIndentation(
          parent.node(),
          /* newLineInsertedByFormatting */ newLineAdded
        );
        parent.update(
          parent.parent(),
          parent.node(),
