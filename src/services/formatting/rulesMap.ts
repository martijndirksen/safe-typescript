...
export class RulesMap {
  ...
  public Initialize(rules: Rule[]) {
    ...
    var rulesBucketConstructionStateList: RulesBucketConstructionState[] = <
      any
    >new Array(this.map.length); //new Array<RulesBucketConstructionState>(this.map.length);
  ...
  public GetRule(context: FormattingContext): Rule {
    var bucketIndex = this.GetRuleBucketIndex(
    ...
    if (bucket != null) {
      for (var i = 0, len = bucket.Rules().length; i < len; i++) {
        ...
        if (rule.Operation.Context.InContext(context)) return rule;
      }
    }
    return null;
  }
...
export class RulesBucket {
  ...
  public AddRule(
    ...
    constructionState: RulesBucketConstructionState[],
    ...
    if (state === undefined) {
      state = constructionState[rulesBucketIndex] =
        new RulesBucketConstructionState();
    }
