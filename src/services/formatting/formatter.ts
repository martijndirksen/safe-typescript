...
module TypeScript.Services.Formatting {
  export class Formatter extends MultipleTokenIndenter {
    ...
    private processToken(token: ISyntaxToken): void {
      ...
      // Push the token
      var currentTokenSpan = new TokenSpan(
        token.kind(),
        position,
        token.width()
      );
      if (!this.parent().hasSkippedOrMissingTokenChild()) {
        if (this.previousTokenSpan) {
          ...
          this.trimWhitespaceInLineRange(
            ...
            this.getLineNumber(currentTokenSpan)
          );
        }
      }
        ...
        if (trivia.isComment() || trivia.isSkippedToken()) {
          ...
          if (this.textSpan().containsTextSpan(currentTokenSpan)) {
            if (trivia.isComment() && this.previousTokenSpan) {
              ...
              // We still want to trim whitespace even if it is the first trivia of the first token. Trim from the beginning of the span to the trivia
              var startLine = this.getLineNumber(
    ...
    private findCommonParents(
      parent1: IndentationNodeContext,
      parent2: IndentationNodeContext
      ...
      Debug.assert(
        deepParentDepth >= shallowParentDepth,
    ...
    private formatPair(
      t1: TokenSpan,
      t1Parent: IndentationNodeContext,
      t2: TokenSpan,
      t2Parent: IndentationNodeContext
    ): void {
      ...
      // Update the context
      this.formattingContext.updateContext(
        t1,
        t1Parent,
        t2,
        t2Parent,
      ...
      if (rule != null) {
        ...
        if (
          (rule.Operation.Action == RuleAction.Space ||
            rule.Operation.Action == RuleAction.Delete) &&
        ...
        if (
          rule.Operation.Action == RuleAction.NewLine &&
          token1Line == token2Line
        ) {
    ...
    private trimWhitespaceInLineRange(
      startLine: number,
      endLine: number,
      token?: TokenSpan
    ): void {
    ...
    private trimWhitespace(line: ITextSnapshotLine, token?: TokenSpan): void {
      ...
      if (
        ...
        token.start() <= line.endPosition() &&
        token.end() >= line.endPosition()
      )
        return;
        ...
        case RuleAction.NewLine:
          {
            if (
              !(
                rule.Flag == RuleFlags.CanDeleteNewLines ||
                this.getLineNumber(t1) == this.getLineNumber(t2)
              )
            ) {
              return;
            }
            ...
            var lineFeedLoc = betweenText.indexOf(
            ...
            if (lineFeedLoc < 0) {
              ...
              lineFeedLoc = betweenText.indexOf(
                this.options.newLineCharacter,
                lineFeedLoc + 1
              );
              if (lineFeedLoc >= 0) {
        ...
        case RuleAction.Space:
          {
            ...
            if (
              betweenSpan.length() > 1 ||
              this.snapshot().getText(betweenSpan) != ' '
            ) {
              this.recordEdit(betweenSpan.start(), betweenSpan.length(), ' ');
              return;
            }
