...
export class FormattingContext {
  ...
  public TokensAreOnSameLine(): boolean {
    if (this.tokensAreOnSameLine === null) {
      var startLine = this.snapshot.getLineNumberFromPosition(
        this.currentTokenSpan.start()
      );
      var endLine = this.snapshot.getLineNumberFromPosition(
  ...
  public BlockIsOnOneLine(node: IndentationNodeContext): boolean {
    ...
    var openBracePosition = start + childOffset(block, block.openBraceToken);
    var closeBracePosition = start + childOffset(block, block.closeBraceToken);
