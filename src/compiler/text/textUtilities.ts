...
export function parseLineStarts(text: ICharacterSequence): number[] {
  ...
  while (index < length) {
    ...
    if (c > CharacterCodes.carriageReturn && c <= 127) {
      ...
      index + 1 < length &&
      text.charCodeAt(index + 1) === CharacterCodes.lineFeed
    ) {
      lineBreakLength = 2;
...
export function getLengthOfLineBreakSlow(
  text: ICharacterSequence,
  index: number,
  c: number
): number {
  if (c === CharacterCodes.carriageReturn) {
    ...
    return next < text.length &&
      CharacterCodes.lineFeed === text.charCodeAt(next)
      ? 2
      : 1;
...
export function getLengthOfLineBreak(
  text: ICharacterSequence,
  index: number
): number {
  var c = text.charCodeAt(index);
