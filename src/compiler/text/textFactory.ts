module TypeScript.TextFactory {
  ...
  function getStartAndLengthOfLineBreakEndingAt(
    ...
    if (c === CharacterCodes.lineFeed) {
      if (
        index > 0 &&
        text.charCodeAt(index - 1) === CharacterCodes.carriageReturn
      ) {
  ...
  class TextBase implements IText {
    ...
    checkSubSpan(span: TextSpan): void {
      if (
        span.start() < 0 ||
        span.start() > this.length() ||
        span.end() > this.length()
      ) {
    ...
    public getLineFromLineNumber(lineNumber: number): ITextLine {
      ...
      if (lineNumber === lineStarts.length - 1) {
        return new TextLine(
          this,
          new TextSpan(first, this.length() - first),
          0,
          lineNumber
        );
      } else {
        getStartAndLengthOfLineBreakEndingAt(
          this,
        ...
        return new TextLine(
          this,
          new TextSpan(first, this.linebreakInfo.startPosition - first),
    ...
    public getLineNumberFromPosition(position: number): number {
      ...
      var lineNumber = ArrayUtilities.binarySearch(
        this._lineStarts(),
        position
      );
      if (lineNumber < 0) {
    ...
    public getLinePosition(position: number): LineAndCharacter {
      ...
      return new LineAndCharacter(
        lineNumber,
        position - this._lineStarts()[lineNumber]
      );
    }
  }
  ...
  class SubText extends TextBase {
    ...
    public subText(span: TextSpan): IText {
      ...
      return new SubText(
        this.text,
        this.getCompositeSpan(span.start(), span.length())
      );
    }
...
module TypeScript.SimpleText {
  ...
  class SimpleStringText implements ISimpleText {
    ...
    private static charArray: number[] = ArrayUtilities.createArray<number>(
      1024,
      0
    );
    ...
    public substr(start: number, length: number, intern: boolean): string {
      if (intern) {
        ...
        var array: number[] =
          length <= SimpleStringText.charArray.length
  ...
  class SimpleScriptSnapshotText implements ISimpleText {
    ...
    public lineMap(): LineMap {
      return new LineMap(
        () => this.scriptSnapshot.getLineStartPositions(),
        this.length()
      );
    }
  }
  ...
  export function fromScriptSnapshot(
