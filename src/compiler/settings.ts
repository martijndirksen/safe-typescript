...
import { DiagnosticInfo } from './core/diagnosticInfo';
...
export class CompilationSettings {
  ...
  public codepage: number | null = null;
...
export class ImmutableCompilationSettings {
  ...
  public allowAutomaticSemicolonInsertion() {
    return this._allowAutomaticSemicolonInsertion;
  }
  public noImplicitAny() {
  ...
  public generateDeclarationFiles() {
    return this._generateDeclarationFiles;
  }
  public useCaseSensitiveFileResolution() {
  ...
  public shouldWarn(diag: Diagnostic, di: DiagnosticInfo) {
    ...
    const almostMatches = this._knownWarnings.filter(
      (kw) =>
        kw2.fileName.toLowerCase().endsWith(kw.fileName.toLowerCase()) &&
        kw.column === kw2.column &&
        kw.code == kw2.code
    );

    if (
      almostMatches.some((kw) => {
        if (kw.line === kw2.line) {
          kw.matched = true;
    ...
    if (almostMatches.length !== 0) {
      var am = almostMatches.sort(
        (k, l) => Math.abs(k.line - kw2.line) - Math.abs(l.line - kw2.line)
      );
      am = am.filter((kw) => !kw.matched);
      if (am && am.length !== 0) {
        console.log(
          'Warning at ' + kw2str(kw2) + ' almost matched known warnings: '
        );
  ...
  public reportUnmatchedKnownWarnings(
    ...
    var sorted = this.matchedOrAlmost.sort(
      (kw1, kw2) => kw1?.category?.localeCompare(kw2.category ?? '') ?? 0
    );
    var currentCategory: string | undefined = undefined;
    var contents = sorted
      .map((kw) => {
        var out = '';
  ...
  public static defaultSettings() {
    if (!ImmutableCompilationSettings._defaultSettings) {
      ImmutableCompilationSettings._defaultSettings =
        ImmutableCompilationSettings.fromCompilationSettings(
  ...
  public static fromCompilationSettings(
  ...
  public toCompilationSettings(): any {
    ...
    for (var name in this) {
      if (this.hasOwnProperty(name) && name.startsWith('_')) {
        result[name.slice(1)] = this[name];
      }
    }
