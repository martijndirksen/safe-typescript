import { IASTSpan, AST } from './ast';
import { Base64VLQFormat } from './base64';
import { Debug } from './core/debug';
import { TextWriter } from './declarationEmitter';
import { Document } from './document';
import { EmitOptions } from './emitter';
import {
  getPrettyName,
...
export class SourceMapper {
  ...
  public decreaseMappingLevel(ast: IASTSpan) {
    ...
    var expectedAstInfo: any =
      expectedAst instanceof AST
        ? SyntaxKind[(<AST>expectedAst).kind()]
        : [expectedAst.start(), expectedAst.end()];
    var astInfo: any =
      ast instanceof AST
        ? SyntaxKind[(<AST>ast).kind()]
        : [ast.start(), ast.end()];
    Debug.assert(
      ast === expectedAst,
    ...
    if (emitOptions.sourceMapRootDirectory()) {
      ...
      if (document.emitToOwnOutputFile()) {
        ...
        this.sourceMapDirectory =
          this.sourceMapDirectory +
          switchToForwardSlashes(
            getRootFilePath(document.fileName).replace(
      ...
      if (isRelative(this.sourceMapDirectory)) {
        ...
        this.sourceMapDirectory =
          emitOptions.commonDirectoryPath() + this.sourceMapDirectory;
        this.sourceMapDirectory = convertToDirectoryPath(
          switchToForwardSlashes(resolvePath(this.sourceMapDirectory))
        );
        this.sourceMapPath = getRelativePathToFixedPath(
          getRootFilePath(jsFilePath),
          this.sourceMapDirectory + prettyMapFileName
        );
      } else {
  ...
  private setNewSourceFilePath(document: Document, emitOptions: EmitOptions) {
    ...
    if (emitOptions.sourceRootDirectory()) {
      ...
      tsFilePath = getRelativePathToFixedPath(
        emitOptions.commonDirectoryPath(),
        tsFilePath
      );
    } else {
  ...
  public emitSourceMapping(): void {
    ...
    for (
      var sourceIndex = 0;
      sourceIndex < this.tsFilePaths.length;
      sourceIndex++
    ) {
      var recordSourceMapping = (
        mappedPosition: SourceMapPosition,
        nameIndex: number
      ) => {
        if (
        ...
        mappingsString =
          mappingsString +
          Base64VLQFormat.encode(
            mappedPosition.emittedColumn - prevEmittedColumn
          );
        ...
        mappingsString =
          mappingsString +
          Base64VLQFormat.encode(sourceIndex - prevSourceIndex);
        ...
        mappingsString =
          mappingsString +
          Base64VLQFormat.encode(
            mappedPosition.sourceLine - 1 - prevSourceLine
          );
        ...
        mappingsString =
          mappingsString +
          Base64VLQFormat.encode(
            mappedPosition.sourceColumn - prevSourceColumn
          );
        ...
        if (nameIndex >= 0) {
          mappingsString =
            mappingsString + Base64VLQFormat.encode(nameIndex - prevNameIndex);
