...
import { integerMultiplyLow32Bits } from './integerUtilities';
...
export class Hash implements RT.Virtual {
  ...
  private static computeFnv1aCharArrayHashCode(
    text: number[],
    start: number,
    len: number
  ): number {
    ...
    for (var i = start; i < end; i++) {
      hashCode = integerMultiplyLow32Bits(hashCode ^ text[i], Hash.FNV_PRIME);
    }

    return hashCode;
  }

  public static computeSimple31BitCharArrayHashCode(
    key: number[],
    start: number,
    len: number
  ): number {
  ...
  public static computeMurmur2StringHashCode(
    key: string,
    seed: number
  ): number {
    ...
    while (numberOfCharsLeft >= 2) {
      ...
      k = integerMultiplyLow32Bits(k, m);
      k ^= k >> r;
      k = integerMultiplyLow32Bits(k, m);

      h = integerMultiplyLow32Bits(h, m);
      h ^= k;
