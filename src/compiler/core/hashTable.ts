module TypeScript.Collections {
  ...
  export class HashTable<TKey, TValue> {
    ...
    constructor(
      ...
      this.entries = ArrayUtilities.createArray<HashTableEntry<TKey, TValue>>(
        size,
        null
      );
    }
    ...
    private computeHashCode(key: TKey): number {
      var hashCode: number =
        this.hash === null ? (<any>key).hashCode : this.hash(key);
    ...
    private grow(): void {
      ...
      var newEntries: HashTableEntry<TKey, TValue>[] =
        ArrayUtilities.createArray<HashTableEntry<TKey, TValue>>(newSize, null);
