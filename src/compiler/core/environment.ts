    ...
    return {
      ...
      readFile(path: string, codepage: number): FileInformation {
        try {
          ...
          if (
            bomChar.charCodeAt(0) === 0xfe &&
            bomChar.charCodeAt(1) === 0xff
          ) {
            streamObj.Charset = 'unicode';
            byteOrderMark = ByteOrderMark.Utf16BigEndian;
          } else if (
            bomChar.charCodeAt(0) === 0xff &&
            bomChar.charCodeAt(1) === 0xfe
          ) {
            streamObj.Charset = 'unicode';
            byteOrderMark = ByteOrderMark.Utf16LittleEndian;
          } else if (
            bomChar.charCodeAt(0) === 0xef &&
            bomChar.charCodeAt(1) === 0xbb
          ) {
          ...
          // Read the whole file
          var contents = streamObj.ReadText(
            -1 /* read from the current position to EOS */
          );
          streamObj.Close();
          ...
          if (err.number === -2147024809) {
            message = TypeScript.getDiagnosticMessage(
              TypeScript.DiagnosticCode.Unsupported_file_encoding,
              null
            );
          } else {
            message = TypeScript.getDiagnosticMessage(
              TypeScript.DiagnosticCode.Cannot_read_file_0_1,
              [path, '' + err.message]
            );
          }
        ...
        function filesInFolder(folder: any, root: string): string[] {
          ...
          if (options.recursive) {
            ...
            for (; !fc.atEnd(); fc.moveNext()) {
              paths = paths.concat(
                filesInFolder(fc.item(), root + '\\' + fc.item().Name)
              );
            }
          }
  ...
  function getNodeEnvironment(): IEnvironment {
    ...
    return {
      ...
      readFile(file: string, codepage: number): FileInformation {
        if (codepage !== null) {
          throw new Error(
            TypeScript.getDiagnosticMessage(
        ...
        try {
          for (var index = 0; index < contents.length; index += chunkLength) {
            ...
            var buffer = new Buffer(
              contents.substr(index, chunkLength),
              'utf8'
            );
            TypeScript.nodeCreateBufferTime +=
              new Date().getTime() - bufferStart;
