  ...
  export var IO = (function () {
    ...
    function getWindowsScriptHostIO(): IIO {
      ...
      return {
        ...
        findFile(rootPath: string, partialFilePath: string): IFindFileResult {
          ...
          while (true) {
            if (fso.FileExists(path)) {
              ...
              rootPath = fso.GetParentFolderName(
          ...
          try {
            ...
            IOUtils.throwIOError(
              TypeScript.getDiagnosticMessage(
                TypeScript.DiagnosticCode.Could_not_delete_file_0,
                [path]
              ),
              e
            );
        ...
        watchFile: <
          (fileName: string, callback: (x: string) => void) => IFileWatcher
        >null,
        run(source, fileName) {
          try {
            eval(source);
            ...
            IOUtils.throwIOError(
              TypeScript.getDiagnosticMessage(
                TypeScript.DiagnosticCode.Error_while_executing_file_0,
                [fileName]
              ),
              e
            );
