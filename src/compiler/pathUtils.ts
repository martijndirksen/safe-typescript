...
module TypeScript {
  export function stripStartAndEndQuotes(str: string) {
    ...
    if (
      str &&
      str.length >= 2 &&
      firstCharCode === str.charCodeAt(str.length - 1) &&
      (firstCharCode === CharacterCodes.singleQuote ||
        firstCharCode === CharacterCodes.doubleQuote)
    ) {
  ...
  export function isSingleQuoted(str: string) {
    return (
      ...
      str.charCodeAt(0) === CharacterCodes.singleQuote
    );
  }
  ...
  export function isDoubleQuoted(str: string) {
    return (
      ...
      str.length >= 2 &&
      str.charCodeAt(0) === str.charCodeAt(str.length - 1) &&
      str.charCodeAt(0) === CharacterCodes.doubleQuote
    );
  }
  ...
  export function trimModName(modName: string) {
    ...
    if (
      modName.length > 5 &&
      modName.substring(modName.length - 5, modName.length) === '.d.ts'
    ) {
      return modName.substring(0, modName.length - 5);
    }
    if (
      modName.length > 3 &&
      modName.substring(modName.length - 3, modName.length) === '.ts'
    ) {
    ...
    if (
      modName.length > 3 &&
      modName.substring(modName.length - 3, modName.length) === '.js'
    ) {
      return modName.substring(0, modName.length - 3);
    }
  ...
  function isFileOfExtension(fname: string, ext: string) {
    ...
    return (
      invariantFname.length > extLength &&
      invariantFname.substring(
        invariantFname.length - extLength,
        invariantFname.length
      ) === invariantExt
    );
  }
  ...
  export function getPrettyName(
    ...
    var modName = treatAsFileName
      ? switchToForwardSlashes(modPath)
      : trimModName(stripStartAndEndQuotes(modPath));
    var components = getPathComponents(modName);
    return components.length
      ? quote
        ? quoteStr(components[components.length - 1])
  ...
  export function getRelativePathToFixedPath(
    fixedModFilePath: string,
    absoluteModPath: string,
    isAbsoultePathURL = true
  ) {
    ...
    for (
      ;
      joinStartIndex < modComponents.length &&
      joinStartIndex < fixedModComponents.length;
      joinStartIndex++
    ) {
      if (
        fixedModComponents[joinStartIndex] !== modComponents[joinStartIndex]
      ) {
        break;
      }
    }
    ...
    if (joinStartIndex !== 0) {
      var relativePath = '';
      var relativePathComponents = modComponents.slice(
        joinStartIndex,
        modComponents.length
      );
  ...
  export function normalizePath(path: string): string {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      if (
        ...
        ArrayUtilities.last(normalizedParts) !== '..' &&
        part === '..'
      ) {
        normalizedParts.pop();
        continue;
