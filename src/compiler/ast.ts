...
export class SoundType {
  ...
  public isIndexable() {
    //Will be true for arrays and index-maps
    return false;
  }
  public toRTTI(): AST {
    //generate code for building the runtime rep. of this type
...
export function structuralEqualsNotIncludingPosition(
  ast1: AST,
  ast2: AST
): boolean {
...
export function structuralEqualsIncludingPosition(
  ast1: AST,
  ast2: AST
): boolean {
...
function commentStructuralEqualsNotIncludingPosition(
  ast1: Comment,
  ast2: Comment
): boolean {
...
function commentStructuralEqualsIncludingPosition(
  ast1: Comment,
  ast2: Comment
): boolean {
...
function commentStructuralEquals(
  ast1: Comment,
  ast2: Comment,
  includingPosition: boolean
): boolean {
...
function astArrayStructuralEquals(
  array1: AST[],
  array2: AST[],
  includingPosition: boolean
): boolean {
  return ArrayUtilities.sequenceEquals(
    array1,
    array2,
    includingPosition
...
function commentArrayStructuralEquals(
  array1: Comment[],
  array2: Comment[],
  includingPosition: boolean
): boolean {
  return ArrayUtilities.sequenceEquals(
    ...
    array2,
    includingPosition
      ? commentStructuralEqualsIncludingPosition
      : commentStructuralEqualsNotIncludingPosition
  );
}
  ...
  public structuralEquals(ast: AST, includingPosition: boolean): boolean {
    return (
      ...
      (ast instanceof Identifier
        ? this._text === (<Identifier>ast)._text
        : false)
    );
  }
}
      ...
      if (ast instanceof NumericLiteral) {
        ...
        return (
          (this._value === n._value ||
            (isNaN(this._value) && isNaN(n._value))) &&
          this._text === n._text
        );
      }
      return false;
...
export class StringLiteral extends AST implements IASTToken {
  ...
  public structuralEquals(
    ...
    return (
      super.structuralEquals(ast, includingPosition) && this._text === ast._text
    );
  }
}
...
export class ArrayLiteralExpression extends AST {
  constructor(
    ...
    public newLineMode: NewLineMode = NewLineMode.None
  ) {
    super();
...
export class ArgumentList extends AST {
  constructor(
    public typeArgumentList: TypeArgumentList,
    public args: ISeparatedSyntaxList2,
...
export function diagnosticFromDecl(
  decl: PullDecl,
  ...
  return new Diagnostic(
    decl.fileName(),
    decl.semanticInfoChain().lineMap(decl.fileName()),
    span.start(),
    span.length(),
    diagnosticKey,
    args
