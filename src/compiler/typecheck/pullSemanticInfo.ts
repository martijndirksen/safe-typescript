...
import { ILogger } from '../diagnostics';
import { Document } from '../document';
import { hasFlag } from '../flags';
import { createIntrinsicsObject, IIndexable } from '../hashTable';
import { normalizePath, isDTSFile } from '../pathUtils';
...
import { PullSymbolBinder } from './pullSymbolBinder';
import {
...
export class SemanticInfoChain {
  ...
  constructor(
    ...
    var globalDecl = new RootPullDecl(
      /*name:*/ '',
      /*fileName:*/ '',
      PullElementKind.Global,
      PullElementFlags.None,
      span,
      this,
      /*isExternalModule:*/ false
    );
    this.documents[0] = new Document(
      this.compiler,
      this,
      /*fileName:*/ '',
      /*referencedFiles:*/ [],
    ...
    this.anyTypeDecl = new NormalPullDecl(
      'any',
      'any',
    ...
    this.booleanTypeDecl = new NormalPullDecl(
      'boolean',
      'boolean',
      PullElementKind.Primitive,
      PullElementFlags.None,
    ...
    this.numberTypeDecl = new NormalPullDecl(
      'number',
      'number',
      PullElementKind.Primitive,
      PullElementFlags.None,
    ...
    this.stringTypeDecl = new NormalPullDecl(
      'string',
      'string',
      PullElementKind.Primitive,
      PullElementFlags.None,
    ...
    this.voidTypeDecl = new NormalPullDecl(
      'void',
      'void',
      PullElementKind.Primitive,
      PullElementFlags.None,
    ...
    this.nullTypeDecl = new RootPullDecl(
      'null',
      '',
      PullElementKind.Primitive,
      PullElementFlags.None,
    ...
    this.undefinedTypeDecl = new RootPullDecl(
      'undefined',
      '',
      PullElementKind.Primitive,
      PullElementFlags.None,
    ...
    this.undefinedValueDecl = new NormalPullDecl(
      'undefined',
      'undefined',
  ...
  private bindPrimitiveSymbol(
    decl: PullDecl,
    newSymbol: PullSymbol
  ): PullSymbol {
  ...
  private addPrimitiveValueSymbol(
    decl: PullDecl,
    type: PullTypeSymbol
  ): PullSymbol {
  ...
  private resetGlobalSymbols(): void {
    ...
    var span = new TextSpan(0, 0);
    var emptyTypeDecl = new PullSynthesizedDecl(
      '{}',
      '{}',
  ...
  public addDocument(document: Document): void {
    ...
    var existingIndex = ArrayUtilities.indexOf(
      this.documents,
  ...
  public removeDocument(fileName: string): void {
    ...
    var index = ArrayUtilities.indexOf(
      this.documents,
      (u) => u.fileName === fileName
    );
    if (index > 0) {
  ...
  public findTopLevelSymbol(
    name: string,
    kind: PullElementKind,
    doNotGoPastThisDecl: PullDecl
  ): PullSymbol {
    ...
    if (!symbol) {
      for (var i = 0, n = this.documents.length; i < n; i++) {
        ...
        var symbol = this.findTopLevelSymbolInDecl(
          topLevelDecl,
          name,
          kind,
          doNotGoPastThisDecl
        ...
        if (
          doNotGoPastThisDecl &&
          topLevelDecl.name == doNotGoPastThisDecl.fileName()
        ) {
          return null;
        }
      }
  ...
  private findTopLevelSymbolInDecl(
    ...
    var doNotGoPastThisPosition =
      doNotGoPastThisDecl &&
      doNotGoPastThisDecl.fileName() === topLevelDecl.fileName()
  ...
  public findExternalModule(id: string) {
    ...
    var dtsFile = id + '.d.ts';
    var dtsCacheID = this.getDeclPathCacheID(
      [dtsFile],
    ...
    var tsFile = id + '.ts';
    var tsCacheID = this.getDeclPathCacheID(
      [tsFile],
      PullElementKind.DynamicModule
    );
  ...
  public findAmbientExternalModuleInGlobalContext(id: string) {
    ...
    if (symbol == undefined) {
      ...
      for (var i = 0; i < this.documents.length; i++) {
        ...
        if (!topLevelDecl.isExternalModule()) {
          var dynamicModules = topLevelDecl.searchChildDecls(
            id,
            PullElementKind.DynamicModule
          );
  ...
  public findDecls(declPath: string[], declKind: PullElementKind): PullDecl[] {
    ...
    var keepSearching =
      declKind & PullElementKind.SomeContainer ||
    ...
    for (var i = 0; i < declPath.length; i++) {
      ...
      for (var j = 0; j < declsToSearch.length; j++) {
        ...
        foundDecls = declsToSearch[j].searchChildDecls(
          path,
          i === declPath.length - 1 ? declKind : PullElementKind.Container
        );
  ...
  public invalidate(
    oldSettings: ImmutableCompilationSettings = null,
    newSettings: ImmutableCompilationSettings = null
  ) {
  ...
  private settingsChangeAffectsSyntax(
    before: ImmutableCompilationSettings,
    after: ImmutableCompilationSettings
  ): boolean {
    ...
    // stored in the AST.
    return (
      before.allowAutomaticSemicolonInsertion() !==
        after.allowAutomaticSemicolonInsertion() ||
  ...
  public getCallResolutionDataForAST(
    ast: AST
  ): PullAdditionalCallResolutionData {
    return this.astCallResolutionDataMap[ast.syntaxID()];
  }

  public setCallResolutionDataForAST(
    ast: AST,
  ...
  public setSignatureSymbolForDecl(
    decl: PullDecl,
  ...
  public getResolver(): PullTypeResolver {
    if (!this._resolver) {
      this._resolver = new PullTypeResolver(
  ...
  public addSyntheticIndexSignature(
    ...
    var indexParameterSymbol = new PullSymbol(
      indexParamName,
    ...
    var indexSigDecl = new PullSynthesizedDecl(
      '',
      '',
      PullElementKind.IndexSignature,
      ...
      containingDecl,
      span,
      containingDecl.semanticInfoChain()
    );
    var indexParamDecl = new PullSynthesizedDecl(
      indexParamName,
      indexParamName,
      PullElementKind.Parameter,
      PullElementFlags.None,
  ...
  public topLevelDecls(): PullDecl[] {
    if (!this._topLevelDecls) {
      this._topLevelDecls = ArrayUtilities.select(this.documents, (u) =>
        u.topLevelDecl()
      );
    }
