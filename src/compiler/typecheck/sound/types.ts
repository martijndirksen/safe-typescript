...
import { SoundType, AST, TypeName, NewLineMode } from '../../ast';
...
import { MkAST, Pair, TcUtil, pair } from './tcUtil';
import { TypeRelations } from './treln';

function substType(
  s: Pair<TVar, SoundType>[],
  descend: boolean
): (t: Pair<NamedType, AST>) => Pair<NamedType, AST> {
  return (t: Pair<NamedType, AST>) =>
    pair(<NamedType>t.fst.subst(s, descend), t.snd);
}
...
export class TConstant extends SoundType {
  ...
  public toRTTI(): AST {
    switch (this.typeName) {
      case TypeName.Any:
        return MkAST.fieldOfRT('Any');
      case TypeName.Number:
        return MkAST.fieldOfRT('Num');
      case TypeName.Bool:
        return MkAST.fieldOfRT('Bool');
      case TypeName.String:
        return MkAST.fieldOfRT('Str');
      case TypeName.Void:
        return MkAST.fieldOfRT('Void');
      case TypeName.Un:
        return MkAST.fieldOfRT('Un'); //only available in the RT for TS*
      case TypeName.Null:
        return MkAST.fieldOfRT('Null'); //only available in the RT for TS*
      default:
...
export class TIndexMap extends SoundType {
  //A pseudo-type, just like TArrow
  constructor(
    public indexType: SoundType,
    public elt: SoundType
  ) {
  ...
  public toRTTI(): AST {
    return MkAST.callExpr(MkAST.fieldOfRT('IndexMapType'), [
      this.indexType.toRTTI(),
      this.elt.toRTTI(),
    ]);
  }
  public equals(t: SoundType) {
...
var fieldToRtti = (f: Field) => {
  var field = [
    { fst: 'name', snd: MkAST.stringConst(f.name) },
    { fst: 'type', snd: f.type.toRTTI() },
  ];
  if (f.optional) {
...
export var toFieldTable = (flds: Field[]) =>
  MkAST.mkObjLit(flds.map((f) => pair(f.name, f.type.toRTTI())));
...
export class TArrow extends SoundType {
  ...
  public subst(s: Pair<TVar, SoundType>[], descend: boolean): SoundType {
    return new TArrow(
      ...
      this.result.subst(s, descend)
    );
  }
  public toRTTI(): AST {
    var mandatory = 0;
    var args = MkAST.mkCleanArray(
      this.args.map((e: TArg) => {
...
var methodToRtti = (m: Method) => {
  var field = [
    { fst: 'name', snd: MkAST.stringConst(m.name) },
    { fst: 'type', snd: m.type.toRTTI() },
  ];
  return MkAST.mkObjLit(field);
};
...
export class StructuredType extends SoundType {
  ...
  private typesMap: { [typeName: string]: Pair<SoundType, AST> } =
    RT.createEmptyMap<Pair<SoundType, AST>>(); //types map is for modules
  private fieldMap: { [fieldName: string]: Field[] } =
    RT.createEmptyMap<Field[]>(); //fieldMap and
  private methodMap: { [methodName: string]: Method[] } =
    RT.createEmptyMap<Method[]>(); //methodMap for fast lookup; arrays for overloading
  constructor(
    tn: TypeName,
  ...
  private lookupMethodOrField(
    map: MethodOrFieldMap,
    f: string,
    overload: number = 0
  ) {
    var x = map[f];
  ...
  private fieldsToString = (fields: Field[]) =>
    fields
      .map((f) => f.name + (f.optional ? '?' : '') + ' : ' + f.type.toString())
      .join('; ');
  public toString() {
    return (
      '{ FIELDS= ' +
...
export class JustType extends StructuredType {
  ...
  constructor(public repr: SoundType) {
    //TypeName is an enum in ast.ts
    super(TypeName.Just);
...
export class TRecord extends StructuredType {
  ...
  public toRTTI() {
    ...
    var r = MkAST.callExpr(MkAST.fieldOfRT('StructuredType'), [
      toMethodTable(this.exposeMethods()),
...
export function isFunctionType(t: SoundType) {
  return (
    t.typeName === TypeName.Record &&
    ((<TRecord>t).callSignature() ? true : false)
  );
}
export class NamedType extends StructuredType {
  ...
  constructor(
    ...
    if (!extendsC.every(isNamedType)) {
      throw new Error(
        'Got heritage from a non-named types : ' +
          extendsC.map((u) => u.toString()).join(', ')
      );
    }
    this.unfree =
      this.unfree && this.extendsC
        ? this.extendsC.every((t) => t.unFree())
        : true;
    this.unfree =
      this.unfree && this.implementsI
        ? this.implementsI.every((t) => t.unFree())
        : true;
  }
  public toRecord(forRTTI: boolean = false): TRecord {
    var r = this.toRecordAux(forRTTI);
    var findDups = (mfs: MethodOrField[], msg: string) => {
      var dups: { [name: string]: MethodOrField[] } =
        RT.createEmptyMap<MethodOrField[]>();
      var hasDup = false;
      mfs.forEach((mf1, i) => {
        if (!dups[mf1.name]) {
          mfs.forEach((mf2, j) => {
    ...
    if (this.extendsC) {
      ...
      for (var i = 0; i < this.extendsC.length; i++) {
        ...
        fields = fields.concat(
          supFields.filter((f) => fields.every((g) => g.name !== f.name))
        );
        methods = methods.concat(
          supMethods.filter((m) => methods.every((n) => m.name !== n.name))
        );
        supFields = [];
        supMethods = [];
      }
  ...
  public substBase(
    ...
    res.implementsI = this.implementsI.map((t: SoundType) =>
      t.subst(s, descend)
    );
    return res;
  }
...
export class TModule extends NamedType {
  ...
  public getMember(f: string, overload: number = 0, circularOk = false) {
    ...
    if (g && !circularOk && this.isCircular(f)) {
      ...
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Unsafe_circular_dependence_on_variable,
        [this.name + '.' + f, TcUtil.Logger.pos(md)]
      );
    }
    return g;
  }
...
export class TObject extends NamedType {
  ...
  public toRTTI() {
    ...
    return MkAST.callExpr(MkAST.fieldOfRT('InstanceType'), [
...
export class TInterface extends NamedType {
  ...
  constructor(
    ...
    super(TypeName.Interface, name, members, methods, [], extendsC, []);
  ...
  public toRTTI() {
    if (this.isArray()) {
      ...
      return MkAST.callExpr(MkAST.fieldOfRT('InterfaceType'), [
        MkAST.stringConst(this.name),
      ]);
    }
  }
...
export class TClass extends NamedType {
  ...
  public objectType() {
    ...
    switch (constr.typeName) {
      ...
      default:
        throw new Error(
          'Unexpected constructor type for class ' +
            this.name +
            ' :: ' +
            constr.toString()
        );
  ...
  public asBinder(fullName?: string[]): TcUtil.LocalTypeBinding {
    if (fullName && fullName.length !== 0) {
      return {
        fst: this.ppname,
        snd: <SoundType>(
          new TVar(TcUtil.mkTypeParamName(fullName, this.ppname), this.ppname)
        ),
      };
    }
...
export class TPoly extends SoundType {
  ...
  constructor(
    public bvars: TVar[],
    public body: SoundType,
    private qualifiedStubName?: string[]
  ) {
    super(TypeName.Poly);
  }
  public isVirtual() {
  ...
  private resolveStub() {
    if (this.qualifiedStubName) {
      ...
      if (t && t.typeName === TypeName.Poly) {
        ...
        throw new Error(
          'Could not resolve type ' + this.qualifiedStubName.join('.')
        );
      }
    }
    return null;
  ...
  public instantiate(args: SoundType[], descend: boolean = true): SoundType {
    ...
    var s: Pair<TVar, SoundType>[] = args.map((t, i) => ({
      fst: this.bvars[i],
      snd: t,
    }));
  ...
  public equals(t: SoundType) {
    if (t.typeName === this.typeName) {
      var q = <TPoly>t;
      return (
        this.body.equals(q.body) &&
        this.bvars.length === q.bvars.length &&
        this.bvars.every((t, i) => t.equals(q.bvars[i]))
      );
    }
    return false;
  }
...
export class TInst extends SoundType {
  constructor(
    public t1: TPoly,
    public args: SoundType[]
  ) {
    super(TypeName.Inst);
    if (this.t1.body.typeName === TypeName.Inst) {
      //collapse adjacent instantiations
  ...
  public subst(s: Pair<TVar, SoundType>[], descend: boolean) {
    return new TInst(
      ...
      this.args.map((t) => t.subst(s, false))
    );
  }
  public unFree() {
  ...
  public equals(t: SoundType) {
    if (t.typeName === this.typeName) {
      ...
      return (
        this.t1.equals(s.t1) &&
        this.args.length === s.args.length &&
        this.args.every((t, i) => t.equals(s.args[i]))
      );
    }
    var u = this.unfold();
