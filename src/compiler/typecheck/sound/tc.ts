...
import { isTypesOnlyLocation } from '../pullTypeResolution';
import { TcEnv, ScopeType } from './tcEnv';
import {
  TcUtil,
  MkAST,
  Pair,
...
import { TranslateTypes } from './translateTypes';
import { TypeRelations } from './treln';
import {
  TInterface,
  toMethodTable,
  toFieldTable,
...
export class SoundTypeChecker {
  ...
  public static buildSignature(
    ...
    if (SoundTypeChecker.globalEnv) {
      //restore the environment built from the previous files
  ...
  public static check(
    ...
    var checker = new SoundTypeChecker(
  ...
  constructor(
    ...
    this.rewriteSource =
      this.rewriteSource && !compilationSettings.noRuntimeChecks();
  }
  ...
  public emitBaseClassHierarchy(): AST[] {
    ...
    var emit = (tname: string, name: string): AST => {
      ...
      return new ExpressionStatement(
        MkAST.callRT('registerType', [
          MkAST.callRT('InterfaceRepr', [
  ...
  public emitInterfaceRegistration(ast: InterfaceDeclaration): AST {
    ...
    var reduceFn = (
      ...
      if (curr instanceof NamedType) {
        ...
        return nt.extendsC
          ? accum.concat(nt.extendsC.reduce(reduceFn, [curr]))
          : [curr];
      } else {
        accum.push(curr);
        return accum;
    ...
    return new ExpressionStatement(
      MkAST.callRT('registerType', [
        MkAST.callRT('InterfaceRepr', [
          ...
          MkAST.mkCleanArray(
            extendsList.map((t) => MkAST.stringConst(t.toString()))
          ),
  ...
  private tcWithResult(r: SoundType, a: AST) {
    ...
    try {
      ...
      switch (r.typeName) {
        ...
        case TypeName.Any:
        case TypeName.Un:
          break; // don't have to return something in these cases
        default:
          if (!res.snd) {
      ...
      //NS: probably not worth catching this exception here
      this.result_t = old;
      throw e;
    }
  }
  ...
  private tcaux(ast: AST): Pair<AST, boolean> {
    return TcUtil.Logger.withCurrentNode(ast, () => {
      ...
      if (TcUtil.Logger.reportWarnings) {
        ...
        if (ast.inferredType) {
          expected_t = TranslateTypes.translateTypeOrSig(
            ast.inferredType,
            this.tcenv
          );
        }
        if (expected_t) {
          var sc = TypeRelations.subTypeOrSig(
            res.fst.soundType,
            expected_t,
            TcUtil.allowDeepSubtyping(ast)
          );
          if (!sc.fst) {
            //can't shallow tag here, since AST may not be an expression form
            TcUtil.Logger.warn(
              'Inferred type incompatible for ' + kind2string(ast.kind())
            );
            TcUtil.Logger.error(
              DiagnosticCode.SEC_Inferred_type_incompatible_with_sound_type,
              [expected_t.toString(), res.fst.soundType.toString()],
              ast
            );
          }
        }
  ...
  public pkg(
    ...
    this.semanticInfoChain.setAliasSymbolForAST(
      checked,
      this.semanticInfoChain.getAliasSymbolForAST(orig)
    );
    ...
    switch (nodeType) {
      ...
      case SyntaxKind.FunctionType:
        try {
          ...
          TcUtil.Logger.error(
            DiagnosticCode.SEC_Feature_not_supported,
            ['Type reference'],
            ast
          );
      ...
      case SyntaxKind.VariableDeclarator:
        ...
        if (
          this.scriptName === 'lib.d.ts' &&
          vd.propertyName.text() === 'Array'
        ) {
      ...
      case SyntaxKind.FunctionPropertyAssignment:
        return this.tcFunctionPropertyAssignment(
      ...
      case SyntaxKind.SimpleArrowFunctionExpression:
        return this.tcSimpleArrowFunctionExpression(
          <SimpleArrowFunctionExpression>ast
        );

      case SyntaxKind.ParenthesizedArrowFunctionExpression:
        return this.tcParenthesizedArrowFunctionExpression(
      ...
      case SyntaxKind.InvocationExpression:
        ...
        switch (ie.expression.kind()) {
          case SyntaxKind.IdentifierName:
            ...
            switch (
              id.text() //NS: Probably unnecessary for SafeTS, although this may still be useful functions to provide in the RT library
            ) {
              case 'wrap':
                return this.tcWrapOrCanTagInvocation(ie, 'wrap');
              case 'canTag':
                return this.tcWrapOrCanTagInvocation(ie, 'canTag');
              default:
                break;
            }
            break;
          case SyntaxKind.MemberAccessExpression:
            ...
            if (
              mae.expression.kind() === SyntaxKind.IdentifierName &&
              (<Identifier>mae.expression).text() === 'RT' &&
              mae.name.text() === 'forceCheckedArray'
            ) {
  ...
  private tcList(ast: ISyntaxList2) {
    ...
    for (var i = 0; i < ast.childCount(); i++) {
      ...
      if (
        c.fst.kind() === SyntaxKind.FunctionDeclaration &&
        this.compilationSettings.tsstarTagging()
      ) {
  ...
  private tcSeparatedList(ast: ISeparatedSyntaxList2) {
    ...
    var res = new ISeparatedSyntaxList2(
      this.doc.fileName,
      checked,
      ast.separatorCount()
    );
    ...
    var withNames: (i: number) => ISyntaxList2 = (i) => {
      if (i === names.length - 1) {
        return this.tcenv.withModule<ISyntaxList2>(names[i].text())(() =>
          this.tcenv.newScope(
            () => <ISyntaxList2>this.tc(ast.moduleElements),
      ...
      switch (v.kind()) {
        case SyntaxKind.VariableStatement:
          ...
          TcUtil.mapSepList2(vs.declaration.declarators, (b: AST) => {
            ...
            if (
              vdr.equalsValueClause &&
              !TcUtil.safeTopLevelInitializer(vdr.equalsValueClause)
            ) {
              TcUtil.Logger.error(
                DiagnosticCode.SEC_Initializer_may_rely_on_uninitialized_top_level_names,
                [],
                vdr
              );
            }
  ...
  private tcTypeParameterList(ast: TypeParameterList) {
    TcUtil.mapSepList2(ast.typeParameters, (a: AST) => {
      ...
      if (tp.kind() !== SyntaxKind.TypeParameter)
  ...
  private tcInterfaceDeclaration(ast: InterfaceDeclaration) {
    ...
    if (!tglobal) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Variable_not_found,
        [ast.identifier.text()],
        ast
      );
      return this.pkg(ast, ast);
    ...
    var tps = ast.typeParameterList
      ? <TypeParameterList>this.tc(ast.typeParameterList)
    ...
    TcUtil.Logger.error(
      DiagnosticCode.SEC_Interface_declaration_mismatch,
      [ast.identifier.text(), tscheme.toString(), tglobal.toString()],
      ast
    );
  ...
  private tcClassDeclaration(ast: ClassDeclaration) {
    ...
    var members = this.tcenv.withClass<ISyntaxList2>(cd.identifier.text())(
      () => {
        var elements = TcUtil.syntaxListMembers(cd.classElements).map(
          (elt: AST) => {
        ...
        return <ISyntaxList2>(
          this.pkg(
            cd.classElements,
            new ISyntaxList2(this.doc.fileName, elements)
          ).fst
        );
      }
    );
    var heritageClauses = this.tcenv.withLocalTypes(
      tparms,
      () => <ISyntaxList2>this.tc(cd.heritageClauses)
    );
    var extendsClause = heritageClauses
      ? <HeritageClause>(
          heritageClauses.firstOrDefault(
            (a: AST, i: number) => a.kind() === SyntaxKind.ExtendsHeritageClause
          )
        )
      : null;
    if (extendsClause) {
      ...
      if (et.typeName !== TypeName.Object && et.typeName !== TypeName.Inst) {
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Invalid_extends_clause,
          [et.toString() + ' is not a class'],
          cd.heritageClauses
        );
      }
    }
    this.checkHeritageClauses(members, heritageClauses);
    var constructorDecl = members.firstOrDefault(
      (a: AST, i: number) => a.kind() === SyntaxKind.ConstructorDeclaration
    );

    var class_t = <TClass>this.tcenv.lookup(cd.identifier.text());
    var obj_t = class_t.objectType();
    if (obj_t.isVirtual()) {
      var cd1 = new ClassDeclaration(
        cd.modifiers,
        cd.identifier,
        cd.typeParameterList,
        heritageClauses,
        members,
        cd.closeBraceToken
      );
    ...
    var extendsC =
      object_t.extendsC && object_t.extendsC.length === 1
        ? <NamedType>object_t.extendsC[0].unfold()
        : null;
    var classArgs: AST[] = [
      MkAST.stringConst(class_t.name), //class name
      ...
      MkAST.mkCleanArray(
        object_t.implementsI.map((i) => MkAST.stringConst(i.toString()))
      ), //implements
    ...
    var classRtti = new MemberVariableDeclaration(
      [PullElementFlags.Static, PullElementFlags.Public],
      new VariableDeclarator(
        MkAST.id('__rtti__'),
        null,
        new EqualsValueClause(MkAST.callRT('registerClass', classArgs))
      )
    );
    this.semanticInfoChain.setDeclForAST(
      classRtti,
      new NormalPullDecl(
        '__rtti__',
        '__rtti__',
        PullElementKind.Property,
        PullElementFlags.Public | PullElementFlags.Static,
        classDecl,
        new TextSpan(0, 0)
      )
    );
    this.semanticInfoChain.setSymbolForAST(classRtti, rttiSym);
    var allMembers = new ISyntaxList2(
      this.doc.fileName,
      TcUtil.syntaxListMembers(members).concat([<AST>classRtti])
    );
    var cd2 = new ClassDeclaration(
      cd.modifiers,
      cd.identifier,
      cd.typeParameterList,
      heritageClauses,
  ...
  private tcMemberVariableDeclaration(ast: MemberVariableDeclaration) {
    ...
    var vd = this.tcenv.newScope(
      () => <VariableDeclarator>this.tc(ast.variableDeclarator),
  ...
  private tcVariableDeclarator(ast: VariableDeclarator) {
    ...
    if (
      !ambient &&
      !vdinit &&
      texpected.typeName !== TypeName.Any &&
      texpected.typeName !== TypeName.Un
    ) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Uninitialized_variable,
        [texpected.toString()],
        ast
      );
    }
    if (vdinit) {
      var sc = TypeRelations.subtype(
        computedType,
        texpected,
        TcUtil.allowDeepSubtyping(ast.equalsValueClause)
      );
      if (sc.fst) {
        vdinit = new EqualsValueClause(
          TcUtil.shallowTag(sc, vdinit.value, texpected)
        );
      } else {
        if (TypeRelations.assignable(texpected, computedType)) {
          vdinit = new EqualsValueClause(
            TcUtil.checkAndTag(vdinit.value, computedType, texpected)
          );
          TcUtil.Logger.warning(
            DiagnosticCode.SEC_Implicit_coercion,
            [computedType.toString(), texpected.toString()],
    ...
    return this.pkg(
      ast,
      new VariableDeclarator(ast.propertyName, ast.typeAnnotation, vdinit),
      texpected
    );
  }
  ...
  private tcImportDeclaration(ast: ImportDeclaration) {
    ...
    switch (mref.kind()) {
      case SyntaxKind.ExternalModuleReference:
        ...
        if (!mod) {
          TcUtil.Logger.error(DiagnosticCode.SEC_Variable_not_found, [
            'module ' + ref,
          ]);
          mod = TConstant.Any;
        }
      ...
      default:
        return TcUtil.NYI(
          'ImportDeclaration with a module reference of kind ' +
            kind2string(mref.kind())
        );
    }
    return this.pkg(ast, ast);
  }
  private tcObjectLiteralExpression(ast: ObjectLiteralExpression) {
    ...
    if (ast.inferredType) {
      this_t = TranslateTypes.translateTypeOrSig(
        ast.inferredType,
        this.tcenv
      ).unfold();
    ...
    var props: AST[] = TcUtil.mapSepList2(ast.propertyAssignments, (p: AST) => {
      if (p.kind() === SyntaxKind.SimplePropertyAssignment) {
        ...
        var fpa = <FunctionPropertyAssignment>(
          this.tcenv.withThisType(this_t, () => this.tc(p))
        );
        methods.push({
        ...
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Unexpected_property_assignment,
          [kind2string(p.kind())],
          p
        );
        return p;
    ...
    var propertyAssignments = new ISeparatedSyntaxList2(
      this.doc.fileName,
      props,
      ast.propertyAssignments.separatorCount()
    );
    ...
    if (!sc.fst) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Object_literal_type_mismatch,
        [this_t.toString(), trec.toString()],
        ast
      );
    }
    var newObj: AST;
    if (this.compilationSettings.secure()) {
      newObj = MkAST.callExpr(MkAST.fieldOfRT('create'), [
        <AST>MkAST.nullConst(),
      ]);
      fields.forEach((f) => {
        newObj = MkAST.callExpr(MkAST.fieldOfRT('setField'), [
          newObj,
          MkAST.stringConst(f.name),
          f.expr,
        ]);
      });
      return this.pkg(ast, TcUtil.shallowTag(sc, newObj, this_t), this_t);
    } else if (this.compilationSettings.tsstarTagging()) {
      newObj = MkAST.callRT('setTag', [
        new ObjectLiteralExpression(propertyAssignments),
        this_t.toRTTI(),
      ]);
  ...
  private tcSimplePropertyAssignment(ast: SimplePropertyAssignment) {
    ...
    if (TcUtil.reservedName(idOrStringText(ast.propertyName))) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Reserved_name,
        [idOrStringText(ast.propertyName)],
        ast
      );
    }
    return this.pkg(
      ast,
      new SimplePropertyAssignment(ast.propertyName, e),
      e.soundType
    );
  }
  private tcFunctionPropertyAssignment(ast: FunctionPropertyAssignment) {
    var blockT = this.auxTcBlockFunction(
      ast,
      TcUtil.callSigTypeParamBindings(ast.callSignature),
      this.symbol(ast).type.getCallSignatures()[0],
      ast.block
    );
    return this.pkg(
      ast,
      new FunctionPropertyAssignment(
        ast.propertyName,
        ast.callSignature,
        blockT.fst
      ),
  ...
  private auxMemberOrMethodType(
    t: SoundType,
    name: string,
    overload: number,
    methodsOk: boolean = false
  ): Field {
    ...
    switch (t.typeName) {
      ...
      case TypeName.Tuple:
        ...
        if (!fld) {
          var ot = <StructuredType>(
            TcEnv.currentEnv.lookupType({ dottedName: 'Object' })
          );
          if (ot) {
      ...
      case TypeName.Number: //implicit conversion to number object
        ...
        if (numClass) {
          return this.auxMemberOrMethodType(
            numClass,
            name,
            overload,
            methodsOk
      ...
      case TypeName.Variable:
        ...
        return cons
          ? this.auxMemberOrMethodType(cons, name, overload, methodsOk)
          : null;
      default:
        return null;
    }
  }
  private tcMemberAccessExpression(
    ...
    var mkProj = (fld?: Field) => {
      ...
      if (!fld) {
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Projecting_nonexistent_field,
          [f, o.soundType.toString()],
          ast
        );
    ...
    switch (t.typeName) {
      case TypeName.Any:
        ...
        if (this.isPure(o)) {
          var ft_lookup: AST = mem(
            mem(MkAST.getRtti(o), id('fieldTable')),
            ast.name
          );
          var mt_lookup: AST = mem(
            mem(MkAST.getRtti(o), id('methodTable')),
            ast.name
          );
          var ob_lookup: AST = mem(MkAST.fieldOfRT('objectMethods'), ast.name);

          var or_exp: AST = new BinaryExpression(
          ...
          var new_ast = MkAST.callRT('shallowTagSwap', [
            MkAST.callRT('getFieldTypeOptim', [
              or_exp,
              o,
              MkAST.stringConst(ast.name.text()),
            ]),
  ...
  private tcQualifiedName(ast: QualifiedName) {
    if (isTypesOnlyLocation(ast)) {
      ...
      var pushNames = (ast: AST): any => {
        switch (ast.kind()) {
          case SyntaxKind.IdentifierName:
            fullName.push((<Identifier>ast).text());
            return;
          case SyntaxKind.QualifiedName:
            fullName.push((<QualifiedName>ast).right.text());
            return pushNames((<QualifiedName>ast).left);
          default:
            return TcUtil.NYI(
  ...
  private tcConstructorDeclaration(ast: ConstructorDeclaration) {
    ...
    var err = (msg: string): any => {
      throw (
        'Non-trivial constructor bodies are not yet supported in [--safe|--secure] mode: got a body with ' +
        msg
      );
      return null;
    };
    ...
    var checkBody = (constr: TArrow, c: TObject) => {
      if (c.extendsC && c.extendsC[0]) {
        ...
        if (super_c.typeName === TypeName.Inst) {
          super_t = [
            TcUtil.mkVariableBinding(
              ast,
              '__super__',
              new TInst(<TPoly>cc.constructSignature(), (<TInst>super_c).args)
            ),
          ];
        } else {
          super_t = [
            TcUtil.mkVariableBinding(ast, '__super__', cc.constructSignature()),
          ];
        }
      }
      var declPath: string[] = this.tcenv.curNamespace();
      var sym = this.semanticInfoChain.findSymbol(
        declPath,
        PullElementKind.All
      );
      if (!sym) {
        ...
        var classAST = <ClassDeclaration>(
          this.semanticInfoChain.getASTForDecl(sym.getDeclarations()[0])
        );
        var initialized = c.exposeFields().every((f: Field) => {
          var initialized =
            constr.args.some((t: TArg) => t.name === f.name && t.isField()) ||
            (c.extendsC &&
              c.extendsC.some((u) =>
                (<NamedType>u.unfold()).hasField(f.name)
              )) ||
            classAST.classElements.any((m: AST) => {
              switch (m.kind()) {
                case SyntaxKind.MemberFunctionDeclaration:
                  return (
                    (<MemberFunctionDeclaration>m).propertyName.text() ===
                    f.name
                  );
                case SyntaxKind.MemberVariableDeclaration:
                  var vd = <MemberVariableDeclaration>m;
                  return (
                    vd.variableDeclarator.equalsValueClause &&
                    vd.variableDeclarator.propertyName.text() === f.name
                  );
                default:
                  return false;
              }
          ...
          if (!initialized) {
            TcUtil.Logger.error(
              DiagnosticCode.SEC_Uninitialized_member,
              [f.name],
              ast
            );
          }
    ...
    switch (ct.typeName) {
      case TypeName.Poly:
        ...
        break;
      case TypeName.Arrow:
        var failed2 = checkBody(
          <TArrow>ct,
          <TObject>(<TArrow>ct).result.unfold()
        );
    ...
    var blockT = this.tcenv.withVariables<Pair<Block, SoundType>>(super_t)(
      () => {
        var csigs = signature.type.getConstructSignatures(); //take the last overload, which is the actual definition
        return this.auxTcBlockFunction(
          ast,
          tparms,
          csigs[csigs.length - 1],
          ast.block,
          true /*voidOk*/
        );
      }
  ...
  private tcSetAccessor(ast: SetAccessor) {
    var t = TranslateTypes.translateTypeOrSig(ast.inferredType, this.tcenv);
    var bindings = TcUtil.mapSepList2(
  ...
  private tcCallSignature(ast: CallSignature) {
    ...
    var t: SoundType = this.tcenv.withLocalTypes(binders, () => {
      ...
      var t = new TArrow(
        TcUtil.mapSepList2(parms, (a: AST) => {
          var p = <Parameter>a;
  ...
  private auxMkFunctionBodyBindings(
    ast: AST,
    tparms: TcUtil.LocalTypeBinding[],
    signature: PullSignatureSymbol,
    block?: Block
  ): Triple<TcUtil.VariableBinding[], TArrow, TcUtil.LocalTypeBinding[]> {
    var bindings = this.tcenv.withLocalTypes(tparms, () => {
      var bindings = signature.parameters.map(
        (p: PullSymbol) =>
      ...
      if (block) {
        bindings = bindings.concat(
          TcUtil.gatherFunctionBindings(block.statements)
        );
      }
      return bindings;
    });
    ...
    switch (t.typeName) {
      case TypeName.Arrow:
        ...
        break;
      case TypeName.Poly:
        arrowType = <TArrow>new TInst(
          <TPoly>t,
          tparms.map((xt) => xt.snd)
        ).unfold();
        break;
      default:
        throw new Error(
          TcUtil.Logger.pos(ast) +
            ' Unexpected type for function ' +
            t.toString()
        );
    }
    return { fst: bindings, snd: arrowType, third: tparms };
  }
  private auxTcBlockFunction(
    ast: AST,
    tparms: TcUtil.LocalTypeBinding[],
    signature: PullSignatureSymbol,
    block: Block,
    voidOk: boolean = false
  ): Pair<Block, SoundType /*Either an arrow or a polytype */> {
    //TODO: check callSig
    var bindingsT = this.auxMkFunctionBodyBindings(
      ast,
      tparms,
      signature,
      block
      ...
      : this.tcenv.newScope(
          () =>
            this.tcenv.withVariables<Block>(bindingsT.fst)(() =>
              this.tcenv.withLocalTypes(
                bindingsT.third,
                () =>
                  <Block>(
                    this.tcWithResult(
                      voidOk ? TConstant.Void : bindingsT.snd.result,
                      block
                    )
                  )
              )
  ...
  private auxTcArrowFunction(
    ast: AST,
    callSig: CallSignature,
    signature: PullSignatureSymbol,
    block: Block,
    body: AST
  ): { block: Block; body: AST; type: SoundType } {
    if (block && !body) {
      var blockT = this.auxTcBlockFunction(
        ast,
        TcUtil.callSigTypeParamBindings(callSig),
        signature,
        block
      );
      ...
      var expr = this.tcenv.newScope(() =>
        this.tcenv.withVariables<AST>(bindingsT.fst, true)(() => this.tc(body))
      );
      var expected_t = bindingsT.snd.result
        ? bindingsT.snd.result
        : TConstant.Any;
      var sc = TypeRelations.subtype(
        expr.soundType,
        expected_t,
        TcUtil.allowDeepSubtyping(body)
      );
      if (!sc.fst) {
        if (TypeRelations.assignable(expected_t, expr.soundType)) {
          TcUtil.Logger.warning(
            DiagnosticCode.SEC_Implicit_coercion,
            [expr.soundType.toString(), expected_t.toString()],
            body
          );
          expr = TcUtil.checkAndTag(expr, expr.soundType, expected_t);
        } else {
          TcUtil.Logger.error(
            DiagnosticCode.SEC_Function_return_type,
            [expected_t.toString(), expr.soundType.toString()],
            ast
          );
      ...
      throw TcUtil.NYI(
        'Unexpected both block and expression in arrow function: ' +
          TcUtil.Logger.pos(ast)
      );
    }
  }
  private tcMemberFunctionDeclaration(ast: MemberFunctionDeclaration) {
    ...
    var checkBody = () => {
      var blockT = this.auxTcBlockFunction(
        ast,
        TcUtil.callSigTypeParamBindings(ast.callSignature),
        callSigs[callSigs.length - 1],
        ast.block
      );
      return this.pkg(
        ast,
        new MemberFunctionDeclaration(
          ast.modifiers,
          ast.propertyName,
          ast.callSignature,
          blockT.fst
  ...
  private tcAnyFunctionDeclaration(ast: FunctionDeclaration) {
    ...
    var blockT = this.tcenv.withoutThis(() =>
      this.auxTcBlockFunction(
        ast,
        TcUtil.callSigTypeParamBindings(ast.callSignature),
        callSig,
        ast.block
  ...
  private tcSimpleArrowFunctionExpression(ast: SimpleArrowFunctionExpression) {
    var bet = this.auxTcArrowFunction(
      ast,
      null,
      this.symbol(ast).type.getCallSignatures()[0],
      ast.block,
    ...
    var f = new SimpleArrowFunctionExpression(
      ast.identifier,
      bet.block,
      bet.body
    );
  ...
  private tcParenthesizedArrowFunctionExpression(
    ast: ParenthesizedArrowFunctionExpression
  ) {
    var bet = this.auxTcArrowFunction(
      ast,
      ast.callSignature,
    ...
    var f = new ParenthesizedArrowFunctionExpression(
      ast.callSignature,
      bet.block,
      bet.body
    );
  ...
  private tcArrayLiteralExpression(ast: ArrayLiteralExpression) {
    ...
    for (var i = 0; i < elts.nonSeparatorCount(); i++) {
      ...
      if (!expected_t) {
        ...
        if (sc.fst) {
          ...
          TcUtil.Logger.error(
            DiagnosticCode.SEC_Array_element_type,
            [expected_t.toString(), elt.soundType.toString()],
            elt
          );
          err = true;
          out.push(TcUtil.shallowTag(sc, elt, expected_t));
    ...
    var elements = new ISeparatedSyntaxList2(
      elts.fileName(),
      out,
      elts.separatorCount()
    );
    ...
    var res: AST = null;
    if (this.compilationSettings.secure()) {
      res = MkAST.callExpr(MkAST.fieldOfRT('newArray'), [
        expected_t.toRTTI(),
        arr,
      ]);
      res.soundType = res_t;
      ...
      this.compilationSettings.tsstarTagging() &&
      expected_t.typeName !== TypeName.UVar
    ) {
      res = err
        ? MkAST.unsafe(arr)
        : MkAST.callRT('setTag', [arr, res_t.toRTTI()]);
    } else {
  ...
  private tcArgumentList(ast: ArgumentList) {
    var targs = ast.typeArgumentList
      ? <TypeArgumentList>this.tc(ast.typeArgumentList)
      : null; //TODO: Check type arguments
    var args = <ISeparatedSyntaxList2>this.tc(ast.args);
    return this.pkg(ast, new ArgumentList(targs, args, ast.closeParenToken));
  }
  private auxTcArgumentList(
    untyped_args: ArgumentList,
    typed_args: ArgumentList,
    t: TArrow,
    default_t?: SoundType
  ): ArgumentList {
    var newargs: AST[] = [];
    ...
    for (; i < typed_args.args.nonSeparatorCount(); i++) {
      ...
      if (i >= t.args.length) {
        if (!formal) {
          TcUtil.Logger.error(
            ...
            [
              t.args.length.toString(),
              typed_args.args.nonSeparatorCount().toString(),
            ],
            typed_args
          );
      ...
      if (formal.variadic) {
        ...
        if (!ft.isArray()) {
          throw (
            'Impossible:  got a variadic argument type ' +
      ...
      var sc = TypeRelations.subtype(
        actual.soundType,
        type,
        TcUtil.allowDeepSubtyping(untyped_args.args.nonSeparatorAt(i))
      );
      if (!sc.fst) {
        if (default_t && default_t.typeName === TypeName.Arrow) {
          //switching to default overload
          return this.auxTcArgumentList(
            untyped_args,
            typed_args,
            <TArrow>default_t
          );
        ...
        if (TypeRelations.assignable(type, actual.soundType)) {
          TcUtil.Logger.warning(
            DiagnosticCode.SEC_Implicit_coercion,
            [actual.soundType.toString(), type.toString()],
            actual
          );
          ...
          TcUtil.Logger.error(
            ...
            [type.toString(), actual.soundType.toString()],
            actual
          );
    ...
    for (; i < t.args.length; i++) {
      if (!t.args[i].optional()) {
        TcUtil.Logger.error(
          ...
          [t.args[i].name, t.args[i].type.toString()],
          typed_args
        );
      }
    }

    var argsList = new ISeparatedSyntaxList2(
      typed_args.args.fileName(),
      newargs,
      typed_args.args.separatorCount()
    );
    return new ArgumentList(
      typed_args.typeArgumentList,
      argsList,
  ...
  private isPure(ast: AST): boolean {
    ...
    switch (ast.kind()) {
      ...
      case SyntaxKind.MemberAccessExpression:
        return (
          this.compilationSettings.noGetters() ||
          this.isPure((<MemberAccessExpression>ast).expression)
        );
      case SyntaxKind.ParenthesizedExpression:
  ...
  private auxTcInvocationTarget(
    ast: AST,
    preArgs: ArgumentList,
    inferredTypeArgs: PullTypeSymbol[],
    overload: number
  ): Pair<
    Pair<SoundType /* any or arrow */, SoundType>,
    (args: ArgumentList) => AST
  > {
    ...
    var instantiate = (t: SoundType) => {
      switch (t.typeName) {
        ...
        case TypeName.Poly:
          ...
          if (
            !tt ||
            (tt.typeName !== TypeName.Arrow && tt.typeName !== TypeName.Any)
          ) {
            TcUtil.Logger.error(
              DiagnosticCode.SEC_Cannot_apply_a_non_function,
              [t.toString()],
              ast
            );
        ...
        default:
          ...
          var instCall = (sig: SoundType, warn: boolean): SoundType => {
            if (sig) {
              var tcall = TcUtil.instantiateType(
                this.tcenv,
                sig,
                preArgs,
                inferredTypeArgs,
              ...
              if (
                !tcall ||
                (tcall.typeName !== TypeName.Arrow &&
                  tcall.typeName !== TypeName.Any)
              ) {
                TcUtil.Logger.error(
                  DiagnosticCode.SEC_Cannot_apply_a_non_function,
                  [t.toString()],
          ...
          return pair(
            instCall(callSig, true),
            defaultSig === callSig ? default_t : instCall(defaultSig, false)
          );
      }
    };
    switch (ast.kind()) {
      case SyntaxKind.MemberAccessExpression:
        ...
        switch (t.typeName) {
          case TypeName.Any:
            TcUtil.Logger.error(
              DiagnosticCode.SEC_Implicit_coercion,
              [TConstant.Any.toString(), '{' + name + ':any}'],
              ast
            );
            var callMethod = (args: ArgumentList) => {
              var allPure: boolean =
                this.isPure(receiver) &&
                TcUtil.sepListMembers(args.args).every((x: AST) => {
                  return this.isPure(x);
                });
              if (!allPure) {
                return MkAST.callRT(
                  'callMethod' + nargs(args),
                  mkArgList(
                    [
                      receiver,
                      receiver.soundType.toRTTI(),
                      MkAST.stringConst(name),
                    ],
                    args
                  )
                ...
                return MkAST.callRT('shallowTagSwap', [
                  MkAST.callRT(
                    'checkMethodArgs' + nargs(args),
                    mkArgList(
                      [
                        receiver,
                        receiver.soundType.toRTTI(),
                  ...
                  new InvocationExpression(
                    new MemberAccessExpression(receiver, mae.name),
                    args
                  ),
                ]);
              }
          ...
          default:
            ...
            if (!fld) {
              ...
              return {
                fst: pair(
                  instantiate(fld.type).fst,
                  default_f === fld
                    ? <SoundType>undefined
                    : instantiate(default_f.type).fst
                ),
                snd: mkCall,
              };
            }
      ...
      case SyntaxKind.ElementAccessExpression: //TODO: Handle index signature here
        ...
        return {
          fst: instantiate(TConstant.Any),
          snd: (args: ArgumentList) =>
            MkAST.callRT(
              'callMethod' + nargs(args),
              mkArgList(
                [receiver2, receiver2.soundType.toRTTI(), methodName],
                args
              )
            ),
        };
      ...
      default:
        ...
        switch (t3.typeName) {
          ...
          default:
            ...
            var mkCall = (args: ArgumentList): AST =>
              new InvocationExpression(receiver3, args);
            return {
              fst: tarr,
              snd: mkCall,
            };
  ...
  private tcForceCheckedArrayInvocation(ast: InvocationExpression) {
    var targs = <ISeparatedSyntaxList2>(
      this.tc(ast.argumentList.typeArgumentList.typeArguments)
    );
    var args = <ISeparatedSyntaxList2>this.tc(ast.argumentList.args);
    var source_t: SoundType = null,
      target_t: SoundType = null;
    if (targs.nonSeparatorCount() !== 2) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Arity_mismatch,
        ['2 type', targs.nonSeparatorCount() + ' type'],
        ast
      );
      source_t = TConstant.Any;
      target_t = TConstant.Any;
    } else {
      source_t = TcUtil.mkCheckedArrayType(
        this.tcenv,
        targs.nonSeparatorAt(0).soundType
      );
    ...
    if (args.nonSeparatorCount() !== 1) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Arity_mismatch,
        ['1', targs.nonSeparatorCount().toString()],
        ast
      );
      return this.pkg(ast, MkAST.unsafe(ast), TConstant.Any);
    }
    ...
    var sc = TypeRelations.subtype(
      arg.soundType,
      source_t,
      TcUtil.allowDeepSubtyping(ast.argumentList.args.nonSeparatorAt(0))
    );
    if (!sc.fst) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Argument_type_mismatch,
        [source_t.toString(), arg.soundType.toString()],
        ast
      );
      return this.pkg(ast, MkAST.unsafe(ast), TConstant.Any);
    }
    var call = MkAST.callRT('__forceCheckedArray', [
      TcUtil.shallowTag(sc, arg, source_t),
      target_t.toRTTI(),
    ]);
    return this.pkg(ast, call, TcUtil.mkArrayType(this.tcenv, target_t));
  }
  private tcWrapOrCanTagInvocation(
    ...
    var callWrap = MkAST.callExpr(MkAST.fieldOfRT(wrapOrCanTag), [
      source_t.toRTTI(),
      target_t.toRTTI(),
      TcUtil.shallowTag(sc, arg, source_t),
    ]);
  ...
  private tcInvocationExpression(ast: InvocationExpression) {
    ...
    var ttcall = this.auxTcInvocationTarget(
      ast.expression,
      typed_args,
      ast.inferredTypeArgs,
      ast.resolvedSignatureIndex
    );
    ...
    switch (tt.typeName) {
      case TypeName.Arrow:
        var tarr = <TArrow>tt;
        var result = call(
          this.auxTcArgumentList(ast.argumentList, typed_args, tarr, def_t)
        );
      ...
      case TypeName.Any:
        var argsSub: AST[] = TcUtil.mapSepList2WithIndex(
          typed_args.args,
          (a: AST, i: number) => {
            var sc = TypeRelations.subtype(
              a.soundType,
              TConstant.Any,
              TcUtil.allowDeepSubtyping(ast.argumentList.args.nonSeparatorAt(i))
            );
            if (!sc.fst)
              TcUtil.Logger.error(
                DiagnosticCode.SEC_Dynamic_call_with_un_typed_argument,
                [],
                a
              );
  ...
  private tcObjectCreationExpression(ast: ObjectCreationExpression) {
    ...
    var typed_args = <ArgumentList>this.tc(ast.argumentList);
    if (
      !(tgtType instanceof StructuredType) ||
      !(<StructuredType>tgtType).constructSignature()
    ) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Invalid_new_target,
        [tgt.soundType.toString()],
        ast.expression
      );
      tgtType = new TClass(
        'Dummy',
    ...
    var csig = (<StructuredType>tgtType).constructSignature(
      ast.resolvedSignatureIndex
    );
    var defSig = (<StructuredType>tgtType).constructSignature(-1);
    var inst = (sig: SoundType) => {
      var ctype = TcUtil.instantiateType(
        this.tcenv,
        sig,
        typed_args,
        ast.inferredTypeArgs
    ...
    var ctype = inst(csig);
    var new_obj: AST = new ObjectCreationExpression(
      tgt,
      this.auxTcArgumentList(
        ast.argumentList,
        typed_args,
        <TArrow>ctype,
        defSig === csig ? <SoundType>undefined : inst(defSig)
      )
    );
  ...
  public computeType(ast: AST): SoundType {
    switch (ast.kind()) {
      ...
      case SyntaxKind.NumericLiteral:
      case SyntaxKind.NumberKeyword:
        return TConstant.Number;
      case SyntaxKind.StringLiteral:
      case SyntaxKind.StringKeyword:
        return TConstant.String;
      case SyntaxKind.VoidKeyword:
        return TConstant.Void;
      case SyntaxKind.ArrayType:
        return TcUtil.mkArrayType(
          this.tcenv,
          this.computeType((<ArrayType>ast).type)
        );
      case SyntaxKind.GenericType:
        ...
        if (nm === 'Array') {
          return TcUtil.mkArrayType(
            this.tcenv,
            this.computeType(
              gt.typeArgumentList.typeArguments.nonSeparatorAt(0)
            )
          );
        } else if (this.compilationSettings.generics()) {
          var tt = <TPoly>(
            TranslateTypes.translateTypeOrSig(this.symbol(gt.name), this.tcenv)
          );
          var targs = TcUtil.mapSepList2(
            gt.typeArgumentList.typeArguments,
            (t) => this.computeType(t)
          );
          var res = new TInst(tt, targs);
          return res;
        } else {
          TcUtil.Logger.error(
            DiagnosticCode.SEC_Generic_types_unsupported,
            [nm],
            ast
          );
          return TConstant.Any;
  ...
  private tcCastExpression(ast: CastExpression) {
    ...
    if (
      this.compilationSettings.inlineCasts() &&
      targ.typeName === TypeName.Object &&
      ast.expression.kind() === SyntaxKind.IdentifierName
    ) {
      var tobj = <TObject>targ;
      var nullGuard = new ParenthesizedExpression(
        [],
        new BinaryExpression(
          SyntaxKind.LogicalOrExpression,
          new BinaryExpression(
      ...
      var strEquals = new BinaryExpression(
        SyntaxKind.EqualsExpression,
      ...
      var instanceOf = new BinaryExpression(
        ...
        arg,
        new Identifier(tobj.fullName.join('.'))
      );
      var tm = new ConditionalExpression(
        new BinaryExpression(
          SyntaxKind.LogicalOrExpression,
          nullGuard,
      ...
      return this.pkg(
        ast,
        MkAST.callRT('wrapFromUn', [arg, targ.toRTTI()]),
        targ
      );
    } else if (targ.isUn()) {
      return this.pkg(
        ast,
        MkAST.callRT('wrapToUn', [arg, arg.soundType.toRTTI()]),
        targ
      );
    } else {
      return this.pkg(
        ast,
        MkAST.callRT('checkAndTag', [
          arg,
          arg.soundType.toRTTI(),
          targ.toRTTI(),
        ]),
        targ
      );
  ...
  private tcAssignIdentifier(
    ast: BinaryExpression,
    lhs: AST /*already type-checked */,
    rhs: AST /*already type-checked*/,
    rhs_orig: AST /* untyped */
  ) {
    var sc = TypeRelations.subtype(
      rhs.soundType,
      lhs.soundType,
      TcUtil.allowDeepSubtyping(rhs_orig)
    );
    if (!sc.fst) {
      if (TypeRelations.assignable(lhs.soundType, rhs.soundType)) {
        TcUtil.Logger.warning(
          DiagnosticCode.SEC_Implicit_coercion,
          [rhs.soundType.toString(), lhs.soundType.toString()],
          lhs
        );
        return this.pkg(
          ast,
          new BinaryExpression(
            ast.kind(),
            lhs,
            TcUtil.checkAndTag(rhs, rhs.soundType, lhs.soundType)
          ),
          rhs.soundType
        );
      }
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Variable_assignment_incompatible_type,
        [
          (<Identifier>ast.left).text(),
          lhs.soundType.toString(),
    ...
    return this.pkg(
      ...
      new BinaryExpression(
        ast.kind(),
        lhs,
        TcUtil.shallowTag(sc, rhs, lhs.soundType)
      ),
      rhs.soundType
    );
  }
  private tcAssignMember(
    ast: BinaryExpression,
    o: AST,
    fn: Identifier,
    rhs: AST,
    ...
    var unsafe = (lhs: AST, rhs: AST) => {
      return this.pkg(
        ast,
        MkAST.unsafe(new BinaryExpression(ast.kind(), lhs, rhs)),
        TConstant.Any
      );
    };
    var tcAssign = (ft: Field) => {
      ...
      if (!ft) {
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Assigning_to_nonexistent_field,
          [fn.text(), o.soundType.toString()],
          ast
        );
      ...
      if (ft.mutable && sc.fst) {
        ...
        if (!ft.mutable) {
          TcUtil.Logger.error(
            DiagnosticCode.SEC_Assigning_to_immutable_field,
            [fn.text()],
            ast
          );
          ...
          if (
            TypeRelations.assignable(ft.type, rhs.soundType) &&
            TypeRelations.isSubtype(o.soundType, TConstant.Any)
          ) {
            return safe(
              mae,
              MkAST.callRT('checkAndTag', [
            ...
            TcUtil.Logger.error(
              DiagnosticCode.SEC_Assigning_incompatible_value_to_field,
              [fn.text(), ft.type.toString(), rhs.soundType.toString()],
              ast
            );
            return unsafe(mae, TcUtil.shallowTag(sc, rhs, ft.type));
          }
    ...
    switch (ot.typeName) {
      case TypeName.Any:
        ...
        if (!sc.fst) {
          TcUtil.Logger.error(
            DiagnosticCode.SEC_Assigning_incompatible_value_to_field,
            [fn.text(), TConstant.Any.toString(), rhs.soundType.toString()],
            ast
          );
          ...
          if (this.isPure(o)) {
            ...
            var new_rhs = MkAST.callRT('checkAndTag', [
              rhs,
              rhs.soundType.toRTTI(),
              MkAST.callRT('getFieldTypeOptim', [
                or_exp,
                o,
                MkAST.stringConst(fn.text()),
              ]),
            ]);
            return this.pkg(
              ast,
              new BinaryExpression(
                ast.kind(),
                new MemberAccessExpression(o, fn),
                new_rhs
              ),
              TConstant.Any
            );
          } else {
            var ce = MkAST.callRT('writeField', [
              o,
              o.soundType.toRTTI(),
              MkAST.stringConst(fn.text()),
              rhs,
      ...
      default:
        ...
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Assigning_to_non_record,
          [fn.text(), o.soundType.toString()],
          o
        );
        return unsafe(new MemberAccessExpression(o, fn), rhs);
  ...
  private tcAssignElement(ast: BinaryExpression, o: AST, key: AST, rhs: AST) {
    ...
    if (o.soundType.unfold().isCheckedArray()) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Assignment_to_index_of_immutable_array_is_not_allowed,
    ...
    if (t_o.indexSignature()) {
      ...
      if (!indexSub.fst) {
        if (TypeRelations.assignable(index.indexType, key.soundType)) {
          ...
          key = MkAST.callRT('checkAndTag', [
            key,
            key.soundType.toRTTI(),
            index.indexType.toRTTI(),
          ]);
        } else {
          TcUtil.Logger.error(
            DiagnosticCode.SEC_Index_map_key_type_mismatch,
            [index.indexType.toString(), key.soundType.toString()],
            ast
          );
      ...
      if (!eltSub.fst) {
        if (TypeRelations.assignable(index.elt, rhs.soundType)) {
          ...
          TcUtil.Logger.error(
            DiagnosticCode.SEC_Index_map_elt_type_mismatch,
            [index.elt.toString(), rhs.soundType.toString()],
            ast
          );
      ...
      if (sc1.fst && sc2.fst && scrhs.fst) {
        ...
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Dynamic_field_assignment_all_not_any,
          [
            o.soundType.toString(),
            key.soundType.toString(),
            rhs.soundType.toString(),
        ...
        return unsafe(
          assignee(o, TConstant.Any),
          TcUtil.shallowTag(scrhs, rhs, TConstant.Any)
        );
      }
    }
  }
  ...
  private getBinOpFromAssignment(aKind: SyntaxKind): SyntaxKind {
    switch (aKind) {
      ...
      default:
        throw new Error(
          "getBinOpFromAssignment doesn't handle this case: " + aKind.toString()
        );
    }
  }

  private tcOpWithAssignment(
    orig: AST,
    lhs_un: AST,
    trhs: SoundType,
    safe: (o: AST, f?: AST, tagRhs?: SoundType) => AST,
    rewrite?: (o: AST, f: AST) => AST
  ): Pair<AST, boolean> {
    var unsafe = (lhs: AST, f?: AST) => MkAST.unsafe(safe(lhs, f));
    var tresult = TranslateTypes.translateTypeOrSig(
      orig.inferredType,
      this.tcenv
    );
    switch (lhs_un.kind()) {
      case SyntaxKind.IdentifierName: //local
        ...
        if (TypeRelations.subtypeZ(tresult, lhs.soundType)) {
          ...
          TcUtil.Logger.warning(
            DiagnosticCode.SEC_Implicit_coercion,
            [tresult.toString(), lhs.soundType.toString()],
            orig
          );
          return this.pkg(
            orig,
            safe(lhs, undefined, lhs.soundType),
            lhs.soundType
          );
        } else {
          TcUtil.Logger.error(
        ...
        switch (t_o.typeName) {
          case TypeName.Any:
            if (TypeRelations.isSubtype(trhs, TConstant.Any)) {
              return this.pkg(
                orig,
                rewrite(o, MkAST.stringConst(mae.name.text())),
                tresult
              );
            } else {
              throw new Error(
                'Got a term that should have been untypeable in TypeScript'
              );
            }
            ...
            if (t_o instanceof StructuredType) {
              ...
              if (tf) {
                if (TypeRelations.subtypeZ(tresult, tf.type)) {
                  ...
                  TcUtil.Logger.warning(
                    DiagnosticCode.SEC_Implicit_coercion,
                    [tresult.toString(), tf.type.toString()],
                    orig
                  );
                  ...
                  TcUtil.Logger.error(
                    DiagnosticCode.SEC_Assigning_incompatible_value_to_field,
                    [f, tf.type.toString(), tresult.toString()]
                  );
              ...
              TcUtil.Logger.error(DiagnosticCode.SEC_Assigning_to_non_record, [
                f,
                o.soundType.toString(),
              ]);
              return this.pkg(orig, unsafe(o, mae.name), tresult);
      ...
      case SyntaxKind.ElementAccessExpression:
        ...
        if (
          t_o.isArray() &&
          index.soundType === TConstant.Number &&
          TypeRelations.subtypeZ(trhs, TcUtil.arrayElementType(t_o))
        ) {
          return this.pkg(orig, safe(o, index), TcUtil.arrayElementType(t_o));
        }

        if (
          TypeRelations.isSubtype(t_o, TConstant.Any) &&
          TypeRelations.isSubtype(trhs, TConstant.Any)
        ) {
          return this.pkg(orig, rewrite(o, index), tresult);
        } else {
          TcUtil.Logger.error(
            DiagnosticCode.SEC_Assignment_with_op_expects_undotted_arguments,
            [o.soundType.toString(), trhs.toString()]
          );
          return this.pkg(orig, unsafe(o, index), tresult);
        }
      ...
      default:
        throw new Error(
          'Got an unexpected l-value in assignment with op at ' +
            TcUtil.Logger.pos(orig) +
            '\n kind is ' +
            kind2string(lhs_un.kind())
        );
    }
  }
      ...
      case SyntaxKind.IdentifierName:
        switch (kind) {
          case SyntaxKind.AssignmentExpression:
            ...
            return this.tcAssignIdentifier(
              ast,
              lhs_typed,
              rhs_typed,
              ast.right
          ...
          default:
            var rebuild_safe = (o: AST, fn?: AST, tagRhs?: SoundType) => {
              ...
              if (tagRhs) {
                rhs_typed = this.pkg(
                  ...
                  TcUtil.checkAndTag(
                    new BinaryExpression(
                      this.getBinOpFromAssignment(ast.kind()),
                      lhs,
                      rhs_typed
                    ),
                    tresult,
            ...
            return this.tcOpWithAssignment(
              ast,
              lhs,
              rhs_typed.soundType,
              rebuild_safe
        ...
        switch (kind) {
          ...
          default:
            var rebuild_safe = (o: AST, fn?: AST, tagRhs?: SoundType) => {
              var mae = this.pkg(
                lhs,
                new MemberAccessExpression(o, <Identifier>fn),
                TConstant.Any
              ).fst;
              if (tagRhs) {
                rhs_typed = this.pkg(
        ...
        switch (kind) {
          ...
          default:
            var rebuild_safe_eae = (o: AST, fn?: AST, tagRhs?: SoundType) => {
              var eae = this.pkg(
                lhs,
                new ElementAccessExpression(o, fn),
                TConstant.Any
              ).fst;
  ...
  private rebuildUnary(operand: AST, e: AST, f?: AST) {
    ...
    switch (operand.kind()) {
      case SyntaxKind.IdentifierName:
        ...
        break;
      case SyntaxKind.MemberAccessExpression:
        term = new MemberAccessExpression(e, <Identifier>f);
        break;
      case SyntaxKind.ElementAccessExpression:
        term = new ElementAccessExpression(e, f);
        break;
      default:
        throw new Error('Impossible');
    }
  ...
  private tcPostfixUnaryOpWithAssignment(ast: PostfixUnaryExpression) {
    var rewrite = (e: AST, f: AST): AST => {
      return MkAST.callRT('assignmentWithUnaryOp', [
        MkAST.stringConst(kind2string(ast.kind())),
        e,
        e.soundType.toRTTI(),
        f,
    ...
    var safe = (e: AST, f?: AST, tagRhs?: SoundType): AST => {
      if (tagRhs) {
        ...
        return new PostfixUnaryExpression(
          ast.kind(),
          this.rebuildUnary(ast.operand, e, f)
        );
      }
    };
    return this.tcOpWithAssignment(
  ...
  private tcUnaryOperation(ast: PrefixUnaryExpression) {
    ...
    if (
      ...
      !TypeRelations.isSubtype(e.soundType, TConstant.Any)
    ) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Unary_operator_expects_an_un_free_type,
        [e.soundType.toString()],
        ast
      );
      return this.pkg(ast, MkAST.unsafe(res), TConstant.Number);
    }
    var result_t = TranslateTypes.translateTypeOrSig(
      ast.inferredType,
      this.tcenv
    );
  ...
  private tcElementAccessExpression(ast: ElementAccessExpression) {
    ...
    switch (t_o.typeName) {
      ...
      case TypeName.Arrow:
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Projection_from_non_records,
          [o.soundType.toString()],
          ast
        );
      default:
        if (this.compilationSettings.secure()) {
          if (o.soundType.unfold().typeName === TypeName.String) {
            return this.pkg(
              ast,
              MkAST.callExpr(MkAST.fieldOfRT('charAt'), [o, key]),
              TConstant.String
            );
          } else {
            return this.pkg(
            ...
            default:
              ...
              if (imap) {
                return this.pkg(
                  ...
                  new ElementAccessExpression(
                    o,
                    TcUtil.force(imap.indexType, key)
                  ),
                  imap.elt
                );
              }
              if (
                !TypeRelations.isSubtype(
                  o.soundType,
                  TConstant.Any,
                  TcUtil.allowDeepSubtyping(ast.expression)
                )
              ) {
                TcUtil.Logger.error(
                  DiagnosticCode.SEC_ElementAccess_from_non_any,
                  [o.soundType.toString(), key.soundType.toString()],
                  ast
                );
              }
              return this.pkg(
  ...
  private tcLogicalOrExpression(ast: BinaryExpression) {
    var e1 = this.tc(ast.left);
    var e2 = this.tc(ast.right);
    var expected_t = ast.inferredType
      ? TranslateTypes.translate(ast.inferredType, this.tcenv)
      : e1.soundType;
    var sc1 = TypeRelations.subtype(
      e1.soundType,
      expected_t,
      TcUtil.allowDeepSubtyping(ast.left)
    );
    var sc2 = TypeRelations.subtype(
      e2.soundType,
      expected_t,
      TcUtil.allowDeepSubtyping(ast.right)
    );
    if (
      ...
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Binary_operator_expects_both_any,
        [e1.soundType.toString(), e2.soundType.toString()],
        ast
      );
      return this.pkg(
        ast,
        MkAST.unsafe(new BinaryExpression(ast.kind(), e1, e2)),
        TConstant.Bool
      );
    }
    if (sc1.fst && sc2.fst) {
      return this.pkg(
        ...
        new BinaryExpression(
          ast.kind(),
          TcUtil.shallowTag(sc1, e1, TConstant.Any),
          TcUtil.shallowTag(sc2, e2, TConstant.Any)
        ),
        expected_t
      );
    } else {
      return this.pkg(
        ast,
        TcUtil.force(TConstant.Bool, new BinaryExpression(ast.kind(), e1, e2)),
        TConstant.Bool
      );
    }
  }
  ...
  private tcThrowStatement(ast: ThrowStatement) {
    ...
    var sc = TypeRelations.subtype(
      exn.soundType,
      TConstant.Any,
      TcUtil.allowDeepSubtyping(ts.expression)
    );
    if (!sc.fst) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Throw_not_any,
        [exn.soundType.toString()],
        ts.expression
      );
    } else {
  ...
  private tcDeleteExpression(ast: DeleteExpression) {
    ...
    switch (expression.kind()) {
      case SyntaxKind.ElementAccessExpression:
        ...
        var safeDelete = MkAST.callExpr(MkAST.fieldOfRT('deleteField'), [
          eae.expression,
      ...
      case SyntaxKind.MemberAccessExpression:
        ...
        if (mae.soundType.unfold() !== TConstant.Any) {
          TcUtil.Logger.error(
            DiagnosticCode.SEC_Delete_operator_expects_any,
            [mae.soundType.toString()],
            ast
          );
          ...
          return this.pkg(
            ast,
            MkAST.callExpr(MkAST.fieldOfRT('deleteField'), [
              mae.expression,
              MkAST.stringConst(mae.name.text()),
            ])
          );
        }
      ...
      default:
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Delete_operator_expects_member_or_element_access,
          [],
          ast
        );
  ...
  private tcConditionalExpression(ast: ConditionalExpression) {
    ...
    var bthen = TypeRelations.subtype(
      thenBod.soundType,
      expected_t,
      TcUtil.allowDeepSubtyping(ast.whenTrue)
    );
    var belse = TypeRelations.subtype(
      elseBod.soundType,
      expected_t,
      TcUtil.allowDeepSubtyping(ast.whenFalse)
    );
    var res: AST = new ConditionalExpression(
      cond,
      TcUtil.shallowTag(bthen, thenBod, expected_t),
      TcUtil.shallowTag(belse, elseBod, expected_t)
    );
    ...
    if (!bthen.fst) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Conditional_branch,
        [expected_t.toString(), thenBod.soundType.toString()],
        ast.whenTrue
      );
    }
    if (!belse.fst) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Conditional_branch,
        [expected_t.toString(), elseBod.soundType.toString()],
        ast.whenFalse
      );
    }
  ...
  private tcCommaExpression(ast: BinaryExpression) {
    ...
    return this.pkg(
      ast,
      new BinaryExpression(SyntaxKind.CommaExpression, e1, e2),
      e2.soundType
    );
  }
  ...
  private tcForStatement(ast: ForStatement) {
    ...
    var vd = fs.variableDeclaration
      ? <VariableDeclaration>this.tc(fs.variableDeclaration)
      : null;
    ...
    var bodyR = fs.statement
      ? this.tcaux(fs.statement)
      : { fst: <AST>null, snd: false };
  ...
  private tcForInStatement(ast: ForInStatement) {
    ...
    var rvalAny = TypeRelations.subtype(
      rval.soundType,
      TConstant.Any,
      TcUtil.allowDeepSubtyping(fis.expression)
    );
    if (!rvalAny.fst) {
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Property_iteration_on_non_any,
        [rval.soundType.toString()],
        fis.expression
      );
    }
    ...
    var guardBlock = (id: Identifier, stmt: AST) => {
      switch (stmt.kind()) {
        case SyntaxKind.Block:
          ...
          var rttiCheck = new IfStatement(
            new BinaryExpression(
              SyntaxKind.EqualsExpression,
              id,
              MkAST.stringConst('__rtti__')
            ),
          ...
          var block = new Block(
            bodyL,
            b.closeBraceLeadingComments,
            b.closeBraceToken
          );
          return this.pkg(fis.statement, block).fst;
        default:
          throw new Error(
            "For/in loop without a block body ... it's a " +
              kind2string(stmt.kind())
          );
      }
    };
    if (fis.variableDeclaration && !fis.left) {
      ...
      var bodyR = this.tcenv.newScope(() =>
        this.tcenv.withVariable<Pair<AST, boolean>>(
          decls,
          iterVar.text(),
          TConstant.String
      ...
      var res = new ForInStatement(
        ...
        fis.left,
        rval,
        guardBlock(MkAST.id(iterVar.text()), bodyR.fst)
      );
      return this.pkg(ast, res, TConstant.Void, bodyR.snd);
    } else if (
      !fis.variableDeclaration &&
      fis.left &&
      fis.left.kind() === SyntaxKind.IdentifierName
    ) {
      var id = <Identifier>fis.left;
      var xt = this.tcenv.lookup(id.text());
      if (
        !xt ||
        (xt.typeName !== TypeName.String && xt.typeName !== TypeName.Any)
      ) {
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Invalid_lval_iteration,
          [
            'Expected a string- or any-typed variable; got ' + xt
              ? xt.toString()
              : 'unknown',
          ],
          fis.left
  ...
  private tcWhileStatement(ast: WhileStatement) {
    ...
    return this.pkg(
      ast,
      new WhileStatement(cond, bodyR.fst),
      TConstant.Void,
      bodyR.snd
    );
  }
  private tcDoStatement(ast: DoStatement) {
    ...
    return this.pkg(
      ast,
      new DoStatement(bodyR.fst, dos.whileKeyword, cond),
      TConstant.Void,
      bodyR.snd
    );
  }
  private tcIfStatement(ast: IfStatement) {
    ...
    return this.pkg(
      ...
      new IfStatement(
        cond,
        thenBodR.fst,
        elseBodR ? <ElseClause>elseBodR.fst : null
      ),
      TConstant.Void,
      thenBodR.snd && elseBodR ? elseBodR.snd : true
    );
  }
  ...
  private tcVariableStatement(ast: VariableStatement) {
    ...
    return this.pkg(
      ast,
      new VariableStatement(ast.modifiers, vd),
      vd.soundType
    );
  }
  ...
  private tcTryStatement(ast: TryStatement) {
    ...
    var finally_clauseR = ast.finallyClause
      ? this.tcaux(ast.finallyClause)
      : null;
    return this.pkg(
      ast,
      new TryStatement(
        <Block>tryBodyR.fst,
        ast.catchClause ? <CatchClause>catch_clauseR.fst : null,
        ast.finallyClause ? <FinallyClause>finally_clauseR.fst : null
      ),
      TConstant.Void,
      tryBodyR.snd &&
        (!catch_clauseR || catch_clauseR.snd) &&
        (!finally_clauseR || finally_clauseR.snd)
    );
  }
  private tcCatchClause(ast: CatchClause) {
    var vd = ast.identifier;
    var err_t = TConstant.Any;
    var catch_bodyR = this.tcenv.withVariable<Pair<AST, boolean>>(
      ast,
      vd.text(),
      err_t
    ...
    return this.pkg(
      ast,
      new CatchClause(vd, ast.typeAnnotation, <Block>catch_bodyR.fst),
      TConstant.Void,
      catch_bodyR.snd
    );
  }
  private tcFinallyClause(ast: FinallyClause) {
    ...
    return this.pkg(
      ast,
      new FinallyClause(<Block>block.fst),
      TConstant.Void,
      block.snd
    );
  }
  private tcReturnStatement(ast: ReturnStatement) {
    ...
    if (result && this.result_t && !sc.fst) {
      if (TypeRelations.assignable(this.result_t, result_t)) {
        TcUtil.Logger.warning(
          DiagnosticCode.SEC_Implicit_coercion,
          [result_t.toString(), this.result_t.toString()],
          ast
        );
        ...
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Function_return_type,
          [this.result_t.toString(), result_t.toString()],
          ast
        );
  ...
  private tcSwitchStatement(ast: SwitchStatement) {
    ...
    switch (guard.soundType.unfold().typeName) {
      ...
      default:
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Switch_guard_not_primitive,
    ...
    return this.tcenv.newScope(() => {
      for (var i = 0; i < ast.switchClauses.childCount(); i++) {
        var caseClauseR = this.tcenv.newScope(() =>
          this.tcaux(ast.switchClauses.childAt(i))
        );
        if (caseClauseR.fst.kind() === SyntaxKind.CaseSwitchClause) {
          var csc = <CaseSwitchClause>caseClauseR.fst;
          if (
            !TypeRelations.subtype(
              csc.expression.soundType,
              guard.soundType,
              TcUtil.allowDeepSubtyping(ast.switchClauses.childAt(i))
            ).fst
          ) {
            TcUtil.Logger.error(
              DiagnosticCode.SEC_Switch_case_incompatible,
              [guard.soundType.toString(), csc.expression.soundType.toString()],
              ast.switchClauses.childAt(i)
            );
          }
        }
        ...
        if (i === ast.switchClauses.childCount() - 1) {
          returns =
            caseClauseR.fst.kind() === SyntaxKind.DefaultSwitchClause &&
            caseClauseR.snd;
        }
      }
      ...
      return this.pkg(
        ast,
        new SwitchStatement(guard, ast.closeParenToken, switchClauses),
        TConstant.Void,
        returns
      );
    });
  ...
  private tcCaseSwitchClause(ast: CaseSwitchClause) {
    ...
    return this.pkg(
      ast,
      new CaseSwitchClause(expr, <ISyntaxList2>st.fst),
      TConstant.Void,
      st.snd
    );
  }
  private tcDefaultSwitchClause(ast: DefaultSwitchClause) {
    ...
    return this.pkg(
      ast,
      new DefaultSwitchClause(<ISyntaxList2>st.fst),
      TConstant.Void,
      st.snd
    );
  }
  ...
  private tcLabeledStatement(ast: LabeledStatement) {
    ...
    return this.pkg(
      ast,
      new LabeledStatement(ast.identifier, st),
      TConstant.Void
    );
  }
