...
///<reference path='translateTypes.ts' />
  ...
  module ScopeStack {
    ...
    export function pushVariable(
      ...
      if (x === 'arguments') {
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Arguments_is_a_reserved_name,
          [],
          a
        );
      }
      var old = lookupVariable(s, x, [
      ...
      if (!allowShadowing && old) {
        //shadowed
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Variable_shadowing,
          [x, TcUtil.Logger.pos(old.snd)],
          a
        );
      }
      var sc = peek(s);
  ...
  export class TcEnv implements RT.Virtual {
    ...
    public inFunctionScope() {
      return this.scopes.scopes.some(
        (s) => s.scopeType === ScopeType.FunctionBlock
      );
    }
    ...
    public curNamespace(stopAtClass: boolean = false): string[] {
      ...
      this.openNamespaces.forEach((t: NamedType) => {
        if (
          t.name !== '<global>' &&
          !(t.typeName === TypeName.Class && stopAtClass)
        ) {
          ns.push(t.name);
        }
      });
        ...
        if (f) {
          if (!circularOk && ns.isCircular(x)) {
            ...
            TcUtil.Logger.error(
              ...
              [ns.name + '.' + x, TcUtil.Logger.pos(md)]
            );
          }
    ...
    public lookupFullName(q: QName, circularOk = false): SoundType {
      ...
      for (var i = 0; i < x.length - 1; i++) {
        ...
        if (!next || next.type.typeName !== TypeName.Module) {
          TcUtil.Logger.warn(
            'Module ' + x[i] + ' not found; full name is ' + x.join('.')
          );
          return null;
        }
    ...
    public withVariables<A>(
      bindings: TcUtil.VariableBinding[],
      allowShadowing: boolean = false
    ): (f: () => A) => A {
      var clear = () => {
    ...
    public withClass<A>(x: string) {
      ...
      if (tclass && tclass.typeName === TypeName.Class) {
        return (f: () => A) =>
          this.withNS<A>(<TClass>tclass)(() => this.newScope(f));
      } else {
    ...
    public newScope<A>(
      f: () => A,
      scopeType: ScopeType = ScopeType.RegularBlock
    ): A {
    ...
    public thisType(): SoundType {
      ...
      if (cn) {
        ...
        switch (t.typeName) {
          ...
          default:
            throw new Error(
              'impossible!: class has an object of type ' + t.toString()
            );
        }
        return;
      }
    ...
    public addModuleMember(
      ...
      for (var i = 0; i < fullPath.length; i++) {
        ...
        if (f.type.typeName !== TypeName.Module) {
          TcUtil.Logger.error(
            DiagnosticCode.SEC_Module_name_clash,
            [fullPath[i]],
            b
          );
          return;
      ...
      if (ns.hasField(fieldName)) {
        ...
        if (decl && !decl.snd) {
          TcUtil.Logger.error(
            DiagnosticCode.SEC_Module_overlapping_field,
            [ns.name, fieldName, TcUtil.Logger.pos(decl.fst)],
            b
          );
        }
        decl.snd = false;
    ...
    public buildSignature(
      elts: ISyntaxList2,
      currentModule: TModule = this.global
    ) {
    ...
    private buildSignatureAux(
      ...
      var tryMergeType = (mod: TModule, a: AST, id: string, t: SoundType) => {
          ...
          return i1.concat(
            i2.filter((f2: Field) => {
              if (i1.some((f1: Field) => f1.name === f2.name)) {
                TcUtil.Logger.error(
                  DiagnosticCode.SEC_Interface_overlapping_field,
                  [name, f2.name],
                  a
                );
                return false;
        ...
        if (
          old_t &&
          old_t.typeName === TypeName.Interface &&
          t.typeName === TypeName.Interface
        ) {
          ...
          var extendsClause = i1.extendsC.concat(
            i2.extendsC.filter((t2) => !i1.extendsC.some((t1) => t1.equals(t2)))
          );
      ...
      TcUtil.syntaxListMembers(elts).forEach((a: AST) => {
        ...
        switch (a.kind()) {
            ...
            var withNames = (currentModule: TModule, i: number) => {
              ...
              if (!nextModuleField) {
                ...
                tryAddMember(
                  currentModule,
                  a,
                  nextModuleField.name,
                  nextModuleField.type
                );
              }
              ...
              this.withModule<void>(nextModuleField.name)(() => {
                if (i === names.length - 1) {
                  this.buildSignatureAux(
                    md.moduleElements,
                    <TModule>nextModuleField.type
                  );
                } else {
          ...
          case SyntaxKind.ClassDeclaration:
            ...
            var constructorType =
              this.symbol(a).type.getConstructorMethod().type; //Strange that it is stashed away in here ... but it is what it is
            var classType = this.withLocalTypes(
              TcUtil.classDeclTypeParams(cd, this),
              () => TranslateTypes.translateType(constructorType, this)
            );
          ...
          case SyntaxKind.InterfaceDeclaration:
            ...
            if (!currentModule.lookupType(id.identifier.text())) {
              ...
              var interfaceType = TranslateTypes.translateType(
                this.symbol(a).type,
                this
              );
              if (
                ...
                (interfaceType.typeName === TypeName.Interface ||
                  (interfaceType.typeName === TypeName.Poly &&
                    (<TPoly>interfaceType).body.typeName ===
                      TypeName.Interface) ||
          ...
          case SyntaxKind.EnumDeclaration:
            ...
            var enumType = TranslateTypes.translateType(
              this.symbol(a).type,
              this
            );
            ...
            if (enumType && enumType.typeName === TypeName.Enum) {
              ...
              enumContainer.addIndexSignature(
                new TIndexMap(enumType, TConstant.String)
              );
              tryAddMember(
                currentModule,
                a,
          ...
          case SyntaxKind.FunctionDeclaration:
            ...
            if (ftype) {
              ...
              if (
                ftyp &&
                (ftyp.typeName === TypeName.Arrow ||
                  ftyp.typeName === TypeName.Poly ||
                  ftyp.callSignature())
              ) {
          ...
          case SyntaxKind.VariableStatement:
            ...
            if (
              hasModifier(vs.modifiers, PullElementFlags.Ambient) ||
              hasModifier(vs.modifiers, PullElementFlags.Exported)
            ) {
              TcUtil.mapSepList2(vs.declaration.declarators, (b: AST) => {
                ...
                if (vdr.inferredType.isType()) {
                  var var_t = TranslateTypes.translateType(
                    <PullTypeSymbol>vdr.inferredType,
                    this
                  );
                  if (
                    hasModifier(vs.modifiers, PullElementFlags.Ambient) &&
                    !TcUtil.isPrimitive(var_t)
                  ) {
                  ...
                  throw new Error(
                    'Type inferred for ' + var_x + ' is not a type'
                  );
                }
              });
            }
