...
import { TcUtil, Pair } from './tcUtil';
import {
  TConstant,
  TRecord,
  TClass,
...
export module TranslateTypes {
  ...
  export function translateTypeOrSig(
    t: PullSymbol,
    tcenv: TcEnv,
    debug: boolean = false
  ): SoundType {
    if (t.isType()) return translateType(<PullTypeSymbol>t, tcenv, debug);
      ...
      if (t.kind === PullElementKind.IndexSignature) {
        ...
        return constructSignatureToPolyArrow(
          <PullSignatureSymbol>t,
          tcenv,
          debug
        );
      }
  ...
  function translateTypeInternal(
    t: PullTypeSymbol,
    tcenv: TcEnv,
    debug: boolean = false
  ): SoundType {
  ...
  function translateTypeBody(
    ...
    if (!t) {
      console.log(
        TcUtil.Logger.pos() +
          ": Warning: TypeScript inferred an undefined type; mapping it to 'any'"
      );
      return TConstant.Any;
    }
    var typeOfSym = t.getTypeOfSymbol();
    if (typeOfSym) {
      var res = tcenv.lookupFullName(
        { dottedName: typeOfSym.fullName() },
        true
      );
      if (res) {
        return res;
  ...
  function addMembersAndMethods(
    ...
    if (t.getConstructSignatures()) {
      t.getConstructSignatures().map((constructSig) =>
        r.addConstructSignature(
          constructSignatureToPolyArrow(constructSig, tcenv, debug)
        )
      );
    }
    ...
    for (var i = 0; mems && i < mems.length; i++) {
      if (
        mems[i].isMethod() ||
        mems[i].kind === PullElementKind.FunctionExpression
      ) {
        var callSigs = mems[i].type.getCallSignatures();
        callSigs.forEach((s: PullSignatureSymbol) =>
          r.addMethod(
            Method(mems[i].name, signatureToPolyArrow(s, tcenv, debug))
          )
        );
      } else {
        r.addField(
  ...
  function fullNameAndParams(
    ...
    if (tcenv.compilationSettings.generics() && t.isGeneric()) {
      if (t.getTypeArguments()) {
        //this is not a binder for the type parameters
        var targs = t
          .getTypeArguments()
          .map((t) => translateTypeInternal(t, tcenv, debug));
        return {
          fullName: fullName,
          targs: targs,
          binders: emptyBinders,
        ...
        var tparams = t.getTypeParameters().map((ptps) => {
          var tv = new TVar(
            TcUtil.mkTypeParamName(fullName, ptps.name),
            ptps.name
          );
  ...
  function translateNamedType(
    ...
    return tcenv.withLocalTypes(nameAndArgs.binders, () => {
      ...
      if (t.isEnum()) {
        ...
        if (tcenv.compilationSettings.generics() && !tcenv.inBuildPhase) {
          console.log(
            TcUtil.Logger.pos() +
              ' type variable ' +
              nameAndArgs.fullName +
              " not found; defaulting to 'any'"
          );
        }
        return TConstant.Any;
      }
      ...
      if (nameAndArgs.targs && !nameAndArgs.tparams) {
        ...
        return new TInst(
          new TPoly([], mkT(), nameAndArgs.fullName),
          nameAndArgs.targs
        );
      }
      ...
      var buildScheme = () => {
        iot.extendsC = t
          .getExtendedTypes()
          .map((pts) => translateTypeInternal(pts, tcenv, debug));
        iot.implementsI = t
          .getImplementedTypes()
          .map((pts) => translateTypeInternal(pts, tcenv, debug));
  ...
  function signatureToPolyArrow(
    f: PullSignatureSymbol,
    tcenv: TcEnv,
    debug = false
  ): SoundType {
    var translateArgs = (f: PullSignatureSymbol) => {
      ...
      for (var i = 0; i < f.parameters.length; i++) {
        ...
        args.push(
          new TArg(
            f.parameters[i].name,
            translateTypeInternal(f.parameters[i].type, tcenv, debug),
            flags,
            f.parameters[i].isVarArg
          )
        );
      }
      return args;
    ...
    var binders = !f.typeParameters
      ...
      : f.typeParameters.map((tp: PullTypeSymbol) => {
          ...
          if (t && t.typeName === TypeName.Variable) {
            ...
            var constraint = resolveConstraint(
              (<PullTypeParameterSymbol>tp).getConstraint()
            );
    ...
    var arr = tcenv.withLocalTypes(
      ...
      () =>
        new TArrow(
          translateArgs(f),
          translateTypeInternal(f.returnType, tcenv, debug)
        )
    );
    return TcUtil.close(binders, arr);
  }
  function signatureToIndexMap(
    i: PullSignatureSymbol,
    tcenv: TcEnv,
    debug = false
  ): TIndexMap {
    if (i.kind !== PullElementKind.IndexSignature)
      return unexpectedSymbol<TIndexMap>(i);
  ...
  function constructSignatureToPolyArrow(
    t: PullSignatureSymbol,
    tcenv: TcEnv,
    debug = false
  ): SoundType {
    var constr_t = signatureToPolyArrow(<PullSignatureSymbol>t, tcenv, debug);
    if (constr_t.typeName === TypeName.Poly) {
      //need to massage it a bit to move the type instantiations to the result
      ...
      if (tarrow.result.typeName === TypeName.Poly) {
        var f2 = new TArrow(
          ...
          new TInst(
            <TPoly>tarrow.result,
            (<TPoly>constr_t).bvars.map((t) => <SoundType>t)
          )
        ); //NS: real type error here: bvars is a TVar[], expect a SoundType[]
