...
import { PullElementFlags, hasModifier } from '../pullFlags';
import { PullTypeSymbol, PullSignatureSymbol } from '../pullSymbols';
import { SoundTypeChecker } from './tc';
import { TcEnv } from './tcEnv';
import { TranslateTypes } from './translateTypes';
import { NamedType, TInst, TPoly, TConstant, TVar, TInterface } from './types';
...
export class MkAST {
  ...
  static argList = (args: AST[]) => {
    return new ISeparatedSyntaxList2(
      TcUtil.currentFile(),
      args,
      args.length - 1
    );
  ...
  static callExpr = (f: AST, args: AST[]): AST => {
    return new InvocationExpression(
      f,
      new ArgumentList(null, MkAST.argList(args), new ASTSpan(0, 0))
    );
  };
  ...
  static mkCleanArray = (args: AST[]) => {
    return new ArrayLiteralExpression(
      MkAST.argList(args),
      NewLineMode.Suppress
    );
  };
  static mkObjLit = (args: Pair<string, AST>[]): AST => {
    var argL = args.map((p: Pair<string, AST>): AST => {
      return new SimplePropertyAssignment(
        MkAST.stringConst(p.fst), //NS: SimplePropertyAssignment used to expect an Identifier; changed it to take an AST for JSON keys
        p.snd
      );
    });
  ...
  static ttConst = (): AST => {
    return new LiteralExpression(SyntaxKind.TrueKeyword, 'true', 'true');
  };
  static ffConst = (): AST => {
    return new LiteralExpression(SyntaxKind.FalseKeyword, 'false', 'false');
  };
...
export module TcUtil {
  ...
  export function markAsVirtual(tif: SoundType) {
    return isCurrentScriptLib_d_ts() && !isPrimitive(tif);
  }
  export function markAsNominal(tif: SoundType) {
    return isCurrentScriptLib_d_ts() && isPrimitive(tif);
  }
  export function shallowTag(
    sc: Pair<boolean, Delta>,
    a: AST,
    ta: SoundType
  ): AST {
    if (sc.fst && sc.snd) {
  ...
  export function force(t: SoundType, a: AST): AST {
    ...
    switch (t.typeName) {
      ...
      case TypeName.Number:
        return new PrefixUnaryExpression(SyntaxKind.PlusExpression, a);
      ...
      case TypeName.Bool:
        return new ConditionalExpression(
          a,
          MkAST.boolConst(true),
          MkAST.boolConst(false)
        );
      default:
        throw new Error('Unknown force type ' + t.toString());
    }
  }
  export function zip<A, B>(a: A[], b: B[]): Pair<A, B>[] {
    ...
    return a.map(
      (x: A, i: number, a: A[]): Pair<A, B> => ({ fst: x, snd: b[i] })
    );
  }
  ...
  export function NYI(msg?: string): Pair<AST, boolean> {
    throw new Error(
      msg
        ? msg + ' not supported in [--secure|--safe] mode'
        : 'Not yet implemented'
    );
  }
  ...
  export function mapSepList2<B>(
    l: ISeparatedSyntaxList2,
    f: (a: AST) => B
  ): B[] {
    var out: B[] = [];
    for (var i = 0; i < l.nonSeparatorCount(); i++) {
  ...
  export function mapSepList2WithIndex<B>(
    l: ISeparatedSyntaxList2,
    f: (a: AST, i: number) => B
  ): B[] {
    var out: B[] = [];
  ...
  export function callSigTypeParamBindings(
    callSig: CallSignature
  ): LocalTypeBinding[] {
    if (callSig && callSig.typeParameterList) {
      return TcUtil.mapSepList2(
        callSig.typeParameterList.typeParameters,
        (a: AST) => {
  ...
  export function typeParameterListBindings(
    tps: TypeParameterList,
    fullName: string[] = []
  ): LocalTypeBinding[] {
    if (tps && tps.typeParameters.nonSeparatorCount() !== 0) {
      return TcUtil.mapSepList2(tps.typeParameters, (t: AST) =>
        (<TVar>t.soundType).asBinder(fullName)
      );
    }
    return [];
  }
  export function classDeclTypeParams(
    cd: ClassDeclaration,
    tcenv: TcEnv
  ): LocalTypeBinding[] {
    if (cd.typeParameterList) {
      ...
      return TcUtil.mapSepList2(
        cd.typeParameterList.typeParameters,
        (t: AST) => {
          var n = (<TypeParameter>t).identifier.text();
          return {
            fst: n,
            snd: <SoundType>new TVar(TcUtil.mkTypeParamName(fullName, n), n),
          };
        }
      );
    }
  ...
  export function isClassType(t: PullTypeSymbol) {
    ...
    return (
      ...
      t
        ...
        .some(
          (s) =>
            s.name === 'prototype' && s.anyDeclHasFlag(PullElementFlags.Static)
        ) &&
  ...
  export function tvarsInScopeForClassElement(elt: AST, tcenv: TcEnv) {
    switch (elt.kind()) {
      ...
      case SyntaxKind.MemberFunctionDeclaration:
        return !hasModifier(
          (<MemberFunctionDeclaration>elt).modifiers,
          PullElementFlags.Static
        );
      case SyntaxKind.MemberVariableDeclaration:
        return !hasModifier(
          (<MemberVariableDeclaration>elt).modifiers,
          PullElementFlags.Static
        );
      ...
      case SyntaxKind.SetAccessor:
        return !hasModifier(
          (<SetAccessor>elt).modifiers,
          PullElementFlags.Static
        );
      case SyntaxKind.GetAccessor:
        return !hasModifier(
          (<GetAccessor>elt).modifiers,
          PullElementFlags.Static
        );
      default:
        TcUtil.Logger.error(
          DiagnosticCode.SEC_Unexpected_class_element,
          [kind2string(elt.kind())],
          elt
        );
        return false;
    }
  ...
  export function mkVariableBinding(
    a: AST,
    name: string,
    type: SoundType
  ): VariableBinding {
  ...
  export function gatherFunctionBindings(
    elts: ISyntaxList2
  ): VariableBinding[] {
    ...
    for (var i = 0; i < elts.childCount(); i++) {
      ...
      switch (a.kind()) {
        case SyntaxKind.FunctionDeclaration:
          ...
          if (
            !TcEnv.currentEnv.symbol(a).anyDeclHasFlag(PullElementFlags.Ambient)
          ) {
            var signature = TcEnv.currentEnv.symbol(a);
            bs.push({
              ast: a,
              name: fd.identifier.text(),
              type: TranslateTypes.translateType(
                signature.type,
                TcEnv.currentEnv
              ),
            });
  ...
  export function isArray(i: SoundType) {
    if (i.typeName === TypeName.Inst) {
      var t = (<TInst>i).t1;
      return (
        t.body.typeName === TypeName.Interface &&
        (<TInterface>t.body).name === 'Array'
      );
    }
    return i.isArray();
  }
    ...
    function captureFree(a: AST): boolean {
      ...
      switch (a.kind()) {
        ...
        case SyntaxKind.ObjectLiteralExpression:
          ...
          return TcUtil.sepListMembers(o.propertyAssignments).every(
            captureFree
          );
        ...
        case SyntaxKind.ConditionalExpression:
          return (
            captureFree((<ConditionalExpression>a).whenTrue) &&
  ...
  function invocationFree(a: AST): boolean {
    switch (a.kind()) {
      ...
      case SyntaxKind.SeparatedList:
        return TcUtil.sepListMembers(<ISeparatedSyntaxList2>a).every(
          invocationFree
        );
      case SyntaxKind.ObjectLiteralExpression:
      ...
      case SyntaxKind.ElementAccessExpression:
        return (
          invocationFree((<ElementAccessExpression>a).expression) &&
      ...
      case SyntaxKind.ConditionalExpression:
        return (
          invocationFree((<ConditionalExpression>a).condition) &&
      ...
      default:
        if (a instanceof BinaryExpression) {
          return (
            invocationFree((<BinaryExpression>a).left) &&
            invocationFree((<BinaryExpression>a).right)
          );
  ...
  export function close(binders: LocalTypeBinding[], t: SoundType) {
    if (binders.length === 0) return t;
      ...
      return new TPoly(
        binders.map((xt) => <TVar>xt.snd),
        t
      );
  }
  export function argumentListTypeArgs(
    tcenv: TcEnv,
    args: ArgumentList,
    ...
    if (args.typeArgumentList && args.typeArgumentList.typeArguments) {
      return TcUtil.mapSepList2(
        args.typeArgumentList.typeArguments,
        (t: AST) => t.soundType
      );
    } else if (inferredTypeArgs && inferredTypeArgs.length !== 0) {
      return inferredTypeArgs.map((t) =>
        TranslateTypes.translateType(t, tcenv)
      );
    } else {
      return null;
    }
  ...
  export function instantiateType(
    ...
    if (targs && targs.length !== 0) {
      if (t.typeName !== TypeName.Poly) {
        TcUtil.Logger.error(DiagnosticCode.SEC_Unexpected_type_arguments, [
          t.toString(),
          overload.toString(),
          targs.map((t) => t.toString()).join(', '),
        ]);
        return t;
      }
      ...
      targs.forEach((t, i) => {
        if (!tpoly.checkInstantiation(i, t)) {
          TcUtil.Logger.error(
            DiagnosticCode.SEC_type_instantiation_not_subtype_constraint,
            [t.toString(), tpoly.bvars[i].toString()]
          );
        }
      });
      ...
      TcUtil.Logger.error(
        DiagnosticCode.SEC_Could_not_infer_type_arguments,
        []
      );
  ...
  function isGenerics(msg: string) {
    switch (msg) {
      ...
      case DiagnosticCode.SEC_Generic_types_unsupported:
        return true;
      default:
        return false;
    }
...
export function kind2string(k: SyntaxKind) {
  switch (k) {
    ...
    case SyntaxKind.SignedRightShiftAssignmentExpression:
      return 'SignedRightShiftAssignmentExpression';
    case SyntaxKind.UnsignedRightShiftAssignmentExpression:
      return 'UnsignedRightShiftAssignmentExpression';
    case SyntaxKind.ConditionalExpression:
    ...
    case SyntaxKind.EqualsWithTypeConversionExpression:
      return 'EqualsWithTypeConversionExpression';
    case SyntaxKind.NotEqualsWithTypeConversionExpression:
      return 'NotEqualsWithTypeConversionExpression';
    case SyntaxKind.EqualsExpression:
    ...
    case SyntaxKind.SignedRightShiftExpression:
      return 'SignedRightShiftExpression';
    case SyntaxKind.UnsignedRightShiftExpression:
      return 'UnsignedRightShiftExpression';
    case SyntaxKind.MultiplyExpression:
    ...
    case SyntaxKind.ExtendsHeritageClause:
      return 'ExtendsHeritageClause';
    case SyntaxKind.ImplementsHeritageClause:
      return 'ImplementsHeritageClause';
    case SyntaxKind.EqualsValueClause:
