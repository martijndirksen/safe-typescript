...
export module TypeRelations {
  ...
  var fail: Pair<boolean, TcUtil.Delta> = {
    fst: false,
    snd: <TcUtil.Delta>undefined,
  };
  ...
  export function assignable(t1: SoundType, t2: SoundType): boolean {
    return (
      (t1.unFree() && !t1.isUn() && t2.typeName === TypeName.Any) ||
  ...
  export function typeEquality(
    ...
    if (TcUtil.isArray(t1) && TcUtil.isArray(t2)) {
      return typeEquality(
        TcUtil.arrayElementType(t1),
        TcUtil.arrayElementType(t2),
        cycles,
        debug
      );
    }
    switch (t1.typeName) {
      ...
      case TypeName.IndexMap:
        if (t2.typeName === TypeName.IndexMap) {
          ...
          var i2 = <TIndexMap>t2;
          return (
            typeEquality(i1.elt, i2.elt, cycles, debug) &&
            typeEquality(i1.indexType, i2.indexType, cycles, debug)
          );
        }
        return false;
      case TypeName.Interface:
        ...
        if (t2.typeName === TypeName.Interface) {
          //TODO: extend cycles?
          return typeEquality(
            (<TInterface>t1).toRecord(),
            (<TInterface>t2).toRecord(),
            cycles,
            debug
          );
        }
  ...
  function deepArrayOrMapSubtyping(
    t1: SoundType,
    t2: SoundType,
    fc: TcUtil.fc,
    cycles: Cycles,
    ...
    var sub: (
      ...
      cycles?: Cycles,
      debug?: any
    ) => boolean = fc.captureFree ? isSubtype : subtypeZ;
    if (TcUtil.isArray(t1) && TcUtil.isArray(t2)) {
      return sub(
        TcUtil.arrayElementType(<TInst>t1),
        TcUtil.arrayElementType(<TInst>t2),
        fc,
        cycles,
        debug
      );
    } else if (
      t1.typeName === TypeName.IndexMap &&
      t2.typeName === TypeName.IndexMap
    ) {
      var i1 = <TIndexMap>t1;
      var i2 = <TIndexMap>t2;
      return (
        sub(i2.indexType, i1.indexType, fc, cycles, debug) &&
        sub(i1.elt, i2.elt, fc, cycles, debug)
      );
    }
    return false;
  }
  function arrowSubtyping(
    a1: TArrow,
    a2: TArrow,
    cycles: Cycles,
    debug?: any
    ...
    for (var i = 0; i < a1.args.length; i++) {
      ...
      if (a2.args[i].variadic && !a1.args[i].variadic) {
        ...
        if (
          !elt_t.isArray() ||
          !subtypeZ(
            (<TInterface>elt_t).arrayElementType(),
            a1.args[i].type,
            TcUtil.fcNeither,
            cycles,
            debug
  ...
  function recordSubtyping(
    ...
    var isDefaultIndexSignature = (
      ...
      if (
        f.type.typeName === TypeName.IndexMap &&
        (<TIndexMap>f.type).indexType.typeName === TypeName.String
      ) {
        return f1s.every((f1) =>
          sub(f1.type, (<TIndexMap>f.type).elt, fc, cycles, debug)
        );
      }
      return false;
    };
        ...
        if (matches.length >= 1) {
          ...
          if (isField) {
            ...
            if (typeEquality(f1.type, f2.type, cycles, debug)) {
              ...
              f1.type.typeName === TypeName.Arrow &&
    ...
    if (fc.fresh || fc.captureFree) {
      var f2 = r2
        .exposeFields()
        .filter((f2) => f2.type.typeName === TypeName.IndexMap);
      if (
        f2.length === 1 &&
    ...
    var dm = result.snd;
    var delta =
      (df.length === 0 && dm.length === 0) ||
      SoundTypeChecker.compilationSettings.tsstarTagging()
        ? zero
        : new TRecord(df, dm); //AR
    return success(delta);
  }
  export function subtypeZ(
    t1: SoundType,
    t2: SoundType,
    fc: TcUtil.fc = TcUtil.fcNeither,
    cycles: Cycles = [],
    debug?: any
  ) {
  ...
  export function subTypeOrSig(
    ...
    if (t1.typeName === TypeName.Arrow && t2.typeName === TypeName.Arrow) {
      ...
      t1.typeName !== TypeName.Arrow &&
      t2.typeName !== TypeName.Arrow
    ) {
  ...
  export function subtype(
        ...
        if (ss instanceof NamedType) {
          return (
            (t.typeName === TypeName.Interface &&
              inHierarchy(t, (<NamedType>ss).implementsI)) ||
    ...
    if (
      t1.equals(t2) ||
      cycles.some((s1s2) => s1s2.fst.equals(t1) && s1s2.snd.equals(t2))
    ) {
      //S-Refl
      return success(zero);
    }
    ...
    if (
      (fc.captureFree && deepArrayOrMapSubtyping(t1, t2, fc, cycles, debug)) ||
      ...
      if (
        ...
        TcUtil.isArray(t2) &&
        TcUtil.isArray(t1) &&
        TcUtil.arrayElementType(t1).typeName === TypeName.UVar
      ) {
        return success(t2);
      }
      return success(zero);
    ...
    switch (t1.typeName) {
      ...
      case TypeName.Record:
        switch (t2.typeName) {
          ...
          case TypeName.Arrow:
            throw new Error(
              'Impossible: Arrow types should never be outside a structure'
            );
      ...
      case TypeName.Interface:
        if (t2.typeName === TypeName.Any && t1.unFree() && !t1.isVirtual()) {
          //S-Any
          if (t1.isNominal()) {
            return success(zero);
          }
          if (SoundTypeChecker.compilationSettings.tsstarTagging()) {
            //AR: if it's an array, return delta even in TS* case
            if (t1.isArray()) {
              return success(t1);
            } else {
              return success(zero);
          ...
          t2 instanceof NamedType &&
          inHierarchy(t2, (<TInterface>t1).extendsC)
        ) {
          if (t1.isNominal()) {
            return success(zero);
          }
            ...
            case TypeName.Interface: //S-IJ
              if (t1.isArray() && t2.isCheckedArray()) {
                ...
                var res = subtype(
                  ...
                  t2,
                  fc,
                  cycles.concat([pair(<NamedType>t1, <NamedType>t2)]),
                  debug
                );
                if (
                  res.fst &&
      ...
      case TypeName.Object:
        ...
        if (t2.typeName === TypeName.Any && t1.unFree()) {
          ...
          t2.typeName === TypeName.Object &&
          inHierarchy(t2, o.extendsC)
        ) {
          //S-Class
          return success(zero);
        } else {
          switch (t2.typeName) {
            case TypeName.Interface: //S-CI
              if (
                inHierarchy(t2, o.implementsI) ||
                o.extendsC.some((t) => isSubtype(t, t2, fc, cycles, debug))
              ) {
                return success(zero);
              } else if (
                isSubtype(
                  (<NamedType>t1).toRecord(),
                  (<NamedType>t2).toRecord(),
                  fc,
                  cycles.concat([pair(<NamedType>t1, <NamedType>t2)]),
                  debug
                )
      ...
      case TypeName.Poly:
        ...
        if (t2.typeName === TypeName.Poly) {
          var q2 = <TPoly>t2;
          if (
            q1.bvars.length === q2.bvars.length &&
            q1.bvars.every((t, i) => t.equals(q2.bvars[i]))
          ) {
