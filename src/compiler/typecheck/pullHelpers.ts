...
import { AST, GetAccessor, SetAccessor } from '../ast';
...
import { PullTypeReferenceSymbol } from './pullTypeInstantiation';
...
export module PullHelpers {
  ...
  export function getSignatureForFuncDecl(functionDecl: PullDecl) {
    ...
    if (
      ...
      functionDecl.kind === PullElementKind.ConstructSignature
    ) {
  ...
  export function getAccessorSymbol(
    getterOrSetter: AST,
  ...
  export function getGetterAndSetterFunction(
    funcDecl: AST,
    semanticInfoChain: SemanticInfoChain
  ): { getter: GetAccessor; setter: SetAccessor } {
  ...
  export function symbolIsEnum(source: PullSymbol): boolean {
    return (
      source &&
      (source.kind & (PullElementKind.Enum | PullElementKind.EnumMember)) !== 0
    );
  }
  ...
  export function symbolIsModule(symbol: PullSymbol) {
    return (
      symbol &&
      (symbol.kind == PullElementKind.Container ||
        isOneDeclarationOfKind(symbol, PullElementKind.Container))
    );
  }
  ...
  export function typeSymbolsAreIdentical(
    a: PullTypeSymbol,
    b: PullTypeSymbol
  ): boolean {
  ...
  export function isSymbolLocal(symbol: PullSymbol) {
    ...
    if (container) {
      ...
      if (
        containerKind &
        (PullElementKind.SomeFunction | PullElementKind.FunctionType)
      ) {
        return true;
      }
