  ...
  export module PullHelpers {
    ...
    export function getGetterAndSetterFunction(
      ...
      if (getter) {
        ...
        result.getter = <GetAccessor>(
          semanticInfoChain.getASTForDecl(getterDecl)
        );
      }
      var setter = accessorSymbol.getSetter();
      if (setter) {
        var setterDecl = setter.getDeclarations()[0];
        result.setter = <SetAccessor>(
          semanticInfoChain.getASTForDecl(setterDecl)
        );
      }
    ...
    export function symbolIsEnum(source: PullSymbol): boolean {
      return (
        source &&
        (source.kind & (PullElementKind.Enum | PullElementKind.EnumMember)) !==
          0
      );
    }
