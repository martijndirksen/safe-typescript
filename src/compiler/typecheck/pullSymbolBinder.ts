...
module TypeScript {
  export class PullSymbolBinder {
    ...
    private findDeclsInContext(
      startingDecl: PullDecl,
      declKind: PullElementKind,
      ...
      if (contextSymbolPath.length) {
        ...
        for (var i = 0; i < contextSymbolPath.length; i++) {
          ...
          copyOfContextSymbolPath[copyOfContextSymbolPath.length] =
            contextSymbolPath[i].name;
        }

        return this.semanticInfoChain.findDecls(
          copyOfContextSymbolPath,
          declKind
        );
      }
    }
    ...
    private getExistingSymbol(
      ...
      var lookingForContainer =
        (searchKind & PullElementKind.SomeContainer) !== 0;
      var name = decl.name;
      if (parent) {
        ...
        if (parentDecl && parentDecl.kind === PullElementKind.Script) {
          return this.semanticInfoChain.findTopLevelSymbol(
            name,
            searchKind,
            decl
          );
          ...
          var prevDecls =
            parentDecl && parentDecl.searchChildDecls(name, searchKind);
    ...
    private checkThatExportsMatch(
      decl: PullDecl,
      prevSymbol: PullSymbol,
      reportError = true
    ): boolean {
      ...
      var prevDecls = prevSymbol.getDeclarations();
      var prevIsExported =
        (prevDecls[prevDecls.length - 1].flags & PullElementFlags.Exported) !==
        0;
      if (
        isExported !== prevIsExported &&
        !prevSymbol.isSignature() &&
        (decl.kind & PullElementKind.SomeSignature) == 0
      ) {
        if (reportError) {
          var ast = this.semanticInfoChain.getASTForDecl(decl);
          this.semanticInfoChain.addDiagnosticFromAST(
            ast,
            DiagnosticCode.All_declarations_of_merged_declaration_0_must_be_exported_or_not_exported,
            [decl.getDisplayName()]
          );
        }
        return false;
    ...
    private bindEnumDeclarationToPullSymbol(enumContainerDecl: PullDecl) {
      ...
      var enumAST = <EnumDeclaration>(
        this.semanticInfoChain.getASTForDecl(enumContainerDecl)
      );

      var isExported = enumContainerDecl.flags & PullElementFlags.Exported;
      var isInitializedModule =
        (enumContainerDecl.flags & PullElementFlags.SomeInitializedModule) != 0;

      var createdNewSymbol = false;

      enumContainerSymbol = <PullContainerSymbol>(
        this.getExistingSymbol(enumContainerDecl, PullElementKind.Enum, parent)
      );
        ...
        if (enumContainerSymbol.kind !== enumDeclKind) {
          ...
          if (isInitializedModule) {
            this.semanticInfoChain.addDiagnosticFromAST(
              ...
              [enumContainerDecl.getDisplayName()]
            );
          }
          enumContainerSymbol = null;
        } else if (
          !this.checkThatExportsMatch(enumContainerDecl, enumContainerSymbol)
        ) {
          enumContainerSymbol = null;
        }
      }
      ...
      if (enumContainerSymbol) {
        ...
        if (!parent) {
          this.semanticInfoChain.cacheGlobalSymbol(
            enumContainerSymbol,
            PullElementKind.Enum
          );
        }
      }
      ...
      if (!enumInstanceSymbol && isInitializedModule) {
        ...
        if (parentInstanceSymbol) {
          if (isExported) {
            ...
            variableSymbol = parentInstanceSymbol.findMember(
              enumName,
              /*lookInParent*/ false
            );
            ...
            if (!variableSymbol) {
              variableSymbol =
                parentInstanceSymbol.findContainedNonMember(enumName);
            }
          } else {
            variableSymbol =
              parentInstanceSymbol.findContainedNonMember(enumName);
          ...
          for (var i = 0; i < siblingDecls.length; i++) {
            ...
            if (
              siblingDecls[i].name == enumName &&
              siblingDecls[i].kind & PullElementKind.SomeValue
            ) {
          ...
          if (augmentedDecl) {
            ...
            if (variableSymbol) {
              if (variableSymbol.isContainer()) {
                variableSymbol = (<PullContainerSymbol>(
                  variableSymbol
                )).getInstanceSymbol();
              } else if (variableSymbol && variableSymbol.isType()) {
                variableSymbol = (<PullTypeSymbol>(
                  variableSymbol
        ...
        if (variableSymbol) {
          ...
          enumInstanceSymbol = new PullSymbol(
            enumName,
        ...
        if (!moduleInstanceTypeSymbol) {
          moduleInstanceTypeSymbol = new PullTypeSymbol(
            '',
        ...
        if (!moduleInstanceTypeSymbol.getAssociatedContainerType()) {
          moduleInstanceTypeSymbol.setAssociatedContainerType(
            enumContainerSymbol
          );
        }
      }
      ...
      if (
        ...
        enumAST.enumElements.nonSeparatorCount() > 0
      ) {
        var multipleEnums =
          ArrayUtilities.where(
            moduleDeclarations,
            (d) => d.kind === PullElementKind.Enum
          ).length > 1;
        if (multipleEnums) {
          var firstVariable = <EnumElement>(
            enumAST.enumElements.nonSeparatorAt(0)
          );
          if (!firstVariable.equalsValueClause) {
            this.semanticInfoChain.addDiagnosticFromAST(
              firstVariable,
              DiagnosticCode.Enums_with_multiple_declarations_must_provide_an_initializer_for_the_first_enum_element,
              null
            );
          }
        }
      ...
      this.bindEnumIndexerDeclsToPullSymbols(
        enumContainerDecl,
        enumContainerSymbol
      );
      ...
      var otherDecls = this.findDeclsInContext(
        enumContainerDecl,
        enumContainerDecl.kind,
        true
      );
    ...
    private bindEnumIndexerDeclsToPullSymbols(
      enumContainerDecl: PullDecl,
      enumContainerSymbol: PullContainerSymbol
    ): void {
      var indexSigDecl = enumContainerDecl
        .getChildDecls()
        .filter((decl) => decl.kind == PullElementKind.IndexSignature)[0];
      var indexParamDecl = indexSigDecl.getChildDecls()[0];

      var syntheticIndexerParameterSymbol = new PullSymbol(
        indexParamDecl.name,
        PullElementKind.Parameter
      );

      syntheticIndexerParameterSymbol.type =
        this.semanticInfoChain.numberTypeSymbol;
      syntheticIndexerParameterSymbol.setResolved();

      var syntheticIndexerSignatureSymbol = new PullSignatureSymbol(
        PullElementKind.IndexSignature
      );
      syntheticIndexerSignatureSymbol.addParameter(
        syntheticIndexerParameterSymbol
      );
      syntheticIndexerSignatureSymbol.returnType =
        this.semanticInfoChain.stringTypeSymbol;
      syntheticIndexerSignatureSymbol.setResolved();

      var enumContainerInstanceTypeSymbol =
        enumContainerSymbol.getInstanceSymbol().type;
      enumContainerInstanceTypeSymbol.addIndexSignature(
        syntheticIndexerSignatureSymbol
      );
    ...
    private bindModuleDeclarationToPullSymbol(moduleContainerDecl: PullDecl) {
      ...
      var moduleNameAST =
        this.semanticInfoChain.getASTForDecl(moduleContainerDecl);
      var moduleDeclAST: AST = getEnclosingModuleDeclaration(moduleNameAST);
      if (!moduleDeclAST) {
        Debug.assert(
          moduleContainerDecl.kind === PullElementKind.DynamicModule
        );
      ...
      var isExported = hasFlag(
        moduleContainerDecl.flags,
        PullElementFlags.Exported
      );
      var searchKind = PullElementKind.SomeContainer;
      var isInitializedModule =
        (moduleContainerDecl.flags & PullElementFlags.SomeInitializedModule) !=
        0;
      ...
      if (parent && moduleKind == PullElementKind.DynamicModule) {
        ...
        this.semanticInfoChain.addDiagnosticFromAST(
          moduleNameAST,
          DiagnosticCode.Ambient_external_module_declaration_must_be_defined_in_global_context,
          null
        );
      }
      ...
      moduleContainerTypeSymbol = <PullContainerSymbol>(
        this.getExistingSymbol(moduleContainerDecl, searchKind, parent)
      );
        ...
        if (moduleContainerTypeSymbol.kind !== moduleKind) {
          ...
          if (isInitializedModule) {
            this.semanticInfoChain.addDiagnosticFromAST(
              moduleNameAST,
              DiagnosticCode.Duplicate_identifier_0,
              [moduleContainerDecl.getDisplayName()]
            );
          }
          ...
          this.semanticInfoChain.addDiagnosticFromAST(
            moduleNameAST,
            DiagnosticCode.Ambient_external_module_declaration_cannot_be_reopened
          );
        } else if (
          !this.checkThatExportsMatch(
            moduleContainerDecl,
            moduleContainerTypeSymbol
          )
        ) {
          moduleContainerTypeSymbol = null;
        }
      ...
      if (moduleContainerTypeSymbol) {
        ...
        moduleContainerTypeSymbol = new PullContainerSymbol(
          modName,
          moduleKind
        );
        createdNewSymbol = true;
      ...
      if (!moduleInstanceSymbol && isInitializedModule) {
        ...
        if (parentInstanceSymbol) {
          if (isExported) {
            ...
            if (!variableSymbol) {
              variableSymbol =
                parentInstanceSymbol.findContainedNonMember(modName);
            }
          } else {
            variableSymbol =
              parentInstanceSymbol.findContainedNonMember(modName);
        ...
        if (variableSymbol) {
          ...
          moduleInstanceSymbol = new PullSymbol(
            modName,
            PullElementKind.Variable
          );
        }
    ...
    private bindImportDeclaration(importDeclaration: PullDecl) {
      ...
      var importDeclAST = <VariableDeclarator>(
        this.semanticInfoChain.getASTForDecl(importDeclaration)
      );
      ...
      importSymbol = <PullTypeAliasSymbol>(
        this.getExistingSymbol(
          importDeclaration,
      ...
      if (importSymbol) {
        this.semanticInfoChain.addDiagnosticFromAST(
          importDeclAST,
          DiagnosticCode.Duplicate_identifier_0,
          [importDeclaration.getDisplayName()]
        );
        importSymbol = null;
      }
    ...
    private bindClassDeclarationToPullSymbol(classDecl: PullDecl) {
      ...
      var classAST = <ClassDeclaration>(
        this.semanticInfoChain.getASTForDecl(classDecl)
      );
      ...
      classSymbol = <PullTypeSymbol>(
        this.getExistingSymbol(classDecl, PullElementKind.SomeType, parent)
      );
      ...
      if (!constructorSymbol) {
        ...
        if (parentDecl) {
          siblingValueDecls = parentDecl.searchChildDecls(
            className,
        ...
        if (constructorSymbol) {
          ...
          constructorSymbol = new PullSymbol(
            className,
            PullElementKind.ConstructorMethod
          );
          constructorTypeSymbol = new PullTypeSymbol(
            '',
      ...
      for (var i = 0; i < typeParameters.length; i++) {
        ...
        if (typeParameter != null) {
          var typeParameterAST = this.semanticInfoChain.getASTForDecl(
            typeParameter.getDeclarations()[0]
          );
          this.semanticInfoChain.addDiagnosticFromAST(
            typeParameterAST,
            DiagnosticCode.Duplicate_identifier_0,
        ...
        typeParameter = new PullTypeParameterSymbol(
          typeParameters[i].name,
          /*_isFunctionTypeParameter*/ false
        );
      ...
      this.bindStaticPrototypePropertyOfClass(
        classSymbol,
    ...
    private bindInterfaceDeclarationToPullSymbol(interfaceDecl: PullDecl) {
      ...
      var interfaceAST = <InterfaceDeclaration>(
        this.semanticInfoChain.getASTForDecl(interfaceDecl)
      );
      var createdNewSymbol = false;
      ...
      interfaceSymbol = <PullTypeSymbol>(
        this.getExistingSymbol(interfaceDecl, PullElementKind.SomeType, parent)
      );
      ...
      if (interfaceSymbol) {
        if (!(interfaceSymbol.kind & acceptableSharedKind)) {
          this.semanticInfoChain.addDiagnosticFromAST(
            interfaceAST,
            DiagnosticCode.Duplicate_identifier_0,
            [interfaceDecl.getDisplayName()]
          );
          interfaceSymbol = null;
        } else if (
          !this.checkThatExportsMatch(interfaceDecl, interfaceSymbol)
        ) {
          interfaceSymbol = null;
        }
      }
      ...
      if (!interfaceSymbol) {
        interfaceSymbol = new PullTypeSymbol(
          interfaceName,
      ...
      // PULLREVIEW: Now that we clean type parameters, searching is redundant
      for (var i = 0; i < typeParameters.length; i++) {
        typeParameter = interfaceSymbol.findTypeParameter(
          typeParameters[i].name
        );
        ...
        if (!typeParameter) {
          ...
          for (var j = 0; j < typeParameterDecls.length; j++) {
            ...
            if (
              typeParameterDeclParent &&
              typeParameterDeclParent === interfaceDecl
            ) {
              var typeParameterAST = this.semanticInfoChain.getASTForDecl(
                typeParameterDecls[0]
              );
              this.semanticInfoChain.addDiagnosticFromAST(
                typeParameterAST,
                DiagnosticCode.Duplicate_identifier_0,
    ...
    private bindObjectTypeDeclarationToPullSymbol(objectDecl: PullDecl) {
      var objectSymbolAST: AST =
        this.semanticInfoChain.getASTForDecl(objectDecl);
    ...
    private bindConstructorTypeDeclarationToPullSymbol(
      constructorTypeDeclaration: PullDecl
    ) {
      var declKind = constructorTypeDeclaration.kind;
      var declFlags = constructorTypeDeclaration.flags;
      var constructorTypeAST = this.semanticInfoChain.getASTForDecl(
        constructorTypeDeclaration
      );

      var constructorTypeSymbol = new PullTypeSymbol(
        '',
        PullElementKind.ConstructorType
      );
      ...
      var signature = new PullSignatureSymbol(
        PullElementKind.ConstructSignature
      );

      var funcDecl = <ConstructorType>(
        this.semanticInfoChain.getASTForDecl(constructorTypeDeclaration)
      );
      ...
      this.bindParameterSymbols(
        funcDecl,
        Parameters.fromParameterList(funcDecl.parameterList),
        constructorTypeSymbol,
        signature
      );
      ...
      for (var i = 0; i < typeParameters.length; i++) {
        typeParameter = constructorTypeSymbol.findTypeParameter(
          typeParameters[i].name
        );
    ...
    private bindVariableDeclarationToPullSymbol(variableDeclaration: PullDecl) {
      ...
      var varDeclAST =
        this.semanticInfoChain.getASTForDecl(variableDeclaration);
      ...
      var isClassConstructorVariable =
        (declFlags & PullElementFlags.ClassConstructorVariable) != 0;

      variableSymbol = this.getExistingSymbol(
        variableDeclaration,
        PullElementKind.SomeValue,
        parent
      );
      ...
      if (variableSymbol) {
        ...
        var prevIsClassConstructorVariable = variableSymbol.anyDeclHasFlag(
          PullElementFlags.ClassConstructorVariable
        );
        var prevIsModuleValue = variableSymbol.allDeclsHaveFlag(
          PullElementFlags.InitializedModule
        );
        var prevIsImplicit = variableSymbol.anyDeclHasFlag(
          PullElementFlags.ImplicitVariable
        );
        var prevIsFunction = prevKind == PullElementKind.Function;
        var prevIsAmbient = variableSymbol.allDeclsHaveFlag(
          PullElementFlags.Ambient
        );
        var isAmbientOrPrevIsAmbient =
          prevIsAmbient ||
          (variableDeclaration.flags & PullElementFlags.Ambient) != 0;
        var prevDecl = variableSymbol.getDeclarations()[0];
        var prevParentDecl = prevDecl.getParentDecl();
        var bothAreGlobal =
          parentDecl &&
          parentDecl.kind == PullElementKind.Script &&
          prevParentDecl.kind == PullElementKind.Script;
        var shareParent =
          bothAreGlobal ||
          prevDecl.getParentDecl() == variableDeclaration.getParentDecl();
        var prevIsParam =
          shareParent &&
          prevKind == PullElementKind.Parameter &&
          declKind == PullElementKind.Variable;

        var acceptableRedeclaration =
          prevIsParam ||
          (isImplicit &&
            ((!isEnumValue && !isClassConstructorVariable && prevIsFunction) || // Enums can't mix with functions
              ((isModuleValue || isEnumValue) &&
                (prevIsModuleValue || prevIsEnum)) || // modules and enums can mix freely
              (isClassConstructorVariable &&
                prevIsModuleValue &&
                isAmbientOrPrevIsAmbient) || // an ambient class can be declared after a module
        ...
        if (
          acceptableRedeclaration &&
          (prevIsClassConstructorVariable || prevIsFunction) &&
          !isAmbientOrPrevIsAmbient
        ) {
          if (prevDecl.fileName() != variableDeclaration.fileName()) {
            this.semanticInfoChain.addDiagnostic(
              diagnosticFromDecl(
                variableDeclaration,
                DiagnosticCode.Module_0_cannot_merge_with_previous_declaration_of_1_in_a_different_file_2,
                [declName, declName, prevDecl.fileName()]
              )
            );
            variableSymbol.type = this.semanticInfoChain
              .getResolver()
              .getNewErrorTypeSymbol(declName);
          }
        }
        ...
        if (!acceptableRedeclaration || prevIsParam) {
          ...
          if (
            (!prevIsParam &&
              (isImplicit ||
                prevIsImplicit ||
                (prevKind & PullElementKind.SomeFunction) !== 0)) ||
            !shareParent
          ) {
            var diagnostic = diagnosticFromDecl(
              variableDeclaration,
              DiagnosticCode.Duplicate_identifier_0,
              [variableDeclaration.getDisplayName()]
            );
            ...
            // double var declaration (keep them separate so we can verify type sameness during type check)
        ...
        if (
          ...
          !this.checkThatExportsMatch(
            variableDeclaration,
            variableSymbol,
            !(isModuleValue && prevIsModuleValue)
          )
        ) {
          variableSymbol.type = this.semanticInfoChain
            .getResolver()
        ...
        if (isClassConstructorVariable) {
          ...
          if (parent) {
            ...
            for (var i = 0; i < members.length; i++) {
              if (
                members[i].name === declName &&
                members[i].kind === PullElementKind.Class
              ) {
          ...
          if (!classTypeSymbol) {
            ...
            if (parentDecl) {
              var childDecls = parentDecl.searchChildDecls(
                declName,
            ...
            if (!classTypeSymbol) {
              classTypeSymbol = <PullTypeSymbol>(
          ...
          if (
            classTypeSymbol &&
            classTypeSymbol.kind !== PullElementKind.Class
          ) {
          ...
          if (
            moduleContainerTypeSymbol &&
            !moduleContainerTypeSymbol.isContainer()
          ) {
            moduleContainerTypeSymbol = null;
          }
    ...
    // properties
    private bindEnumMemberDeclarationToPullSymbol(
      propertyDeclaration: PullDecl
    ) {
      var declFlags = propertyDeclaration.flags;
      var declKind = propertyDeclaration.kind;
      var propDeclAST = <EnumElement>(
        this.semanticInfoChain.getASTForDecl(propertyDeclaration)
      );
      ...
      if (propertySymbol) {
        this.semanticInfoChain.addDiagnostic(
          diagnosticFromDecl(
            propertyDeclaration,
            DiagnosticCode.Duplicate_identifier_0,
            [propertyDeclaration.getDisplayName()]
          )
        );
      }
    ...
    // parameters
    private bindParameterSymbols(
      functionDeclaration: AST,
      parameterList: IParameters,
      funcType: PullTypeSymbol,
      signatureSymbol: PullSignatureSymbol
    ) {
      ...
      if (parameterList) {
        for (var i = 0, n = parameterList.length; i < n; i++) {
          ...
          var isProperty = hasFlag(
            decl.flags,
            PullElementFlags.PropertyParameter
          );
          var parameterSymbol = new PullSymbol(
            id.valueText(),
            PullElementKind.Parameter
          );
    ...
    private bindFunctionDeclarationToPullSymbol(functionDeclaration: PullDecl) {
      ...
      var funcDeclAST = <FunctionDeclaration>(
        this.semanticInfoChain.getASTForDecl(functionDeclaration)
      );
      ...
      functionSymbol = this.getExistingSymbol(
        functionDeclaration,
        PullElementKind.SomeValue,
        parent
      );
      ...
      if (functionSymbol) {
        ...
        var previousIsAmbient = functionSymbol.allDeclsHaveFlag(
          PullElementFlags.Ambient
        );
        var isAmbientOrPreviousIsAmbient: boolean =
          previousIsAmbient ||
          (functionDeclaration.flags & PullElementFlags.Ambient ? true : false);
        var acceptableRedeclaration: boolean =
          (functionSymbol.kind === PullElementKind.Function &&
            (isSignature ||
              functionSymbol.allDeclsHaveFlag(PullElementFlags.Signature))) ||
          (functionSymbol.allDeclsHaveFlag(
            PullElementFlags.InitializedModule
          ) &&
            isAmbientOrPreviousIsAmbient);
        if (!acceptableRedeclaration) {
          this.semanticInfoChain.addDiagnosticFromAST(
            funcDeclAST,
            DiagnosticCode.Duplicate_identifier_0,
            [functionDeclaration.getDisplayName()]
          );
          functionSymbol.type = this.semanticInfoChain
            .getResolver()
            .getNewErrorTypeSymbol(funcName);
        }
      }
      ...
      if (!functionTypeSymbol) {
        functionTypeSymbol = new PullTypeSymbol(
          '',
      ...
      var signature = isSignature
        ? new PullSignatureSymbol(PullElementKind.CallSignature)
        : new PullDefinitionSignatureSymbol(PullElementKind.CallSignature);
      ...
      var funcDecl = <FunctionDeclaration>(
        this.semanticInfoChain.getASTForDecl(functionDeclaration)
      );
      this.bindParameterSymbols(
        funcDecl,
    ...
    private bindFunctionExpressionToPullSymbol(
      functionExpressionDeclaration: PullDecl
    ) {
      var declKind = functionExpressionDeclaration.kind;
      var declFlags = functionExpressionDeclaration.flags;
      var ast = this.semanticInfoChain.getASTForDecl(
        functionExpressionDeclaration
      );

      var parameters =
        ast.kind() === SyntaxKind.SimpleArrowFunctionExpression
          ? Parameters.fromIdentifier(
              (<SimpleArrowFunctionExpression>ast).identifier
            )
      ...
      var functionTypeSymbol = new PullTypeSymbol(
        '',
        PullElementKind.FunctionType
      );
      functionTypeSymbol.setFunctionSymbol(functionSymbol);
      ...
      var name =
        funcExpAST.kind() === SyntaxKind.FunctionExpression
      ...
      var signature = new PullDefinitionSignatureSymbol(
        PullElementKind.CallSignature
      );
    ...
    private bindFunctionTypeDeclarationToPullSymbol(
      functionTypeDeclaration: PullDecl
    ) {
      var declKind = functionTypeDeclaration.kind;
      var declFlags = functionTypeDeclaration.flags;
      var funcTypeAST = <FunctionType>(
        this.semanticInfoChain.getASTForDecl(functionTypeDeclaration)
      );
      ...
      this.bindParameterSymbols(
        funcTypeAST,
        Parameters.fromParameterList(funcTypeAST.parameterList),
        functionTypeSymbol,
        signature
      );
    ...
    private bindMethodDeclarationToPullSymbol(methodDeclaration: PullDecl) {
      ...
      if (
        ...
        (methodSymbol.kind !== PullElementKind.Method ||
          (!isSignature &&
            !methodSymbol.allDeclsHaveFlag(PullElementFlags.Signature)))
      ) {
        this.semanticInfoChain.addDiagnosticFromAST(
          methodAST,
          DiagnosticCode.Duplicate_identifier_0,
          [methodDeclaration.getDisplayName()]
        );
        methodSymbol = null;
      }
      ...
      var nameAST =
        methodAST.kind() === SyntaxKind.MemberFunctionDeclaration
      ...
      var signature = isSignature
        ? new PullSignatureSymbol(sigKind)
        : new PullDefinitionSignatureSymbol(sigKind);
      ...
      for (var i = 0; i < typeParameters.length; i++) {
        ...
        typeParameterAST = <TypeParameter>(
          this.semanticInfoChain.getASTForDecl(typeParameters[i])
        );
      ...
      this.bindParameterSymbols(
        funcDecl,
        Parameters.fromParameterList(getParameterList(funcDecl)),
        methodTypeSymbol,
        signature
      );
    ...
    private bindStaticPrototypePropertyOfClass(
      classTypeSymbol: PullTypeSymbol,
      constructorTypeSymbol: PullTypeSymbol
    ) {
      var prototypeStr = 'prototype';

      var prototypeSymbol = constructorTypeSymbol.findMember(
        prototypeStr,
        /*lookInParent*/ false
      );
      if (prototypeSymbol && !prototypeSymbol.getIsSynthesized()) {
        ...
        this.semanticInfoChain.addDiagnostic(
          diagnosticFromDecl(
            prototypeSymbol.getDeclarations()[0],
            DiagnosticCode.Duplicate_identifier_0,
            [prototypeSymbol.getDisplayName()]
          )
        );
      }
      ...
      if (!prototypeSymbol || !prototypeSymbol.getIsSynthesized()) {
        var prototypeDecl = new PullSynthesizedDecl(
          prototypeStr,
          prototypeStr,
          PullElementKind.Property,
          PullElementFlags.Public | PullElementFlags.Static,
          constructorTypeSymbol.getDeclarations()[0],
          classTypeSymbol.getDeclarations()[0].getSpan(),
          this.semanticInfoChain
        );

        prototypeSymbol = new PullSymbol(
          prototypeStr,
          PullElementKind.Property
        );
        ...
        if (prototypeSymbol.type && prototypeSymbol.type.isGeneric()) {
          ...
          prototypeSymbol.type = resolver.instantiateTypeToAny(
            prototypeSymbol.type,
    ...
    private bindConstructorDeclarationToPullSymbol(
      constructorDeclaration: PullDecl
    ) {
      var declKind = constructorDeclaration.kind;
      var declFlags = constructorDeclaration.flags;
      var constructorAST = <ConstructorDeclaration>(
        this.semanticInfoChain.getASTForDecl(constructorDeclaration)
      );
      ...
      var constructSignature = isSignature
        ? new PullSignatureSymbol(PullElementKind.ConstructSignature)
        : new PullDefinitionSignatureSymbol(PullElementKind.ConstructSignature);
      ...
      this.bindParameterSymbols(
        constructorAST,
        Parameters.fromParameterList(constructorAST.parameterList),
        constructorTypeSymbol,
        constructSignature
      );
      ...
      for (var i = 0; i < typeParameters.length; i++) {
        constructSignature.addTypeParameter(
    ...
    private bindConstructSignatureDeclarationToPullSymbol(
      constructSignatureDeclaration: PullDecl
    ) {
      var parent = this.getParent(constructSignatureDeclaration, true);
      var constructorAST = <ConstructSignature>(
        this.semanticInfoChain.getASTForDecl(constructSignatureDeclaration)
      );

      var constructSignature = new PullSignatureSymbol(
        PullElementKind.ConstructSignature
      );
      ...
      for (var i = 0; i < typeParameters.length; i++) {
        typeParameter = constructSignature.findTypeParameter(
          typeParameters[i].name
        );
      ...
      var funcDecl = this.semanticInfoChain.getASTForDecl(
    ...
    private bindCallSignatureDeclarationToPullSymbol(
      callSignatureDeclaration: PullDecl
    ) {
      var parent = this.getParent(callSignatureDeclaration, true);
      var callSignatureAST = <CallSignature>(
        this.semanticInfoChain.getASTForDecl(callSignatureDeclaration)
      );

      var callSignature = new PullSignatureSymbol(
        PullElementKind.CallSignature
      );
    ...
    private bindIndexSignatureDeclarationToPullSymbol(
      indexSignatureDeclaration: PullDecl
    ) {
      var indexSignature = new PullSignatureSymbol(
        PullElementKind.IndexSignature
      );
      ...
      var funcDecl = <IndexSignature>(
        this.semanticInfoChain.getASTForDecl(indexSignatureDeclaration)
      );
      this.bindParameterSymbols(
        funcDecl,
        Parameters.fromParameter(funcDecl.parameter),
        null,
        indexSignature
      );
    ...
    // getters and setters
    private bindGetAccessorDeclarationToPullSymbol(
      getAccessorDeclaration: PullDecl
    ) {
      var declKind = getAccessorDeclaration.kind;
      var declFlags = getAccessorDeclaration.flags;
      var funcDeclAST = <GetAccessor>(
        this.semanticInfoChain.getASTForDecl(getAccessorDeclaration)
      );
      ...
      accessorSymbol = <PullAccessorSymbol>(
        parent.findMember(funcName, /*lookInParent*/ false)
      );
      ...
      if (accessorSymbol) {
        if (!accessorSymbol.isAccessor()) {
          this.semanticInfoChain.addDiagnosticFromAST(
            funcDeclAST,
            DiagnosticCode.Duplicate_identifier_0,
            [getAccessorDeclaration.getDisplayName()]
          );
          accessorSymbol = null;
        } else {
          ...
          if (getterSymbol) {
            this.semanticInfoChain.addDiagnosticFromAST(
              funcDeclAST,
              DiagnosticCode.Getter_0_already_declared,
              [getAccessorDeclaration.getDisplayName()]
            );
            accessorSymbol = null;
            getterSymbol = null;
    ...
    private bindSetAccessorDeclarationToPullSymbol(
      setAccessorDeclaration: PullDecl
    ) {
      var declKind = setAccessorDeclaration.kind;
      var declFlags = setAccessorDeclaration.flags;
      var funcDeclAST = <SetAccessor>(
        this.semanticInfoChain.getASTForDecl(setAccessorDeclaration)
      );
      ...
      if (accessorSymbol) {
        if (!accessorSymbol.isAccessor()) {
          this.semanticInfoChain.addDiagnosticFromAST(
            funcDeclAST,
            DiagnosticCode.Duplicate_identifier_0,
            [setAccessorDeclaration.getDisplayName()]
          );
          accessorSymbol = null;
        } else {
          ...
          if (setterSymbol) {
            this.semanticInfoChain.addDiagnosticFromAST(
              funcDeclAST,
              DiagnosticCode.Setter_0_already_declared,
              [setAccessorDeclaration.getDisplayName()]
            );
            accessorSymbol = null;
            setterSymbol = null;
