...
import { IBitMatrix, BitMatrix } from '../core/bitMatrix';
import { Debug } from '../core/debug';
import { Diagnostic, getDiagnosticMessage } from '../core/diagnosticCore';
import { Errors } from '../core/errors';
import { Document } from '../document';
import { lastParameterIsRest } from '../emitter';
import { hasFlag, TypeRelationshipFlags } from '../flags';
...
import { PullElementKind, PullElementFlags, hasModifier } from './pullFlags';
...
export class PullTypeResolver {
  ...
  private cachedIArgumentsInterfaceType() {
    if (!this._cachedIArgumentsInterfaceType) {
      this._cachedIArgumentsInterfaceType = <PullTypeSymbol>(
        this.getSymbolFromDeclPath('IArguments', [], PullElementKind.Interface)
      );
    }
  ...
  private resolveExternalModuleReference(
    ...
    if (isRelative(originalIdText)) {
      ...
      symbol = this.semanticInfoChain.findAmbientExternalModuleInGlobalContext(
  ...
  private resolveVariableDeclaratorOrParameterOrEnumElement(
    ...
    var hasTypeExpr =
      typeExpr !== null || varDeclOrParameter.kind() === SyntaxKind.EnumElement;
  ...
  private resolveSetterArgumentTypeAnnotation(
    ...
    if (
      ...
      var parameter = <Parameter>(
        setterFunctionDeclarationAst.parameterList.parameters.nonSeparatorAt(0)
      );
  ...
  private typeCheckContinueStatement(
    ...
    if (!this.inIterationStatement(ast)) {
      ...
      if (
        !ArrayUtilities.contains(continuableLabels, ast.identifier.valueText())
      ) {
        ...
        if (!foundMatchingParameter) {
          ...
          context.postDiagnostic(
            this.semanticInfoChain.diagnosticFromAST(
              ...
              [
                (<Parameter>(
                  parameterList.parameters.nonSeparatorAt(currentParameterIndex)
                )).identifier.text(),
                nameAST.text(),
              ]
            )
        ...
        if (
          ...
          context.postDiagnostic(
            this.semanticInfoChain.diagnosticFromAST(
              ...
              [
                typeArg.toString(enclosingSymbol, /*useConstraintInName*/ true),
      ...
      context.postDiagnostic(
        this.semanticInfoChain.diagnosticFromAST(
          ...
          [
            ...
            lhsType.toString(enclosingDecl ? enclosingDecl.getSymbol() : null),
          ]
        )
      );
      return this.getNewErrorTypeSymbol(rhsName);
    ...
    while (ast) {
      switch (ast.kind()) {
        case SyntaxKind.ExtendsHeritageClause:
          ...
          return (
            heritageClause.parent.parent.kind() === SyntaxKind.ClassDeclaration
          );
  ...
  private resolveLogicalAndExpression(
    ...
    return this.resolveAST(binex.right, /*isContextuallyTyped:*/ false, context)
      .type;
  }
      ...
      if (signatures[i].isGeneric() && typeParameters.length) {
        ...
        if (
          ...
          resolvedSigMap.push({
            ...
            index: i,
            inferredTypeArgs: signatures[i].isGeneric() ? inferredTypeArgs : [],
          }); //AR: shallowTag
        }
      }
    }
    ...
    if (expressionType && parameterType) {
      var expressionTypeGenerativeTypeClassification =
        expressionTypeEnclosingType
          ...
          : GenerativeTypeClassification.Unknown;
      var parameterTypeGenerativeTypeClassification = parameterTypeEnclosingType
  ...
  private typeCheckFunctionOverloads(
    ...
    if (
      ...
      if (
        signatureForVisibilityCheck.anyDeclHasFlag(PullElementFlags.Private) !=
        signature.anyDeclHasFlag(PullElementFlags.Private)
      ) {
        errorCode =
          DiagnosticCode.Overload_signatures_must_all_be_public_or_private;
      } else if (
        signatureForVisibilityCheck.anyDeclHasFlag(PullElementFlags.Exported) !=
        ...
        signatureForVisibilityCheck.anyDeclHasFlag(PullElementFlags.Ambient) !=
        signature.anyDeclHasFlag(PullElementFlags.Ambient)
      ) {
        errorCode =
          DiagnosticCode.Overload_signatures_must_all_be_ambient_or_non_ambient;
      } else if (
        signatureForVisibilityCheck.anyDeclHasFlag(PullElementFlags.Optional) !=
  ...
  private checkFunctionTypePrivacy(
    ...
    if (
      ...
      (funcDeclAST.kind() === SyntaxKind.SetAccessor &&
        funcDeclAST.parent.parent.kind() === SyntaxKind.ObjectLiteralExpression)
    ) {
      return;
    }
            ...
            if (declKind === PullElementKind.Class) {
              ...
              for (var i = 0; i < constructorDecls.length; i++) {
                constructorDecls[i].flags =
                  constructorDecls[i].flags | PullElementFlags.MustCaptureThis;
              }
            }
            break;
          }
  ...
  private typeCheckMembersAgainstIndexer(
    ...
    if (stringSignature || numberSignature) {
      ...
      for (var i = 0; i < members.length; i++) {
        ...
        if (
          (member.name ||
            (member.kind === PullElementKind.Property && member.name === '')) &&
  ...
  private typeCheckIfTypeExtendsType(
    ...
    if (!foundError && typeSymbol.hasOwnIndexSignatures()) {
      foundError = !this.sourceIndexSignaturesAreSubtypeOfTargetIndexSignatures(
        typeSymbol,
        extendedType,
        classOrInterface,
        context,
    ...
    if (!foundError && typeSymbol.isClass()) {
      ...
      if (typeConstructorTypeMembers.length) {
        ...
        for (var i = 0; i < typeConstructorTypeMembers.length; i++) {
          ...
          var extendedConstructorTypeProp = extendedConstructorType.findMember(
            propName,
            /*lookInParent*/ true
          );
    ...
    for (var i = 0; i < indexSignaturesFromThisBaseType.length; i++) {
      ...
      if (parameterTypeIsString) {
        if (allInheritedSignatures.stringSignatureWithBaseOrigin) {
          if (
            allInheritedSignatures.stringSignatureWithBaseOrigin.baseOrigin !==
              baseTypeSymbol &&
            !this.typesAreIdentical(
        ...
        if (allInheritedSignatures.numberSignatureWithBaseOrigin) {
          if (
            allInheritedSignatures.numberSignatureWithBaseOrigin.baseOrigin !==
              baseTypeSymbol &&
            !this.typesAreIdentical(
...
export class TypeComparisonInfo {
  ...
  public addMessage(message: string) {
    if (!this.onlyCaptureFirstError && this.message) {
      this.message = this.message + EOL + this.indentString() + message;
    } else {
...
export function getPropertyAssignmentNameTextFromIdentifier(identifier: AST): {
  actualText: string;
  memberName: string;
} {
