...
///<reference path='..\references.ts' />
  ...
  export class PullSymbol {
    ...
    private findAliasedType(
      scopeSymbol: PullSymbol,
      skipScopeSymbolAliasesLookIn?: boolean,
      lookIntoOnlyExportedAlias?: boolean,
      aliasSymbols: PullTypeAliasSymbol[] = [],
      visitedScopeDeclarations: PullDecl[] = []
    ): PullTypeAliasSymbol[] {
      ...
      for (var i = 0; i < scopeDeclarations.length; i++) {
        ...
        if (!ArrayUtilities.contains(visitedScopeDeclarations, scopeDecl)) {
          ...
          for (var j = 0; j < childDecls.length; j++) {
            ...
            if (
              childDecl.kind === PullElementKind.TypeAlias &&
              (!lookIntoOnlyExportedAlias ||
                childDecl.flags & PullElementFlags.Exported)
            ) {
              var symbol = <PullTypeAliasSymbol>childDecl.getSymbol();

              if (
                PullContainerSymbol.usedAsSymbol(symbol, this) || // this is symbol is used as this alias
                (this.rootSymbol &&
                  PullContainerSymbol.usedAsSymbol(symbol, this.rootSymbol))
              ) {
                // the root symbol of the alias is used as import symbol
                aliasSymbols.push(symbol);
                return aliasSymbols;
              }

              if (
                !skipScopeSymbolAliasesLookIn &&
                this.isExternalModuleReferenceAlias(symbol) &&
                (!symbol.assignedContainer().hasExportAssignment() ||
                  (symbol
                    ...
                    symbol
                      .assignedContainer()
                      .getExportAssignedContainerSymbol().kind ==
                      PullElementKind.DynamicModule))
              ) {
                // It is a dynamic module)) {
      ...
      for (var i = 0; i < scopeSymbolAliasesToLookIn.length; i++) {
        ...
        var result = this.findAliasedType(
          scopeSymbolAlias.assignedContainer().hasExportAssignment()
            ? scopeSymbolAlias
                .assignedContainer()
                .getExportAssignedContainerSymbol()
            : scopeSymbolAlias.assignedContainer(),
          /*skipScopeSymbolAliasesLookIn*/ false,
          /*lookIntoOnlyExportedAlias*/ true,
          aliasSymbols,
          visitedScopeDeclarations
        );
        if (result) {
          return result;
        }
    ...
    public getExternalAliasedSymbols(scopeSymbol: PullSymbol) {
      ...
      if (
        scopePath.length &&
        scopePath[scopePath.length - 1].kind === PullElementKind.DynamicModule
      ) {
    ...
    private isExternalModuleReferenceAlias(aliasSymbol: PullTypeAliasSymbol) {
      if (aliasSymbol) {
        ...
        if (
          aliasSymbol.assignedType() &&
          aliasSymbol.assignedType() != aliasSymbol.assignedContainer()
        ) {
          return false;
        }
        ...
        // Its internal module
        if (
          aliasSymbol.assignedContainer() &&
          aliasSymbol.assignedContainer().kind != PullElementKind.DynamicModule
        ) {
          return false;
        }
      ...
      if (scopeSymbol) {
        if (this.kind != PullElementKind.TypeAlias) {
          ...
          for (var i = 0; i < scopePath.length; i++) {
            var internalAliases = this.findAliasedType(
              scopeSymbol,
              /*skipScopeSymbolAliasesLookIn*/ true,
              /*lookIntoOnlyExportedAlias*/ true
            );
            if (internalAliases) {
    ...
    public getAliasSymbolName(
      ...
      if (
        externalAliases &&
        this.isExternalModuleReferenceAlias(
          externalAliases[externalAliases.length - 1]
        )
      ) {
        var aliasFullName = '';
        for (
          var i = 1, symbolsLen = externalAliases.length;
          i < symbolsLen;
          i++
        ) {
          aliasFullName =
            aliasFullName + '.' + aliasPartsNameGetter(externalAliases[i]);
        }
    ...
    public getName(
      ...
      var aliasName = this.getAliasSymbolName(
        scopeSymbol,
        (symbol) => symbol.getName(scopeSymbol, useConstraintInName),
    ...
    public getDisplayName(
      ...
      var aliasDisplayName = this.getAliasSymbolName(
        scopeSymbol,
        (symbol) => symbol.getDisplayName(scopeSymbol, useConstraintInName),
        (symbol) => symbol.getDisplayName(),
    ...
    public pathToRoot() {
      ...
      while (node) {
        if (node.isType()) {
          var associatedContainerSymbol = (<PullTypeSymbol>(
            node
          )).getAssociatedContainerType();
    ...
    public fullQualifiedName(): string[] {
      ...
      var ix = fn.indexOf('<');
      this._fullQualifiedName =
        ix >= 0 ? fn.substring(0, ix).split('.') : fn.split('.');
    ...
    public fullNameAux(scopeSymbol?: PullSymbol): string {
      ...
      var aliasFullName = this.getAliasSymbolName(
        scopeSymbol,
        (symbol) => symbol.fullName(scopeSymbol),
        (symbol) => symbol.getNamePartForFullName()
      );
      ...
      for (var i = 1; i < path.length; i++) {
        var aliasFullName = path[i].getAliasSymbolName(
          scopeSymbol,
          (symbol) => symbol.fullName(scopeSymbol),
          (symbol) => symbol.getNamePartForFullName()
        );
        ...
        if (
          path[i].kind == PullElementKind.DynamicModule &&
          !isQuoted(scopedName)
        ) {
    ...
    public getScopedName(
      ...
      for (var i = 1; i < path.length; i++) {
        ...
        if (
          kind === PullElementKind.Container ||
          kind === PullElementKind.DynamicModule
        ) {
          var aliasFullName = path[i].getAliasSymbolName(
            scopeSymbol,
            (symbol) =>
    ...
    public getTypeNameEx(
      ...
      var type = this.type;
      if (type) {
        var memberName: MemberName = getPrettyTypeName
          ? this.getTypeNameForFunctionSignature(
              '',
              scopeSymbol,
              getPrettyTypeName
            )
          : null;
        if (!memberName) {
          memberName = type.getScopedNameEx(
            scopeSymbol,
    ...
    private getTypeNameForFunctionSignature(
      prefix: string,
      scopeSymbol?: PullSymbol,
      getPrettyTypeName?: boolean
    ) {
      ...
      if (
        type &&
        !type.isNamedTypeSymbol() &&
        this.kind != PullElementKind.Property &&
        this.kind != PullElementKind.Variable &&
        this.kind != PullElementKind.Parameter
      ) {
        var signatures = type.getCallSignatures();
        if (
          signatures.length == 1 ||
          (getPrettyTypeName && signatures.length)
        ) {
    ...
    public getNameAndTypeNameEx(scopeSymbol?: PullSymbol) {
      ...
      if (type) {
        ...
        var memberName: MemberName = this.getTypeNameForFunctionSignature(
          nameStr,
          scopeSymbol
        );
        if (!memberName) {
    ...
    static getTypeParameterString(
      typars: PullTypeSymbol[],
      scopeSymbol?: PullSymbol,
      useContraintInName?: boolean
    ) {
      return PullSymbol.getTypeParameterStringEx(
        typars,
        scopeSymbol,
        /*getTypeParamMarkerInfo:*/ undefined,
        useContraintInName
      ).toString();
    }

    static getTypeParameterStringEx(
      ...
      if (typeParameters && typeParameters.length) {
        ...
        for (var i = 0; i < typeParameters.length; i++) {
          ...
          builder.add(
            typeParameters[i].getScopedNameEx(
              scopeSymbol,
              /*skipTypeParametersInName*/ false,
              useContraintInName
            )
    ...
    static getIsExternallyVisible(
      symbol: PullSymbol,
      fromIsExternallyVisibleSymbol: PullSymbol,
      inIsExternallyVisibleSymbols: PullSymbol[]
    ) {
      ...
      Debug.assert(
        ArrayUtilities.last(inIsExternallyVisibleSymbols) ===
          fromIsExternallyVisibleSymbol
      );
    ...
    public isExternallyVisible(
      ...
      if (this.rootSymbol) {
        return PullSymbol.getIsExternallyVisible(
          this.rootSymbol,
          this,
          inIsExternallyVisibleSymbols
        );
      ...
      if (
        container.kind == PullElementKind.DynamicModule ||
        (container.getAssociatedContainerType() &&
          container.getAssociatedContainerType().kind ==
            PullElementKind.DynamicModule)
      ) {
        var containerSymbol =
          container.kind == PullElementKind.DynamicModule
      ...
      if (
        !this.anyDeclHasFlag(PullElementFlags.Exported) &&
        kind != PullElementKind.Property &&
        kind != PullElementKind.Method
      ) {
        return false;
      }
    ...
    private static getDefaultConstructorSymbolForDocComments(
      ...
      if (classSymbol.getHasDefaultConstructor()) {
        ...
        if (extendedTypes.length) {
          return PullSymbol.getDefaultConstructorSymbolForDocComments(
            extendedTypes[0]
          );
        }
      }
    ...
    public docComments(useConstructorAsClass?: boolean): string {
      ...
      if (this._docComments === null) {
        ...
        if (
          ...
          var paramComment = this.getParameterDocCommentText(
            this.getDisplayName(),
            funcDocComments
          );
          if (paramComment != '') {
          ...
          var paramSelfComment = this.getDocCommentText(
            this.getDocCommentArray(this)
          );
          if (paramSelfComment != '') {
          ...
          if (getSymbolComments) {
            ...
            if (docComments == '') {
              if (this.kind == PullElementKind.CallSignature) {
                ...
                if (
                  callTypeSymbol &&
                  callTypeSymbol.getCallSignatures().length == 1
                ) {
    ...
    private getParameterDocCommentText(
      param: string,
      fncDocComments: Comment[]
    ) {
      if (
        fncDocComments.length === 0 ||
        fncDocComments[0].kind() !== SyntaxKind.MultiLineCommentTrivia
      ) {
      ...
      for (var i = 0; i < fncDocComments.length; i++) {
        ...
        for (
          ...
          // Parameter name
          if (
            param !== commentContents.substr(j, param.length) ||
            !this.isSpaceChar(commentContents, j + param.length)
          ) {
          ...
          var paramHelpString = commentContents.substring(
            j,
            endOfParam < 0 ? commentContents.length : endOfParam
          );

          // Find alignement spaces to remove
          var paramSpacesToRemove: number = undefined;
          var paramLineIndex =
            commentContents.substring(0, j).lastIndexOf('\n') + 1;
          if (paramLineIndex !== 0) {
            if (
              paramLineIndex < j &&
              commentContents.charAt(paramLineIndex + 1) === '\r'
            ) {
              paramLineIndex++;
            }
          }
          var startSpaceRemovalIndex = this.consumeLeadingSpace(
            commentContents,
            paramLineIndex
          );
          if (
            startSpaceRemovalIndex !== j &&
            commentContents.charAt(startSpaceRemovalIndex) === '*'
          ) {
    ...
    private cleanJSDocComment(content: string, spacesToRemove?: number) {
      ...
      for (var l = 0; l < lines.length; l++) {
        ...
        for (
          var i = line.indexOf('@', cleanLinePos.start);
          0 <= i && i < cleanLinePos.end;
          i = line.indexOf('@', i + 1)
        ) {
          // We have encoutered @.
          ...
          if (
            line.indexOf('param', i + 1) === i + 1 &&
            this.isSpaceChar(line, i + 6)
          ) {
            // It is param tag.
    ...
    private consumeLeadingSpace(
      line: string,
      startIndex: number,
      maxSpacesToRemove?: number
    ) {
    ...
    private isSpaceChar(line: string, index: number) {
      ...
      if (index < length) {
        ...
        return (
          charCode === CharacterCodes.space || charCode === CharacterCodes.tab
        );
      }
    ...
    private cleanDocCommentLine(
      line: string,
      jsDocStyleComment: boolean,
      jsDocLineSpaceToRemove?: number
    ) {
      ...
      if (nonSpaceIndex !== -1) {
        ...
        if (jsDocStyleComment && line.charAt(nonSpaceIndex) === '*') {
          ...
          nonSpaceIndex = this.consumeLeadingSpace(
            line,
            startIndex,
            jsDocLineSpaceToRemove
          );
  ...
  export class PullSignatureSymbol extends PullSymbol {
    private _memberTypeParameterNameCache: IIndexable<PullTypeParameterSymbol> =
      null;
    ...
    public addTypeParameter(typeParameter: PullTypeParameterSymbol) {
      ...
      if (!this._memberTypeParameterNameCache) {
        this._memberTypeParameterNameCache =
          createIntrinsicsObject<PullTypeParameterSymbol>();
      }

      this.typeParameters[this.typeParameters.length] = typeParameter;

      this._memberTypeParameterNameCache[typeParameter.getName()] =
        typeParameter;
    }
      ...
      if (!this._memberTypeParameterNameCache) {
        ...
        if (this.typeParameters) {
          for (var i = 0; i < this.typeParameters.length; i++) {
            this._memberTypeParameterNameCache[
              this.typeParameters[i].getName()
            ] = <PullTypeParameterSymbol>this.typeParameters[i];
          }
        }
      }
      ...
      if (this._stringConstantOverload === undefined) {
        ...
        for (var i = 0; i < params.length; i++) {
          ...
          if (
            ...
            (<PullPrimitiveTypeSymbol>paramType).isStringConstant()
          ) {
    ...
    static getSignatureTypeMemberName(
    ...
    static getSignaturesTypeNameEx(
      signatures: PullSignatureSymbol[],
      prefix: string,
      shortform: boolean,
      brackets: boolean,
      ...
      for (var i = 0; i < len; i++) {
        ...
        result.push(
          signature.getSignatureTypeNameEx(
            prefix,
            shortform,
            brackets,
            scopeSymbol
      ...
      if (getPrettyTypeName && result.length && len > 1) {
        ...
        var overloadString = getLocalizedText(DiagnosticCode._0_overload_s, [
          (foundDefinition ? len - 2 : len - 1).toString(),
        ]);
    ...
    public toString(scopeSymbol?: PullSymbol, useConstraintInName?: boolean) {
      var s = this.getSignatureTypeNameEx(
        this.getScopedNameEx().toString(),
        /*shortform*/ false,
        /*brackets*/ false,
        scopeSymbol,
        /*getParamMarkerInfo*/ undefined,
        useConstraintInName
      ).toString();
      return s;
    }

    public getSignatureTypeNameEx(
      prefix: string,
      shortform: boolean,
      brackets: boolean,
      scopeSymbol?: PullSymbol,
      ...
      typeParamterBuilder.add(
        PullSymbol.getTypeParameterStringEx(
      ...
      for (var i = 0; i < paramLen; i++) {
        ...
        builder.add(
          MemberName.create(
            varArgPrefix +
              params[i].getScopedNameEx(scopeSymbol).toString() +
              optionalString +
              typeString
          )
        );
    ...
    public wrapsSomeTypeParameter(
      ...
      if (
        ...
        signature.returnType.wrapsSomeTypeParameter(typeParameterArgumentMap)
      ) {
      ...
      if (!wrapsSomeTypeParameter) {
        ...
        for (var i = 0; i < parameters.length; i++) {
          ...
          if (
            parameters[i].type.wrapsSomeTypeParameter(typeParameterArgumentMap)
          ) {
            wrapsSomeTypeParameter = true;
            break;
          }
    ...
    public wrapsSomeNestedTypeIntoInfiniteExpansion(
      typeBeingWrapped: PullTypeSymbol,
      isCheckingTypeArgumentList: boolean,
      knownWrapMap: IBitMatrix
    ): boolean {
      if (this.inWrapCheck) {
      ...
      if (
        knownWrapMap.valueAt(
          this.pullSymbolID,
          typeBeingWrapped.pullSymbolID
        ) != undefined
      ) {
        return knownWrapMap.valueAt(
          this.pullSymbolID,
          typeBeingWrapped.pullSymbolID
        );
      }

      knownWrapMap.setValueAt(
        this.pullSymbolID,
        typeBeingWrapped.pullSymbolID,
        isCheckingTypeArgumentList
      );
      ...
      if (
        this.returnType &&
        this.returnType._wrapsSomeNestedTypeIntoInfiniteExpansionRecurse(
          typeBeingWrapped,
          isCheckingTypeArgumentList,
          knownWrapMap
        )
      ...
      if (!wrapsSomeWrappedTypeParameter) {
        ...
        for (var i = 0; i < parameters.length; i++) {
          if (
            parameters[i].type &&
            parameters[i].type._wrapsSomeNestedTypeIntoInfiniteExpansionRecurse(
              typeBeingWrapped,
              isCheckingTypeArgumentList,
              knownWrapMap
            )
  ...
  export class PullTypeSymbol extends PullSymbol {
    ...
    private _containedNonMemberContainerCache: IIndexable<PullTypeSymbol> =
      null;
    ...
    public isArrayNamedTypeReference() {
      if (this._isArrayNamedTypeReference === undefined) {
        this._isArrayNamedTypeReference =
          this.computeIsArrayNamedTypeReference();
      }
    ...
    public isClass() {
      return (
        this.kind == PullElementKind.Class || this._constructorMethod != null
      );
    }
    public isFunction() {
      return (
        (this.kind &
          (PullElementKind.ConstructorType | PullElementKind.FunctionType)) !=
        0
      );
    }
    ...
    public findContainedNonMemberType(
      typeName: string,
      kind = PullElementKind.None
    ): PullTypeSymbol {
      ...
      if (nonMemberSymbol && kind != PullElementKind.None) {
        nonMemberSymbol =
          (nonMemberSymbol.kind & kind) != 0 ? nonMemberSymbol : null;
      }

      return nonMemberSymbol;
    }

    public findContainedNonMemberContainer(
      ...
      var nonMemberSymbol =
        this._containedNonMemberContainerCache[containerName];
    ...
    public addEnclosedMemberType(enclosedType: PullTypeSymbol): void {
      ...
      this._enclosedMemberTypes[this._enclosedMemberTypes.length] =
        enclosedType;
    ...
    public addEnclosedMemberContainer(enclosedContainer: PullTypeSymbol): void {
      ...
      this._enclosedMemberContainers[this._enclosedMemberContainers.length] =
        enclosedContainer;
    ...
    public addEnclosedNonMember(enclosedNonMember: PullSymbol): void {
      ...
      if (!this._containedNonMemberNameCache) {
        this._containedNonMemberNameCache =
          createIntrinsicsObject<PullSymbol>();
      }

      this._containedNonMemberNameCache[enclosedNonMember.name] =
        enclosedNonMember;
    }

    public addEnclosedNonMemberType(
      enclosedNonMemberType: PullTypeSymbol
    ): void {
      ...
      if (!this._containedNonMemberTypeNameCache) {
        this._containedNonMemberTypeNameCache =
          createIntrinsicsObject<PullTypeSymbol>();
      }

      this._containedNonMemberTypeNameCache[enclosedNonMemberType.name] =
        enclosedNonMemberType;
    }

    public addEnclosedNonMemberContainer(
      enclosedNonMemberContainer: PullTypeSymbol
    ): void {
      ...
      this._containedNonMemberContainerCache[enclosedNonMemberContainer.name] =
    ...
    // GTODO
    public addConstructorTypeParameter(
    ...
    private canUseSimpleInstantiationCache(
      substitutingTypes: PullTypeSymbol[]
    ): boolean {
      return (
        substitutingTypes.length === 1 &&
        substitutingTypes[0].kind !== PullElementKind.ObjectType
      );
    }

    public addSpecialization(
      specializedVersionOfThisType: PullTypeSymbol,
      substitutingTypes: PullTypeSymbol[]
    ): void {
      ...
      if (this.canUseSimpleInstantiationCache(substitutingTypes)) {
        ...
        this._simpleInstantiationCache[substitutingTypes[0].pullSymbolID] =
          specializedVersionOfThisType;
      } else {
        ...
        this._complexInstantiationCache[
          getIDForTypeSubstitutions(substitutingTypes)
        ] = specializedVersionOfThisType;
      }
    ...
    public addConstructSignature(
      constructSignature: PullSignatureSymbol
    ): void {
      if (!this._constructSignatures) {
        this._constructSignatures = [];
      }

      this._constructSignatures[this._constructSignatures.length] =
        constructSignature;
    ...
    private addUnhiddenSignaturesFromBaseType(
      ...
      if (!derivedTypeSignatures) {
        RT.applyVariadic<PullSignatureSymbol>(
          signaturesBeingAggregated,
          'push',
          baseTypeSignatures
        );
      ...
      for (var i = 0; i < baseTypeSignatures.length; i++) {
        ...
        var signatureIsHidden = ArrayUtilities.any(
          derivedTypeSignatures,
          (sig) =>
            resolver.signaturesAreIdentical(
              baseSignature,
              sig,
    ...
    public getCallSignatures(): PullSignatureSymbol[] {
      ...
      if (this._extendedTypes && this.kind === PullElementKind.Interface) {
        for (var i = 0; i < this._extendedTypes.length; i++) {
          ...
          // identical when return types are ignored.
          this.addUnhiddenSignaturesFromBaseType(
            this._callSignatures,
            this._extendedTypes[i].getCallSignatures(),
            signatures
          );
        }
      }
    ...
    public getConstructSignatures(): PullSignatureSymbol[] {
      ...
      if (this._extendedTypes && this.kind === PullElementKind.Interface) {
        for (var i = 0; i < this._extendedTypes.length; i++) {
          ...
          this.addUnhiddenSignaturesFromBaseType(
            this._constructSignatures,
            this._extendedTypes[i].getConstructSignatures(),
            signatures
          );
        }
      }
    ...
    public getIndexSignatures(): PullSignatureSymbol[] {
      ...
      if (this._extendedTypes) {
        for (var i = 0; i < this._extendedTypes.length; i++) {
          ...
          this.addUnhiddenSignaturesFromBaseType(
            this._indexSignatures,
            this._extendedTypes[i].getIndexSignatures(),
            signatures
          );
        }
      }
    ...
    public addTypeThatExtendsThisType(type: PullTypeSymbol): void {
      ...
      this._typesThatExtendThisType[this._typesThatExtendThisType.length] =
        type;
    }
    ...
    public hasBase(
      ...
      if (
        this === potentialBase ||
        this.getRootSymbol() === potentialBase ||
        this === potentialBase.getRootSymbol()
      ) {
        return true;
      }
    ...
    public isValidBaseKind(
      ...
      return !!(
        baseType.kind &
        (PullElementKind.Interface | PullElementKind.Class)
      );
    }
    ...
    public getAllMembers(
      searchDeclKind: PullElementKind,
      memberVisiblity: GetAllMembersVisiblity
    ): PullSymbol[] {
      ...
      if (this._members.length !== 0) {
        ...
        for (var i = 0, n = this._members.length; i < n; i++) {
          ...
          if (
            member.kind & searchDeclKind &&
            (memberVisiblity !== GetAllMembersVisiblity.externallyVisible ||
              !member.anyDeclHasFlag(PullElementFlags.Private))
          ) {
      ...
      if (this._extendedTypes) {
        ...
        var extenedMembersVisibility =
          memberVisiblity !== GetAllMembersVisiblity.all
            ? GetAllMembersVisiblity.externallyVisible
            : GetAllMembersVisiblity.all;

        for (var i = 0, n = this._extendedTypes.length; i < n; i++) {
          var extendedMembers = this._extendedTypes[i].getAllMembers(
            searchDeclKind,
            /*memberVisiblity*/ extenedMembersVisibility
          );
          ...
          for (var j = 0, m = extendedMembers.length; j < m; j++) {
            ...
            if (
              !(
                ...
                this._memberNameCache[extendedMember.name]
              )
            ) {
    ...
    public getNamePartForFullName(): string {
      ...
      var typarString = PullSymbol.getTypeParameterString(
        typars,
        this,
        /*useConstraintInName:*/ true
      );
    ...
    public getScopedName(
      ...
      return this.getScopedNameEx(
        scopeSymbol,
        skipTypeParametersInName,
        useConstraintInName,
        /*getPrettyTypeName*/ false,
        /*getTypeParamMarkerInfskipInternalAliasName*/ false,
        skipInternalAliasName
      ).toString();
    }
    ...
    public isNamedTypeSymbol(): boolean {
      var kind = this.kind;
      if (
        kind === PullElementKind.Primitive || // primitives
    ...
    public getScopedNameEx(
      ...
      if (this.isArrayNamedTypeReference()) {
        ...
        var elementMemberName = elementType
          ? elementType.isArrayNamedTypeReference() ||
            elementType.isNamedTypeSymbol()
            ...
            : elementType.getMemberTypeNameEx(
                /*topLevel*/ false,
                scopeSymbol,
                getPrettyTypeName
              )
      ...
      if (!this.isNamedTypeSymbol()) {
        return this.getMemberTypeNameEx(
          /*topLevel*/ true,
          scopeSymbol,
          getPrettyTypeName
        );
      ...
      if (skipTypeParametersInName) {
        ...
        var builder = new MemberNameArray();
        builder.prefix = super.getScopedName(
          scopeSymbol,
          skipTypeParametersInName,
          useConstraintInName,
          skipInternalAliasName
    ...
    public getTypeOfSymbol() {
      ...
      if (
        associatedContainerType &&
        associatedContainerType.isNamedTypeSymbol()
      ) {
      ...
      if (
        ...
        !PullHelpers.isSymbolLocal(functionSymbol)
      ) {
    ...
    private getMemberTypeNameEx(
      ...
      if (
        ...
        if (typeOfSymbol) {
          var nameForTypeOf = typeOfSymbol.getScopedNameEx(
            scopeSymbol,
            /*skipTypeParametersInName*/ true
          );
        ...
        for (var i = 0; i < members.length; i++) {
          if (
            members[i].kind == PullElementKind.Method &&
            members[i].type.hasOnlyOverloadCallSignatures()
          ) {
            // Add all Call signatures of the method
            var methodCallSignatures = members[i].type.getCallSignatures();
            var nameStr =
              members[i].getDisplayName(scopeSymbol) +
              (members[i].isOptional ? '?' : '');
            var methodMemberNames = PullSignatureSymbol.getSignaturesTypeNameEx(
            ...
            if (
              memberTypeName.isArray() &&
              (<MemberNameArray>memberTypeName).delim === delim
            ) {
        ...
        var getPrettyFunctionOverload =
          getPrettyTypeName && !curlies && this.hasOnlyOverloadCallSignatures();
        ...
        if (callSignatures.length > 0) {
          signatureMemberName = PullSignatureSymbol.getSignaturesTypeNameEx(
            callSignatures,
            /*prefix*/ '',
            useShortFormSignature,
            /*brackets*/ false,
            scopeSymbol,
            getPrettyFunctionOverload
          );
          allMemberNames.addAll(signatureMemberName);
        }
        ...
        if (
          curlies ||
          (!getPrettyFunctionOverload && signatureCount > 1 && topLevel)
        ) {
    ...
    public getGenerativeTypeClassification(
    ...
    public wrapsSomeTypeParameter(
      typeParameterArgumentMap: CandidateInferenceInfo[]
    ): boolean;
    public wrapsSomeTypeParameter(
      typeParameterArgumentMap: PullTypeSymbol[]
    ): boolean;
    public wrapsSomeTypeParameter(typeParameterArgumentMap: any): boolean {
      ...
      if (type.isTypeParameter()) {
        ...
        if (
          constraint &&
          constraint.wrapsSomeTypeParameter(typeParameterArgumentMap)
        ) {
          return true;
        }
      ...
      if (!wrapsSomeTypeParameter) {
        ...
        if (typeArguments) {
          for (var i = 0; i < typeArguments.length; i++) {
            if (
              typeArguments[i].wrapsSomeTypeParameter(typeParameterArgumentMap)
            ) {
              wrapsSomeTypeParameter = true;
              break;
            }
      ...
      if (!(type.kind & PullElementKind.SomeInstantiatableType) || !type.name) {
        if (!wrapsSomeTypeParameter) {
          ...
          for (var i = 0; i < members.length; i++) {
            if (
              members[i].type.wrapsSomeTypeParameter(typeParameterArgumentMap)
            ) {
              wrapsSomeTypeParameter = true;
              break;
            }
    ...
    public wrapsSomeNestedTypeIntoInfiniteExpansion(
      ...
      if (!this.isArrayNamedTypeReference() && this.isNamedTypeSymbol()) {
        var knownWrapMap = BitMatrix.getBitMatrix(
          /*allowUndefinedValues:*/ true
        );
        var result = this._wrapsSomeNestedTypeIntoInfiniteExpansionRecurse(
          typeBeingWrapped,
          /*isCheckingTypeArgumentList:*/ false,
          knownWrapMap
        );
    ...
    private isTypeEquivalentToRootSymbol() {
      if (this.isTypeReference()) {
        if (this.getIsSpecialized()) {
          ...
          var rootTypeArguments = (<PullTypeSymbol>(
            this.getRootSymbol()
          )).getTypeArguments();
          if (typeArguments) {
            for (var i = 0; i < typeArguments.length; i++) {
              if (
                !typeArguments[i].isTypeParameter() && // The typeArgument is not type parameter
                !(
                  rootTypeArguments &&
                  rootTypeArguments[i] == typeArguments[i].getRootSymbol()
                )
              ) {
                // Root symbol of the type argument is not same as type argument of rootSymbol
                return false;
              }
            }
            return true;
    ...
    private isTypeBeingWrapped(typeBeingWrapped: PullTypeSymbol) {
      ...
      if (typeBeingWrapped.isTypeEquivalentToRootSymbol()) {
        return this.isTypeBeingWrapped(
          <PullTypeSymbol>typeBeingWrapped.getRootSymbol()
        );
      }
    ...
    public _wrapsSomeNestedTypeIntoInfiniteExpansionRecurse(
      typeBeingWrapped: PullTypeSymbol,
      isCheckingTypeArgumentList: boolean,
      knownWrapMap: IBitMatrix
    ): boolean {
      if (this.isArrayNamedTypeReference()) {
        return this.getElementType()._wrapsSomeNestedTypeIntoInfiniteExpansionRecurse(
          typeBeingWrapped,
          isCheckingTypeArgumentList,
          knownWrapMap
        );
      ...
      wrapsSomeWrappedTypeParameter =
        this._wrapsSomeNestedTypeIntoInfiniteExpansionWorker(
          typeBeingWrapped,
          isCheckingTypeArgumentList,
          knownWrapMap
        );
    ...
    private _wrapsSomeNestedTypeIntoInfiniteExpansionWorker(
      typeBeingWrapped: PullTypeSymbol,
      isCheckingTypeArgumentList: boolean,
      knownWrapMap: IBitMatrix
    ): boolean {
      ...
      if (typeArguments) {
        for (var i = 0; i < typeArguments.length; i++) {
          if (
            typeArguments[i]._wrapsSomeNestedTypeIntoInfiniteExpansionRecurse(
              typeBeingWrapped,
              /*isCheckingTypeArgumentList:*/ true,
              knownWrapMap
            )
          ) {
            return true;
      ...
      // If it is not a named type symbol or this type is a generic type, verify members
        ...
        for (var i = 0; i < members.length; i++) {
          if (
            members[i].type &&
            members[i].type._wrapsSomeNestedTypeIntoInfiniteExpansionRecurse(
              typeBeingWrapped,
              isCheckingTypeArgumentList,
              knownWrapMap
            )
        ...
        for (var i = 0; i < sigs.length; i++) {
          if (
            sigs[i].wrapsSomeNestedTypeIntoInfiniteExpansion(
              typeBeingWrapped,
              isCheckingTypeArgumentList,
              knownWrapMap
            )
  ...
  export class PullErrorTypeSymbol extends PullPrimitiveTypeSymbol {
    constructor(
      private anyType: PullTypeSymbol,
      name: string
    ) {
      super(name);
  ...
  export class PullContainerSymbol extends PullTypeSymbol {
    ...
    public hasExportAssignment() {
      return (
        !!this.assignedValue || !!this.assignedType || !!this.assignedContainer
      );
    }

    static usedAsSymbol(
      containerSymbol: PullSymbol,
      symbol: PullSymbol
    ): boolean {
      ...
      var containerExportSymbol =
        moduleSymbol.getExportAssignedContainerSymbol();
      if (valueExportSymbol || typeExportSymbol || containerExportSymbol) {
        return (
          valueExportSymbol == symbol ||
          typeExportSymbol == symbol ||
          containerExportSymbol == symbol ||
          PullContainerSymbol.usedAsSymbol(containerExportSymbol, symbol)
        );
      }
  ...
  export class PullTypeAliasSymbol extends PullTypeSymbol {
    ...
    public setIsUsedAsValue(value: boolean): void {
      ...
      this._resolveDeclaredSymbol();
      var resolver = this._getResolver();
      var importDeclStatement = <ImportDeclaration>(
        resolver.semanticInfoChain.getASTForDecl(this.getDeclarations()[0])
      );
      var aliasSymbol = <PullTypeAliasSymbol>(
        resolver.semanticInfoChain.getAliasSymbolForAST(
          importDeclStatement.moduleReference
        )
      );
      if (aliasSymbol) {
    ...
    public getAllMembers(
      searchDeclKind: PullElementKind,
      memberVisibility: GetAllMembersVisiblity
    ): PullSymbol[] {
      if (this._assignedType) {
        return this._assignedType.getAllMembers(
          searchDeclKind,
          memberVisibility
        );
      }
  ...
  export class PullTypeParameterSymbol extends PullTypeSymbol {
    ...
    constructor(
      name: string,
      private _isFunctionTypeParameter: boolean
    ) {
  ...
  function getIDForTypeSubstitutionsFromObjectType(
    type: PullTypeSymbol
  ): string {
    var structure = '';
    ...
    if (type.isResolved) {
      ...
      if (members && members.length) {
        for (var j = 0; j < members.length; j++) {
          structure +=
            members[j].name +
            '@' +
            getIDForTypeSubstitutions([members[j].type]);
        }
      }
      ...
      if (constructSignatures && constructSignatures.length) {
        for (var j = 0; j < constructSignatures.length; j++) {
          structure +=
            'new' +
            getIDForTypeSubstitutionFromSignature(constructSignatures[j]);
        }
      }
      ...
      if (indexSignatures && indexSignatures.length) {
        for (var j = 0; j < indexSignatures.length; j++) {
          structure +=
            '[]' + getIDForTypeSubstitutionFromSignature(indexSignatures[j]);
        }
      }
    }
  ...
  function getIDForTypeSubstitutionFromSignature(
    ...
    if (parameters && parameters.length) {
      for (var k = 0; k < parameters.length; k++) {
        structure +=
          parameters[k].name +
          '@' +
          getIDForTypeSubstitutions([parameters[k].type]);
      }
    }
