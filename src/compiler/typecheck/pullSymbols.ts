...
import { SoundType, Comment, ImportDeclaration } from '../ast';
import {
...
export class PullSymbol {
            ...
            if (
              ...
              (!symbol.assignedContainer().hasExportAssignment() ||
                (symbol
                  ...
                  symbol.assignedContainer().getExportAssignedContainerSymbol()
  ...
  private getDocCommentsOfDecl(decl: PullDecl): Comment[] {
    var ast = decl.ast();
  ...
  private getDocCommentArray(symbol: PullSymbol) {
    ...
    for (var i = 0; i < decls.length; i++) {
      if (isParameter && decls[i].kind == PullElementKind.Property) {
    ...
    if (this._docComments === null) {
      ...
      if (
        ...
        if (this.kind == PullElementKind.FunctionType) {
          // @ts-expect-error Weird, this = symbol?
          var functionSymbol = (<PullTypeSymbol>this).getFunctionSymbol();
        ...
        if (getSymbolComments) {
          ...
          if (docComments == '') {
            if (this.kind == PullElementKind.CallSignature) {
              // @ts-expect-error Weird, this = symbol?
              var callTypeSymbol = (<PullSignatureSymbol>this).functionType;
              if (
                callTypeSymbol &&
    ...
    if (nonSpaceIndex !== -1) {
      ...
      return {
        start: nonSpaceIndex,
          ...
          line.charAt(line.length - 1) === '\r' ? line.length - 1 : line.length,
...
export class PullSignatureSymbol extends PullSymbol {
    ...
    if (!this._memberTypeParameterNameCache) {
      ...
      if (this.typeParameters) {
        for (var i = 0; i < this.typeParameters.length; i++) {
          this._memberTypeParameterNameCache[this.typeParameters[i].getName()] =
  ...
  public wrapsSomeNestedTypeIntoInfiniteExpansion(
    ...
    if (
      knownWrapMap.valueAt(this.pullSymbolID, typeBeingWrapped.pullSymbolID) !=
      undefined
    ) {
...
export class PullTypeSymbol extends PullSymbol {
  ...
  private isTypeBeingWrapped(typeBeingWrapped: PullTypeSymbol): boolean {
  ...
  if (type.isResolved) {
    ...
    if (members && members.length) {
      for (var j = 0; j < members.length; j++) {
        structure +=
          members[j].name + '@' + getIDForTypeSubstitutions([members[j].type]);
      }
    }
    ...
    if (constructSignatures && constructSignatures.length) {
      for (var j = 0; j < constructSignatures.length; j++) {
        structure +=
          'new' + getIDForTypeSubstitutionFromSignature(constructSignatures[j]);
      }
    }
