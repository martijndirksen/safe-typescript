...
module TypeScript {
  ...
  export class PullTypeReferenceSymbol extends PullTypeSymbol {
    public static createTypeReference(
      type: PullTypeSymbol
    ...
    constructor(public referencedTypeSymbol: PullTypeSymbol) {
      ...
      Debug.assert(
        referencedTypeSymbol != null,
        'Type root symbol may not be null'
      );
    ...
    public setAssociatedContainerType(type: PullTypeSymbol): void {
      Debug.fail(
        'Reference symbol ' + this.pullSymbolID + ': setAssociatedContainerType'
      );
    }
    ...
    public findContainedNonMemberContainer(
      ...
      this.ensureReferencedTypeIsResolved();
      return this.referencedTypeSymbol.findContainedNonMemberContainer(
        containerName,
        kind
      );
    }
    ...
    public addEnclosedMemberContainer(enclosedContainer: PullTypeSymbol): void {
      Debug.fail(
        'Reference symbol ' + this.pullSymbolID + ': addEnclosedMemberContainer'
      );
    }
    ...
    public addEnclosedNonMemberType(
      ...
      Debug.fail(
        'Reference symbol ' + this.pullSymbolID + ': addEnclosedNonMemberType'
      );
    }
    ...
    public findContainedNonMemberType(
      ...
      return this.referencedTypeSymbol.findContainedNonMemberType(
        typeName,
        kind
      );
    }
    ...
    public setHasDefaultConstructor(hasOne = true): void {
      Debug.fail(
        'Reference symbol ' + this.pullSymbolID + ':setHasDefaultConstructor'
      );
    }
    ...
    public addSpecialization(
      ...
      this.ensureReferencedTypeIsResolved();
      return this.referencedTypeSymbol.addSpecialization(
    ...
    public addCallSignature(callSignature: PullSignatureSymbol): void {
      Debug.fail(
        'Reference symbol ' + this.pullSymbolID + ': addCallSignature'
      );
    }
    public addConstructSignature(
      ...
      Debug.fail(
        'Reference symbol ' + this.pullSymbolID + ': addConstructSignature'
      );
    }
    ...
    public isValidBaseKind(
      ...
      this.ensureReferencedTypeIsResolved();
      return this.referencedTypeSymbol.isValidBaseKind(
        baseType,
        isExtendedType
      );
    }
    ...
    public getAllMembers(
      ...
      return this.referencedTypeSymbol.getAllMembers(
        searchDeclKind,
        memberVisiblity
      );
    }
  ...
  export class PullInstantiatedTypeReferenceSymbol extends PullTypeReferenceSymbol {
    ...
    private _allInstantiatedMemberNameCache: { [name: string]: PullSymbol } =
      null;
    ...
    public getGenerativeTypeClassification(
      ...
      if (
        ...
        var typeReferenceTypeArguments = (<PullTypeSymbol>(
          this.getRootSymbol()
        )).getTypeArguments();
        ...
        if (!typeReferenceTypeArguments) {
          ...
          for (var i = 0; i < typeParameters.length; i++) {
            typeParametersMap[typeParameters[i].pullSymbolID] =
              typeParameters[i];
          }

          var rootThis = PullHelpers.getRootType(this);
          var wrapsSomeTypeParameters =
            rootThis.wrapsSomeTypeParameter(typeParametersMap);

          // It's a wrap of a wrap
          if (
            wrapsSomeTypeParameters &&
            this.wrapsSomeNestedTypeIntoInfiniteExpansion(enclosingType)
          ) {
            this._generativeTypeClassification =
              GenerativeTypeClassification.InfinitelyExpanding;
          } else if (wrapsSomeTypeParameters) {
            this._generativeTypeClassification =
              GenerativeTypeClassification.Open;
          } else {
            this._generativeTypeClassification =
              GenerativeTypeClassification.Closed;
          }
        } else {
          var i = 0;

          while (i < typeReferenceTypeArguments.length) {
            referenceTypeArgument = <PullTypeSymbol>(
              typeReferenceTypeArguments[i].getRootSymbol()
            );

            if (
              referenceTypeArgument.wrapsSomeTypeParameter(
    ...
    public static create(
      ...
      if (type.getIsSpecialized() && typeArguments && typeArguments.length) {
        for (var i = 0; i < typeArguments.length; i++) {
          reconstructedTypeArgumentList[reconstructedTypeArgumentList.length] =
            resolver.instantiateType(
              typeArguments[i],
        ...
        for (var i = 0; i < typeArguments.length; i++) {
          typeParameterArgumentMap[typeArguments[i].pullSymbolID] =
            reconstructedTypeArgumentList[i];
        }
      } else {
          ...
          if (typeParameterArgumentMap.hasOwnProperty(typeParameterID)) {
            ...
            if (tyArg) {
              reconstructedTypeArgumentList[
                reconstructedTypeArgumentList.length
              ] = tyArg;
            }
          }
        }
      ...
      if (!instantiateFunctionTypeParameters) {
        instantiation = <PullInstantiatedTypeReferenceSymbol>(
          rootType.getSpecialization(reconstructedTypeArgumentList)
        );
      ...
      var isReferencedType =
        (type.kind & PullElementKind.SomeInstantiatableType) != 0;
      ...
      if (type.isTypeReference() && type.isGeneric()) {
        ...
        for (var typeParameterID in typeParameterArgumentMap) {
          if (typeParameterArgumentMap.hasOwnProperty(typeParameterID)) {
            initializationMap[+typeParameterID] =
              typeParameterArgumentMap[+typeParameterID];
          }
        }

        var oldMap = typeParameterArgumentMap;

        var outerTypeMap = (<PullInstantiatedTypeReferenceSymbol>type)
          ._typeParameterArgumentMap;
        ...
        for (var typeParameterID in outerTypeMap) {
          if (outerTypeMap.hasOwnProperty(typeParameterID)) {
            ...
            innerSubstitution =
              typeParameterArgumentMap[outerSubstitution.pullSymbolID];
            ...
            if (
              ...
              initializationMap[+typeParameterID] =
                typeParameterArgumentMap[
                  outerTypeMap[typeParameterID].pullSymbolID
                ];
              ...
              if (!outerSubstitution.isTypeParameter()) {
                canRelatePreInstantiatedTypeParametersToRootTypeParameters =
                  false;
              }
            } else {
              canRelatePreInstantiatedTypeParametersToRootTypeParameters =
        ...
        if (
          ...
          for (var i = 0; i < typeParameters.length; i++) {
            typeParameterArgumentMap[typeParameters[i].pullSymbolID] =
              initializationMap[typeParameters[i].pullSymbolID];
          }
      ...
      instantiation = new PullInstantiatedTypeReferenceSymbol(
        instantiateRoot ? rootType : type,
        typeParameterArgumentMap
      );

      if (!instantiateFunctionTypeParameters) {
        rootType.addSpecialization(
          instantiation,
          reconstructedTypeArgumentList
        );
      }
    ...
    public getTypeArguments(): PullTypeSymbol[] {
      ...
      if (!this._typeArgumentReferences) {
        ...
        if (typeParameters.length) {
          ...
          for (var i = 0; i < typeParameters.length; i++) {
            typeArgument = <PullTypeSymbol>(
              this._typeParameterArgumentMap[typeParameters[i].pullSymbolID]
            );
      ...
      if (!this._instantiatedMembers) {
        ...
        for (var i = 0; i < referencedMembers.length; i++) {
          ...
          if (!this._instantiatedMemberNameCache[referencedMember.name]) {
            ...
            if (
              !referencedMember.type.wrapsSomeTypeParameter(
                this._typeParameterArgumentMap
              )
            ) {
              instantiatedMember = referencedMember;
            } else {
              instantiatedMember = new PullSymbol(
                referencedMember.name,
                referencedMember.kind
              );
              instantiatedMember.setRootSymbol(referencedMember);
              instantiatedMember.type = this._getResolver().instantiateType(
                referencedMember.type,
                this._typeParameterArgumentMap
              );
              instantiatedMember.isOptional = referencedMember.isOptional;
            }

            this._instantiatedMemberNameCache[instantiatedMember.name] =
              instantiatedMember;
          } else {
            instantiatedMember =
              this._instantiatedMemberNameCache[referencedMember.name];
          }

          this._instantiatedMembers[this._instantiatedMembers.length] =
            instantiatedMember;
        }
      }
    ...
    public findMember(name: string, lookInParent = true): PullSymbol {
      ...
      if (!memberSymbol) {
        var referencedMemberSymbol = this.referencedTypeSymbol.findMember(
          name,
          lookInParent
        );
        ...
        if (referencedMemberSymbol) {
          memberSymbol = new PullSymbol(
            referencedMemberSymbol.name,
            referencedMemberSymbol.kind
          );
          memberSymbol.setRootSymbol(referencedMemberSymbol);

          this._getResolver().resolveDeclaredSymbol(referencedMemberSymbol);

          memberSymbol.type = this._getResolver().instantiateType(
            referencedMemberSymbol.type,
            this._typeParameterArgumentMap
          );
    ...
    public getAllMembers(
      ...
      var allReferencedMembers = this.referencedTypeSymbol.getAllMembers(
        searchDeclKind,
        memberVisiblity
      );

      if (!this._allInstantiatedMemberNameCache) {
        this._allInstantiatedMemberNameCache =
      ...
      for (var i = 0; i < allReferencedMembers.length; i++) {
        ...
        if (this._allInstantiatedMemberNameCache[referencedMember.name]) {
          requestedMembers[requestedMembers.length] =
            this._allInstantiatedMemberNameCache[referencedMember.name];
        } else {
          if (
            !referencedMember.type.wrapsSomeTypeParameter(
              this._typeParameterArgumentMap
            ...
            requestedMember = new PullSymbol(
              referencedMember.name,
              referencedMember.kind
            );
            requestedMember.setRootSymbol(referencedMember);

            requestedMember.type = this._getResolver().instantiateType(
              referencedMember.type,
              this._typeParameterArgumentMap
            );
            requestedMember.isOptional = referencedMember.isOptional;

            this._allInstantiatedMemberNameCache[requestedMember.name] =
              requestedMember;
    ...
    public getConstructorMethod(): PullSymbol {
      ...
      if (!this._instantiatedConstructorMethod) {
        var referencedConstructorMethod =
          this.referencedTypeSymbol.getConstructorMethod();
        this._instantiatedConstructorMethod = new PullSymbol(
          referencedConstructorMethod.name,
          referencedConstructorMethod.kind
        );
        this._instantiatedConstructorMethod.type =
          PullInstantiatedTypeReferenceSymbol.create(
            this._getResolver(),
            referencedConstructorMethod.type,
            this._typeParameterArgumentMap
          );
      }
    ...
    public getAssociatedContainerType(): PullTypeSymbol {
      ...
      if (!this._instantiatedAssociatedContainerType) {
        var referencedAssociatedContainerType =
          this.referencedTypeSymbol.getAssociatedContainerType();

        if (referencedAssociatedContainerType) {
          this._instantiatedAssociatedContainerType =
            PullInstantiatedTypeReferenceSymbol.create(
              this._getResolver(),
              referencedAssociatedContainerType,
              this._typeParameterArgumentMap
            );
        }
      }
    ...
    public getCallSignatures(): PullSignatureSymbol[] {
      ...
      for (var i = 0; i < referencedCallSignatures.length; i++) {
        ...
        if (
          !referencedCallSignatures[i].wrapsSomeTypeParameter(
            this._typeParameterArgumentMap
          )
        ) {
          this._instantiatedCallSignatures[
            this._instantiatedCallSignatures.length
          ] = referencedCallSignatures[i];
        } else {
          this._instantiatedCallSignatures[
            this._instantiatedCallSignatures.length
    ...
    public getConstructSignatures(): PullSignatureSymbol[] {
      ...
      var referencedConstructSignatures =
      ...
      for (var i = 0; i < referencedConstructSignatures.length; i++) {
        this._getResolver().resolveDeclaredSymbol(
          referencedConstructSignatures[i]
        );

        if (
          !referencedConstructSignatures[i].wrapsSomeTypeParameter(
            this._typeParameterArgumentMap
          )
        ) {
          this._instantiatedConstructSignatures[
    ...
    public getIndexSignatures(): PullSignatureSymbol[] {
      ...
      for (var i = 0; i < referencedIndexSignatures.length; i++) {
        ...
        if (
          !referencedIndexSignatures[i].wrapsSomeTypeParameter(
            this._typeParameterArgumentMap
          )
        ) {
          this._instantiatedIndexSignatures[
            this._instantiatedIndexSignatures.length
          ] = referencedIndexSignatures[i];
        } else {
          this._instantiatedIndexSignatures[
            this._instantiatedIndexSignatures.length
          ] = this._getResolver().instantiateSignature(
