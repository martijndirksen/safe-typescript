...
///<reference path='..\references.ts' />
  ...
  function containingModuleHasExportAssignment(ast: AST): boolean {
    ...
    while (ast) {
      if (ast.kind() === SyntaxKind.ModuleDeclaration) {
        ...
        return moduleDecl.moduleElements.any(
          (m) => m.kind() === SyntaxKind.ExportAssignment
        );
      } else if (ast.kind() === SyntaxKind.SourceUnit) {
        var sourceUnit = <SourceUnit>ast;
        return sourceUnit.moduleElements.any(
          (m) => m.kind() === SyntaxKind.ExportAssignment
        );
      }
  ...
  function isParsingAmbientModule(
    ast: AST,
    ...
    while (ast) {
      if (ast.kind() === SyntaxKind.ModuleDeclaration) {
        if (
          hasModifier(
            (<ModuleDeclaration>ast).modifiers,
  ...
  function preCollectImportDecls(
    ast: AST,
    context: DeclCollectionContext
  ): void {
    ...
    if (
      hasModifier(importDecl.modifiers, PullElementFlags.Exported) &&
      !containingModuleHasExportAssignment(ast)
    ) {
      declFlags |= PullElementFlags.Exported;
    }

    var decl = new NormalPullDecl(
      importDecl.identifier.valueText(),
      importDecl.identifier.text(),
  ...
  function preCollectScriptDecls(
    ...
    if (isExternalModule) {
      ...
      var moduleContainsExecutableCode = containsExecutableCode(
      ...
      var decl: PullDecl = new NormalPullDecl(
        valueText,
        fileName,
        kind,
        declFlags,
  ...
  function preCollectEnumDecls(
    ...
    if (
      (hasModifier(enumDecl.modifiers, PullElementFlags.Exported) ||
        isParsingAmbientModule(enumDecl, context)) &&
      !containingModuleHasExportAssignment(enumDecl)
    ) {
      declFlags |= PullElementFlags.Exported;
    }

    if (
      hasModifier(enumDecl.modifiers, PullElementFlags.Ambient) ||
      isParsingAmbientModule(enumDecl, context) ||
      context.isDeclareFile
    ) {
      declFlags |= PullElementFlags.Ambient;
    }
    ...
    var enumDeclaration = new NormalPullDecl(
      enumName,
    ...
    var enumIndexerDecl = new NormalPullDecl(
      '',
      '',
      PullElementKind.IndexSignature,
      PullElementFlags.Signature,
      enumDeclaration,
      span
    );
    var enumIndexerParameter = new NormalPullDecl(
      'x',
      'x',
    ...
    var valueDecl = new NormalPullDecl(
      enumDeclaration.name,
      enumDeclaration.getDisplayName(),
      PullElementKind.Variable,
      enumDeclaration.flags,
      context.getParent(),
      enumDeclaration.getSpan()
    );
  ...
  function createEnumElementDecls(
    propertyDecl: EnumElement,
    context: DeclCollectionContext
  ): void {
    var parent = context.getParent();
    ...
    var decl = new PullEnumElementDecl(
      propertyDecl.propertyName.valueText(),
      propertyDecl.propertyName.text(),
      parent,
      span
    );
  ...
  function preCollectModuleDecls(
    ...
    var moduleContainsExecutableCode = containsExecutableCode(
      moduleDecl.moduleElements
    );

    var isDynamic = moduleDecl.stringLiteral !== null;

    if (
      (hasModifier(moduleDecl.modifiers, PullElementFlags.Exported) ||
        isParsingAmbientModule(moduleDecl, context)) &&
      !containingModuleHasExportAssignment(moduleDecl)
    ) {
      declFlags |= PullElementFlags.Exported;
    }

    if (
      hasModifier(moduleDecl.modifiers, PullElementFlags.Ambient) ||
      isParsingAmbientModule(moduleDecl, context) ||
      context.isDeclareFile
    ) {
      declFlags |= PullElementFlags.Ambient;
  ...
  function createModuleVariableDecl(
    decl: PullDecl,
    moduleNameAST: AST,
    context: DeclCollectionContext
  ): void {
  ...
  function containsExecutableCode(members: ISyntaxList2): boolean {
    for (var i = 0, n = members.childCount(); i < n; i++) {
      ...
      if (member.kind() === SyntaxKind.ModuleDeclaration) {
        ...
        member.kind() !== SyntaxKind.InterfaceDeclaration &&
        member.kind() !== SyntaxKind.ImportDeclaration
      ) {
  ...
  function preCollectClassDecls(
    ...
    if (
      (hasModifier(classDecl.modifiers, PullElementFlags.Exported) ||
        isParsingAmbientModule(classDecl, context)) &&
      !containingModuleHasExportAssignment(classDecl)
    ) {
      declFlags |= PullElementFlags.Exported;
    }

    if (
      hasModifier(classDecl.modifiers, PullElementFlags.Ambient) ||
      isParsingAmbientModule(classDecl, context) ||
      context.isDeclareFile
    ) {
      declFlags |= PullElementFlags.Ambient;
  ...
  function preCollectObjectTypeDecls(
    ...
    if (
      parent &&
      (parent.kind === PullElementKind.WithBlock ||
        parent.flags & PullElementFlags.DeclaredInAWithBlock)
    ) {
  ...
  function preCollectInterfaceDecls(
    ...
    if (
      (hasModifier(interfaceDecl.modifiers, PullElementFlags.Exported) ||
        isParsingAmbientModule(interfaceDecl, context)) &&
      !containingModuleHasExportAssignment(interfaceDecl)
    ) {
      declFlags |= PullElementFlags.Exported;
    }
  ...
  function preCollectParameterDecl(
    argDecl: Parameter,
    context: DeclCollectionContext
  ): void {
    var declFlags = PullElementFlags.None;
    ...
    if (
      argDecl.questionToken !== null ||
      argDecl.equalsValueClause !== null ||
      argDecl.dotDotDotToken !== null
    ) {
    ...
    var decl = new NormalPullDecl(
      argDecl.identifier.valueText(),
      argDecl.identifier.text(),
      PullElementKind.Parameter,
      declFlags,
      parent,
      span
    ...
    var isPublicOrPrivate = hasModifier(
      argDecl.modifiers,
      PullElementFlags.Public | PullElementFlags.Private
    );
    ...
    if (isPublicOrPrivate && isInConstructor) {
      ...
      var propDecl = new NormalPullDecl(
        argDecl.identifier.valueText(),
        argDecl.identifier.text(),
  ...
  function preCollectTypeParameterDecl(
    typeParameterDecl: TypeParameter,
    context: DeclCollectionContext
  ): void {
    var declFlags = PullElementFlags.None;

    var span = TextSpan.fromBounds(
      typeParameterDecl.start(),
      typeParameterDecl.end()
    );
    ...
    var decl = new NormalPullDecl(
      typeParameterDecl.identifier.valueText(),
      typeParameterDecl.identifier.text(),
      PullElementKind.TypeParameter,
      declFlags,
      parent,
      span
    );
  ...
  function createPropertySignature(
    propertyDecl: PropertySignature,
    context: DeclCollectionContext
  ): void {
  ...
  function createMemberVariableDeclaration(
    memberDecl: MemberVariableDeclaration,
    context: DeclCollectionContext
  ): void {
    var declFlags = PullElementFlags.None;
    var declType = PullElementKind.Property;
    ...
    var decl = new NormalPullDecl(
      memberDecl.variableDeclarator.propertyName.valueText(),
      memberDecl.variableDeclarator.propertyName.text(),
      declType,
      declFlags,
      parent,
      span
    ...
    context.semanticInfoChain.setDeclForAST(
      memberDecl.variableDeclarator,
      decl
    );
  ...
  function createVariableDeclaration(
    ...
    if (
      (hasModifier(modifiers, PullElementFlags.Exported) ||
        isParsingAmbientModule(varDecl, context)) &&
      !containingModuleHasExportAssignment(varDecl)
    ) {
      declFlags |= PullElementFlags.Exported;
    }

    if (
      hasModifier(modifiers, PullElementFlags.Ambient) ||
      isParsingAmbientModule(varDecl, context) ||
      context.isDeclareFile
    ) {
      declFlags |= PullElementFlags.Ambient;
  ...
  function createFunctionTypeDeclaration(
    functionTypeDeclAST: FunctionType,
    context: DeclCollectionContext
  ): void {
    ...
    var span = TextSpan.fromBounds(
      functionTypeDeclAST.start(),
      functionTypeDeclAST.end()
    );
  ...
  // constructor types
  function createConstructorTypeDeclaration(
    constructorTypeDeclAST: ConstructorType,
    context: DeclCollectionContext
  ): void {
    var declFlags = PullElementFlags.None;
    ...
    var span = TextSpan.fromBounds(
      constructorTypeDeclAST.start(),
      constructorTypeDeclAST.end()
    );
  ...
  function createFunctionDeclaration(
    ...
    if (
      (hasModifier(funcDeclAST.modifiers, PullElementFlags.Exported) ||
        isParsingAmbientModule(funcDeclAST, context)) &&
      !containingModuleHasExportAssignment(funcDeclAST)
    ) {
      declFlags |= PullElementFlags.Exported;
    }

    if (
      hasModifier(funcDeclAST.modifiers, PullElementFlags.Ambient) ||
      isParsingAmbientModule(funcDeclAST, context) ||
      context.isDeclareFile
    ) {
      declFlags |= PullElementFlags.Ambient;
    ...
    var decl = new NormalPullDecl(
      funcDeclAST.identifier.valueText(),
      funcDeclAST.identifier.text(),
      declType,
      declFlags,
      parent,
      span
  ...
  function createAnyFunctionExpressionDeclaration(
    ...
    id: Identifier,
    context: DeclCollectionContext,
    displayName: Identifier = null
  ): void {
    var declFlags = PullElementFlags.None;

    if (
      functionExpressionDeclAST.kind() ===
    ...
    var span = TextSpan.fromBounds(
      functionExpressionDeclAST.start(),
      functionExpressionDeclAST.end()
    );
    ...
    var decl: PullDecl = new PullFunctionExpressionDecl(
      name,
      declFlags,
      parent,
      span,
    ...
    if (
      ...
      var simpleArrow = <SimpleArrowFunctionExpression>(
      ...
      var span = TextSpan.fromBounds(
        simpleArrow.identifier.start(),
        simpleArrow.identifier.end()
      );

      var decl: PullDecl = new NormalPullDecl(
        simpleArrow.identifier.valueText(),
        simpleArrow.identifier.text(),
        PullElementKind.Parameter,
        declFlags,
        parent,
        span
  ...
  function createMemberFunctionDeclaration(
    ...
    var decl = new NormalPullDecl(
      funcDecl.propertyName.valueText(),
  ...
  // index signatures
  function createIndexSignatureDeclaration(
    indexSignatureDeclAST: IndexSignature,
    context: DeclCollectionContext
  ): void {
    var declFlags = PullElementFlags.Signature;
    ...
    var span = TextSpan.fromBounds(
      indexSignatureDeclAST.start(),
      indexSignatureDeclAST.end()
    );
  ...
  // call signatures
  function createCallSignatureDeclaration(
    callSignature: CallSignature,
    context: DeclCollectionContext
  ): void {
    var isChildOfObjectType =
      callSignature.parent &&
      callSignature.parent.parent &&
  ...
  function createMethodSignatureDeclaration(
  ...
  function createConstructSignatureDeclaration(
    constructSignatureDeclAST: ConstructSignature,
    context: DeclCollectionContext
  ): void {
    var declFlags = PullElementFlags.Signature;
    ...
    var span = TextSpan.fromBounds(
      constructSignatureDeclAST.start(),
      constructSignatureDeclAST.end()
    );
  ...
  // class constructors
  function createClassConstructorDeclaration(
    constructorDeclAST: ConstructorDeclaration,
    context: DeclCollectionContext
  ): void {
    var declFlags = PullElementFlags.None;
    ...
    var span = TextSpan.fromBounds(
      constructorDeclAST.start(),
      constructorDeclAST.end()
    );
  ...
  function createGetAccessorDeclaration(
    getAccessorDeclAST: GetAccessor,
    context: DeclCollectionContext
  ): void {
    var declFlags = PullElementFlags.Public;
    ...
    var span = TextSpan.fromBounds(
      getAccessorDeclAST.start(),
      getAccessorDeclAST.end()
    );
    ...
    var decl = new NormalPullDecl(
      getAccessorDeclAST.propertyName.valueText(),
      getAccessorDeclAST.propertyName.text(),
      declType,
      declFlags,
      parent,
      span
  ...
  function createFunctionExpressionDeclaration(
  ...
  function createSetAccessorDeclaration(
    setAccessorDeclAST: SetAccessor,
    context: DeclCollectionContext
  ): void {
    var declFlags = PullElementFlags.Public;
    ...
    var span = TextSpan.fromBounds(
      setAccessorDeclAST.start(),
      setAccessorDeclAST.end()
    );
    ...
    var decl = new NormalPullDecl(
      setAccessorDeclAST.propertyName.valueText(),
      setAccessorDeclAST.propertyName.text(),
      declType,
      declFlags,
      parent,
      span
  ...
  function preCollectObjectLiteralDecls(
    ast: AST,
    context: DeclCollectionContext
  ): void {
    var span = TextSpan.fromBounds(ast.start(), ast.end());
  ...
  function preCollectSimplePropertyAssignmentDecls(
    propertyAssignment: SimplePropertyAssignment,
    context: DeclCollectionContext
  ): void {
    var assignmentText = getPropertyAssignmentNameTextFromIdentifier(
      propertyAssignment.propertyName
    );
    var span = TextSpan.fromBounds(
      propertyAssignment.start(),
      propertyAssignment.end()
    );
  ...
  function preCollectFunctionPropertyAssignmentDecls(
    propertyAssignment: FunctionPropertyAssignment,
    context: DeclCollectionContext
  ): void {
    var assignmentText = getPropertyAssignmentNameTextFromIdentifier(
      propertyAssignment.propertyName
  ...
  function preCollectDecls(ast: AST, context: DeclCollectionContext) {
    switch (ast.kind()) {
      ...
      case SyntaxKind.MemberVariableDeclaration:
        createMemberVariableDeclaration(
      ...
      case SyntaxKind.MemberFunctionDeclaration:
        createMemberFunctionDeclaration(
      ...
      case SyntaxKind.SimplePropertyAssignment:
        preCollectSimplePropertyAssignmentDecls(
          <SimplePropertyAssignment>ast,
          context
        );
        break;
      case SyntaxKind.FunctionPropertyAssignment:
        preCollectFunctionPropertyAssignmentDecls(
  ...
  function isContainer(decl: PullDecl): boolean {
    return (
      decl.kind === PullElementKind.Container ||
      decl.kind === PullElementKind.DynamicModule ||
      decl.kind === PullElementKind.Enum
    );
  }
  ...
  function postCollectDecls(ast: AST, context: DeclCollectionContext) {
    ...
    if (
      ...
      ast.kind() === SyntaxKind.StringLiteral
    ) {
      if (
    ...
    if (ast.kind() === SyntaxKind.EnumDeclaration) {
      ...
      computeEnumElementConstantValues(
        <EnumDeclaration>ast,
        currentDecl,
        context
      );
  ...
  function computeEnumElementConstantValues(
    ast: EnumDeclaration,
    enumDecl: PullDecl,
    context: DeclCollectionContext
  ): void {
    ...
    for (var i = 0, n = ast.enumElements.nonSeparatorCount(); i < n; i++) {
      ...
      var enumElementDecl = <PullEnumElementDecl>(
        ArrayUtilities.first(
          enumMemberDecls,
          (d) => context.semanticInfoChain.getASTForDecl(d) === enumElement
        )
      );
      ...
      if (enumElement.equalsValueClause === null) {
        ...
        enumElementDecl.constantValue = computeEnumElementConstantValue(
          enumElement.equalsValueClause.value,
          enumMemberDecls,
          context
        );
  ...
  function computeEnumElementConstantValue(
    expression: AST,
    enumMemberDecls: PullDecl[],
    context: DeclCollectionContext
  ): number {
    ...
    if (isIntegerLiteralAST(expression)) {
      ...
      return value && expression.kind() === SyntaxKind.NegateExpression
        ? -value
        : value;
      ...
      switch (expression.kind()) {
        case SyntaxKind.IdentifierName:
          ...
          var matchingEnumElement = <PullEnumElementDecl>(
            ArrayUtilities.firstOrDefault(
              enumMemberDecls,
              (d) => d.name === name.valueText()
            )
          );
        ...
        case SyntaxKind.LeftShiftExpression:
          ...
          var left = computeEnumElementConstantValue(
            binaryExpression.left,
            enumMemberDecls,
            context
          );
          var right = computeEnumElementConstantValue(
  ...
  export module DeclarationCreator {
    export function create(
      ...
      var declCollectionContext = new DeclCollectionContext(
        document,
        semanticInfoChain,
        compilationSettings.propagateEnumConstants()
      );
