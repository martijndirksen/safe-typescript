...
import { ISeparatedSyntaxList2, AST } from '../ast';
import { IBitMatrix, BitMatrix } from '../core/bitMatrix';
import { IBitVector, BitVector } from '../core/bitVector';
...
export class ArgumentInferenceContext {
  public inferenceCache: IBitMatrix = BitMatrix.getBitMatrix(
  ...
  constructor(resolver: PullTypeResolver, argumentsOrParameters: any) {
    ...
    if (argumentsOrParameters instanceof ISeparatedSyntaxList2) {
      //.nonSeparatorAt != undefined) {
  ...
  public alreadyRelatingTypes(
    objectType: PullTypeSymbol,
    parameterType: PullTypeSymbol
  ) {
    if (
      this.inferenceCache.valueAt(
        objectType.pullSymbolID,
        parameterType.pullSymbolID
      )
    ) {
      return true;
    } else {
      this.inferenceCache.setValueAt(
        objectType.pullSymbolID,
        parameterType.pullSymbolID,
        true
      );
  ...
  public resetRelationshipCache() {
    ...
    this.inferenceCache = BitMatrix.getBitMatrix(
      /*allowUndefinedValues:*/ false
    );
  }
  ...
  public getArgumentTypeSymbolAtIndex(
    i: number,
    ...
    if (this.fixedParameterTypes && i < this.getInferenceArgumentCount()) {
      ...
      return this.resolver.resolveAST(
        this.argumentASTs.nonSeparatorAt(i),
        true,
        context
      ).type;
    }
  ...
  public inferArgumentTypes(
    resolver: PullTypeResolver,
    context: PullTypeResolutionContext
  ): {
    results: { param: PullTypeParameterSymbol; type: PullTypeSymbol }[];
    unfit: boolean;
  } {
    ...
    for (var infoKey in this.candidateCache) {
      if (this.candidateCache.hasOwnProperty(infoKey)) {
        ...
        bestCommonType = resolver.widenType(
          resolver.findBestCommonType(
            collection,
            context,
...
export class PullContextualTypeContext {
  ...
  public recordProvisionallyTypedSymbol(symbol: PullSymbol) {
    this.provisionallyTypedSymbols[this.provisionallyTypedSymbols.length] =
      symbol;
  }
...
export class PullTypeResolutionContext {
  ...
  constructor(
    ...
    if (inTypeCheck) {
      ...
      this.typeCheckedNodes = BitVector.getBitVector(
        /*allowUndefinedValues:*/ false
      );
    }
  }
  ...
  public pushContextualType(
    type: PullTypeSymbol,
    provisional: boolean,
    substitutions: PullTypeSymbol[]
  ) {
    this.contextStack.push(
      new PullContextualTypeContext(type, provisional, substitutions)
    );
  }
  ...
  public getContextualType(): PullTypeSymbol {
    var context = !this.contextStack.length
      ? null
    ...
    if (context) {
      ...
      if (
        ...
        (<PullTypeParameterSymbol>type).getConstraint()
      ) {
  ...
  public setTypeInContext(symbol: PullSymbol, type: PullTypeSymbol) {
    ...
    if (this.contextStack.length && this.inProvisionalResolution()) {
      ...
      ].recordProvisionallyTypedSymbol(symbol);
    }
  }
  ...
  public setSymbolForAST(ast: AST, symbol: PullSymbol): void {
    this.contextStack[this.contextStack.length - 1].setSymbolForAST(
      ast,
      symbol
    );
  }
