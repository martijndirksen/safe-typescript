...
module SymbolDisplay {
   ...
   */
  export function toMinimalDisplayParts(
    symbol: ISymbol,
    location: ILocation,
  ...
  function toDisplayPartsWorker(
    ...
    if (minimal) {
      ...
      if (semanticModel == null) {
        ...
        throw Errors.argument(
          'semanticModel',
          'Semantic model must be provided in order to provide minimal type qualification.'
        );
      }
    }
    ...
    var result: Part[] = [];
    symbol.accept(
      new Visitor(format, location, semanticModel, minimal, result)
    );
    return result;
  }
  ...
  class Visitor implements ISymbolVisitor {
    ...
    constructor(
      ...
      if (isFirstSymbolVisited) {
        this.notFirstVisitor = new Visitor(
          format,
          location,
          semanticModel,
          minimal,
    ...
    private addKeyword(kind: SyntaxKind): void {
      this.builder.push(
        new Part(PartKind.Keyword, SyntaxFacts.getText(kind), null)
      );
    }

    private addPunctuation(kind: SyntaxKind): void {
      this.builder.push(
        new Part(PartKind.Punctuation, SyntaxFacts.getText(kind), null)
      );
    }
    ...
    private visitTypeParameter(symbol: ITypeParameterSymbol): void {
      this.builder.push(
        new Part(PartKind.TypeParameterName, symbol.name(), symbol)
      );
    }
    ...
    private visitEnumType(symbol: IEnumTypeSymbol): void {
      ...
      this.builder.push(
        new Part(this.getPartKind(symbol), symbol.name(), symbol)
      );
    }
    ...
    private shouldVisitModule(moduleSymbol: IModuleSymbol): boolean {
      if (
        this.format.typeQualificationStyle() !==
    ...
    private addNameAndTypeArguments(
      symbol: ISymbol,
    ...
    private visitAnonymousType(symbol: IAnonymousTypeSymbol): void {
      ...
      if (symbol.signatureCount() === 1) {
        ...
        if (
          signature.kind() === SymbolKind.ConstructSignature ||
      ...
      if (
        EnumUtilities.hasFlag(
          this.format.typeOptions(),
    ...
    private addTypeParameterConstraint(
      ...
      if (
        ...
        EnumUtilities.hasFlag(
          this.format.genericsOptions(),
          GenericsOptions.IncludeTypeConstraints
        )
      ) {
        this.addSpace();
    ...
    private nameBoundSuccessfullyToSameSymbol(
      symbol: ISymbol,
      arity: number
    ): boolean {
    ...
    private getMinimallyQualifyLookupOptions(): LookupOptions {
      var token = this.location
        .syntaxTree()
        .sourceUnit()
        .findToken(this.location.textSpan().start());
    ...
    private visitVariable(symbol: IVariableSymbol): void {
      ...
      if (
        ...
        EnumUtilities.hasFlag(
          this.format.memberOptions(),
          MemberOptions.IncludeConstantValue
        ) &&
        symbol.hasValue() &&
    ...
    private addAccessibilityIfRequired(symbol: ISymbol): void {
      ...
      if (
        EnumUtilities.hasFlag(
          this.format.memberOptions(),
          MemberOptions.IncludeAccessibility
        ) &&
        (containingType === null ||
    ...
    private addMemberModifiersIfRequired(symbol: ISymbol): void {
      ...
      if (
        EnumUtilities.hasFlag(
          this.format.memberOptions(),
          MemberOptions.IncludeModifiers
        ) &&
        (containingType == null ||
          containingType.typeKind() !== TypeKind.Interface)
      ) {
    ...
    private addEnumConstantValue(
      enumType: IObjectTypeSymbol,
      value: any
    ): void {
    ...
    private addLiteralValue(value: any): void {
      ...
      this.builder.push(
        new Part(
          typeof value === 'number'
            ? PartKind.NumericLiteral
            : PartKind.StringLiteral,
          stringified,
          null
        )
      );
