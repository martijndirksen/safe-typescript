...
module TypeScript {
  export class SyntaxTreeToAstVisitor implements ISyntaxVisitor {
    ...
    public static visit(
      ...
      var visitor = incrementalAST
        ? new SyntaxTreeToIncrementalAstVisitor(
            fileName,
            syntaxTree.lineMap(),
            compilationSettings
          )
        : new SyntaxTreeToAstVisitor(
            fileName,
            syntaxTree.lineMap(),
            compilationSettings
          );
    ...
    private setCommentsAndSpan(
      ...
      ast.setPreComments(
        this.convertTokenLeadingComments(firstToken, fullStart)
      );
      ast.setPostComments(
        this.convertNodeTrailingComments(node, lastToken, fullStart)
      );
    }
    ...
    public setSpan(
      ...
      firstToken = element.firstToken(),
      lastToken = element.lastToken()
    ): void {
      ...
      var desiredLimChar =
        fullStart + element.fullWidth() - trailingTriviaWidth;
    ...
    public visitSeparatedSyntaxList(
      ...
      for (var i = 0, n = list.childCount(); i < n; i++) {
        if (i % 2 === 0) {
          ...
          this.previousTokenTrailingComments =
            this.convertTokenTrailingComments(
              separatorToken,
              this.position +
                separatorToken.leadingTriviaWidth() +
                separatorToken.width()
            );
          this.movePast(separatorToken);
        }
      }

      var result = new ISeparatedSyntaxList2(
        this.fileName,
        array,
        list.separatorCount()
      );
      ...
      for (var i = 0, n = triviaList.count(); i < n; i++) {
        ...
        if (trivia.isComment()) {
          var hasTrailingNewLine =
            i + 1 < n && triviaList.syntaxTriviaAt(i + 1).isNewLine();
          result.push(
            this.convertComment(
              trivia,
              commentStartPosition,
    ...
    private convertTokenLeadingComments(
      token: ISyntaxToken,
      commentStartPosition: number
    ): Comment[] {
      if (token === null) {
    ...
    private convertTokenTrailingComments(
      token: ISyntaxToken,
      commentStartPosition: number
    ): Comment[] {
      if (
        token === null ||
        !token.hasTrailingComment() ||
        token.hasTrailingNewLine()
      ) {
        return null;
      }
    ...
    private convertNodeTrailingComments(
      node: SyntaxNode,
      ...
      if (
        lastToken === null ||
        !lastToken.hasTrailingComment() ||
        lastToken.hasTrailingNewLine()
      ) {
        return null;
      }
      ...
      return this.convertComments(
        ...
        nodeStart + node.fullWidth() - lastToken.trailingTriviaWidth()
      );
    }
    ...
    public visitExternalModuleReference(
      node: ExternalModuleReferenceSyntax
    ...
    public visitModuleNameModuleReference(
      node: ModuleNameModuleReferenceSyntax
    ...
    public visitClassDeclaration(
      ...
      var heritageClauses = node.heritageClauses
        ? this.visitSyntaxList(node.heritageClauses)
        : null;

      this.movePast(node.openBraceToken);
      var members = this.visitSyntaxList(node.classElements);

      var closeBraceToken = this.createTokenSpan(
        this.position,
    ...
    public visitHeritageClause(node: HeritageClauseSyntax): HeritageClause {
      ...
      var result = new HeritageClause(
        node.extendsOrImplementsKeyword.tokenKind === SyntaxKind.ExtendsKeyword
    ...
    public visitModuleDeclaration(
      ...
      var stringLiteral = node.stringLiteral
    ...
    public visitFunctionDeclaration(
      ...
      var result = new FunctionDeclaration(
        this.visitModifiers(node.modifiers),
        name,
        callSignature,
        block
      );
    ...
    public visitEnumElement(node: EnumElementSyntax): EnumElement {
      ...
      var value =
        node.equalsValueClause !== null
          ? this.visitEqualsValueClause(node.equalsValueClause)
          : null;
    ...
    public visitExportAssignment(
      node: ExportAssignmentSyntax
    ): ExportAssignment {
      var start = this.position;
    ...
    public visitVariableDeclarator(
      ...
      var init = node.equalsValueClause
        ? this.visitEqualsValueClause(node.equalsValueClause)
        : null;
    ...
    public visitEqualsValueClause(
      node: EqualsValueClauseSyntax
    ): EqualsValueClause {
      var start = this.position;
      var afterEqualsComments = this.convertTokenTrailingComments(
        node.equalsToken,
        this.position +
          node.equalsToken.leadingTriviaWidth() +
          node.equalsToken.width()
      );

      this.movePast(node.equalsToken);
      var value: AST = node.value.accept(this);
      value.setPreComments(
        this.mergeComments(afterEqualsComments, value.preComments())
      );
    ...
    public visitPrefixUnaryExpression(
    ...
    public visitArrayLiteralExpression(
      node: ArrayLiteralExpressionSyntax
    ): ArrayLiteralExpression {
      var start = this.position;
      var openStart =
        this.position + node.openBracketToken.leadingTriviaWidth();
      this.movePast(node.openBracketToken);

      var expressions = this.visitSeparatedSyntaxList(node.expressions);

      var closeStart =
        this.position + node.closeBracketToken.leadingTriviaWidth();
    ...
    public visitOmittedExpression(
      node: OmittedExpressionSyntax
    ): OmittedExpression {
      var start = this.position;
    ...
    public visitParenthesizedExpression(
      ...
      var openParenTrailingComments = this.convertTokenTrailingComments(
        openParenToken,
        start + openParenToken.leadingTriviaWidth() + openParenToken.width()
      );
    ...
    public visitSimpleArrowFunctionExpression(
      ...
      var block = node.block ? this.visitBlock(node.block) : null;
      var expression: AST = node.expression
        ? node.expression.accept(this)
        : null;

      var result = new SimpleArrowFunctionExpression(
        identifier,
        block,
        expression
      );
    ...
    public visitParenthesizedArrowFunctionExpression(
      ...
      var result = new ParenthesizedArrowFunctionExpression(
        callSignature,
        block,
        expression
      );
    ...
    public visitBlock(node: BlockSyntax): Block {
      ...
      var closeBraceLeadingComments = this.convertTokenLeadingComments(
        node.closeBraceToken,
        this.position
      );
      var closeBraceToken = this.createTokenSpan(
    ...
    public visitParameter(node: ParameterSyntax): Parameter {
      ...
      var dotDotDotToken = this.createTokenSpan(
        this.position,
        node.dotDotDotToken
      );
      ...
      var questionToken = this.createTokenSpan(
        this.position,
        node.questionToken
      );
      this.movePast(node.questionToken);
      var typeExpr = this.visitTypeAnnotation(node.typeAnnotation);
      var init: EqualsValueClause = node.equalsValueClause
        ? node.equalsValueClause.accept(this)
        : null;
    ...
    public visitPostfixUnaryExpression(
      ...
      var result = new PostfixUnaryExpression(
        node.kind() === SyntaxKind.PostIncrementExpression
    ...
    public visitElementAccessExpression(
    ...
    public visitArgumentList(node: ArgumentListSyntax): ArgumentList {
      ...
      if (
        node.args.fullWidth() === 0 &&
        node.closeParenToken.fullWidth() === 0
      ) {
        // If the argument list was empty, and closing paren is missing, set the argument ofsets to be the open paren trivia
        var openParenTokenEnd =
          start +
          node.openParenToken.leadingTriviaWidth() +
          node.openParenToken.width();
        this.setSpanExplicit(
          args,
          openParenTokenEnd,
          openParenTokenEnd + node.openParenToken.trailingTriviaWidth()
        );
      }

      var closeParenToken = this.createTokenSpan(
        this.position,
    ...
    public visitConstructSignature(
      node: ConstructSignatureSyntax
    ): ConstructSignature {
      var start = this.position;
    ...
    public visitPropertySignature(
      node: PropertySignatureSyntax
    ): PropertySignature {
      var start = this.position;
    ...
    public visitTypeParameter(node: TypeParameterSyntax): TypeParameter {
      ...
      var identifier = this.visitIdentifier(node.identifier);
      var constraint: Constraint = node.constraint
    ...
    public visitIfStatement(node: IfStatementSyntax): IfStatement {
      ...
      var elseBod: ElseClause = node.elseClause
    ...
    public visitExpressionStatement(
      ...
      var preComments = this.convertTokenLeadingComments(
        node.firstToken(),
        start
      );
      var expression = node.expression.accept(this);
      ...
      var postComments = this.convertComments(
        node.semicolonToken.trailingTrivia(),
        this.position +
          node.semicolonToken.leadingTriviaWidth() +
          node.semicolonToken.width()
      );
      this.movePast(node.semicolonToken);
    ...
    public visitIndexMemberDeclaration(
      node: IndexMemberDeclarationSyntax
    ...
    public visitMemberFunctionDeclaration(
      ...
      var result = new MemberFunctionDeclaration(
        this.visitModifiers(node.modifiers),
        name,
        callSignature,
        block
    ...
    public visitObjectCreationExpression(
    ...
    public visitCaseSwitchClause(
      node: CaseSwitchClauseSyntax
    ): CaseSwitchClause {
      var start = this.position;
    ...
    public visitDefaultSwitchClause(
      node: DefaultSwitchClauseSyntax
    ): DefaultSwitchClause {
      var start = this.position;
    ...
    public visitBreakStatement(node: BreakStatementSyntax): BreakStatement {
      ...
      var identifier: Identifier = node.identifier
    ...
    public visitForStatement(node: ForStatementSyntax): ForStatement {
      ...
      this.movePast(node.openParenToken);
      var variableDeclaration: VariableDeclaration = node.variableDeclaration
        ? node.variableDeclaration.accept(this)
        : null;
      var initializer: AST = node.initializer
    ...
    public visitObjectLiteralExpression(
      ...
      var closeStart =
        this.position + node.closeBraceToken.leadingTriviaWidth();
      this.movePast(node.closeBraceToken);
    ...
    public visitSimplePropertyAssignment(
      node: SimplePropertyAssignmentSyntax
      ...
      var postComments = this.convertNodeTrailingComments(
        node,
        node.lastToken(),
        start
      );
      ...
      var afterColonComments = this.convertTokenTrailingComments(
        node.colonToken,
        this.position +
          node.colonToken.leadingTriviaWidth() +
          node.colonToken.width()
      );

      this.movePast(node.colonToken);
      var expression: AST = node.expression.accept(this);
      expression.setPreComments(
        this.mergeComments(afterColonComments, expression.preComments())
      );
    ...
    public visitFunctionPropertyAssignment(
      node: FunctionPropertyAssignmentSyntax
    ...
    public visitFunctionExpression(
      ...
      var name =
        node.identifier === null ? null : this.visitIdentifier(node.identifier);
    ...
    public visitDeleteExpression(
      node: DeleteExpressionSyntax
    ): DeleteExpression {
      var start = this.position;
    ...
    public visitDebuggerStatement(
      node: DebuggerStatementSyntax
    ): DebuggerStatement {
      var start = this.position;
  ...
  class SyntaxTreeToIncrementalAstVisitor extends SyntaxTreeToAstVisitor {
    ...
    public visitParenthesizedArrowFunctionExpression(
      ...
      var result: ParenthesizedArrowFunctionExpression =
        this.getAndMovePastAST(node);
      if (!result) {
