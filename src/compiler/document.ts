module TypeScript {
  export class Document {
    ...
    private cacheSyntaxTreeInfo(syntaxTree: SyntaxTree): void {
      ...
      this._isExternalModule =
        this.hasImplicitImport(leadingComments) ||
        this.hasTopLevelImportOrExport(sourceUnit);
    ...
    private getAmdDependency(comment: string): string {
      var amdDependencyRegEx =
        /^\/\/\/\s*<amd-dependency\s+path=('|")(.+?)\1/gim;
      var match = amdDependencyRegEx.exec(comment);
      return match ? match[2] : null;
    }

    private hasImplicitImport(
      sourceUnitLeadingComments: ISyntaxTrivia[]
    ): boolean {
    ...
    private hasTopLevelImportOrExport(node: SourceUnitSyntax): boolean {
      ...
      for (var i = 0, n = node.moduleElements.childCount(); i < n; i++) {
        ...
        if (
          firstToken !== null &&
          firstToken.tokenKind === SyntaxKind.ExportKeyword
        ) {
          return true;
        }
        ...
        if (moduleElement.kind() === SyntaxKind.ImportDeclaration) {
          ...
          if (
            importDecl.moduleReference.kind() ===
    ...
    public update(
      ...
      if (
        ...
        Debug.shouldAssert(AssertionLevel.Normal)
      ) {
        ...
        if (Debug.shouldAssert(AssertionLevel.VeryAggressive)) {
          var oldTextPrefix = oldText.getText(
            0,
            textChangeRange.span().start()
          );
          var newTextPrefix = newText.getText(
            0,
            textChangeRange.span().start()
          );
          ...
          var newTextSuffix = newText.getText(
    ...
    public topLevelDecl(): PullDecl {
      if (this._topLevelDecl === null) {
        this._topLevelDecl = DeclarationCreator.create(
          this,
          this._semanticInfoChain,
          this._compiler.compilationSettings()
        );
