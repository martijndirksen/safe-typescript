...
module TypeScript {
  ...
  export class DeclarationEmitter {
    ...
    constructor(
      private emittingFileName: string,
      public document: Document,
      private compiler: TypeScriptCompiler,
      private emitOptions: EmitOptions,
      private semanticInfoChain: SemanticInfoChain
    ) {
      this.declFile = new TextWriter(
        emittingFileName,
        this.document.byteOrderMark !== ByteOrderMark.None,
        OutputFileType.Declaration
      );
    }
    ...
    private emitDeclarationsForAST(ast: AST) {
      switch (ast.kind()) {
        case SyntaxKind.VariableStatement:
          return this.emitDeclarationsForVariableStatement(
            <VariableStatement>ast
          );
        case SyntaxKind.PropertySignature:
          return this.emitPropertySignature(<PropertySignature>ast);
        case SyntaxKind.VariableDeclarator:
          return this.emitVariableDeclarator(
        ...
        case SyntaxKind.ConstructorDeclaration:
          return this.emitDeclarationsForConstructorDeclaration(
        ...
        case SyntaxKind.FunctionDeclaration:
          return this.emitDeclarationsForFunctionDeclaration(
            <FunctionDeclaration>ast
          );
        case SyntaxKind.MemberFunctionDeclaration:
          return this.emitMemberFunctionDeclaration(
            <MemberFunctionDeclaration>ast
          );
        case SyntaxKind.ClassDeclaration:
          return this.emitDeclarationsForClassDeclaration(
            <ClassDeclaration>ast
          );
        case SyntaxKind.InterfaceDeclaration:
          return this.emitDeclarationsForInterfaceDeclaration(
            <InterfaceDeclaration>ast
          );
        case SyntaxKind.ImportDeclaration:
          return this.emitDeclarationsForImportDeclaration(
            <ImportDeclaration>ast
          );
        case SyntaxKind.ModuleDeclaration:
          return this.emitDeclarationsForModuleDeclaration(
            <ModuleDeclaration>ast
          );
        case SyntaxKind.EnumDeclaration:
          return this.emitDeclarationsForEnumDeclaration(<EnumDeclaration>ast);
        case SyntaxKind.ExportAssignment:
          return this.emitDeclarationsForExportAssignment(
    ...
    private canEmitDeclarations(declAST: AST): boolean {
      ...
      if (
        ...
        container.kind() === SyntaxKind.SourceUnit
      ) {
    ...
    private emitDeclFlags(declarationAST: AST, typeString: string) {
      this.declFile.Write(
        this.getDeclFlagsString(
          this.semanticInfoChain.getDeclForAST(declarationAST),
          typeString
        )
      );
    }

    private emitTypeNamesMember(
      memberName: MemberName,
      emitIndent: boolean = false
    ) {
    ...
    private emitDeclarationComments(astOrSymbol: any, endLine = true) {
      ...
      var declComments: Comment[] = astOrSymbol.docComments
        ? astOrSymbol.docComments()
        : docComments(astOrSymbol);
    ...
    private emitVariableDeclarator(
      varDecl: VariableDeclarator,
      isFirstVarInList: boolean,
      isLastVarInList: boolean
    ) {
    ...
    private emitDeclarationsForMemberVariableDeclaration(
    ...
    private emitDeclarationsForVariableStatement(
      variableStatement: VariableStatement
    ) {
      this.emitDeclarationsForVariableDeclaration(
        variableStatement.declaration
      );
    }

    private emitDeclarationsForVariableDeclaration(
      ...
      for (var i = 0; i < varListCount; i++) {
        this.emitVariableDeclarator(
          <VariableDeclarator>variableDeclaration.declarators.nonSeparatorAt(i),
          i == 0,
          i == varListCount - 1
        );
      }
    ...
    private emitParameterList(
      ...
      this.emitParameters(
        isPrivate,
        Parameters.fromParameterList(parameterList)
      );
      this.declFile.Write(')');
    }
    ...
    private emitParameters(
      ...
      for (var i = 0; i < argsLen; i++) {
        this.emitArgDecl(
          parameterList.astAt(i),
          parameterList.identifierAt(i),
          parameterList.isOptionalAt(i),
          isPrivate
        );
        if (i < argsLen - 1) {
          this.declFile.Write(', ');
      ...
      if (hasLastParameterRestParameter) {
        ...
        this.emitArgDecl(
          parameterList.astAt(index),
          parameterList.identifierAt(index),
          parameterList.isOptionalAt(index),
          isPrivate
        );
      }
    }
    ...
    private emitMemberFunctionDeclaration(funcDecl: MemberFunctionDeclaration) {
      ...
      if (funcDecl.block) {
        ...
        hasModifier(funcDecl.modifiers, PullElementFlags.Private) &&
        this.isOverloadedCallSignature(funcDecl)
      ) {
        ...
        var firstSignature = callSignatures[0].isDefinition()
          ? callSignatures[1]
          : callSignatures[0];
        var firstSignatureDecl = firstSignature.getDeclarations()[0];
        var firstFuncDecl =
          this.semanticInfoChain.getASTForDecl(firstSignatureDecl);
    ...
    private emitDeclarationsForFunctionDeclaration(
      ...
      if (
        id !== '' ||
        !funcDecl.identifier ||
        funcDecl.identifier.text().length > 0
      ) {
        this.declFile.Write(id);
    ...
    private emitBaseList(
      bases: ISeparatedSyntaxList2,
      useExtendsList: boolean
    ) {
      if (bases && bases.nonSeparatorCount() > 0) {
        ...
        for (var i = 0; i < basesLen; i++) {
          ...
          var baseType = <PullTypeSymbol>(
            this.semanticInfoChain.getSymbolForAST(base)
          );
    ...
    private emitDeclarationsForGetAccessor(funcDecl: GetAccessor): void {
      this.emitMemberAccessorDeclaration(
        funcDecl,
        funcDecl.modifiers,
    ...
    private emitMemberAccessorDeclaration(
      funcDecl: AST,
    ...
    private emitClassMembersFromConstructorDefinition(
      funcDecl: ConstructorDeclaration
    ) {
      if (funcDecl.parameterList) {
        ...
        for (var i = 0; i < argsLen; i++) {
          var parameter = <Parameter>(
            funcDecl.parameterList.parameters.nonSeparatorAt(i)
          );
    ...
    private emitHeritageClause(clause: HeritageClause) {
      this.emitBaseList(
        clause.typeNames,
        clause.kind() === SyntaxKind.ExtendsHeritageClause
      );
    }
    ...
    private getEnclosingContainer(ast: AST): AST {
      ...
      while (ast) {
        if (
          ast.kind() === SyntaxKind.ClassDeclaration ||
    ...
    private emitTypeParameters(
      ...
      funcSignature?: PullSignatureSymbol
    ) {
      ...
      for (var i = 0; i < typars.length; i++) {
        ...
        var memberName = typars[i].getScopedNameEx(
          containerSymbol,
    ...
    private emitDeclarationsForInterfaceDeclaration(
      ...
      var interfacePullDecl =
        this.semanticInfoChain.getDeclForAST(interfaceDecl);
    ...
    private emitDeclarationsForImportDeclaration(
      ...
      var isExportedImportDecl = hasModifier(
        importDeclAST.modifiers,
        PullElementFlags.Exported
      );

      if (
        isExportedImportDecl ||
        importSymbol.typeUsedExternally() ||
        PullContainerSymbol.usedAsSymbol(
          importSymbol.getContainer(),
          importSymbol
        )
      ) {
        ...
        if (
          ...
          this.declFile.WriteLine(
            this.getFullName(
              (<ModuleNameModuleReference>importDeclAST.moduleReference)
                .moduleName
            ) + ';'
          );
        }
    ...
    public getFullName(name: AST): string {
      if (name.kind() === SyntaxKind.IdentifierName) {
        ...
        return (
          this.getFullName(dotExpr.left) + '.' + this.getFullName(dotExpr.right)
        );
      }
    }

    private emitDeclarationsForEnumDeclaration(
      moduleDecl: EnumDeclaration
    ): void {
      ...
      for (var j = 0; j < membersLen; j++) {
        ...
        var enumElementDecl = <PullEnumElementDecl>(
          this.semanticInfoChain.getDeclForAST(enumElement)
        );
    ...
    private emitDeclarationsForModuleDeclaration(
      ...
      var name: AST =
        ...
        ArrayUtilities.first(getModuleNames(moduleDecl.name));
    ...
    private resolveScriptReference(document: Document, reference: string) {
      ...
      var documentDir = convertToDirectoryPath(
        switchToForwardSlashes(getRootFilePath(document.fileName))
      );
      var resolvedReferencePath = this.emitOptions.resolvePath(
        documentDir + reference
      );
    ...
    private emitReferencePaths(sourceUnit: SourceUnit) {
      ...
      if (this.document.emitToOwnOutputFile()) {
        ...
        for (var j = 0; j < scriptReferences.length; j++) {
          var currentReference = this.resolveScriptReference(
            this.document,
            scriptReferences[j]
          );
          ...
          if (
            document &&
            (document.emitToOwnOutputFile() ||
              document.isDeclareFile() ||
              !addedGlobalDocument)
          ) {
      ...
      // Emit the references
      var emittingFilePath = documents.length
        ? getRootFilePath(this.emittingFileName)
        : null;
      for (var i = 0; i < documents.length; i++) {
        ...
        if (document.isDeclareFile()) {
          ...
          declFileName = this.compiler.mapOutputFileName(
            document,
            this.emitOptions,
            (fileName: string, flag: boolean) =>
              TypeScriptCompiler.mapToDTSFileName(fileName, flag)
          );
        }

        // Get the relative path
        declFileName = getRelativePathToFixedPath(
          emittingFilePath,
          declFileName,
          false
        );
        this.declFile.WriteLine(
          '/// <reference path="' + declFileName + '" />'
        );
      }
