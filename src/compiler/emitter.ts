...
export class Emitter {
  ...
  public emitVariableDeclarator(varDecl: VariableDeclarator) {
    ...
    if (
      ...
      var parentDecl = pullDecl && pullDecl.getParentDecl();
      var parentIsModule =
        parentDecl && parentDecl.flags & PullElementFlags.SomeInitializedModule;
  ...
  public emitScriptElements(sourceUnit: SourceUnit) {
    if (
      ...
      if (this.emitOptions.compilationSettings().serviceMode()) {
        ...
        this.writeToOutput(
          '//SOUND TYPE CHECKER FAILED ON ' + scriptName + '; NO CODE TO EMIT\n'
        );
      }
      return;
    }
    ...
    if (isExternalModule) {
      ...
      if (isNonElidedExternalModule) {
        ...
        if (
          ...
          exportAssignmentValueSymbol &&
          !(
            exportAssignmentValueSymbol.kind & PullElementKind.SomeTypeReference
          )
        ) {
          this.emitIndent();
          this.writeToOutput(
  ...
  public emitClass(classDecl: ClassDeclaration) {
    ...
    if (
      (temp === EmitContainer.Module || temp === EmitContainer.DynamicModule) &&
  ...
  private requiresExtendsBlock(moduleElements: ISyntaxList2): boolean {
    for (var i = 0, n = moduleElements.childCount(); i < n; i++) {
      ...
      if (moduleElement.kind() === SyntaxKind.ModuleDeclaration) {
        ...
        if (
          !hasModifier(classDeclaration.modifiers, PullElementFlags.Ambient) &&
  ...
  public emitParenthesizedExpression(
    ...
    if (
      parenthesizedExpression.expression.kind() === SyntaxKind.CastExpression &&
  ...
  private isNotAmbientOrHasInitializer(
    ...
    return (
      ...
      this.firstVariableDeclarator(variableStatement).equalsValueClause !== null
    );
  }
