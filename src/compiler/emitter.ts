...
module TypeScript {
  ...
  export class EmitOptions {
    ...
    public sharedOutputFile() {
      return this._sharedOutputFile;
    }
    public sourceRootDirectory() {
      return this._sourceRootDirectory;
    }
    public sourceMapRootDirectory() {
    ...
    constructor(
      ...
      if (
        settings.moduleGenTarget() === ModuleGenTarget.Unspecified &&
        compiler._isDynamicModuleCompilation()
      ) {
        ...
        if (settings.mapRoot()) {
          if (settings.sourceRoot()) {
            this._diagnostic = new Diagnostic(
              ...
              0,
              0,
              DiagnosticCode.Options_mapRoot_and_sourceRoot_cannot_be_specified_without_specifying_sourcemap_option,
              null
            );
            return;
          } else {
            this._diagnostic = new Diagnostic(
              ...
              0,
              0,
              DiagnosticCode.Option_mapRoot_cannot_be_specified_without_specifying_sourcemap_option,
              null
            );
            return;
          }
          ...
          this._diagnostic = new Diagnostic(
            ...
            0,
            0,
            DiagnosticCode.Option_sourceRoot_cannot_be_specified_without_specifying_sourcemap_option,
            null
          );
          return;
        }
      ...
      this._sourceMapRootDirectory = convertToDirectoryPath(
        switchToForwardSlashes(settings.mapRoot())
      );
      this._sourceRootDirectory = convertToDirectoryPath(
        switchToForwardSlashes(settings.sourceRoot())
      );
      ...
      if (
        ...
        if (settings.outFileOption()) {
          this._sharedOutputFile = switchToForwardSlashes(
            resolvePath(settings.outFileOption())
          );
        }

        if (settings.outDirOption()) {
          this._outputDirectory = convertToDirectoryPath(
            switchToForwardSlashes(resolvePath(settings.outDirOption()))
          );
        }
    ...
    private determineCommonDirectoryPath(compiler: TypeScriptCompiler): void {
      ...
      for (var i = 0, len = fileNames.length; i < len; i++) {
        ...
        if (!document.isDeclareFile()) {
          ...
          if (commonComponentsLength === -1) {
            ...
            for (
              var j = 0;
              j < commonComponentsLength && j < fileComponents.length;
              j++
            ) {
              if (commonComponents[j] !== fileComponents[j]) {
                ...
                if (j === 0) {
                  if (
                    this._outputDirectory || // there is --outDir specified
                    ...
                    this._diagnostic = new Diagnostic(
                      ...
                      0,
                      0,
                      DiagnosticCode.Cannot_find_the_common_subdirectory_path_for_the_input_files,
                      null
                    );
                    return;
                  }
            ...
            if (
              !updatedPath &&
              fileComponents.length < commonComponentsLength
            ) {
      ...
      this._commonDirectoryPath =
        commonComponents.slice(0, commonComponentsLength).join('/') + '/';
    }
  }
  ...
  export function lastParameterIsRest(parameterList: ParameterList): boolean {
    ...
    return (
      parameters.nonSeparatorCount() > 0 &&
      (<Parameter>parameters.nonSeparatorAt(parameters.nonSeparatorCount() - 1))
        .dotDotDotToken !== null
    );
  }
  ...
  export class Emitter {
    ...
    private getEnclosingDecl() {
      ...
      var enclosingDecl =
        declStackLen > 0 ? this.declStack[declStackLen - 1] : null;
    ...
    public shouldEmitImportDeclaration(importDeclAST: ImportDeclaration) {
      return importDeclarationIsElided(
        importDeclAST,
        this.semanticInfoChain,
        this.emitOptions.compilationSettings()
      );
    }
    ...
    public emitImportDeclaration(importDeclAST: ImportDeclaration) {
      ...
      var isAmdCodeGen =
        this.emitOptions.compilationSettings().moduleGenTarget() ==
        ModuleGenTarget.Asynchronous;
      ...
      var associatedParentSymbol = parentSymbol
        ? parentSymbol.getAssociatedContainerType()
        : null;
      var associatedParentSymbolKind = associatedParentSymbol
        ? associatedParentSymbol.kind
        : PullElementKind.None;
      ...
      if (
        ...
        if (
          importSymbol.getExportAssignedTypeSymbol() ||
          ...
          if (
            valueSymbol &&
            (valueSymbol.kind == PullElementKind.Method ||
              valueSymbol.kind == PullElementKind.Function)
          ) {
            needsPropertyAssignment = true;
          } else {
      ...
      if (isAmdCodeGen && isExternalModuleReference) {
        ...
        if (isExternalModuleReference) {
          this.writeToOutput(
            'require(' +
              (<ExternalModuleReference>aliasAST).stringLiteral.text() +
              ')'
          );
        } else {
          this.emitJavascript(
            (<ModuleNameModuleReference>aliasAST).moduleName,
            false
          );
        }
    ...
    public emitComments(
      ast: AST,
      pre: boolean,
      onlyPinnedOrTripleSlashComments: boolean = false
    ) {
      ...
      if (ast && ast.kind() !== SyntaxKind.Block) {
        if (
          ast.parent.kind() === SyntaxKind.SimpleArrowFunctionExpression ||
          ast.parent.kind() === SyntaxKind.ParenthesizedArrowFunctionExpression
        ) {
          return;
        }
      }
      ...
      if (pre) {
        ...
        if (onlyPinnedOrTripleSlashComments) {
          preComments = ArrayUtilities.where(preComments, (c) =>
            this.isPinnedOrTripleSlash(c)
          );
        }
    ...
    public emitObjectLiteralExpression(objectLiteral: ObjectLiteralExpression) {
      ...
      this.emitCommaSeparatedList(
        objectLiteral,
        objectLiteral.propertyAssignments,
        /*buffer:*/ ' ',
        /*preserveNewLines:*/ true,
        /*forceNewLines*/ objectLiteral.newLineMode
      );
      this.writeToOutput('}');
    ...
    public emitArrayLiteralExpression(arrayLiteral: ArrayLiteralExpression) {
      ...
      this.emitCommaSeparatedList(
        ...
        /*buffer:*/ '',
        /*preserveNewLines:*/ true,
        arrayLiteral.newLineMode
      );
      this.writeToOutput(']');
      this.recordSourceMappingEnd(arrayLiteral);
    }

    public emitObjectCreationExpression(
      ...
      if (objectCreationExpression.argumentList) {
        ...
        this.emitCommaSeparatedList(
          ...
          objectCreationExpression.argumentList.args,
          /*buffer:*/ '',
    ...
    public getConstantDecl(
      dotExpr: MemberAccessExpression
    ): PullEnumElementDecl {
    ...
    public emitInvocationExpression(callNode: InvocationExpression) {
      ...
      if (
        target.kind() === SyntaxKind.MemberAccessExpression &&
        (<MemberAccessExpression>target).expression.kind() ===
        ...
        if (
          callNode.expression.kind() === SyntaxKind.SuperKeyword &&
      ...
      this.writeToOutputWithSourceMapRecord(
        ')',
        callNode.argumentList.closeParenToken
      );
    ...
    private emitParameterList(list: ParameterList): void {
      ...
      this.emitCommentsArray(
        list.openParenTrailingComments,
        /*trailing:*/ true
      );
    ...
    private emitFunctionBodyStatements(
      name: string,
      funcDecl: AST,
      parameterList: IParameters,
      block: Block,
      bodyExpression: AST
    ): void {
      this.writeLineToOutput(' {');
      ...
      if (block) {
        ...
        this.emitCommentsArray(
          block.closeBraceLeadingComments,
          /*trailing:*/ false
        );
      } else {
    ...
    private getImportDecls(fileName: string): PullDecl[] {
      var topLevelDecl = this.semanticInfoChain.topLevelDecl(
      ...
      for (var i = 0, n = queue.length; i < n; i++) {
        ...
        if (decl.kind & PullElementKind.TypeAlias) {
          var importStatementAST = <ImportDeclaration>(
            this.semanticInfoChain.getASTForDecl(decl)
          );
          if (
            ...
            if (
              typeSymbol &&
              typeSymbol !== this.semanticInfoChain.anyTypeSymbol &&
              !typeSymbol.isError()
            ) {
              result.push(decl);
            }
    ...
    public getModuleImportAndDependencyList(sourceUnit: SourceUnit) {
      ...
      if (importDecls.length) {
        for (var i = 0; i < importDecls.length; i++) {
          ...
          var importStatementSymbol = <PullTypeAliasSymbol>(
            importStatementDecl.getSymbol()
          );
          var importStatementAST = <ImportDeclaration>(
            this.semanticInfoChain.getASTForDecl(importStatementDecl)
          );
          ...
          if (importStatementSymbol.isUsedAsValue()) {
            ...
            dependencyList += (<ExternalModuleReference>(
    ...
    public shouldCaptureThis(ast: AST) {
      if (ast.kind() === SyntaxKind.SourceUnit) {
        var scriptDecl = this.semanticInfoChain.topLevelDecl(
          this.document.fileName
        );
    ...
    public emitEnum(moduleDecl: EnumDeclaration) {
      ...
      if (temp === EmitContainer.Prog && isExported) {
        ...
        var dotMod =
          svModuleName !== ''
            ? (parentIsDynamic ? 'exports' : svModuleName) + '.'
            : svModuleName;
        this.writeToOutput('}');
        this.recordSourceMappingNameEnd();
        this.writeToOutput(
      ...
      if (temp !== EmitContainer.Prog && isExported) {
        ...
        if (parentIsDynamic) {
          ...
          this.emitIndent();
          this.writeToOutput(
            'var ' + this.moduleName + ' = exports.' + this.moduleName + ';'
          );
        } else {
          this.writeLineToOutput('');
          this.emitIndent();
    ...
    private getModuleDeclToVerifyChildNameCollision(
      moduleDecl: PullDecl,
      changeNameIfAnyDeclarationInContext: boolean
    ) {
    ...
    private getModuleName(
      ...
      moduleDecl = this.getModuleDeclToVerifyChildNameCollision(
        moduleDecl,
    ...
    public emitSingleModuleDeclaration(
      moduleDecl: ModuleDeclaration,
      moduleName: IASTToken
    ) {
      ...
      if (moduleName === moduleDecl.stringLiteral) {
        ...
        if (isLastName) {
          ...
          this.emitIndent();
          this.emitSingleModuleDeclaration(
            moduleDecl,
            moduleNames[nameIndex + 1]
          );
          this.writeLineToOutput('');
        }
      }
    ...
    public emitEnumElement(varDecl: EnumElement): void {
      ...
      var pullDecl = <PullEnumElementDecl>(
        this.semanticInfoChain.getDeclForAST(varDecl)
      );
    ...
    public emitSimpleArrowFunctionExpression(
      arrowFunction: SimpleArrowFunctionExpression
    ): void {
      this.emitAnyArrowFunctionExpression(
        arrowFunction,
        null /*arrowFunction.getNameText()*/,
        Parameters.fromIdentifier(arrowFunction.identifier),
        arrowFunction.block,
        arrowFunction.expression
      );
    }

    public emitParenthesizedArrowFunctionExpression(
    ...
    private emitAnyArrowFunctionExpression(
      arrowFunction: AST,
      funcName: string,
    ...
    public emitConstructor(funcDecl: ConstructorDeclaration) {
      ...
      this.emitCommentsArray(
        funcDecl.block.closeBraceLeadingComments,
        /*trailing:*/ false
      );
      ...
      this.writeToOutputWithSourceMapRecord(
        '}',
        funcDecl.block.closeBraceToken
      );
    ...
    public emitGetAccessor(accessor: GetAccessor): void {
      ...
      this.emitFunctionBodyStatements(
        null,
        accessor,
        Parameters.fromParameterList(accessor.parameterList),
        accessor.block,
        /*bodyExpression:*/ null
      );
    ...
    public emitFunctionExpression(funcDecl: FunctionExpression): void {
      ...
      var parameters = Parameters.fromParameterList(
    ...
    public emitFunction(funcDecl: FunctionDeclaration) {
      ...
      if (funcDecl.block) {
        ...
        if (
          (this.emitState.container === EmitContainer.Module ||
            this.emitState.container === EmitContainer.DynamicModule) &&
          pullFunctionDecl &&
          hasFlag(pullFunctionDecl.flags, PullElementFlags.Exported)
        ) {
          this.writeLineToOutput('');
          this.emitIndent();
          var modName =
    ...
    public emitVariableDeclaration(declaration: VariableDeclaration) {
      ...
      var isAmbientWithoutInit =
        pullVarDecl &&
        hasFlag(pullVarDecl.flags, PullElementFlags.Ambient) &&
        varDecl.equalsValueClause === null;
      if (!isAmbientWithoutInit) {
        ...
        for (
          var i = 0, n = declaration.declarators.nonSeparatorCount();
          i < n;
          i++
        ) {
    ...
    private emitMemberVariableDeclaration(varDecl: MemberVariableDeclaration) {
      Debug.assert(
        !hasModifier(varDecl.modifiers, PullElementFlags.Static) &&
      ...
      var quotedOrNumber =
        isQuoted(varDeclName) ||
        varDecl.variableDeclarator.propertyName.kind() !==
      ...
      this.writeToOutputWithSourceMapRecord(
        varDecl.variableDeclarator.propertyName.text(),
    ...
    public emitVariableDeclarator(varDecl: VariableDeclarator) {
      ...
      if (
        pullDecl &&
        (pullDecl.flags & PullElementFlags.Ambient) === PullElementFlags.Ambient
      ) {
        ...
        var parentIsModule =
          parentDecl &&
          parentDecl.flags & PullElementFlags.SomeInitializedModule;
    ...
    private symbolIsUsedInItsEnclosingContainer(
      symbol: PullSymbol,
      dynamic = false
    ) {
        ...
        if (enclosingDecl) {
          ...
          if (parentDecl) {
            ...
            while (symbolDeclarationEnclosingContainer) {
              if (
                symbolDeclarationEnclosingContainer.kind ===
                (dynamic
              ...
              symbolDeclarationEnclosingContainer =
                symbolDeclarationEnclosingContainer.getParentDecl();
            }
            ...
            if (symbolDeclarationEnclosingContainer && enclosingContainer) {
              var same =
                symbolDeclarationEnclosingContainer === enclosingContainer;

              // initialized module object variables are bound to their parent's decls
              if (
                !same &&
                symbol.anyDeclHasFlag(PullElementFlags.InitializedModule)
              ) {
                same =
                  symbolDeclarationEnclosingContainer ===
                  enclosingContainer.getParentDecl();
              }
    ...
    private getPotentialDeclPathInfoForEmit(pullSymbol: PullSymbol) {
      ...
      var symbolContainerDeclPath = parentDecl
        ? parentDecl.getParentPath()
        : <PullDecl[]>[];
      ...
      return {
        potentialPath: symbolContainerDeclPath,
        startingIndex: startingIndex,
      };
    }

    // Emit the dotted names using the decl path
    private emitDottedNameFromDeclPath(
      declPath: PullDecl[],
      ...
      for (var i = startingIndex; i <= lastIndex; i++) {
        if (
          declPath[i].kind === PullElementKind.DynamicModule ||
          ...
          this.writeToOutput(
            this.getModuleName(
              declPath[i],
              /* changeNameIfAnyDeclarationInContext */ true
            ) + '.'
          );
        }
      }
    ...
    private emitSymbolContainerNameInEnclosingContext(pullSymbol: PullSymbol) {
      ...
      this.emitDottedNameFromDeclPath(
        potentialDeclPath,
        startingIndex,
        potentialDeclPath.length - 1
      );
    }
    ...
    public emitName(name: Identifier, addThis: boolean) {
      ...
      if (name.text().length > 0) {
        ...
        var isLocalAlias =
          pullSymbolAlias &&
          pullSymbolAlias.getDeclarations()[0].getParentDecl() ==
        ...
        if (
          ...
          if (pullSymbolContainer) {
            ...
            if (pullSymbolContainerKind === PullElementKind.Class) {
              ...
              PullHelpers.symbolIsModule(pullSymbolContainer) ||
              pullSymbolContainerKind === PullElementKind.Enum ||
              pullSymbolContainer.anyDeclHasFlag(
                PullElementFlags.InitializedModule | PullElementFlags.Enum
              )
            ) {
              // If property or, say, a constructor being invoked locally within the module of its definition
              if (
                pullSymbolKind === PullElementKind.Property ||
                pullSymbolKind === PullElementKind.EnumMember
              ) {
                this.emitSymbolContainerNameInEnclosingContext(pullSymbol);
              } else if (
                pullSymbol.anyDeclHasFlag(PullElementFlags.Exported) &&
                pullSymbolKind === PullElementKind.Variable &&
                !pullSymbol.anyDeclHasFlag(
                  PullElementFlags.InitializedModule | PullElementFlags.Enum
                )
              ) {
                this.emitSymbolContainerNameInEnclosingContext(pullSymbol);
              ...
              pullSymbolContainerKind === PullElementKind.DynamicModule ||
              pullSymbolContainer.anyDeclHasFlag(
      ...
      if (this.sourceMapper) {
        ...
        if (name) {
          if (this.sourceMapper.currentNameIndex.length > 0) {
            var parentNameIndex =
              this.sourceMapper.currentNameIndex[
    ...
    public recordSourceMappingStart(ast: IASTSpan) {
      if (this.sourceMapper && isValidAstNode(ast)) {
        ...
        var siblings =
          this.sourceMapper.currentMappings[
    ...
    private emitParameterPropertyAndMemberVariableAssignments(): void {
      ...
      if (constructorDecl && constructorDecl.parameterList) {
        for (
          var i = 0,
            n = constructorDecl.parameterList.parameters.nonSeparatorCount();
          i < n;
          i++
        ) {
          var parameter = <Parameter>(
            constructorDecl.parameterList.parameters.nonSeparatorAt(i)
          );
          var parameterDecl = this.semanticInfoChain.getDeclForAST(parameter);
          if (
            hasFlag(parameterDecl.flags, PullElementFlags.PropertyParameter)
          ) {
            this.emitIndent();
            this.recordSourceMappingStart(parameter);
            this.writeToOutputWithSourceMapRecord(
              'this.' + parameter.identifier.text(),
      ...
      for (
        var i = 0, n = this.thisClassNode.classElements.childCount();
        i < n;
        i++
      ) {
        if (
          this.thisClassNode.classElements.childAt(i).kind() ===
          SyntaxKind.MemberVariableDeclaration
        ) {
          var varDecl = <MemberVariableDeclaration>(
            this.thisClassNode.classElements.childAt(i)
          );
          if (
            !hasModifier(varDecl.modifiers, PullElementFlags.Static) &&
            varDecl.variableDeclarator.equalsValueClause
    ...
    private isOnSameLine(pos1: number, pos2: number): boolean {
      var lineMap = this.document.lineMap();
      return (
        lineMap.getLineNumberFromPosition(pos1) ===
        lineMap.getLineNumberFromPosition(pos2)
      );
    }

    private emitCommaSeparatedList(
      parent: AST,
      ...
      preserveNewLines =
        preserveNewLines && newLineMode !== NewLineMode.Suppress;
      ...
      var startLine =
        forceNewLines ||
        (preserveNewLines &&
          !this.isOnSameLine(parent.end(), list.nonSeparatorAt(0).end()));
      ...
      if (
        ...
        !this.isOnSameLine(
          parent.end(),
          list.nonSeparatorAt(list.nonSeparatorCount() - 1).end()
        )
      ) {
        this.writeLineToOutput('');
        this.emitIndent();
    ...
    public emitList(
      list: ISyntaxList2,
      useNewLineSeparator = true,
      startInclusive = 0,
      endExclusive = list.childCount()
    ) {
      if (list === null) {
        return;
      }
    ...
    public emitSeparatedList(
      list: ISeparatedSyntaxList2,
      useNewLineSeparator = true,
      startInclusive = 0,
      endExclusive = list.nonSeparatorCount()
    ) {
      if (list === null) {
        return;
      }
    ...
    private getCopyrightComments(): Comment[] {
      ...
      if (preComments) {
        ...
        for (var i = 0, n = preComments.length; i < n; i++) {
          ...
          if (lastComment) {
            var lastCommentLine = lineMap.getLineNumberFromPosition(
              lastComment.end()
            );
            var commentLine = lineMap.getLineNumberFromPosition(
              comment.start()
            );
        ...
        // a copyright header.
        var lastCommentLine = lineMap.getLineNumberFromPosition(
          ArrayUtilities.last(copyrightComments).end()
        );
        var astLine = lineMap.getLineNumberFromPosition(
          this.copyrightElement.start()
        );
    ...
    public emitScriptElements(sourceUnit: SourceUnit) {
      if (
        this.emitOptions
          .compilationSettings()
          .shouldInstrument(sourceUnit.fileName()) &&
        (this.emitOptions.compilationSettings().safe() ||
          this.emitOptions.compilationSettings().secure()) &&
        !sourceUnit.rewrittenModuleElements
      ) {
        if (this.emitOptions.compilationSettings().serviceMode()) {
          this.writeToOutput('//SOMETHING WENT WRONG; TRY REFRESHING THE PAGE');
        } else {
          var scriptName = sourceUnit
            .fileName()
            .substring(sourceUnit.fileName().lastIndexOf('/') + 1);
          this.writeToOutput(
            '//SOUND TYPE CHECKER FAILED ON ' +
              scriptName +
              '; NO CODE TO EMIT\n'
          );
        }
      ...
      var list = sourceUnit.rewrittenModuleElements
        ? sourceUnit.rewrittenModuleElements
      ...
      var isNonElidedExternalModule =
        isExternalModule && !scriptIsElided(sourceUnit);
      if (isNonElidedExternalModule) {
        ...
        if (
          ...
          var importAndDependencyList =
            this.getModuleImportAndDependencyList(sourceUnit);
          importList += importAndDependencyList.importList;
          dependencyList += importAndDependencyList.dependencyList + ']';

          this.writeLineToOutput(
            'define(' + dependencyList + ',' + ' function(' + importList + ') {'
          );
        }
      }
      ...
      if (isExternalModule) {
        ...
        externalModule = this.semanticInfoChain.getDeclForAST(
      ...
      if (isExternalModule) {
        ...
        if (isNonElidedExternalModule) {
          ...
          var exportAssignmentValueSymbol = (<PullContainerSymbol>(
            externalModule.getSymbol()
          ...
          if (
            ...
            // AMD
            if (
              exportAssignmentIdentifier &&
              exportAssignmentValueSymbol &&
              !(
                exportAssignmentValueSymbol.kind &
                PullElementKind.SomeTypeReference
              )
            ) {
              ...
              this.emitIndent();
              this.writeLineToOutput(
                'return ' + exportAssignmentIdentifier + ';'
              );
              this.indenter.decreaseIndent();
            }
            this.writeToOutput('});');
            ...
            this.emitIndent();
            this.writeToOutput(
              'module.exports = ' + exportAssignmentIdentifier + ';'
            );
          }
    ...
    public emitConstructorStatements(funcDecl: ConstructorDeclaration) {
      ...
      var emitPropertyAssignmentsAfterSuperCall =
        getExtendsHeritageClause(this.thisClassNode.heritageClauses) !== null;
      var propertyAssignmentIndex = emitPropertyAssignmentsAfterSuperCall
        ? 1
        : 0;
      var lastEmittedNode: AST = null;
    ...
    public emitAccessorMemberDeclaration(
      funcDecl: AST,
      name: IASTToken,
      className: string,
      isProto: boolean
      ...
      var accessors = PullHelpers.getGetterAndSetterFunction(
        funcDecl,
        this.semanticInfoChain
      );
      if (accessors.getter) {
        ...
        this.emitAccessorBody(
          accessors.getter,
          accessors.getter.parameterList,
          accessors.getter.block
        );
        this.writeLineToOutput(',');
      }
      ...
      if (accessors.setter) {
        ...
        this.emitAccessorBody(
          accessors.setter,
          accessors.setter.parameterList,
          accessors.setter.block
        );
        this.writeLineToOutput(',');
      }
    ...
    public emitClass(classDecl: ClassDeclaration) {
      ...
      var hasBaseClass =
        getExtendsHeritageClause(classDecl.heritageClauses) !== null;
      ...
      if (hasBaseClass) {
        baseTypeReference = getExtendsHeritageClause(
      ...
      if (constrDecl) {
        ...
        this.indenter.increaseIndent();
        this.writeLineToOutput(
          'function ' + classDecl.identifier.text() + '() {'
        );
        this.recordSourceMappingNameStart('constructor');
        if (hasBaseClass) {
          this.emitIndent();
      ...
      if (
        (temp === EmitContainer.Module ||
          temp === EmitContainer.DynamicModule) &&
        hasFlag(pullDecl.flags, PullElementFlags.Exported)
      ) {
        this.writeLineToOutput('');
        this.emitIndent();
        var modName =
          temp === EmitContainer.Module ? this.moduleName : 'exports';
        this.writeToOutputWithSourceMapRecord(
        ...
        if (memberDecl.kind() === SyntaxKind.MemberVariableDeclaration) {
          ...
          if (
            ...
            if (
              ...
              this.writeToOutput(
                classDecl.identifier.text() + '[' + varDeclName + ']'
              );
            } else {
              this.writeToOutput(
                classDecl.identifier.text() + '.' + varDeclName
    ...
    private emitClassMemberFunctionDeclaration(
      ...
      if (
        isQuoted(functionName) ||
        funcDecl.propertyName.kind() !== SyntaxKind.IdentifierName
      ) {
        this.writeToOutput('[' + functionName + '] = ');
      } else {
        this.writeToOutput('.' + functionName + ' = ');
    ...
    private requiresExtendsBlock(moduleElements: ISyntaxList2): boolean {
      for (var i = 0, n = moduleElements.childCount(); i < n; i++) {
        ...
        if (moduleElement.kind() === SyntaxKind.ModuleDeclaration) {
          ...
          if (
            !hasModifier(moduleAST.modifiers, PullElementFlags.Ambient) &&
            this.requiresExtendsBlock(moduleAST.moduleElements)
          ) {
            return true;
          }
          ...
          if (
            ...
            getExtendsHeritageClause(classDeclaration.heritageClauses) !== null
          ) {
            return true;
          }
        }
    ...
    public emitParenthesizedExpression(
      parenthesizedExpression: ParenthesizedExpression
    ): void {
      if (
        parenthesizedExpression.expression.kind() ===
          SyntaxKind.CastExpression &&
        parenthesizedExpression.openParenTrailingComments === null
      ) {
        ...
        this.writeToOutput('(');
        this.emitCommentsArray(
          parenthesizedExpression.openParenTrailingComments,
          /*trailing:*/ false
        );
        this.emit(parenthesizedExpression.expression);
        this.writeToOutput(')');
    ...
    public emitPrefixUnaryExpression(expression: PrefixUnaryExpression): void {
      ...
      switch (nodeType) {
        ...
        case SyntaxKind.NegateExpression:
          this.writeToOutput('-');
          if (
            expression.operand.kind() === SyntaxKind.NegateExpression ||
            expression.operand.kind() === SyntaxKind.PreDecrementExpression
          ) {
            this.writeToOutput(' ');
          }
          this.emit(expression.operand);
        ...
        case SyntaxKind.PlusExpression:
          this.writeToOutput('+');
          if (
            expression.operand.kind() === SyntaxKind.PlusExpression ||
            expression.operand.kind() === SyntaxKind.PreIncrementExpression
          ) {
            this.writeToOutput(' ');
          }
          this.emit(expression.operand);
    ...
    public emitPostfixUnaryExpression(
    ...
    private canEmitDottedNameMemberAccessExpression(
      ...
      if (
        memberExpressionNodeType == SyntaxKind.IdentifierName ||
        memberExpressionNodeType == SyntaxKind.MemberAccessExpression
      ) {
        var memberAccessSymbol = this.getSymbolForEmit(expression).symbol;
        var memberAccessExpressionSymbol = this.getSymbolForEmit(
          expression.expression
        ).symbol;
        if (
          memberAccessSymbol &&
          memberAccessExpressionSymbol && // We have symbols resolved for this expression and access
          !this.semanticInfoChain.getAliasSymbolForAST(expression.expression) && // The access is not off alias
          (PullHelpers.symbolIsModule(memberAccessExpressionSymbol) ||
            memberAccessExpressionSymbol.kind === PullElementKind.Enum ||
            memberAccessExpressionSymbol.anyDeclHasFlag(
              PullElementFlags.InitializedModule | PullElementFlags.Enum
            ))
        ) {
          // container is module
          ...
          if (
            memberAccessSymbolKind === PullElementKind.Property ||
            memberAccessSymbolKind === PullElementKind.EnumMember ||
            (memberAccessSymbol.anyDeclHasFlag(PullElementFlags.Exported) &&
              memberAccessSymbolKind === PullElementKind.Variable &&
              !memberAccessSymbol.anyDeclHasFlag(
                PullElementFlags.InitializedModule | PullElementFlags.Enum
              )) ||
            (memberAccessSymbol.anyDeclHasFlag(PullElementFlags.Exported) &&
    ...
    private emitDottedNameMemberAccessExpressionWorker(
      expression: MemberAccessExpression,
      potentialPath: PullDecl[],
      startingIndex: number,
      lastIndex: number
    ) {
      this.recordSourceMappingStart(expression);
      if (expression.expression.kind() == SyntaxKind.MemberAccessExpression) {
        ...
        this.emitDottedNameFromDeclPath(
          ...
          startingIndex,
          lastIndex - 2
        ); // We would be emitting two identifiers as part of member access
    ...
    private emitDottedNameMemberAccessExpressionRecurse(
      expression: MemberAccessExpression,
      potentialPath: PullDecl[],
      startingIndex: number,
      lastIndex: number
      ...
      if (lastIndex - startingIndex < 1) {
        // Member expression emits alteast two identifiers
    ...
    private emitDottedNameMemberAccessExpression(
      expression: MemberAccessExpression
    ) {
      var memberAccessSymbol = this.getSymbolForEmit(expression).symbol;
      // Get the decl path info to emit this expression using declPath
      var potentialDeclInfo =
        this.getPotentialDeclPathInfoForEmit(memberAccessSymbol);
      this.emitDottedNameMemberAccessExpressionWorker(
        expression,
        potentialDeclInfo.potentialPath,
        potentialDeclInfo.startingIndex,
        potentialDeclInfo.potentialPath.length
      );
    }

    public emitMemberAccessExpression(
      expression: MemberAccessExpression
    ): void {
    ...
    public emitBinaryExpression(expression: BinaryExpression): void {
      ...
      switch (expression.kind()) {
        ...
        default: {
          this.emit(expression.left);
          var binOp = SyntaxFacts.getText(
            SyntaxFacts.getOperatorTokenFromBinaryExpression(expression.kind())
          );
          if (binOp === 'instanceof') {
    ...
    public emitSimplePropertyAssignment(
      property: SimplePropertyAssignment
    ): void {
    ...
    public emitFunctionPropertyAssignment(
      ...
      var parameters = Parameters.fromParameterList(
        funcProp.callSignature.parameterList
      );
      this.emitFunctionParameters(parameters);
      this.writeToOutput(')');

      this.emitFunctionBodyStatements(
        funcProp.propertyName.text(),
        funcProp,
        parameters,
        funcProp.block,
        /*bodyExpression:*/ null
    ...
    public emitExpressionStatement(statement: ExpressionStatement): void {
      var isArrowExpression =
        statement.expression.kind() ===
          SyntaxKind.SimpleArrowFunctionExpression ||
        statement.expression.kind() ===
    ...
    private emitSwitchClauseBody(body: ISyntaxList2): void {
      if (
        body.childCount() === 1 &&
        body.childAt(0).kind() === SyntaxKind.Block
      ) {
    ...
    public emitRegularExpressionLiteral(
    ...
    public emitConstructorDeclaration(
      ...
      if (declaration.block) {
        ...
        this.emitComments(
          declaration,
          /*pre:*/ true,
          /*onlyPinnedOrTripleSlashComments:*/ true
        );
      }
    }

    public shouldEmitFunctionDeclaration(
      declaration: FunctionDeclaration
    ): boolean {
      return (
        declaration.preComments() !== null ||
        (!hasModifier(declaration.modifiers, PullElementFlags.Ambient) &&
          declaration.block !== null)
      );
    }
    ...
    public shouldEmitModuleDeclaration(
    ...
    public shouldEmitInterfaceDeclaration(
    ...
    private firstVariableDeclarator(
      ...
      return <VariableDeclarator>(
        statement.declaration.declarators.nonSeparatorAt(0)
      );
    }
    ...
    private isNotAmbientOrHasInitializer(
      ...
      return (
        !hasModifier(variableStatement.modifiers, PullElementFlags.Ambient) ||
        this.firstVariableDeclarator(variableStatement).equalsValueClause !==
          null
      );
    }
    ...
    public shouldEmitVariableStatement(statement: VariableStatement): boolean {
      return (
        statement.preComments() !== null ||
        this.isNotAmbientOrHasInitializer(statement)
      );
    }
    ...
    private emit(ast: AST): void {
      ...
      switch (ast.kind()) {
        ...
        case SyntaxKind.ExportAssignment:
          return this.setExportAssignmentIdentifier(
            (<ExportAssignment>ast).identifier.text()
          );
        case SyntaxKind.ClassDeclaration:
        ...
        case SyntaxKind.SimpleArrowFunctionExpression:
          return this.emitSimpleArrowFunctionExpression(
            <SimpleArrowFunctionExpression>ast
          );
        case SyntaxKind.ParenthesizedArrowFunctionExpression:
          return this.emitParenthesizedArrowFunctionExpression(
    ...
    private emitWorker(ast: AST): void {
      ...
      switch (ast.kind()) {
        ...
        case SyntaxKind.RegularExpressionLiteral:
          return this.emitRegularExpressionLiteral(
            <RegularExpressionLiteral>ast
          );
        ...
        case SyntaxKind.ObjectCreationExpression:
          return this.emitObjectCreationExpression(
        ...
        case SyntaxKind.SimplePropertyAssignment:
          return this.emitSimplePropertyAssignment(
            <SimplePropertyAssignment>ast
          );
        case SyntaxKind.FunctionPropertyAssignment:
          return this.emitFunctionPropertyAssignment(
  ...
  export function getLastConstructor(
    classDecl: ClassDeclaration
  ): ConstructorDeclaration {
    return <ConstructorDeclaration>(
      classDecl.classElements.lastOrDefault(
        (e) => e.kind() === SyntaxKind.ConstructorDeclaration
      )
    );
  }
