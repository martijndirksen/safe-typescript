...
var definitions: ITypeDefinition[] = [
  ...
  <any>{
    ...
    baseType: 'SyntaxNode',
    interfaces: ['IModuleReferenceSyntax'],
    children: [<any>{ name: 'moduleName', type: 'INameSyntax' }],
    isTypeScriptSpecific: true,
  },
  <any>{
  ...
  <any>{
    ...
    children: [
      ...
      <any>{
        ...
        isToken: true,
        tokenKinds: ['ExtendsKeyword', 'ImplementsKeyword'],
      },
      <any>{
        name: 'typeNames',
  ...
  <any>{
    ...
    children: [
      <any>{
        name: 'propertyName',
        isToken: true,
        tokenKinds: ['IdentifierName', 'StringLiteral', 'NumericLiteral'],
      },
      <any>{
        name: 'typeAnnotation',
  ...
  <any>{
    name: 'TupleTypeLeftSpreadSyntax',
    baseType: 'SyntaxNode',
    interfaces: ['ITypeSyntax'],
    children: [
      <any>{ name: 'openBracketToken', isToken: true },
      <any>{ name: 'spreadLeft', type: 'SpreadTypeSyntax' },
      <any>{ name: 'commaToken', isToken: true },
      <any>{
        name: 'types',
        isSeparatedList: true,
        requiresAtLeastOneItem: true,
  ...
  <any>{
    ...
    children: [
      ...
      <any>{
        name: 'operatorToken',
        isToken: true,
        tokenKinds: ['PlusPlusToken', 'MinusMinusToken'],
      },
    ],
  },
  <any>{
  ...
  <any>{
    ...
    children: [
      ...
      <any>{
        name: 'switchClauses',
        isList: true,
        elementType: 'ISwitchClauseSyntax',
      },
      <any>{ name: 'closeBraceToken', isToken: true },
    ],
  },
  ...
  <any>{
    ...
    children: [
      ...
      <any>{
        ...
        isSeparatedList: true,
        elementType: 'IPropertyAssignmentSyntax',
      },
      <any>{ name: 'closeBraceToken', isToken: true },
    ],
  },
  ...
  <any>{
    ...
    baseType: 'SyntaxNode',
    interfaces: ['IStatementSyntax'],
    children: [<any>{ name: 'semicolonToken', isToken: true }],
  },
  <any>{
  ...
  for (var i = 0; i < definition.children.length; i++) {
    ...
    if (!child.isOptional && !child.isToken) {
      result +=
        '        if (' +
        child.name +
        " === null) { throw Errors.argumentNull('" +
        child.name +
        "'); }\r\n";
    }
  }
...
function generateIfKindCheck(
  child: IMemberDefinition,
  tokenKinds: string[],
  indent: string
): string {
  var result = '';
...
function generateDefaultCase(child: IMemberDefinition, indent: string): string {
  ...
  result +=
    indent + "                throw Errors.argument('" + child.name + "');\r\n";

  return result;
}

function generateSwitchKindCheck(
  child: IMemberDefinition,
  tokenKinds: string[],
  indent: string
): string {
  ...
  if (identifierName.length > 0) {
    result +=
      indent +
      '        if (!SyntaxFacts.isIdentifierName(' +
      child.name +
      '.tokenKind)) {\r\n';
...
function generateFactoryMethod(definition: ITypeDefinition): string {
  return (
    generateFactory1Method(definition) + generateFactory2Method(definition)
  );
}
...
function generateAcceptMethods(definition: ITypeDefinition): string {
  ...
  result +=
    ...
    getNameWithoutSuffix(definition) +
    '(this);\r\n';
  result += '    }\r\n';
...
function generateKindMethod(definition: ITypeDefinition): string {
  ...
  if (!hasKind) {
    ...
    result +=
      '        return SyntaxKind.' + getNameWithoutSuffix(definition) + ';\r\n';
    result += '    }\r\n';
  }
...
function generateFirstTokenMethod(definition: ITypeDefinition): string {
  ...
  for (var i = 0; i < definition.children.length; i++) {
    ...
    if (child.isToken) {
      ...
      result +=
        '(token = ' + getPropertyAccess(child) + '.firstToken()) !== null';
      result += ') { return token; }\r\n';
    }
  }
  ...
  if (definition.name === 'SourceUnitSyntax') {
    ...
    for (var i = definition.children.length - 1; i >= 0; i--) {
      ...
      if (child.isToken) {
        ...
        result +=
          '(token = ' + getPropertyAccess(child) + '.lastToken()) !== null';
        result += ') { return token; }\r\n';
      }
    }
  ...
  for (var i = definition.children.length - 1; i >= 0; i--) {
    ...
    if (child.isList || child.isSeparatedList) {
      result +=
        '        ' +
        getPropertyAccess(child) +
        '.insertChildrenInto(array, index);\r\n';
    } else if (child.isOptional) {
      result +=
        '        if (' +
        getPropertyAccess(child) +
        ' !== null) { array.splice(index, 0, ' +
        getPropertyAccess(child) +
        '); }\r\n';
    } else {
      result +=
        '        array.splice(index, 0, ' + getPropertyAccess(child) + ');\r\n';
    }
  }
...
function generateTriviaMethods(definition: ITypeDefinition): string {
  var result = '\r\n';
  result +=
    '    public withLeadingTrivia(trivia: ISyntaxTriviaList): ' +
    definition.name +
    ' {\r\n';
  result +=
    '        return <' +
  ...
  result += '    }\r\n\r\n';
  result +=
    '    public withTrailingTrivia(trivia: ISyntaxTriviaList): ' +
    definition.name +
    ' {\r\n';
  result +=
    '        return <' +
...
function generateIsTypeScriptSpecificMethod(
  definition: ITypeDefinition
): string {
    ...
    for (var i = 0; i < definition.children.length; i++) {
      ...
      if (child.isOptional) {
        result +=
          ...
          ' !== null && ' +
          getPropertyAccess(child) +
          '.isTypeScriptSpecific()) { return true; }\r\n';
      } else {
        result +=
          '        if (' +
          getPropertyAccess(child) +
...
function generateNode(definition: ITypeDefinition): string {
  var result =
    '    export class ' + definition.name + ' extends ' + definition.baseType;
...
function generateRewriter(): string {
  ...
  for (const definition of definitions) {
    lines.push(
      '',
      `        public visit${getNameWithoutSuffix(definition)}(node: ${
        definition.name
      }): any {`
    );
    ...
    if (definition.children.length === 0) {
      ...
      lines.push('            return node.update(');

      for (const child of definition.children) {
        const optCheck = child.isOptional
          ? `node.${child.name} === null ? null :`
          : '';

        if (child.isToken) {
          lines.push(
            `                ${optCheck} this.visitToken(node.${child.name}),`
          );
        } else if (child.isList) {
          lines.push(
            `                ${optCheck} this.visitList(node.${child.name}),`
          );
        } else if (child.isSeparatedList) {
          lines.push(
            `                ${optCheck} this.visitSeparatedList(node.${child.name}),`
          );
        } else if (child.type === 'SyntaxKind') {
          lines.push(`                ${optCheck} node.kind(),`);
        } else if (isNodeOrToken(child)) {
          lines.push(
            `                ${optCheck} <${child.type}>this.visitNodeOrToken(node.${child.name}),`
          );
        } else {
          lines.push(
            `                ${optCheck} <${child.type}>this.visitNode(node.${child.name}),`
          );
        }
      }
    }
...
function generateToken(
  ...
  var leadingTriviaWidth = leading
    ? 'getTriviaWidth(this._leadingTriviaInfo)'
    : '0';
  var trailingTriviaWidth = trailing
    ? 'getTriviaWidth(this._trailingTriviaInfo)'
    : '0';

  if (leading && trailing) {
    result +=
      '        public fullWidth(): number { return ' +
      leadingTriviaWidth +
      ' + this.width() + ' +
      trailingTriviaWidth +
      '; }\r\n';
  ...
  result +=
    '        public hasLeadingTrivia(): boolean { return ' +
    (leading ? 'true' : 'false') +
    '; }\r\n';
  result +=
    '        public hasLeadingComment(): boolean { return ' +
    (leading ? 'hasTriviaComment(this._leadingTriviaInfo)' : 'false') +
    '; }\r\n';
  result +=
    '        public hasLeadingNewLine(): boolean { return ' +
    (leading ? 'hasTriviaNewLine(this._leadingTriviaInfo)' : 'false') +
    '; }\r\n';
  result +=
    '        public hasLeadingSkippedText(): boolean { return false; }\r\n';
  result +=
    '        public leadingTriviaWidth(): number { return ' +
    (leading ? 'getTriviaWidth(this._leadingTriviaInfo)' : '0') +
    '; }\r\n';
  result +=
  ...
  result +=
    '        public hasTrailingTrivia(): boolean { return ' +
    (trailing ? 'true' : 'false') +
    '; }\r\n';
  result +=
    '        public hasTrailingComment(): boolean { return ' +
    (trailing ? 'hasTriviaComment(this._trailingTriviaInfo)' : 'false') +
    '; }\r\n';
  result +=
    '        public hasTrailingNewLine(): boolean { return ' +
    (trailing ? 'hasTriviaNewLine(this._trailingTriviaInfo)' : 'false') +
    '; }\r\n';
  result +=
    '        public hasTrailingSkippedText(): boolean { return false; }\r\n';
  result +=
    '        public trailingTriviaWidth(): number { return ' +
    (trailing ? 'getTriviaWidth(this._trailingTriviaInfo)' : '0') +
    '; }\r\n';
  result +=
  ...
  for (var i = 0; i < definitions.length; i++) {
    ...
    for (var j = 0; j < definition.children.length; j++) {
      ...
      if (child.isToken) {
        if (child.isOptional) {
          result +=
            '            this.visitOptionalToken(node.' + child.name + ');\r\n';
        } else {
        ...
        result +=
          '            this.visitSeparatedList(node.' + child.name + ');\r\n';
      } else if (isNodeOrToken(child)) {
        if (child.isOptional) {
          result +=
            '            this.visitOptionalNodeOrToken(node.' +
            child.name +
            ');\r\n';
        } else {
          result +=
            '            this.visitNodeOrToken(node.' + child.name + ');\r\n';
        }
      } else if (child.type !== 'SyntaxKind') {
        if (child.isOptional) {
          result +=
            '            this.visitOptionalNode(node.' + child.name + ');\r\n';
        } else {
...
function generateKeywordCondition(
  ...
  if (keywords.length === 1) {
    ...
    for (var i = currentCharacter; i < length; i++) {
      ...
      result +=
        'array[' + index + '] === CharacterCodes.' + keywordText.substr(i, 1);
    }
