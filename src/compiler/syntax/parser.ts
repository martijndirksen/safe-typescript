  ...
  class NormalParserSource implements IParserSource, ISlidingWindowSource {
    ...
    constructor(
      ...
      this.slidingWindow = new SlidingWindow(
        this,
        ArrayUtilities.createArray<ISyntaxToken>(
  ...
  class IncrementalParserSource implements IParserSource {
    ...
    private tryPeekTokenFromOldSourceUnit(n: number): ISyntaxToken {
      ...
      for (var i = 0; i < n; i++) {
        ...
        if (
          !this.canReuseTokenFromOldSourceUnit(currentPosition, interimToken)
        ) {
          return null;
        }
    ...
    public moveToNextToken(): void {
      ...
      if (this._oldSourceUnitCursor.currentToken() === currentToken) {
        ...
        var absolutePosition =
          this.absolutePosition() + currentToken.fullWidth();
        var previousToken = currentToken;
        this._normalParserSource.resetToPosition(
          absolutePosition,
          previousToken
        );
  ...
  class ParserImpl {
    ...
    private addSkippedTokensBeforeNode(
      ...
      if (skippedTokens.length > 0) {
        ...
        var newToken = this.addSkippedTokensBeforeToken(
          oldToken,
          skippedTokens
        );
    ...
    private setStrictMode(isInStrictMode: boolean) {
      ...
      this.factory = isInStrictMode
        ? Syntax.strictModeFactory
        : Syntax.normalModeFactory;
    }
    ...
    private isIndexSignature(tokenIndex: number): boolean {
      return (
        this.peekToken(tokenIndex).tokenKind === SyntaxKind.OpenBracketToken
      );
    }
    ...
    private parseUnaryExpressionOrLower(): IUnaryExpressionSyntax {
      ...
      if (SyntaxFacts.isPrefixUnaryExpressionOperatorToken(currentTokenKind)) {
        var operatorKind =
          SyntaxFacts.getPrefixUnaryExpressionFromOperatorToken(
            currentTokenKind
          );
      ...
      if (typeArgumentList === null || !isOpenParenOrDot) {
        ...
        if (isDot) {
          ...
          return this.factory.argumentList(
            typeArgumentList,
            Syntax.emptyToken(SyntaxKind.OpenParenToken),
            Syntax.emptySeparatedList,
            Syntax.emptyToken(SyntaxKind.CloseParenToken)
          );
        } else {
    ...
    private isExpectedListTerminator(
      currentListType: ListParsingState
    ): boolean {
      switch (currentListType) {
        case ListParsingState.SourceUnit_ModuleElements:
