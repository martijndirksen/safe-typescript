...
class NormalParserSource implements IParserSource, ISlidingWindowSource {
  ...
  constructor(
    ...
    this.slidingWindow = new SlidingWindow(
      this,
      ArrayUtilities.createArray<ISyntaxToken>(/*defaultWindowSize:*/ 32, null),
      null,
      copy,
      grow
    );
  ...
  public releaseRewindPoint(rewindPoint: IParserRewindPoint): void {
    ...
    this.slidingWindow.releaseAndUnpinAbsoluteIndex(
      (<any>rewindPoint).absoluteIndex
    );
  ...
  public fetchMoreItems(
    ...
    sourceIndex: number,
    _window: CheckedArray<any>,
    destinationIndex: number,
    spaceAvailable: number
  ): number {
    // Assert disabled because it is actually expensive enugh to affect perf.
    // Debug.assert(spaceAvailable > 0);
    var window: ISyntaxToken[] = RT.forceCheckedArray<any, ISyntaxToken>(
      _window
    );
  ...
  public currentTokenAllowingRegularExpression(): ISyntaxToken {
    ...
    var token = this.slidingWindow.currentItem(
...
class IncrementalParserSource implements IParserSource {
  ...
  constructor(
    ...
    this._normalParserSource = new NormalParserSource(
      oldSyntaxTree.fileName(),
      newText,
      oldSyntaxTree.parseOptions().languageVersion()
    );
  }

  private static extendToAffectedRange(
    changeRange: TextChangeRange,
    ...
    //   However, if this turns out to be wrong, we may have to increase how much
    // futher we look back.
    //
  ...
  public getRewindPoint(): IParserRewindPoint {
    ...
    var oldSourceUnitCursorIndex =
      this._oldSourceUnitCursor.getAndPinCursorIndex();
  ...
  public rewind(rewindPoint: IParserRewindPoint): void {
    ...
    this._oldSourceUnitCursor.rewindToPinnedCursorIndex(
      rewindPoint.oldSourceUnitCursorIndex
    );
  ...
  private syncCursorToNewTextIfBehind() {
    while (true) {
      ...
      if (
        ...
        currentElement.fullWidth() > Math.abs(this._changeDelta)
      ) {
  ...
  private intersectsWithChangeRangeSpanInOriginalText(
    start: number,
    length: number
  ) {
    return (
      this._changeRange !== null &&
      this._changeRange.span().intersectsWith(start, length)
    );
  }
  ...
  private canReuseTokenFromOldSourceUnit(
    position: number,
    token: ISyntaxToken
  ): boolean {
    ...
    if (this._oldSourceUnitCursor.currentToken() === currentToken) {
      ...
      if (this._changeRange !== null) {
        ...
        if (this.absolutePosition() >= changeRangeSpanInNewText.end()) {
          this._changeDelta +=
            this._changeRange.newLength() - this._changeRange.span().length();
          this._changeRange = null;
        }
      }
    }
...
class ParserImpl {
  ...
  constructor(
    fileName: string,
    lineMap: LineMap,
    source: IParserSource,
    parseOptions: ParseOptions,
    private newText_forDebuggingPurposesOnly: ISimpleText
  ) {
  ...
  public currentTokenStart(): number {
    return (
      this.source.absolutePosition() + this.currentToken().leadingTriviaWidth()
    );
  }
  ...
  private eatIdentifierNameToken(): ISyntaxToken {
    ...
    if (SyntaxFacts.isAnyKeyword(token.tokenKind)) {
      this.moveToNextToken();
      return convertToIdentifierName(token);
    }
  ...
  private canEatExplicitOrAutomaticSemicolon(
    allowWithoutNewline: boolean
  ): boolean {
    var token = this.currentToken();
  ...
  private eatExplicitOrAutomaticSemicolon(
    allowWithoutNewline: boolean
  ): ISyntaxToken {
    var token = this.currentToken();
    ...
    if (this.canEatAutomaticSemicolon(allowWithoutNewline)) {
      ...
      var semicolonToken = emptyToken(SyntaxKind.SemicolonToken);
      ...
      if (!this.parseOptions.allowAutomaticSemicolonInsertion()) {
        ...
        this.addDiagnostic(
          new Diagnostic(
            ...
            0,
            DiagnosticCode.Automatic_semicolon_insertion_not_allowed,
            null
          )
        );
      }
  ...
  private createMissingToken(
    ...
    return emptyToken(expectedKind);
  }

  private getExpectedTokenDiagnostic(
    expectedKind: SyntaxKind,
    actual: ISyntaxToken
  ): Diagnostic {
    var token = this.currentToken();
    ...
    if (
      SyntaxFacts.isAnyKeyword(expectedKind) ||
      SyntaxFacts.isAnyPunctuation(expectedKind)
    ) {
      ...
      if (actual !== null && SyntaxFacts.isAnyKeyword(actual.tokenKind)) {
        return new Diagnostic(
          ...
          this.currentTokenStart(),
          token.width(),
          DiagnosticCode.Identifier_expected_0_is_a_keyword,
  ...
  private addSkippedTokenAfterNodeOrToken(
    nodeOrToken: ISyntaxNodeOrToken,
    skippedToken: ISyntaxToken
  ): ISyntaxNodeOrToken {
    if (nodeOrToken.isToken()) {
      return this.addSkippedTokenAfterToken(
        <ISyntaxToken>nodeOrToken,
        skippedToken
      );
    } else if (nodeOrToken.isNode()) {
      return this.addSkippedTokenAfterNode(
        <SyntaxNode>nodeOrToken,
        skippedToken
      );
    } else {
      throw Errors.invalidOperation();
  ...
  private addSkippedTokenAfterNode(
    node: SyntaxNode,
    skippedToken: ISyntaxToken
  ): SyntaxNode {
  ...
  private addSkippedTokensBeforeNode(
    node: SyntaxNode,
  ...
  private addSkippedTokensBeforeToken(
    token: ISyntaxToken,
    skippedTokens: ISyntaxToken[]
  ): ISyntaxToken {
  ...
  public parseSyntaxTree(isDeclaration: boolean): SyntaxTree {
    ...
    var allDiagnostics = this.source
      .tokenDiagnostics()
      .concat(this.diagnostics);
    allDiagnostics.sort(
      (a: Diagnostic, b: Diagnostic) => a.start() - b.start()
    );
  ...
  private setStrictMode(isInStrictMode: boolean) {
    ...
    this.factory = isInStrictMode ? strictModeFactory : normalModeFactory;
  }
  ...
  private parseSourceUnit(): SourceUnitSyntax {
    ...
    var result = this.parseSyntaxList(
      ListParsingState.SourceUnit_ModuleElements,
      (parser, items) => ParserImpl.updateStrictModeState(parser, items)
    );
    var moduleElements = result.list;

    this.setStrictMode(savedIsInStrictMode);

    var sourceUnit = this.factory.sourceUnit(
      moduleElements,
      this.currentToken()
    );
    sourceUnit = <SourceUnitSyntax>(
      this.addSkippedTokensBeforeNode(sourceUnit, result.skippedTokens)
    );

    if (Debug.shouldAssert(AssertionLevel.Aggressive)) {
      Debug.assert(
        sourceUnit.fullWidth() ===
  ...
  private static updateStrictModeState(
    parser: ParserImpl,
    items: CheckedArray<any>
  ): void {
    if (!parser.isInStrictMode) {
      ...
      parser.setStrictMode(
        SyntaxFacts.isUseStrictDirective(items[items.length - 1])
      );
    }
  }
  ...
  private isModuleElement(inErrorRecovery: boolean): boolean {
    ...
    return (
      this.isImportDeclaration(modifierCount) ||
  ...
  private isImportDeclaration(modifierCount: number): boolean {
    ...
    return (
      this.currentToken().tokenKind === SyntaxKind.ImportKeyword &&
  ...
  private isExportAssignment(): boolean {
    return (
      this.currentToken().tokenKind === SyntaxKind.ExportKeyword &&
  ...
  private tryParseTypeArgumentList(
    ...
    if (!inExpression) {
      ...
      result = this.parseSeparatedSyntaxList(
        ListParsingState.TypeArgumentList_Types
      );
      typeArguments = result.list;
      lessThanToken = this.addSkippedTokensAfterToken(
        lessThanToken,
      ...
      return this.factory.typeArgumentList(
        lessThanToken,
        typeArguments,
        greaterThanToken
      );
    ...
    if (
      greaterThanToken.fullWidth() === 0 ||
      !this.canFollowTypeArgumentListInExpression(this.currentToken().kind())
    ) {
      ...
      this.releaseRewindPoint(rewindPoint);
      var typeArgumentList = this.factory.typeArgumentList(
        lessThanToken,
        typeArguments,
        greaterThanToken
      );
  ...
  private parseName(): INameSyntax {
    ...
    while (
      shouldContinue &&
      this.currentToken().tokenKind === SyntaxKind.DotToken
    ) {
  ...
  private isEnumDeclaration(modifierCount: number): boolean {
    ...
    return (
      this.currentToken().tokenKind === SyntaxKind.EnumKeyword &&
      this.isIdentifier(this.peekToken(1))
    );
  }
  ...
  private parseEnumDeclaration(): EnumDeclarationSyntax {
    ...
    var enumElements: ISeparatedSyntaxList = emptySeparatedList;

    if (openBraceToken.width() > 0) {
      var result = this.parseSeparatedSyntaxList(
        ListParsingState.EnumDeclaration_EnumElements
      );
      enumElements = result.list;
      openBraceToken = this.addSkippedTokensAfterToken(
        openBraceToken,
        result.skippedTokens
      );
    }
  ...
  private isEnumElement(inErrorRecovery: boolean): boolean {
    if (
      ...
      this.currentNode().kind() === SyntaxKind.EnumElement
    ) {
      return true;
    }
  ...
  private isClassDeclaration(modifierCount: number): boolean {
    ...
    return (
      this.currentToken().tokenKind === SyntaxKind.ClassKeyword &&
      this.isIdentifier(this.peekToken(1))
    );
  }

  private parseHeritageClauses(): ISyntaxList {
    var heritageClauses: ISyntaxList = emptyList;

    if (this.isHeritageClause()) {
      var result = this.parseSyntaxList(
        ListParsingState.ClassOrInterfaceDeclaration_HeritageClauses
      );
  ...
  private parseClassDeclaration(): ClassDeclarationSyntax {
    ...
    var typeParameterList = this.parseOptionalTypeParameterList(
      /*requireCompleteTypeParameterList:*/ false
    );
    var heritageClauses = this.parseHeritageClauses();
    var openBraceToken = this.eatToken(SyntaxKind.OpenBraceToken);
    var classElements: ISyntaxList = emptyList;
  ...
  private static isPublicOrPrivateKeyword(token: ISyntaxToken): boolean {
    return (
      token.tokenKind === SyntaxKind.PublicKeyword ||
  ...
  private isAccessor(inErrorRecovery: boolean): boolean {
    ...
    if (
      this.peekToken(index).tokenKind !== SyntaxKind.GetKeyword &&
  ...
  private parseAccessor(checkForStrictMode: boolean): SyntaxNode {
    ...
    if (this.currentToken().tokenKind === SyntaxKind.GetKeyword) {
      return this.parseGetMemberAccessorDeclaration(
        modifiers,
        checkForStrictMode
      );
    } else if (this.currentToken().tokenKind === SyntaxKind.SetKeyword) {
      return this.parseSetMemberAccessorDeclaration(
        modifiers,
        checkForStrictMode
      );
    } else {
  ...
  private parseGetMemberAccessorDeclaration(
    modifiers: ISyntaxList,
    checkForStrictMode: boolean
  ): GetAccessorSyntax {
    ...
    var block = this.parseBlock(
      /*parseStatementsEvenWithNoOpenBrace:*/ false,
      checkForStrictMode
    );
  ...
  private parseSetMemberAccessorDeclaration(
    modifiers: ISyntaxList,
    checkForStrictMode: boolean
  ): SetAccessorSyntax {
  ...
  private isConstructorDeclaration(): boolean {
    ...
    var index = this.modifierCount();
    return (
      this.peekToken(index).tokenKind === SyntaxKind.ConstructorKeyword &&
  ...
  private isMemberFunctionDeclaration(inErrorRecovery: boolean): boolean {
    ...
    while (true) {
      ...
      if (
        this.isPropertyName(token, inErrorRecovery) &&
  ...
  private parseMemberFunctionDeclaration(): MemberFunctionDeclarationSyntax {
    ...
    while (true) {
      var currentToken = this.currentToken();
      if (
        this.isPropertyName(currentToken, /*inErrorRecovery:*/ false) &&
    ...
    var newCallSignature =
      this.tryAddUnexpectedEqualsGreaterThanToken(callSignature);
    ...
    return this.factory.memberFunctionDeclaration(
      modifiers,
      propertyName,
      callSignature,
      block,
  ...
  private parseMemberVariableDeclaration(): MemberVariableDeclarationSyntax {
    ...
    return this.factory.memberVariableDeclaration(
      modifiers,
      variableDeclarator,
      semicolon
    );
  ...
  private parseIndexMemberDeclaration(): IndexMemberDeclarationSyntax {
    ...
    return this.factory.indexMemberDeclaration(
      modifiers,
      indexSignature,
      semicolonToken
    );
  ...
  private tryAddUnexpectedEqualsGreaterThanToken(
    callSignature: CallSignatureSyntax
  ): CallSignatureSyntax {
    var token0 = this.currentToken();

    var hasEqualsGreaterThanToken =
      token0.tokenKind === SyntaxKind.EqualsGreaterThanToken;
    if (hasEqualsGreaterThanToken) {
      ...
      if (callSignature.lastToken()) {
        ...
        var diagnostic = new Diagnostic(
          ...
          this.currentTokenStart(),
          token0.width(),
          DiagnosticCode.Unexpected_token_0_expected,
  ...
  private isModuleDeclaration(modifierCount: number): boolean {
    ...
    if (this.currentToken().tokenKind === SyntaxKind.ModuleKeyword) {
      ...
      return (
        ...
        token1.tokenKind === SyntaxKind.StringLiteral
      );
    }
  ...
  private parseModuleDeclaration(): ModuleDeclarationSyntax {
    ...
    var moduleElements: ISyntaxList = emptyList;
    if (openBraceToken.width() > 0) {
      var result = this.parseSyntaxList(
        ListParsingState.ModuleDeclaration_ModuleElements
      );
  ...
  private isInterfaceDeclaration(modifierCount: number): boolean {
    ...
    return (
      this.currentToken().tokenKind === SyntaxKind.InterfaceKeyword &&
      this.isIdentifier(this.peekToken(1))
    );
  }
  ...
  private parseObjectType(): ObjectTypeSyntax {
    ...
    var typeMembers: ISeparatedSyntaxList = emptySeparatedList;
    if (openBraceToken.width() > 0) {
      var result = this.parseSeparatedSyntaxList(
        ListParsingState.ObjectType_TypeMembers
      );
  ...
  private isTypeMember(inErrorRecovery: boolean): boolean {
    ...
    return (
      this.isCallSignature(/*tokenIndex:*/ 0) ||
  ...
  private parseIndexSignature(): IndexSignatureSyntax {
    ...
    return this.factory.indexSignature(
      openBracketToken,
      parameter,
      closeBracketToken,
      typeAnnotation
  ...
  private isConstructSignature(): boolean {
    ...
    var token1 = this.peekToken(1);
    return (
      token1.tokenKind === SyntaxKind.LessThanToken ||
      token1.tokenKind === SyntaxKind.OpenParenToken
    );
  }
  ...
  private isMethodSignature(inErrorRecovery: boolean): boolean {
    if (this.isPropertyName(this.currentToken(), inErrorRecovery)) {
      ...
      // id?(
      if (
        this.peekToken(1).tokenKind === SyntaxKind.QuestionToken &&
  ...
  private isPropertySignature(inErrorRecovery: boolean): boolean {
    ...
    if (
      ParserImpl.isModifier(currentToken) &&
      !currentToken.hasTrailingNewLine() &&
  ...
  private isHeritageClause(): boolean {
    var token0 = this.currentToken();
    return (
      token0.tokenKind === SyntaxKind.ExtendsKeyword ||
  ...
  private parseHeritageClause(): HeritageClauseSyntax {
    ...
    Debug.assert(
      extendsOrImplementsKeyword.tokenKind === SyntaxKind.ExtendsKeyword ||
        extendsOrImplementsKeyword.tokenKind === SyntaxKind.ImplementsKeyword
    );

    var result = this.parseSeparatedSyntaxList(
      ListParsingState.HeritageClause_TypeNameList
    );
    var typeNames = result.list;
    extendsOrImplementsKeyword = this.addSkippedTokensAfterToken(
    ...
    return this.factory.heritageClause(
      extendsOrImplementsKeyword.tokenKind === SyntaxKind.ExtendsKeyword
        ? SyntaxKind.ExtendsHeritageClause
        : SyntaxKind.ImplementsHeritageClause,
      extendsOrImplementsKeyword,
      typeNames
    );
  }
  ...
  private isStatement(inErrorRecovery: boolean): boolean {
    ...
    return (
      this.isLabeledStatement(currentToken) ||
  ...
  private isLabeledStatement(currentToken: ISyntaxToken): boolean {
    return (
      this.isIdentifier(currentToken) &&
      this.peekToken(1).tokenKind === SyntaxKind.ColonToken
    );
  }
  ...
  private parseForOrForInStatement(): IIterationStatementSyntax {
    ...
    if (currentToken.tokenKind === SyntaxKind.VarKeyword) {
      ...
      return this.parseForStatementWithNoVariableDeclarationOrInitializer(
        forKeyword,
        openParenToken
      );
    } else {
      ...
      return this.parseForOrForInStatementWithInitializer(
        forKeyword,
        openParenToken
      );
    }
  ...
  private parseForOrForInStatementWithVariableDeclaration(
    ...
    if (this.currentToken().tokenKind === SyntaxKind.InKeyword) {
      return this.parseForInStatementWithVariableDeclarationOrInitializer(
        forKeyword,
        openParenToken,
        variableDeclaration,
        null
  ...
  private parseForOrForInStatementWithInitializer(
    forKeyword: ISyntaxToken,
    openParenToken: ISyntaxToken
  ): IIterationStatementSyntax {
    // Debug.assert(forKeyword.tokenKind === SyntaxKind.ForKeyword && openParenToken.tokenKind === SyntaxKind.OpenParenToken);
  ...
  private parseForStatementWithNoVariableDeclarationOrInitializer(
    forKeyword: ISyntaxToken,
    openParenToken: ISyntaxToken
  ): ForStatementSyntax {
    // Debug.assert(forKeyword.tokenKind === SyntaxKind.ForKeyword && openParenToken.tokenKind === SyntaxKind.OpenParenToken);
  ...
  private parseForStatementWithVariableDeclarationOrInitializer(
    ...
    if (
      this.currentToken().tokenKind !== SyntaxKind.SemicolonToken &&
  ...
  private parseBreakStatement(): BreakStatementSyntax {
    ...
    if (
      !this.canEatExplicitOrAutomaticSemicolon(/*allowWithoutNewline:*/ false)
    ) {
      if (this.isIdentifier(this.currentToken())) {
  ...
  private parseSwitchStatement() {
    ...
    var switchClauses: ISyntaxList = emptyList;
    if (openBraceToken.width() > 0) {
      var result = this.parseSyntaxList(
        ListParsingState.SwitchStatement_SwitchClauses
      );
  ...
  private parseCaseSwitchClause(): CaseSwitchClauseSyntax {
    ...
    if (colonToken.fullWidth() > 0) {
      var result = this.parseSyntaxList(
        ListParsingState.SwitchClause_Statements
      );
      statements = result.list;
      colonToken = this.addSkippedTokensAfterToken(
        colonToken,
        result.skippedTokens
      );
    }

    return this.factory.caseSwitchClause(
      caseKeyword,
      expression,
      colonToken,
      statements
  ...
  private parseDefaultSwitchClause(): DefaultSwitchClauseSyntax {
    ...
    return this.factory.defaultSwitchClause(
      defaultKeyword,
      colonToken,
      statements
    );
  ...
  private parseVariableDeclaration(
    ...
    varKeyword = this.addSkippedTokensAfterToken(
      varKeyword,
      result.skippedTokens
    );
  ...
  private isVariableDeclarator(): boolean {
    if (
      this.currentNode() !== null &&
      this.currentNode().kind() === SyntaxKind.VariableDeclarator
    ) {
      return true;
    }
  ...
  private parseVariableDeclarator(
    ...
    var propertyName = allowPropertyName
    ...
    return this.factory.variableDeclarator(
      propertyName,
      typeAnnotation,
      equalsValueClause
    );
  ...
  private isEqualsValueClause(inParameter: boolean): boolean {
    ...
    if (!this.previousToken().hasTrailingNewLine()) {
      ...
      if (token0.tokenKind === SyntaxKind.OpenBraceToken && inParameter) {
        return false;
      }
  ...
  private parseAssignmentExpression(allowIn: boolean): IExpressionSyntax {
    return this.parseSubExpression(
      ExpressionPrecedence.AssignmentExpressionPrecedence,
      allowIn
    );
  }
  ...
  private parseUnaryExpressionOrLower(): IUnaryExpressionSyntax {
    ...
    if (SyntaxFacts.isPrefixUnaryExpressionOperatorToken(currentTokenKind)) {
      var operatorKind =
        SyntaxFacts.getPrefixUnaryExpressionFromOperatorToken(currentTokenKind);
  ...
  private parseSubExpression(
    ...
    if (precedence <= ExpressionPrecedence.AssignmentExpressionPrecedence) {
      ...
      var parethesizedArrowFunction =
        this.tryParseParenthesizedArrowFunctionExpression();
    ...
    return this.parseBinaryOrConditionalExpressions(
      precedence,
      allowIn,
      leftOperand
    );
  ...
  private parseBinaryOrConditionalExpressions(
    precedence: number,
    allowIn: boolean,
    leftOperand: IExpressionSyntax
  ): IExpressionSyntax {
    while (true) {
      ...
      if (SyntaxFacts.isBinaryExpressionOperatorToken(token0Kind)) {
        ...
        var tokenKind =
          mergedToken === null ? token0Kind : mergedToken.syntaxKind;

        var binaryExpressionKind =
          SyntaxFacts.getBinaryExpressionFromOperatorToken(tokenKind);
        ...
        if (
          ...
          !this.isRightAssociative(binaryExpressionKind)
        ) {
          break;
        }
        ...
        var operatorToken =
          mergedToken === null
            ? token0
            : token(mergedToken.syntaxKind)
                .withLeadingTrivia(token0.leadingTrivia())
                .withTrailingTrivia(
                  this.peekToken(mergedToken.tokenCount - 1).trailingTrivia()
                );
        ...
        leftOperand = this.factory.binaryExpression(
          ...
          leftOperand,
          operatorToken,
          this.parseSubExpression(newPrecedence, allowIn)
        );
        continue;
      }
      ...
      if (
        token0Kind === SyntaxKind.QuestionToken &&
        precedence <= ExpressionPrecedence.ConditionalExpressionPrecedence
      ) {
  ...
  private tryMergeBinaryExpressionTokens(): {
    tokenCount: number;
    ...
    if (
      token0.tokenKind === SyntaxKind.GreaterThanToken &&
      !token0.hasTrailingTrivia()
    ) {
      ...
      if (storage[0] === SyntaxKind.GreaterThanToken) {
        ...
        return {
          tokenCount: 2,
          syntaxKind: SyntaxKind.GreaterThanEqualsToken,
        };
      }
    }
  ...
  private parseMemberExpressionOrLower(
    inObjectCreation: boolean
  ): IMemberExpressionSyntax {
    if (this.currentToken().tokenKind === SyntaxKind.NewKeyword) {
    ...
    return this.parseMemberExpressionRest(
      expression,
      /*allowArguments:*/ false,
      /*inObjectCreation:*/ inObjectCreation
    );
  }
  ...
  private parseCallExpressionOrLower(): IMemberExpressionSyntax {
    ...
    if (this.currentToken().tokenKind === SyntaxKind.SuperKeyword) {
      ...
      expression = this.parseMemberExpressionOrLower(
    ...
    while (true) {
      ...
      switch (currentTokenKind) {
        case SyntaxKind.OpenParenToken:
          ...
          expression = this.factory.invocationExpression(
            expression,
            this.parseArgumentList(/*typeArgumentList:*/ null)
          );
          continue;
    ...
    switch (currentTokenKind) {
      ...
      case SyntaxKind.MinusMinusToken:
        ...
        return this.factory.postfixUnaryExpression(
          SyntaxFacts.getPostfixUnaryExpressionFromOperatorToken(
            currentTokenKind
          ),
          expression,
          this.eatAnyToken()
  ...
  private tryParseGenericArgumentList(): ArgumentListSyntax {
    ...
    var typeArgumentList = this.tryParseTypeArgumentList(
      /*inExpression:*/ true
    );
    var token0 = this.currentToken();
    ...
    if (typeArgumentList === null || !isOpenParenOrDot) {
      ...
      if (isDot) {
        ...
        var diagnostic = new Diagnostic(
          ...
          this.currentTokenStart(),
          token0.width(),
          DiagnosticCode.A_parameter_list_must_follow_a_generic_type_argument_list_expected,
          null
        );
        this.addDiagnostic(diagnostic);

        return this.factory.argumentList(
          typeArgumentList,
          emptyToken(SyntaxKind.OpenParenToken),
          emptySeparatedList,
          emptyToken(SyntaxKind.CloseParenToken)
        );
      } else {
  ...
  private parseArgumentList(
    ...
    var _arguments = emptySeparatedList;

    if (openParenToken.fullWidth() > 0) {
      var result = this.parseSeparatedSyntaxList(
        ListParsingState.ArgumentList_AssignmentExpressions
      );
      _arguments = result.list;
      openParenToken = this.addSkippedTokensAfterToken(
        openParenToken,
        result.skippedTokens
      );
    }
  ...
  private parseElementAccessExpression(
    ...
    if (
      this.currentToken().tokenKind === SyntaxKind.CloseBracketToken &&
      inObjectCreation
    ) {
      ...
      var diagnostic = new Diagnostic(
        ...
        DiagnosticCode.new_T_cannot_be_used_to_create_an_array_Use_new_Array_T_instead,
        null
      );
      this.addDiagnostic(diagnostic);

      argumentExpression = emptyToken(SyntaxKind.IdentifierName);
    } else {
  ...
  private tryReparseDivideAsRegularExpression(): IPrimaryExpressionSyntax {
    ...
    if (
      currentToken.tokenKind === SyntaxKind.SlashToken ||
  ...
  private parseObjectCreationExpression(): IMemberExpressionSyntax {
    ...
    var expression = this.parseObjectCreationExpressionOrLower(
      /*inObjectCreation:*/ true
    );
    var argumentList = this.tryParseArgumentList();

    var result = this.factory.objectCreationExpression(
      newKeyword,
      expression,
      argumentList
    );
  ...
  private parseObjectCreationExpressionOrLower(
    inObjectCreation: boolean
  ): IMemberExpressionSyntax {
    if (this.currentToken().tokenKind === SyntaxKind.NewKeyword) {
      return this.parseObjectCreationExpression();
  ...
  private parseCastExpression(): CastExpressionSyntax {
    ...
    return this.factory.castExpression(
      lessThanToken,
      type,
      greaterThanToken,
      expression
  ...
  private tryParseParenthesizedArrowFunctionExpression(): ParenthesizedArrowFunctionExpressionSyntax {
    ...
    if (this.isDefinitelyArrowFunctionExpression()) {
      ...
      return this.parseParenthesizedArrowFunctionExpression(
        /*requiresArrow:*/ false
      );
    }
    ...
    var arrowFunction = this.parseParenthesizedArrowFunctionExpression(
      /*requiresArrow:*/ true
    );
  ...
  private parseParenthesizedArrowFunctionExpression(
    ...
    var callSignature = this.parseCallSignature(
      /*requireCompleteTypeParameterList:*/ true
    );

    if (
      requireArrow &&
      this.currentToken().tokenKind !== SyntaxKind.EqualsGreaterThanToken
    ) {
      return null;
    }

    var equalsGreaterThanToken = this.eatToken(
    ...
    return this.factory.parenthesizedArrowFunctionExpression(
      callSignature,
  ...
  private tryParseArrowFunctionBlock(): BlockSyntax {
    if (this.isBlock()) {
      ...
      if (
        this.isStatement(/*inErrorRecovery:*/ false) &&
        ...
        return this.parseBlock(
          /*parseStatementsEvenWithNoOpenBrace:*/ true,
          /*checkForStrictMode:*/ false
        );
      } else {
        return null;
  ...
  private isDefinitelyArrowFunctionExpression(): boolean {
    ...
    if (token1.tokenKind === SyntaxKind.CloseParenToken) {
      ...
      return (
        token2.tokenKind === SyntaxKind.ColonToken ||
    ...
    token2 = this.peekToken(2);
    if (
      token1.tokenKind === SyntaxKind.PublicKeyword ||
      token1.tokenKind === SyntaxKind.PrivateKeyword
    ) {
    ...
    if (token2.tokenKind === SyntaxKind.QuestionToken) {
      ...
      if (
        token3.tokenKind === SyntaxKind.ColonToken ||
  ...
  private parsePropertyAssignment(
    inErrorRecovery: boolean
  ): IPropertyAssignmentSyntax {
  ...
  private isFunctionPropertyAssignment(inErrorRecovery: boolean): boolean {
    return (
      this.isPropertyName(this.currentToken(), inErrorRecovery) &&
      this.isCallSignature(/*index:*/ 1)
    );
  }
  ...
  private parseFunctionPropertyAssignment(): FunctionPropertyAssignmentSyntax {
    ...
    var block = this.parseBlock(
      /*parseBlockEvenWithNoOpenBrace:*/ false,
      /*checkForStrictMode:*/ true
    );

    return this.factory.functionPropertyAssignment(
      propertyName,
      callSignature,
      block
    );
  ...
  private parseArrayLiteralExpression(): ArrayLiteralExpressionSyntax {
    ...
    var result = this.parseSeparatedSyntaxList(
      ListParsingState.ArrayLiteralExpression_AssignmentExpressions
    );
  ...
  private parseBlock(
    parseBlockEvenWithNoOpenBrace: boolean,
    checkForStrictMode: boolean
  ): BlockSyntax {
    var openBraceToken = this.eatToken(SyntaxKind.OpenBraceToken);

    var statements: ISyntaxList = emptyList;

    if (parseBlockEvenWithNoOpenBrace || openBraceToken.width() > 0) {
      var savedIsInStrictMode = this.isInStrictMode;

      var processItems = checkForStrictMode
        ? (parser: ParserImpl, items: CheckedArray<any>) =>
            ParserImpl.updateStrictModeState(parser, items)
        : <(p: ParserImpl, i: CheckedArray<any>) => void>null;
      var result = this.parseSyntaxList(
        ListParsingState.Block_Statements,
        processItems
      );
      statements = result.list;
      openBraceToken = this.addSkippedTokensAfterToken(
  ...
  private parseCallSignature(
    requireCompleteTypeParameterList: boolean
  ): CallSignatureSyntax {
    var typeParameterList = this.parseOptionalTypeParameterList(
    ...
    return this.factory.callSignature(
      typeParameterList,
      parameterList,
      typeAnnotation
    );
  ...
  private parseOptionalTypeParameterList(
    requireCompleteTypeParameterList: boolean
  ): TypeParameterListSyntax {
    if (this.currentToken().tokenKind !== SyntaxKind.LessThanToken) {
      return null;
    ...
    var result = this.parseSeparatedSyntaxList(
      ListParsingState.TypeParameterList_TypeParameters
    );
    ...
    if (
      ...
      this.releaseRewindPoint(rewindPoint);
      var typeParameterList = this.factory.typeParameterList(
        lessThanToken,
        typeParameters,
        greaterThanToken
      );
  ...
  private parseParameterList(): ParameterListSyntax {
    var openParenToken = this.eatToken(SyntaxKind.OpenParenToken);
    var parameters: ISeparatedSyntaxList = emptySeparatedList;

    if (openParenToken.width() > 0) {
      var result = this.parseSeparatedSyntaxList(
        ListParsingState.ParameterList_Parameters
      );
    ...
    var closeParenToken = this.eatToken(SyntaxKind.CloseParenToken);
    return this.factory.parameterList(
      openParenToken,
      parameters,
      closeParenToken
    );
  ...
  private parseOptionalTypeAnnotation(
  ...
  private parseTypeAnnotation(
    ...
    var type =
      allowStringLiteral &&
      this.currentToken().tokenKind === SyntaxKind.StringLiteral
  ...
  private parseFunctionType(): FunctionTypeSyntax {
    ...
    return this.factory.functionType(
      typeParameterList,
      parameterList,
      equalsGreaterThanToken,
      returnType
  ...
  private parseConstructorType(): ConstructorTypeSyntax {
    ...
    return this.factory.constructorType(
      newKeyword,
      typeParameterList,
      parameterList,
      equalsGreaterThanToken,
  ...
  private isParameter(): boolean {
    if (
      this.currentNode() !== null &&
      this.currentNode().kind() === SyntaxKind.Parameter
    ) {
      return true;
    }
    ...
    if (
      ParserImpl.isModifier(token) &&
      !this.isModifierUsedAsParameterIdentifier(token)
    ) {
      return true;
    }
  ...
  private parseParameter(): ParameterSyntax {
    ...
    while (true) {
      ...
      if (
        ParserImpl.isModifier(currentToken) &&
        !this.isModifierUsedAsParameterIdentifier(currentToken)
      ) {
        modifierArray.push(this.eatAnyToken());
        continue;
      }
  ...
  private parseSyntaxList(
    currentListType: ListParsingState,
    processItems: (parser: ParserImpl, items: CheckedArray<any>) => void = null
  ): { skippedTokens: ISyntaxToken[]; list: ISyntaxList } {
    var savedListParsingState = this.listParsingState;
  ...
  private parseSeparatedSyntaxList(currentListType: ListParsingState): {
    skippedTokens: ISyntaxToken[];
  ...
  private abortParsingListOrMoveToNextToken(
    currentListType: ListParsingState,
    ...
    for (
      var state = ListParsingState.LastListParsingState;
      state >= ListParsingState.FirstListParsingState;
      state >>= 1
    ) {
      if ((this.listParsingState & state) !== 0) {
        if (
          this.isExpectedListTerminator(state) ||
          this.isExpectedListItem(state, /*inErrorRecovery:*/ true)
        ) {
  ...
  private tryParseExpectedListItem(
    currentListType: ListParsingState,
    inErrorRecovery: boolean,
    items: ISyntaxNodeOrToken[],
    processItems: (parser: ParserImpl, items: CheckedArray<any>) => void
  ): void {
  ...
  private listIsTerminated(currentListType: ListParsingState): boolean {
    return (
      this.isExpectedListTerminator(currentListType) ||
      this.currentToken().tokenKind === SyntaxKind.EndOfFileToken
    );
  }
  ...
  private returnZeroOrOneLengthSyntaxNodeOrTokenArray(
    array: ISyntaxNodeOrToken[]
  ) {
    if (array.length <= 1) {
  ...
  private parseSyntaxListWorker(
    ...
    while (true) {
      ...
      if (newItemsCount === oldItemsCount) {
        ...
        var abort = this.abortParsingListOrMoveToNextToken(
          currentListType,
          items,
          skippedTokens
        );
  ...
  private parseSeparatedSyntaxListWorker(currentListType: ListParsingState): {
    skippedTokens: ISyntaxToken[];
    list: ISeparatedSyntaxList;
  } {
    ...
    var allowAutomaticSemicolonInsertion =
      separatorKind === SyntaxKind.SemicolonToken;
    ...
    while (true) {
      ...
      var currentToken = this.currentToken();
      if (
        currentToken.tokenKind === separatorKind ||
        currentToken.tokenKind === SyntaxKind.CommaToken
      ) {
      ...
      if (
        ...
        this.canEatAutomaticSemicolon(/*allowWithoutNewline:*/ false)
      ) {
        items.push(
    ...
    var result = separatedList(items);
  ...
  private isExpectedTypeParameterList_TypeParametersTerminator(): boolean {
    ...
    if (
      token.tokenKind === SyntaxKind.OpenParenToken ||
  ...
  private isExpectedClassOrInterfaceDeclaration_HeritageClausesTerminator(): boolean {
    var token0 = this.currentToken();
    if (
      token0.tokenKind === SyntaxKind.OpenBraceToken ||
  ...
  private isExpectedArgumentList_AssignmentExpressionsTerminator(): boolean {
    var token0 = this.currentToken();
    return (
      token0.tokenKind === SyntaxKind.CloseParenToken ||
  ...
  private isExpectedTryBlock_StatementsTerminator(): boolean {
    return (
      this.currentToken().tokenKind === SyntaxKind.CatchKeyword ||
  ...
  private getExpectedListElementType(
    currentListType: ListParsingState
  ): string {
    switch (currentListType) {
      case ListParsingState.SourceUnit_ModuleElements:
        return getLocalizedText(
          DiagnosticCode.module_class_interface_enum_import_or_statement,
          null
        );
      ...
      case ListParsingState.ClassDeclaration_ClassElements:
        return getLocalizedText(
          DiagnosticCode.constructor_function_accessor_or_variable,
          null
        );
      ...
      case ListParsingState.ObjectType_TypeMembers:
        return getLocalizedText(
          DiagnosticCode.call_construct_index_property_or_function_signature,
          null
        );
...
export function parse(
  ...
  var source = new NormalParserSource(
    fileName,
    text,
...
export function incrementalParse(
  ...
  return new ParserImpl(
    ...
    newText.lineMap(),
    source,
    oldSyntaxTree.parseOptions(),
    newText
  ).parseSyntaxTree(oldSyntaxTree.isDeclaration());
}
