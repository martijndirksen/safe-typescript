...
module TypeScript {
  ...
  export class SlidingWindow {
    ...
    constructor(
      // Underlying source that we retrieve items from.
      ...
      //copy
      private copy: (
        win: CheckedArray<any>,
        sourceIndex: number,
        dest: CheckedArray<any>,
        destinationIndex: number,
        length: number
      ) => void,
      //grow
    ...
    private addMoreItemsToWindow(argument: any): boolean {
      ...
      var amountFetched = this.source.fetchMoreItems(
        argument,
    ...
    private tryShiftOrGrowWindow(): void {
      ...
      var currentIndexIsPastWindowHalfwayPoint =
        this.currentRelativeItemIndex > this.window.length >>> 1;
    ...
    public setAbsoluteIndex(absoluteIndex: number): void {
      ...
      if (
        absoluteIndex >= this.windowAbsoluteStartIndex &&
        absoluteIndex < this.windowAbsoluteEndIndex()
      ) {
