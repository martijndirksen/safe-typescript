...
export function columnForEndOfToken(
  ...
  return (
    columnForStartOfToken(token, syntaxInformationMap, options) + token.width()
  );
}
...
export function columnForStartOfToken(
  ...
  var firstTokenInLine =
    syntaxInformationMap.firstTokenInLineContainingToken(token);
...
function columnForLeadingTextInReverse(
  ...
  for (var i = leadingTextInReverse.length - 1; i >= 0; i--) {
    ...
    column = columnForPositionInStringWorker(
      text,
      text.length,
      column,
      options
...
function columnForPositionInStringWorker(
  input: string,
  position: number,
  startColumn: number,
  options: IFormattingOptions
...
export function indentString(
  ...
  return '\t'.repeat(numberOfTabs) + ' '.repeat(numberOfSpaces);
}

export function indentTrivia(
  column: number,
  options: IFormattingOptions
): ISyntaxTrivia {
  return whitespace(indentString(column, options));
}
