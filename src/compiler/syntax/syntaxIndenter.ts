...
export class SyntaxIndenter extends SyntaxRewriter {
  ...
  constructor(
    ...
    this.indentationTrivia = indentTrivia(this.indentationAmount, this.options);
  }
  ...
  public visitToken(token: ISyntaxToken): ISyntaxToken {
    ...
    if (this.lastTriviaWasNewLine) {
      ...
      result = token.withLeadingTrivia(
        this.indentTriviaList(token.leadingTrivia())
      );
    }
  ...
  private indentSegment(segment: string): string {
    ...
    var newFirstNonWhitespaceColumn =
      firstNonWhitespaceColumn + this.indentationAmount;

    // Compute an indentation string for that.
    var indentationString = indentString(
      newFirstNonWhitespaceColumn,
      this.options
    );

    // Join the new indentation and the original string without its indentation.
    return indentationString + segment.substring(firstNonWhitespacePos);
  }

  private indentWhitespace(
    trivia: ISyntaxTrivia,
    indentThisTrivia: boolean,
    result: ISyntaxTrivia[]
  ): void {
    if (!indentThisTrivia) {
    ...
    result.push(whitespace(newIndentation));
  }

  private indentSingleLineOrSkippedText(
    trivia: ISyntaxTrivia,
    indentThisTrivia: boolean,
    result: ISyntaxTrivia[]
  ): void {
  ...
  private indentMultiLineComment(
    trivia: ISyntaxTrivia,
    indentThisTrivia: boolean,
    result: ISyntaxTrivia[]
  ): void {
    ...
    result.push(multiLineComment(newText));
  }

  public static indentNode(
    node: ISyntaxNode,
    indentFirstToken: boolean,
    indentAmount: number,
  ...
  public static indentNodes(
    ...
    var indenter = new SyntaxIndenter(indentFirstToken, indentAmount, options);
    var result: SyntaxNode[] = ArrayUtilities.select<SyntaxNode, SyntaxNode>(
      nodes,
      (n) => <SyntaxNode>n.accept(indenter)
    );
