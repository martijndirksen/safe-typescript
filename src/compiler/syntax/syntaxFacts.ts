...
import { ExpressionStatementSyntax } from './syntaxNodes.generated';
import { ISyntaxToken } from './syntaxToken';

export module SyntaxFacts {
  ...
  export function isStandardKeyword(kind: SyntaxKind): boolean {
    return (
      kind >= SyntaxKind.FirstStandardKeyword &&
      kind <= SyntaxKind.LastStandardKeyword
    );
  }

  export function isFutureReservedKeyword(kind: SyntaxKind): boolean {
    return (
      kind >= SyntaxKind.FirstFutureReservedKeyword &&
      kind <= SyntaxKind.LastFutureReservedKeyword
    );
  }

  export function isFutureReservedStrictKeyword(kind: SyntaxKind): boolean {
    return (
      kind >= SyntaxKind.FirstFutureReservedStrictKeyword &&
      kind <= SyntaxKind.LastFutureReservedStrictKeyword
    );
  }
  ...
  export function isPrefixUnaryExpressionOperatorToken(
    tokenKind: SyntaxKind
  ): boolean {
    return (
      getPrefixUnaryExpressionFromOperatorToken(tokenKind) !== SyntaxKind.None
    );
  }
  ...
  export function getPrefixUnaryExpressionFromOperatorToken(
    tokenKind: SyntaxKind
  ): SyntaxKind {
    switch (tokenKind) {
  ...
  export function getPostfixUnaryExpressionFromOperatorToken(
    tokenKind: SyntaxKind
  ): SyntaxKind {
    switch (tokenKind) {
      case SyntaxKind.PlusPlusToken:
  ...
  export function getBinaryExpressionFromOperatorToken(
    tokenKind: SyntaxKind
  ): SyntaxKind {
    switch (tokenKind) {
  ...
  export function getOperatorTokenFromBinaryExpression(
    tokenKind: SyntaxKind
  ): SyntaxKind {
    switch (tokenKind) {
  ...
  export function isAnyDivideOrRegularExpressionToken(
    kind: SyntaxKind
  ): boolean {
    switch (kind) {
      case SyntaxKind.SlashToken:
  ...
  export function isDirectivePrologueElement(
  ...
  export function isIdentifierNameOrAnyKeyword(token: ISyntaxToken): boolean {
    ...
    return (
      tokenKind === SyntaxKind.IdentifierName ||
      SyntaxFacts.isAnyKeyword(tokenKind)
    );
  }
}
