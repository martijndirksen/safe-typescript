...
import {
  ...
  childOffsetAt,
  childOffset,
  findSkippedTokenInTrailingTriviaList,
...
export class PositionedElement {
  ...
  public static create(
    ...
    if (element.isNode()) {
      ...
      return new PositionedSeparatedList(
        parent,
        <ISeparatedSyntaxList>element,
        fullStart
      );
    } else {
...
export class PositionedToken extends PositionedNodeOrToken {
  ...
  public previousToken(includeSkippedTokens: boolean = false): PositionedToken {
    ...
    return this.root()
      .node()
      .findToken(start - 1, includeSkippedTokens);
  }
...
export class PositionedSkippedToken extends PositionedToken {
  ...
  public nextToken(includeSkippedTokens: boolean = false): PositionedToken {
    ...
    return this.root()
      .node()
      .findToken(this.parentToken().fullEnd(), includeSkippedTokens);
  }
}
