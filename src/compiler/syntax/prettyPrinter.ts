...
module TypeScript.PrettyPrinter {
  ...
  class PrettyPrinterImpl implements ISyntaxVisitor {
    ...
    private newLineCountBetweenModuleElements(
      element1: IModuleElementSyntax,
      element2: IModuleElementSyntax
    ): number {
    ...
    private newLineCountBetweenClassElements(
      element1: IClassElementSyntax,
      element2: IClassElementSyntax
    ): number {
      if (element1 === null || element2 === null) {
        return 0;
      }
    ...
    private newLineCountBetweenStatements(
      element1: IClassElementSyntax,
      element2: IClassElementSyntax
    ): number {
      if (element1 === null || element2 === null) {
    ...
    private newLineCountBetweenSwitchClauses(
      element1: ISwitchClauseSyntax,
      element2: ISwitchClauseSyntax
    ): number {
      if (element1 === null || element2 === null) {
        return 0;
      }
    ...
    private getIndentation(count: number): string {
      for (var i = this.indentations.length; i <= count; i++) {
        var text =
          i === 0 ? '' : this.indentations[i - 1] + this.indentWhitespace;
    ...
    private appendModuleElements(list: ISyntaxList): void {
      ...
      for (var i = 0, n = list.childCount(); i < n; i++) {
        ...
        var newLineCount = this.newLineCountBetweenModuleElements(
          lastModuleElement,
          moduleElement
        );
    ...
    public visitClassDeclaration(node: ClassDeclarationSyntax): void {
      ...
      for (var i = 0, n = node.classElements.childCount(); i < n; i++) {
        ...
        var newLineCount = this.newLineCountBetweenClassElements(
          lastClassElement,
          classElement
        );
    ...
    private appendObjectType(
      node: ObjectTypeSyntax,
    ...
    private appendBlockOrSemicolon(
      block: BlockSyntax,
    ...
    private appendStatements(statements: ISyntaxList): void {
      ...
      for (var i = 0, n = statements.childCount(); i < n; i++) {
        ...
        var newLineCount = this.newLineCountBetweenStatements(
          lastStatement,
          statement
        );
    ...
    public visitSwitchStatement(node: SwitchStatementSyntax): void {
      ...
      for (var i = 0, n = node.switchClauses.childCount(); i < n; i++) {
        ...
        var newLineCount = this.newLineCountBetweenSwitchClauses(
          lastSwitchClause,
          switchClause
        );
    ...
    private appendSwitchClauseStatements(node: ISwitchClauseSyntax): void {
      if (
        node.statements.childCount() === 1 &&
        node.statements.childAt(0).kind() === SyntaxKind.Block
      ) {
        this.ensureSpace();
