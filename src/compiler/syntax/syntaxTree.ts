module TypeScript {
  export class SyntaxTree {
    ...
    public toJSON(key: any): any {
      ...
      result.languageVersion =
        LanguageVersion[this._parseOptions.languageVersion()];
  ...
  class GrammarCheckerWalker extends PositionTrackingWalker {
    ...
    private pushDiagnostic1(
      elementFullStart: number,
      ...
      this.diagnostics.push(
        new Diagnostic(
          this.syntaxTree.fileName(),
          this.syntaxTree.lineMap(),
          elementFullStart + element.leadingTriviaWidth(),
          element.width(),
          diagnosticKey,
          args
        )
    ...
    private checkParameterListAcessibilityModifiers(
      ...
      for (var i = 0, n = node.parameters.childCount(); i < n; i++) {
        ...
        if (i % 2 === 0) {
          ...
          if (
            this.checkParameterAccessibilityModifiers(
              node,
              parameter,
              parameterFullStart
            )
    ...
    private checkParameterAccessibilityModifiers(
      parameterList: ParameterListSyntax,
      parameter: ParameterSyntax,
      parameterFullStart: number
    ): boolean {
      if (parameter.modifiers.childCount() > 0) {
        ...
        for (var i = 0, n = modifiers.childCount(); i < n; i++) {
          ...
          if (
            this.checkParameterAccessibilityModifier(
              parameterList,
              modifier,
              modifierFullStart,
              i
    ...
    private checkParameterAccessibilityModifier(
      parameterList: ParameterListSyntax,
      modifier: ISyntaxToken,
      modifierFullStart: number,
      modifierIndex: number
    ): boolean {
      if (
        modifier.tokenKind !== SyntaxKind.PublicKeyword &&
        ...
        if (this.inAmbientDeclaration) {
          ...
          this.currentConstructor.parameterList === parameterList
        ) {
          this.pushDiagnostic1(
            modifierFullStart,
            modifier,
    ...
    private checkForTrailingSeparator(
      parent: ISyntaxElement,
      ...
      for (var i = 0, n = list.childCount(); i < n; i++) {
        ...
        if (i === n - 1) {
          this.pushDiagnostic1(
            currentElementFullStart,
            child,
            DiagnosticCode.Trailing_separator_not_allowed
          );
        }
    ...
    private checkForAtLeastOneElement(
      parent: ISyntaxElement,
      list: ISeparatedSyntaxList,
      expected: string
    ): boolean {
    ...
    public visitParameterList(node: ParameterListSyntax): void {
      if (
        this.checkParameterListAcessibilityModifiers(node) ||
    ...
    public visitHeritageClause(node: HeritageClauseSyntax): void {
      if (
        this.checkForTrailingSeparator(node, node.typeNames) ||
        this.checkForAtLeastOneElement(
          node,
          node.typeNames,
          getLocalizedText(DiagnosticCode.type_name, null)
        )
      ) {
        this.skip(node);
        return;
    ...
    public visitVariableDeclaration(node: VariableDeclarationSyntax): void {
      if (
        this.checkForTrailingSeparator(node, node.variableDeclarators) ||
        this.checkForAtLeastOneElement(
          node,
          node.variableDeclarators,
          getLocalizedText(DiagnosticCode.identifier, null)
        )
      ) {
        this.skip(node);
        return;
    ...
    public visitTypeArgumentList(node: TypeArgumentListSyntax): void {
      if (
        this.checkForTrailingSeparator(node, node.typeArguments) ||
        this.checkForAtLeastOneElement(
          node,
          node.typeArguments,
          getLocalizedText(DiagnosticCode.identifier, null)
    ...
    public visitTypeParameterList(node: TypeParameterListSyntax): void {
      if (
        this.checkForTrailingSeparator(node, node.typeParameters) ||
        this.checkForAtLeastOneElement(
          node,
          node.typeParameters,
          getLocalizedText(DiagnosticCode.identifier, null)
    ...
    private checkIndexSignatureParameter(node: IndexSignatureSyntax): boolean {
      ...
      if (parameter.dotDotDotToken) {
        ...
        parameter.typeAnnotation.type.kind() !== SyntaxKind.StringKeyword &&
    ...
    private checkClassDeclarationHeritageClauses(
      node: ClassDeclarationSyntax
    ): boolean {
      var heritageClauseFullStart = this.childFullStart(
        node,
      ...
      for (var i = 0, n = node.heritageClauses.childCount(); i < n; i++) {
        ...
        var heritageClause = <HeritageClauseSyntax>(
          node.heritageClauses.childAt(i)
        );

        if (
          heritageClause.extendsOrImplementsKeyword.tokenKind ===
          SyntaxKind.ExtendsKeyword
        ) {
          if (seenExtendsClause) {
            this.pushDiagnostic1(
              heritageClauseFullStart,
              heritageClause,
    ...
    private checkForDisallowedDeclareModifier(modifiers: ISyntaxList): boolean {
      if (this.inAmbientDeclaration) {
        ...
        var declareToken = SyntaxUtilities.getToken(
          modifiers,
        ...
        if (declareToken) {
          this.pushDiagnostic1(
            this.childFullStart(modifiers, declareToken),
            declareToken,
    ...
    private checkForRequiredDeclareModifier(
      ...
      if (!this.inAmbientDeclaration && this.syntaxTree.isDeclaration()) {
        ...
        if (
          !SyntaxUtilities.containsToken(modifiers, SyntaxKind.DeclareKeyword)
        ) {
          this.pushDiagnostic1(
            this.childFullStart(moduleElement, typeKeyword),
            typeKeyword.firstToken(),
      ...
      if (!this.inAmbientDeclaration && !this.syntaxTree.isDeclaration()) {
        ...
        for (var i = 0, n = moduleElements.childCount(); i < n; i++) {
          ...
          if (inFunctionOverloadChain) {
            ...
            if (
              functionDeclaration.identifier.valueText() !==
              ...
              this.pushDiagnostic1(
                identifierFullStart,
                functionDeclaration.identifier,
                DiagnosticCode.Function_overload_name_must_be_0,
                [functionOverloadChainName]
              );
              return true;
            }
    ...
    private checkClassOverloads(node: ClassDeclarationSyntax): boolean {
      if (
        ...
        var classElementFullStart = this.childFullStart(
          node,
          node.classElements
        );
        ...
        for (var i = 0, n = node.classElements.childCount(); i < n; i++) {
          ...
          if (inFunctionOverloadChain) {
            if (classElement.kind() !== SyntaxKind.MemberFunctionDeclaration) {
              this.pushDiagnostic1(
                ...
                classElement.firstToken(),
                DiagnosticCode.Function_implementation_expected
              );
              return true;
            }
            var propertyNameFullStart: number;
            memberFunctionDeclaration = <MemberFunctionDeclarationSyntax>(
              classElement
            );
            if (
              memberFunctionDeclaration.propertyName.valueText() !==
              functionOverloadChainName
            ) {
            ...
            isStaticOverload = SyntaxUtilities.containsToken(
              memberFunctionDeclaration.modifiers,
          ...
          if (classElement.kind() === SyntaxKind.MemberFunctionDeclaration) {
            ...
            functionOverloadChainName =
              memberFunctionDeclaration.propertyName.valueText();
            isInStaticOverloadChain = SyntaxUtilities.containsToken(
              memberFunctionDeclaration.modifiers,
              SyntaxKind.StaticKeyword
            );
            ...
            classElement.kind() === SyntaxKind.ConstructorDeclaration
          ) {
    ...
    private checkForReservedName(
      ...
      while (current !== null) {
        if (current.kind() === SyntaxKind.QualifiedName) {
          ...
          tokenFullStart =
            nameFullStart + this.childFullStart(qualifiedName, token);
        ...
        switch (token.valueText()) {
          ...
          case 'void':
            this.pushDiagnostic(
              tokenFullStart + token.leadingTriviaWidth(),
              token.width(),
              diagnosticKey,
              [token.valueText()]
            );
    ...
    private checkInterfaceDeclarationHeritageClauses(
    ...
    public visitInterfaceDeclaration(node: InterfaceDeclarationSyntax): void {
      if (
        this.checkForReservedName(
          node,
          node.identifier,
          DiagnosticCode.Interface_name_cannot_be_0
        ) ||
        ...
        if (
          ...
          if (seenStaticModifier) {
            ...
            this.pushDiagnostic1(
              ...
              [modifier.text(), previousToken.text()]
            );
            return true;
          }
          ...
          this.pushDiagnostic1(
            modifierFullStart,
            modifier,
            DiagnosticCode._0_modifier_cannot_appear_on_a_class_element,
            [modifier.text()]
          );
          return true;
        }
    ...
    private checkGetAccessorParameter(
      node: SyntaxNode,
    ...
    private checkIndexMemberModifiers(
      node: IndexMemberDeclarationSyntax
    ): boolean {
      if (node.modifiers.childCount() > 0) {
        ...
        this.pushDiagnostic1(
          modifierFullStart,
          node.modifiers.childAt(0),
          DiagnosticCode.Modifiers_cannot_appear_here
        );
        return true;
      }
    ...
    private checkEcmaScriptVersionIsAtLeast(
      parent: ISyntaxElement,
      node: ISyntaxElement,
    ...
    public visitGetAccessor(node: GetAccessorSyntax): void {
      if (
        this.checkForAccessorDeclarationInAmbientContext(node) ||
        this.checkEcmaScriptVersionIsAtLeast(
          node,
          node.getKeyword,
          LanguageVersion.EcmaScript5,
          DiagnosticCode.Accessors_are_only_available_when_targeting_ECMAScript_5_and_higher
        ) ||
        this.checkForDisallowedModifiers(node, node.modifiers) ||
        this.checkClassElementModifiers(node.modifiers) ||
        this.checkGetAccessorParameter(
          node,
          node.getKeyword,
          node.parameterList
        )
    ...
    private checkForAccessorDeclarationInAmbientContext(
      accessor: SyntaxNode
    ): boolean {
      if (this.inAmbientDeclaration) {
        this.pushDiagnostic1(
          this.position(),
          accessor,
          DiagnosticCode.Accessors_are_not_allowed_in_ambient_contexts,
          null
        );
        return true;
      }
    ...
    private checkSetAccessorParameter(
      node: SyntaxNode,
    ...
    public visitSetAccessor(node: SetAccessorSyntax): void {
      if (
        ...
        this.checkForDisallowedModifiers(node, node.modifiers) ||
        this.checkClassElementModifiers(node.modifiers) ||
        this.checkSetAccessorParameter(
          node,
          node.setKeyword,
          node.parameterList
        )
    ...
    public visitEnumDeclaration(node: EnumDeclarationSyntax): void {
      if (
        (this.checkForReservedName(
          node,
          node.identifier,
          DiagnosticCode.Enum_name_cannot_be_0
        ) ||
          this.checkForDisallowedDeclareModifier(node.modifiers) ||
          this.checkForRequiredDeclareModifier(
            node,
      ...
      for (var i = 0, n = modifiers.childCount(); i < n; i++) {
        ...
        if (
          ...
          this.pushDiagnostic1(
            modifierFullStart,
            modifier,
            DiagnosticCode._0_modifier_cannot_appear_on_a_module_element,
            [modifier.text()]
          );
          return true;
        }
        ...
        if (modifier.tokenKind === SyntaxKind.DeclareKeyword) {
          ...
          if (seenDeclareModifier) {
            this.pushDiagnostic1(
              ...
              DiagnosticCode._0_modifier_must_precede_1_modifier,
              [
                SyntaxFacts.getText(SyntaxKind.ExportKeyword),
                SyntaxFacts.getText(SyntaxKind.DeclareKeyword),
              ]
            );
            return;
          }
    ...
    private checkForDisallowedImportDeclaration(
      node: ModuleDeclarationSyntax
    ): boolean {
      var currentElementFullStart = this.childFullStart(
        node,
        node.moduleElements
      );
      ...
      for (var i = 0, n = node.moduleElements.childCount(); i < n; i++) {
        ...
        if (child.kind() === SyntaxKind.ImportDeclaration) {
          ...
          if (
            importDeclaration.moduleReference.kind() ===
            ...
            if (node.stringLiteral === null) {
              this.pushDiagnostic1(
                currentElementFullStart,
                importDeclaration,
                DiagnosticCode.Import_declarations_in_an_internal_module_cannot_reference_an_external_module,
                null
              );
            }
          }
    ...
    private checkForDisallowedDeclareModifierOnImportDeclaration(
      modifiers: ISyntaxList
    ): boolean {
      var declareToken = SyntaxUtilities.getToken(
        modifiers,
    ...
    public visitImportDeclaration(node: ImportDeclarationSyntax): any {
      if (
        this.checkForDisallowedDeclareModifierOnImportDeclaration(
          node.modifiers
        ) ||
        this.checkModuleElementModifiers(node.modifiers)
      ) {
    ...
    public visitModuleDeclaration(node: ModuleDeclarationSyntax): void {
      if (
        this.checkForReservedName(
          node,
          node.name,
          DiagnosticCode.Module_name_cannot_be_0
        ) ||
        this.checkForDisallowedDeclareModifier(node.modifiers) ||
        this.checkForRequiredDeclareModifier(
          node,
      ...
      if (node.stringLiteral) {
        if (
          ...
          var stringLiteralFullStart = this.childFullStart(
            node,
            node.stringLiteral
          );
          this.pushDiagnostic1(
            stringLiteralFullStart,
            node.stringLiteral,
    ...
    private checkForDisallowedExports(
      node: ISyntaxElement,
      moduleElements: ISyntaxList
    ): boolean {
        ...
        for (var i = 0, n = moduleElements.childCount(); i < n; i++) {
          ...
          if (child.kind() === SyntaxKind.ExportAssignment) {
            this.pushDiagnostic1(
              moduleElementFullStart,
              child,
              DiagnosticCode.Export_assignment_not_allowed_in_module_with_exported_element
            );
            return true;
          }
    ...
    private checkForMultipleExportAssignments(
      node: ISyntaxElement,
      moduleElements: ISyntaxList
    ): boolean {
      var moduleElementFullStart = this.childFullStart(node, moduleElements);
        ...
        if (child.kind() === SyntaxKind.ExportAssignment) {
          if (seenExportAssignment) {
            this.pushDiagnostic1(
              moduleElementFullStart,
              child,
              DiagnosticCode.Module_cannot_have_multiple_export_assignments
            );
            errorFound = true;
          }
    ...
    private checkForDisallowedExportAssignment(
      node: ModuleDeclarationSyntax
    ): boolean {
      var moduleElementFullStart = this.childFullStart(
        node,
        node.moduleElements
      );
      ...
      for (var i = 0, n = node.moduleElements.childCount(); i < n; i++) {
        ...
        if (child.kind() === SyntaxKind.ExportAssignment) {
          this.pushDiagnostic1(
            moduleElementFullStart,
            child,
            DiagnosticCode.Export_assignment_cannot_be_used_in_internal_modules
          );
    ...
    private checkForDisallowedModifiers(
      parent: ISyntaxElement,
      modifiers: ISyntaxList
    ): boolean {
      ...
      for (var i = 0, n = modifiers.childCount(); i < n; i++) {
        ...
        if (child.kind() !== SyntaxKind.PublicKeyword) {
          this.pushDiagnostic1(
            ...
            [SyntaxFacts.getText(child.kind())]
          );
          return true;
        }
