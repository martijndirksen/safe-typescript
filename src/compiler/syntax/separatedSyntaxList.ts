...
import { emptyTriviaList, ISyntaxTriviaList } from './syntaxTriviaList';
...
class SingletonSeparatedSyntaxList implements ISeparatedSyntaxList {
  ...
  public findTokenInternal(
    ...
    return (<any>this.item).findTokenInternal(
      new PositionedSeparatedList(parent, this, fullStart),
      position,
      fullStart
    );
  }
  ...
  public insertChildrenInto(array: ISyntaxElement[], index: number): void {
    if (index === 0) {
      ...
      RT.applyVariadic<any>(
        array,
        'splice',
        [index, <any>0].concat(this.elements)
      );
