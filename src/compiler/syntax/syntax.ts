...
import { normalModeFactory } from './syntaxFactory.generated';
import { SyntaxKind } from './syntaxKind';
import { emptyList, ISyntaxList } from './syntaxList';
...
export function emptySourceUnit() {
  return normalModeFactory.sourceUnit(
    emptyList,
    token(SyntaxKind.EndOfFileToken, { text: '' })
  );
}
...
export function numericLiteralExpression(text: string): IUnaryExpressionSyntax {
  return token(SyntaxKind.NumericLiteral, { text: text });
}

export function stringLiteralExpression(text: string): IUnaryExpressionSyntax {
  return token(SyntaxKind.StringLiteral, { text: text });
}
...
export function assignmentExpression(
  ...
  return normalModeFactory.binaryExpression(
...
export function isUnterminatedStringLiteral(token: ISyntaxToken): boolean {
  if (token && token.kind() === SyntaxKind.StringLiteral) {
    var text = token.text();
    return (
      text.length < 2 || text.charCodeAt(text.length - 1) !== text.charCodeAt(0)
    );
  }
    ...
    if (positionedToken.kind() === SyntaxKind.EndOfFileToken) {
      ...
      return (
        (position < positionedToken.end() &&
          (positionedToken.kind() === SyntaxKind.StringLiteral ||
            positionedToken.kind() === SyntaxKind.RegularExpressionLiteral)) ||
        (position <= positionedToken.end() &&
          isUnterminatedStringLiteral(positionedToken.token()))
      );
    }
