module TypeScript.Syntax {
  export function emptySourceUnit() {
    return Syntax.normalModeFactory.sourceUnit(
      Syntax.emptyList,
      Syntax.token(SyntaxKind.EndOfFileToken, { text: '' })
    );
  }

  export function getStandaloneExpression(
    ...
    if (
      ...
      if (
        parentNode.kind() === SyntaxKind.QualifiedName &&
        (<QualifiedNameSyntax>parentNode).right === token
      ) {
        return parentPositionedNode;
      } else if (
        parentNode.kind() === SyntaxKind.MemberAccessExpression &&
        (<MemberAccessExpressionSyntax>parentNode).name === token
      ) {
        return parentPositionedNode;
      }
    }
  ...
  export function isInModuleOrTypeContext(
    positionedToken: PositionedToken
  ): boolean {
    if (positionedToken !== null) {
      var positionedNodeOrToken =
        Syntax.getStandaloneExpression(positionedToken);
  ...
  export function isInTypeOnlyContext(
    positionedToken: PositionedToken
  ): boolean {
  ...
  export function nodeStructuralEquals(
    node1: SyntaxNode,
    node2: SyntaxNode
  ): boolean {
    if (node1 === null) {
  ...
  export function nodeOrTokenStructuralEquals(
    node1: ISyntaxNodeOrToken,
    node2: ISyntaxNodeOrToken
  ): boolean {
    if (node1 === node2) {
      return true;
    }
    ...
    if (node1.isToken()) {
      return node2.isToken()
        ? tokenStructuralEquals(<ISyntaxToken>node1, <ISyntaxToken>node2)
        : false;
    }

    return node2.isNode()
      ? nodeStructuralEquals(<SyntaxNode>node1, <SyntaxNode>node2)
      : false;
  }

  export function tokenStructuralEquals(
    token1: ISyntaxToken,
    token2: ISyntaxToken
  ): boolean {
    if (token1 === token2) {
    ...
    return (
      ...
      Syntax.triviaListStructuralEquals(
        token1.leadingTrivia(),
        token2.leadingTrivia()
      ) &&
      Syntax.triviaListStructuralEquals(
        token1.trailingTrivia(),
        token2.trailingTrivia()
      )
    );
  }

  export function triviaListStructuralEquals(
    triviaList1: ISyntaxTriviaList,
    triviaList2: ISyntaxTriviaList
  ): boolean {
  ...
  export function triviaStructuralEquals(
    trivia1: ISyntaxTrivia,
    trivia2: ISyntaxTrivia
  ): boolean {
    return (
      trivia1.kind() === trivia2.kind() &&
      trivia1.fullWidth() === trivia2.fullWidth() &&
      trivia1.fullText() === trivia2.fullText()
    );
  }

  export function listStructuralEquals(
    list1: ISyntaxList,
    list2: ISyntaxList
  ): boolean {
  ...
  export function separatedListStructuralEquals(
    list1: ISeparatedSyntaxList,
    list2: ISeparatedSyntaxList
  ): boolean {
    if (list1.childCount() !== list2.childCount()) {
      return false;
    }
  ...
  export function elementStructuralEquals(
    element1: ISyntaxElement,
    ...
    if (element1.isToken()) {
      return tokenStructuralEquals(
        <ISyntaxToken>element1,
        <ISyntaxToken>element2
      );
      ...
      return separatedListStructuralEquals(
        <ISeparatedSyntaxList>element1,
        <ISeparatedSyntaxList>element2
      );
    }
  ...
  export function numericLiteralExpression(
    text: string
  ...
  export function isSuperInvocationExpression(
    node: IExpressionSyntax
  ): boolean {
    return (
      node.kind() === SyntaxKind.InvocationExpression &&
      (<InvocationExpressionSyntax>node).expression.kind() ===
        SyntaxKind.SuperKeyword
    );
  }

  export function isSuperInvocationExpressionStatement(
    node: SyntaxNode
  ): boolean {
    return (
  ...
  export function isSuperMemberAccessExpression(
    node: IExpressionSyntax
  ): boolean {
    return (
      node.kind() === SyntaxKind.MemberAccessExpression &&
      (<MemberAccessExpressionSyntax>node).expression.kind() ===
        SyntaxKind.SuperKeyword
    );
  }

  export function isSuperMemberAccessInvocationExpression(
    node: SyntaxNode
  ): boolean {
    return (
      node.kind() === SyntaxKind.InvocationExpression &&
  ...
  export function assignmentExpression(
    ...
    return Syntax.normalModeFactory.binaryExpression(
  ...
  export function nodeHasSkippedOrMissingTokens(node: SyntaxNode): boolean {
    for (var i = 0; i < node.childCount(); i++) {
      ...
      if (child !== null && child.isToken()) {
        ...
        if (
          token.hasSkippedToken() ||
          (token.width() === 0 && token.kind() !== SyntaxKind.EndOfFileToken)
        ) {
          return true;
        }
      }
  ...
  export function isUnterminatedStringLiteral(token: ISyntaxToken): boolean {
    if (token && token.kind() === SyntaxKind.StringLiteral) {
      ...
      return (
        text.length < 2 ||
        text.charCodeAt(text.length - 1) !== text.charCodeAt(0)
      );
    }
  ...
  export function isUnterminatedMultilineCommentTrivia(
  ...
  export function isEntirelyInsideCommentTrivia(
    trivia: ISyntaxTrivia,
    fullStart: number,
    position: number
  ): boolean {
    if (trivia && trivia.isComment() && position > fullStart) {
      ...
      if (position < end) {
        ...
        return (
          ...
          isUnterminatedMultilineCommentTrivia(trivia)
        );
      }
    }
  ...
  export function isEntirelyInsideComment(
    ...
    if (positionedToken.kind() === SyntaxKind.EndOfFileToken) {
      ...
      if (
        position <=
        fullStart + positionedToken.token().leadingTriviaWidth()
      ) {
        triviaList = positionedToken.token().leadingTrivia();
    ...
    if (triviaList) {
      ...
      for (var i = 0, n = triviaList.count(); i < n; i++) {
        ...
        if (position <= fullStart) {
          ...
          position <= fullStart + trivia.fullWidth() &&
          trivia.isComment()
        ) {
    ...
    return (
      ...
      isEntirelyInsideCommentTrivia(lastTriviaBeforeToken, fullStart, position)
    );
  }

  export function isEntirelyInStringOrRegularExpressionLiteral(
    sourceUnit: SourceUnitSyntax,
    position: number
  ): boolean {
    var positionedToken = sourceUnit.findToken(position);
    ...
    if (positionedToken) {
      if (positionedToken.kind() === SyntaxKind.EndOfFileToken) {
        ...
        return (
          positionedToken &&
          positionedToken.token().trailingTriviaWidth() === 0 &&
          isUnterminatedStringLiteral(positionedToken.token())
        );
        ...
        return (
          (position < positionedToken.end() &&
            (positionedToken.kind() === TypeScript.SyntaxKind.StringLiteral ||
  ...
  function findSkippedTokenInTriviaList(
    positionedToken: PositionedToken,
    position: number,
    lookInLeadingTriviaList: boolean
    ...
    if (triviaList && triviaList.hasSkippedToken()) {
      for (var i = 0, n = triviaList.count(); i < n; i++) {
        ...
        if (
          trivia.isSkippedToken() &&
          position >= fullStart &&
          position <= fullStart + triviaWidth
        ) {
          return new PositionedSkippedToken(
            positionedToken,
            trivia.skippedToken(),
            fullStart
          );
  ...
  function findSkippedTokenOnLeftInTriviaList(
    positionedToken: PositionedToken,
    position: number,
    lookInLeadingTriviaList: boolean
  ): PositionedSkippedToken {
  ...
  export function findSkippedTokenInLeadingTriviaList(
    positionedToken: PositionedToken,
    position: number
  ): PositionedSkippedToken {
    return findSkippedTokenInTriviaList(
      positionedToken,
      position,
      /*lookInLeadingTriviaList*/ true
    );
  }

  export function findSkippedTokenInTrailingTriviaList(
    positionedToken: PositionedToken,
    position: number
  ): PositionedSkippedToken {
    return findSkippedTokenInTriviaList(
      positionedToken,
      position,
      /*lookInLeadingTriviaList*/ false
    );
  }

  export function findSkippedTokenInPositionedToken(
    positionedToken: PositionedToken,
    position: number
  ): PositionedSkippedToken {
    ...
    return findSkippedTokenInTriviaList(
      positionedToken,
      position,
      /*lookInLeadingTriviaList*/ positionInLeadingTriviaList
    );
  }

  export function findSkippedTokenOnLeft(
    positionedToken: PositionedToken,
    position: number
  ): PositionedSkippedToken {
    var positionInLeadingTriviaList = position < positionedToken.start();
    return findSkippedTokenOnLeftInTriviaList(
      positionedToken,
      position,
      /*lookInLeadingTriviaList*/ positionInLeadingTriviaList
    );
  ...
  export function hasAncestorOfKind(
    positionedToken: PositionedElement,
    kind: SyntaxKind
  ): boolean {
