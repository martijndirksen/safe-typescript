import { PositionedElement, PositionedNode } from './positionedElement';
import { IModuleElementSyntax } from './syntaxElement';
...
export class SyntaxUtilities {
  public static isAngleBracket(positionedElement: PositionedElement): boolean {
    ...
    if (
      parent !== null &&
      (element.kind() === SyntaxKind.LessThanToken ||
        element.kind() === SyntaxKind.GreaterThanToken)
    ) {
      switch (parent.kind()) {
  ...
  public static getExportKeyword(
    ...
    switch (moduleElement.kind()) {
      ...
      case SyntaxKind.ImportDeclaration:
        return SyntaxUtilities.getToken(
          (<any>moduleElement).modifiers,
  ...
  public static isAmbientDeclarationSyntax(
    positionNode: PositionedNode
  ): boolean {
    if (!positionNode) {
      return false;
    }
    ...
    switch (node.kind()) {
      ...
      case SyntaxKind.EnumDeclaration:
        if (
          SyntaxUtilities.containsToken(
            <ISyntaxList>(<any>node).modifiers,
      ...
      case SyntaxKind.MemberVariableDeclaration:
        if (node.isClassElement() || node.isModuleElement()) {
          return SyntaxUtilities.isAmbientDeclarationSyntax(
            positionNode.containingNode()
          );
        }

      case SyntaxKind.EnumElement:
        return SyntaxUtilities.isAmbientDeclarationSyntax(
          positionNode.containingNode().containingNode()
        );
