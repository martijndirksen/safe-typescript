...
export class SyntaxDedenter extends SyntaxRewriter {
  ...
  public visitToken(token: ISyntaxToken): ISyntaxToken {
    ...
    if (this.lastTriviaWasNewLine) {
      ...
      result = token.withLeadingTrivia(
        this.dedentTriviaList(token.leadingTrivia())
      );
    }
    ...
    for (var i = 0, n = triviaList.count(); i < n && !this.isAborted(); i++) {
      ...
      if (dedentThisTrivia) {
        if (trivia.kind() === SyntaxKind.WhitespaceTrivia) {
          ...
          var hasFollowingNewLine =
            i < triviaList.count() - 1 &&
            triviaList.syntaxTriviaAt(i + 1).kind() ===
  ...
  private dedentSegment(
    ...
    this.dedentationAmount =
      firstNonWhitespaceColumn - newFirstNonWhitespaceColumn;
    Debug.assert(this.dedentationAmount >= 0);

    // Compute an indentation string for that.
    var indentationStr = indentString(
  ...
  private dedentWhitespace(
    ...
    var newIndentation = this.dedentSegment(
      trivia.fullText(),
      hasFollowingNewLineTrivia
    );
    return whitespace(newIndentation);
  }
  ...
  private dedentMultiLineComment(trivia: ISyntaxTrivia): ISyntaxTrivia {
    ...
    for (var i = 1; i < segments.length; i++) {
      ...
      segments[i] = this.dedentSegment(
        segment,
        /*hasFollowingNewLineTrivia*/ false
      );
    }
    ...
    return multiLineComment(result);
  }

  public static dedentNode(
    node: ISyntaxNode,
    dedentFirstToken: boolean,
    dedentAmount: number,
