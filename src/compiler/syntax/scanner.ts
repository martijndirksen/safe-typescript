import type { CheckedArray } from '../../runtime/rt';
  ...
  var isIdentifierStartCharacter: boolean[] =
    ArrayUtilities.createArray<boolean>(
      CharacterCodes.maxAsciiCharacter,
      false
    );
  var isIdentifierPartCharacter: boolean[] =
    ArrayUtilities.createArray<boolean>(
      CharacterCodes.maxAsciiCharacter,
      false
    );
  ...
  for (
    ...
    if (character >= CharacterCodes.a && character <= CharacterCodes.z) {
      ...
      character >= CharacterCodes._0 &&
      character <= CharacterCodes._9
    ) {
      isIdentifierPartCharacter[character] = true;
  ...
  export class Scanner implements ISlidingWindowSource {
    ...
    private scanMultiLineCommentTriviaLength(
      diagnostics: Diagnostic[]
    ): number {
      this.slidingWindow.moveToNextItem();
      this.slidingWindow.moveToNextItem();
