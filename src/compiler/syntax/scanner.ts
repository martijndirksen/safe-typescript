...
var isIdentifierStartCharacter: boolean[] = ArrayUtilities.createArray<boolean>(
  CharacterCodes.maxAsciiCharacter,
  false
);
var isIdentifierPartCharacter: boolean[] = ArrayUtilities.createArray<boolean>(
  CharacterCodes.maxAsciiCharacter,
  false
);
var isNumericLiteralStart: boolean[] = ArrayUtilities.createArray<boolean>(
  CharacterCodes.maxAsciiCharacter,
  false
);
for (
  var character = 0;
  character < CharacterCodes.maxAsciiCharacter;
  character++
) {
...
for (
  var keywordKind = SyntaxKind.FirstKeyword;
...
export class Scanner implements ISlidingWindowSource {
  ...
  public scan(
    ...
    var leadingTriviaInfo = this.scanTriviaInfo(
      diagnostics,
      /*isTrailing: */ false
    );

    var start = this.slidingWindow.absoluteIndex();
    var kindAndFlags = this.scanSyntaxToken(
      diagnostics,
      allowRegularExpression
    );
    var end = this.slidingWindow.absoluteIndex();

    var trailingTriviaInfo = this.scanTriviaInfo(
      diagnostics,
      /*isTrailing: */ true
    );

    var isVariableWidthKeyword =
      (kindAndFlags & SyntaxConstants.IsVariableWidthKeyword) !== 0;
  ...
  private createToken(
    ...
    isVariableWidthKeyword: boolean
  ): ISyntaxToken {
  ...
  private static triviaWindow: number[] = ArrayUtilities.createArray<number>(
    2048,
    0
  );

  // Scans a subsection of 'text' as trivia.
  public static scanTrivia(
    text: ISimpleText,
    start: number,
    length: number,
    isTrailing: boolean
    ...
    var scanner = new Scanner(
      /*fileName:*/ null,
      text.subText(new TextSpan(start, length)),
      LanguageVersion.EcmaScript5,
      Scanner.triviaWindow
    );
    return scanner.scanTrivia(text, start, isTrailing);
  }

  private scanTrivia(
    underlyingText: ISimpleText,
    underlyingTextStart: number,
    isTrailing: boolean
  ): ISyntaxTriviaList {
      ...
      if (!this.slidingWindow.isAtEndOfSource()) {
        ...
        switch (ch) {
          ...
          case CharacterCodes.byteOrderMark:
            ...
            trivia.push(
              this.scanWhitespaceTrivia(underlyingText, underlyingTextStart)
            );
            continue;
  ...
  private scanTriviaInfo(
    ...
    while (true) {
      ...
      return (
        (width << SyntaxConstants.TriviaFullWidthShift) | hasCommentOrNewLine
      );
    }
  }
  ...
  private scanMultiLineCommentTriviaLength(diagnostics: Diagnostic[]): number {
    ...
    while (true) {
      ...
      if (
        ...
        this.slidingWindow.peekItemN(1) === CharacterCodes.slash
      ) {
        this.slidingWindow.moveToNextItem();
        this.slidingWindow.moveToNextItem();
        width += 2;
  ...
  private scanLineTerminatorSequenceLength(ch: number): number {
    ...
    if (
      ...
      this.currentCharCode() === CharacterCodes.lineFeed
    ) {
      this.slidingWindow.moveToNextItem();
      return 2;
    } else {
  ...
  private isIdentifierStart(interpretedChar: number): boolean {
    ...
    return (
      interpretedChar > CharacterCodes.maxAsciiCharacter &&
  ...
  private tryFastScanIdentifierOrKeyword(firstCharacter: number): SyntaxKind {
    ...
    var window: number[] = RT.forceCheckedArray<any, number>(
    ...
    if (currentIndex === endIndex) {
      ...
      character === CharacterCodes.backslash ||
      character > CharacterCodes.maxAsciiCharacter
    ) {
      ...
      if (isKeywordStartCharacter[firstCharacter]) {
        kind = ScannerUtilities.identifierKind(
          window,
          startIndex,
          identifierLength
        );
      ...
      slidingWindow.setAbsoluteIndex(
        slidingWindow.absoluteIndex() + identifierLength
      );
  ...
  private slowScanIdentifierOrKeyword(diagnostics: Diagnostic[]): SyntaxKind {
    ...
    if (
      keywordKind >= SyntaxKind.FirstKeyword &&
      keywordKind <= SyntaxKind.LastKeyword
    ) {
      if (sawUnicodeEscape) {
  ...
  private isOctalNumericLiteral(): boolean {
    return (
      this.currentCharCode() === CharacterCodes._0 &&
      CharacterInfo.isOctalDigit(+this.slidingWindow.peekItemN(1))
    );
  }
    ...
    if (this.languageVersion() >= LanguageVersion.EcmaScript5) {
      diagnostics.push(
        new Diagnostic(
          ...
          this.text.lineMap(),
          position,
          this.absoluteIndex() - position,
          DiagnosticCode.Octal_literals_are_not_available_when_targeting_ECMAScript_5_and_higher,
          null
        )
      );
    }
  ...
  private scanDecimalNumericLiteral(): void {
    ...
    if (ch === CharacterCodes.e || ch === CharacterCodes.E) {
      ...
      if (CharacterInfo.isDecimalDigit(+nextChar1)) {
        ...
        nextChar1 === CharacterCodes.minus ||
        nextChar1 === CharacterCodes.plus
      ) {
        // e+ or E+ or e- or E-
  ...
  private scanDotToken(diagnostics: Diagnostic[]): SyntaxKind {
    ...
    this.slidingWindow.moveToNextItem();
    if (
      this.currentCharCode() === CharacterCodes.dot &&
  ...
  private scanDefaultCharacter(
    ...
    diagnostics.push(
      new Diagnostic(
        ...
        position,
        1,
        DiagnosticCode.Unexpected_character_0,
        [messageText]
      )
    );
      ...
      if (ch === CharacterCodes.backslash) {
        ...
        diagnostics.push(
          new Diagnostic(
            this.fileName,
            this.text.lineMap(),
            Math.min(this.slidingWindow.absoluteIndex(), this.text.length()),
            1,
            DiagnosticCode.Missing_close_quote_character,
            null
          )
        );
        break;
  ...
  public substring(start: number, end: number, intern: boolean): string {
    ...
    if (intern) {
      return DefaultStringTable.addCharArray(window, offset, length);
    } else {
      return StringUtilities.fromCharCodeArray(
        window.slice(offset, offset + length)
      );
    }
  }

  private createIllegalEscapeDiagnostic(
    start: number,
    end: number
  ): Diagnostic {
  ...
  public static isValidIdentifier(
    text: ISimpleText,
    languageVersion: LanguageVersion
  ): boolean {
    var scanner = new Scanner(
    ...
    return (
      errors.length === 0 &&
      SyntaxFacts.isIdentifierNameOrAnyKeyword(token) &&
