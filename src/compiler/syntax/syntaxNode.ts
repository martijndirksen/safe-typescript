...
import {
  elementStructuralEquals,
  findSkippedTokenInPositionedToken,
  findSkippedTokenOnLeft,
} from './syntax';
...
import { SourceUnitSyntax } from './syntaxNodes.generated';
import { ISyntaxToken } from './syntaxToken';
import { SyntaxTokenReplacer } from './syntaxTokenReplacer';
...
export class SyntaxNode implements ISyntaxNodeOrToken {
  ...
  constructor(parsedInStrictMode: boolean) {
    this._data = parsedInStrictMode
      ? SyntaxConstants.NodeParsedInStrictModeMask
      : 0;
  }
  ...
  public firstToken(): ISyntaxToken {
    for (var i = 0, n = this.childCount(); i < n; i++) {
      ...
      if (element !== null) {
        if (
          element.fullWidth() > 0 ||
          element.kind() === SyntaxKind.EndOfFileToken
        ) {
  ...
  public withLeadingTrivia(trivia: ISyntaxTriviaList): SyntaxNode {
    return this.replaceToken(
      this.firstToken(),
      this.firstToken().withLeadingTrivia(trivia)
    );
  }
  ...
  public withTrailingTrivia(trivia: ISyntaxTriviaList): SyntaxNode {
    return this.replaceToken(
      this.lastToken(),
      this.lastToken().withTrailingTrivia(trivia)
    );
  }
  ...
  private computeData(): number {
    ...
    var isIncrementallyUnusable =
      (this._data & SyntaxConstants.NodeIncrementallyUnusableMask) !== 0 ||
      slotCount === 0;
  ...
  public findToken(
    position: number,
    includeSkippedTokens: boolean = false
  ): PositionedToken {
  ...
  private tryGetEndOfFileAt(position: number): PositionedToken {
    if (
      this.kind() === SyntaxKind.SourceUnit &&
      position === this.fullWidth()
    ) {
      // @ts-expect-error suspicious cast
      var sourceUnit = <SourceUnitSyntax>this;
      return new PositionedToken(
        new PositionedNode(null, sourceUnit, 0),
        sourceUnit.endOfFileToken,
        sourceUnit.moduleElements.fullWidth()
      );
    }
  ...
  public findTokenOnLeft(
    position: number,
    includeSkippedTokens: boolean = false
  ): PositionedToken {
    var positionedToken = this.findToken(
      position,
      /*includeSkippedTokens*/ false
    );
    ...
    if (includeSkippedTokens) {
      positionedToken =
        findSkippedTokenOnLeft(positionedToken, position) || positionedToken;
    }
  ...
  public findCompleteTokenOnLeft(
    position: number,
    includeSkippedTokens: boolean = false
  ): PositionedToken {
    var positionedToken = this.findToken(
  ...
  public width(): number {
    return (
      this.fullWidth() - this.leadingTriviaWidth() - this.trailingTriviaWidth()
    );
  }
