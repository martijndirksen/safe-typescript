...
export function deferredTrivia(
  kind: SyntaxKind,
  text: ISimpleText,
  fullStart: number,
  fullWidth: number
...
export var lineFeedTrivia: ISyntaxTrivia = trivia(
  SyntaxKind.NewLineTrivia,
  '\n'
);
export var carriageReturnTrivia: ISyntaxTrivia = trivia(
  SyntaxKind.NewLineTrivia,
  '\r'
);
export var carriageReturnLineFeedTrivia: ISyntaxTrivia = trivia(
  SyntaxKind.NewLineTrivia,
  '\r\n'
);
...
export function splitMultiLineCommentTriviaIntoMultipleLines(
  ...
  for (var i = 0; i < triviaText.length; i++) {
    ...
    switch (ch) {
      case CharacterCodes.carriageReturn:
        if (
          i < triviaText.length - 1 &&
          triviaText.charCodeAt(i + 1) === CharacterCodes.lineFeed
        ) {
          // Consume the \r
          i++;
        }
