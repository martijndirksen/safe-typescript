...
module TypeScript {
  ...
  export class TypeScriptCompiler {
    ...
    public getSoundSemanticDiagnostics(fileName: string) {
      ...
      errors.forEach((d: Diagnostic) =>
        this.semanticInfoChain.addDiagnostic(d)
      );
      return errors;
    }
      ...
      for (var i = 0, n = path.length; i < n; i++) {
        ...
        switch (current.kind()) {
          ...
          case SyntaxKind.ObjectCreationExpression:
            if (propagateContextualTypes) {
              var isNew =
                current.kind() === SyntaxKind.ObjectCreationExpression;
              var callExpression = <{ argumentList: ArgumentList }>(
                (<any>current)
              );
                ...
                if (callResolutionResults.actualParametersContextTypeSymbols) {
                  ...
                  if (argExpression) {
                    for (
                      var j = 0,
                        m =
                          callExpression.argumentList.args.nonSeparatorCount();
                      j < m;
                      j++
                    ) {
                      if (
          ...
          case SyntaxKind.ObjectLiteralExpression:
            if (propagateContextualTypes) {
              ...
              if (memeberAST) {
                ...
                resolutionContext.pushContextualType(
                  contextualType,
                  false,
                  null
                );
    ...
    public pullGetVisibleDeclsFromAST(
      ast: AST,
      document: Document
    ): PullDecl[] {
      var resolver = this.semanticInfoChain.getResolver();
    ...
    private moveNextDeclarationEmitPhase(): boolean {
      ...
      if (this.index < this.fileNames.length) {
        ...
        this._sharedDeclarationEmitter =
          this.compiler._emitDocumentDeclarations(
            document,
            this._emitOptions,
            (file) => {
