...
(function (RT) {
  ...
  _ = Object.prototype.__rtti__ =
    RT.Any; /* Functions and Arrays inherit __rtti__ from Object.prototype, Object.create(..) may not */
  var namedTypesCache = {};
    ...
    switch (t2.tt) {
        ...
        switch (t1.tt) {
          case TT.INTERFACE:
            ...
            sub = subtype(
              t1.structuredType,
              t2.structuredType,
              extendContext(cxt, t1, t2, NameRelation.SUBTYPE)
            );
            if (sub.fst) {
      ...
      case TT.INDEX_MAP:
        if (t1.tt === TT.INDEX_MAP) {
          return {
            fst:
              equalTypes(t1.key, t2.key, cxt) &&
              equalTypes(t1.value, t2.value, cxt),
            snd: zero,
          };
        } else {
          return { fst: false, snd: zero };
      ...
      case TT.JUST_TYPE:
        return {
          fst: subtype(t1.tt === TT.JUST_TYPE ? t1.base : t1, t2.base, cxt).fst,
          snd: zero,
        };
      default:
      //falls off
    ...
    switch (t1.tt) {
      ...
      case TT.INTERFACE:
        ...
        var b = equalTypes(
          t1.structuredType,
          t2.structuredType,
          extendContext(cxt, t1, t2, NameRelation.EQUALITY)
        );
        if (b) {
      ...
      case TT.INDEX_MAP:
        return (
          equalTypes(t1.key, t1.key, cxt) && equalTypes(t1.value, t1.value, cxt)
        );
      case TT.JUST_TYPE:
        return equalTypes(t1.base, t2.base, cxt);
      case TT.STRUCTURED_TYPE:
        return (
          eqflds(t1.fieldTable, t2.fieldTable) &&
  ...
  function checkAndTag(v, from, to) {
    ...
    switch (to.tt) {
      ...
      case TT.STRUCTURED_TYPE:
        ...
        v.__rtti__ = combine(
          v.__rtti__ || RT.Any /* t_v is stale at this point */,
  ...
  function resolveMethod(o, from, mname) {
    ...
    var t_o = o.__rtti__ || RT.Any;
    return (
      t_o.methodTable[mname] ||
      RT.objectMethods[mname] ||
      from.methodTable[mname]
    );
  }
  ...
  function callMethod(o, from, m, args, argTypes) {
    ...
    if (t1 === undefined) {
      return callFunction(
        readField(o, from, m),
        RT.Any /* readField gives type Any */,
        args,
        argTypes
      );
    }
...
RT.registerType(
  RT.InterfaceRepr(
    'Object',
    {
      ...
      isPrototypeOf: RT.ArrowType(
        [RT.InterfaceType('Object')],
        RT.Bool,
        undefined
      ),
...
RT.registerType(
  RT.InterfaceRepr(
    'String',
    {
      ...
      localeCompare: RT.ArrowType(
        [RT.Str, RT.ArrayType(RT.Str), RT.JustType('CollatorOptions')],
        RT.Num,
        undefined,
        2
      ),
      localeCompare: RT.ArrowType(
        [RT.Str, RT.Str, RT.JustType('CollatorOptions')],
        RT.Num,
        undefined,
        2
      ),
      ...
      match: RT.ArrowType(
        [RT.InterfaceType('RegExp')],
        RT.ArrayType(RT.Str),
        undefined
      ),
      replace: RT.ArrowType([RT.Str, RT.Str], RT.Str, undefined),
      replace: RT.ArrowType(
        [
          RT.Str,
          RT.StructuredType(
            {
              '<call>': RT.ArrowType(
                [RT.Str, RT.ArrayType(RT.Any)],
                RT.Str,
                {
      ...
      replace: RT.ArrowType(
        [RT.InterfaceType('RegExp'), RT.Str],
        RT.Str,
        undefined
      ),
      replace: RT.ArrowType(
      ...
      split: RT.ArrowType(
        [RT.InterfaceType('RegExp'), RT.Num],
        RT.ArrayType(RT.Str),
        undefined,
        1
      ),
...
RT.registerType(
  RT.InterfaceRepr(
    'Number',
    {
      ...
      toLocaleString: RT.ArrowType(
        [RT.ArrayType(RT.Str), RT.JustType('NumberFormatOptions')],
        RT.Str,
        undefined,
        1
      ),
      toLocaleString: RT.ArrowType(
        [RT.Str, RT.JustType('NumberFormatOptions')],
        RT.Str,
        undefined,
        1
      ),
